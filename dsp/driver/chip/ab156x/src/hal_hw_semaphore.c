...
hal_hw_semaphore_status_t hw_semaphore_take(hal_hw_semaphore_id_t id)
{
   ...
   hal_nvic_save_and_set_interrupt_mask(&int_mask);
    if (id < 32) {
        if ((SMPH->CH1_STATUS0 & (1 << id)) != 0) {
            hal_nvic_restore_interrupt_mask(int_mask);
            return HAL_HW_SEMAPHORE_STATUS_TAKE_ERROR;
        }
        SMPH->CH1_LOCK0 = (1 << id);
        hal_nvic_restore_interrupt_mask(int_mask);
        return ((SMPH->CH1_STATUS0 & (1 << id)) == 0) ? HAL_HW_SEMAPHORE_STATUS_TAKE_ERROR : HAL_HW_SEMAPHORE_STATUS_OK;
    } else {
        id -= 32;
        if ((SMPH->CH1_STATUS1 & (1 << id)) != 0) {
            hal_nvic_restore_interrupt_mask(int_mask);
            return HAL_HW_SEMAPHORE_STATUS_TAKE_ERROR;
        }
        SMPH->CH1_LOCK1 = (1 << id);
        hal_nvic_restore_interrupt_mask(int_mask);
        return ((SMPH->CH1_STATUS1 & (1 << id)) == 0) ? HAL_HW_SEMAPHORE_STATUS_TAKE_ERROR : HAL_HW_SEMAPHORE_STATUS_OK;
    }
}
...
hal_hw_semaphore_status_t hw_semaphore_give(hal_hw_semaphore_id_t id)
{
    ...
    if (id < 32) {
        ...
        SMPH->CH1_RELEASE0 = (1 << id);
        hal_nvic_restore_interrupt_mask(int_mask);
        return ((SMPH->CH1_STATUS0 & (1 << id)) == 0) ? HAL_HW_SEMAPHORE_STATUS_OK : HAL_HW_SEMAPHORE_STATUS_GIVE_ERROR;
    } else {
        id -= 32;
        if ((SMPH->CH1_STATUS1 & (1 << id)) == 0) {
            hal_nvic_restore_interrupt_mask(int_mask);
        ...
        SMPH->CH1_RELEASE1 = (1 << id);
        hal_nvic_restore_interrupt_mask(int_mask);
        return ((SMPH->CH1_STATUS1 & (1 << id)) == 0) ? HAL_HW_SEMAPHORE_STATUS_OK : HAL_HW_SEMAPHORE_STATUS_GIVE_ERROR;
    }
}
#endif 
