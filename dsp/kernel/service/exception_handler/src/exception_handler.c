...
/* if dsp occurs exception and it trigger another exception when processing it */
__attribute__((aligned(8))) static exception_context_t exception_context_1;
exception_context_t *const exception_context_pointer_1 = &exception_context_1;
...
/******************************************************************************/
extern void exception_platform_assert_hook();
__attribute__((optimize("-O0"))) void platform_assert(const char *expr, const char *file, int line)
{
    ...
    static uint32_t platform_assert_count = 0;
    if (platform_assert_count == 0) {
        /* call exception platform assert hook function */
        exception_platform_assert_hook();
        platform_assert_count = 1;
    }
...
extern void exception_light_assert_hook();
__attribute__((optimize("-O0"))) void light_assert(const char *expr, const char *file, int line)
{
    static uint32_t primask_backup_assert = 0;
    static uint32_t light_assert_count = 0;

    if (light_assert_count == 0) {
        /* call exception light assert hook function */
        exception_light_assert_hook();
        light_assert_count = 1;
    }
                                        ...
                                        const memory_region_type *static_regions, exception_user_regions_t *pEUR)
{
    ...
    pESM->packet_size = sizeof(exception_sharemem_t);
...
void exception_dsp_fault_handler(void)
{
    ...
    /* Update Exception Count,it should not occur exception before here */
    exception_info.count += 1;

    if (exception_info.count > 1) {
        /* ARx has shift in exception_context, so need to revise it */
        ReviseAddressRegsitersLocation(exception_context_pointer_1);
    } else {
        /* ARx has shift in exception_context, so need to revise it */
    ...
    if (exception_info.count > 1) {
        ...
        PrepareExceptionShareMemory(exception_sharemem_pointer, exception_info_pointer, exception_context_pointer_1, memory_regions, &exception_user_regions);
    } else {
        /* Prepare exception context for the master */
