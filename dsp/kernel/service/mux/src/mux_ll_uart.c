...
mux_status_t mux_open_ll(mux_port_t port, const char *user_name, mux_handle_t *p_handle, mux_callback_t callback, void *user_data)
{
    ...
    if (g_mux_ll_config == NULL) {
        g_mux_ll_config = MUX_LL_CONFIG_SHMEM_POINTER;
    }
...
ATTR_TEXT_IN_FAST_MEM mux_status_t mux_rx_ll(mux_handle_t handle, mux_buffer_t *buffer, uint32_t *receive_done_data_len)
{
    ...
    *receive_done_data_len = mux_ringbuffer_read((mux_ringbuffer_t *)&user_config->rxbuf, buffer->p_buf, buffer->buf_size);
    ...
    LOG_MSGID_I(common, "[mux_rx_ll] [%02x] expect size=%d real size=%d, ud_len=%d, vd_len=%d", 5, user_config->rxbuf.uid, buffer->buf_size, *receive_done_data_len,\
        mux_ringbuffer_data_length(&user_config->rxbuf), vd_len);
    return MUX_STATUS_OK;
}
...
ATTR_TEXT_IN_FAST_MEM mux_status_t mux_tx_ll(mux_handle_t handle, const mux_buffer_t buffers[], uint32_t buffers_counter, uint32_t *send_done_data_len)
{
    ...
    *send_done_data_len = send_len;
    ...
    LOG_MSGID_I(common, "[mux_tx_ll] uid=%02x send_len=%d ud_len=%d vd_len=%d", 4, txbuf->uid, send_len, mux_ringbuffer_data_length(txbuf), vd_len);

    ret = hal_ccni_set_event(CCNI_DSP0_TO_CM4_LL_UART, (hal_ccni_message_t*)msg_array);
    if (HAL_CCNI_STATUS_OK != ret) {
        RB_LOG_W("dsp ccni send error, return = %d", 1, ret);
    } else {
...
void mux_ll_uart_rx_event_from_mcu_handler(hal_ccni_event_t event, void * msg)
{
    ...
    mux_ll_user_callback_timestamp_t *callback_timestamp;
    (void)msg;
    ...
    for (uid = 0; uid < g_mux_ll_config->user_count; uid++) {
        ...
        if (ll_user_config->dcid != HAL_CORE_DSP0) {
            continue;
        }
        data_len = mux_ringbuffer_data_length(&ll_user_config->rxbuf);
        if ((data_len > 0) && ll_user_config->callback) {
            callback_timestamp = &ll_user_config->callback_timestamp[cbts_idx++ % MUX_LL_CALLBACK_TIMESTAMP_BUFFER_MAX];
            hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &callback_timestamp->start_timestamp);;
            ll_user_config->callback(user_id_to_handle(uid, (uint32_t)ll_user_config->rxbuf.port_idx + MUX_LL_UART_0), MUX_EVENT_READY_TO_READ, data_len, ll_user_config->user_data);
            hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &callback_timestamp->end_timestamp);
            hal_gpt_get_duration_count(callback_timestamp->start_timestamp, callback_timestamp->end_timestamp, &duration_count);
            if (duration_count > 100) {
...
mux_status_t mux_control_ll(mux_handle_t handle, mux_ctrl_cmd_t command, mux_ctrl_para_t *para)
{
    ...
    case MUX_CMD_GET_LL_USER_RX_BUFFER_DATA_SIZE:
        ...
        RB_LOG_I("[mux_control_ll] uid[%02u] rx_ud_len:%u", 2, uid, para->mux_ll_user_rx_data_len);
        break;
...
dchs_mode_t dchs_get_device_mode(void)
{
    ...
    while (g_mux_ll_config->device_mode == DCHS_MODE_ERROR) {
        timeout = mux_get_tick_elapse(start_count);
        if (timeout > 100000) {
            RB_LOG_W("[dchs_get_device_mode] waiting for sync", 0);
