...
/**
 * Internal api for exception msgid.
 */
#ifdef MTK_SYSLOG_VERSION_2
#if defined(MTK_SYSLOG_SUB_FEATURE_MSGID_TO_STRING_LOG_SUPPORT) || defined(MTK_DEBUG_PLAIN_LOG_ENABLE)
#define LOG_EXCEPTION_MSGID(cpu_id, message, arg_cnt, ...) \
do { \
    log_print_exception_log((message), \
                            ##__VA_ARGS__); \
} while (0)
#else
#define LOG_EXCEPTION_MSGID(cpu_id, message, arg_cnt, ...) \
do { \
    ...
    log_print_exception_msgid_log((cpu_id), \
                            (msg_id_string), \
                            (arg_cnt), \
                            ##__VA_ARGS__); \
} while (0)
#endif /* MTK_SYSLOG_SUB_FEATURE_MSGID_TO_STRING_LOG_SUPPORT || MTK_DEBUG_PLAIN_LOG_ENABLE */
#endif /* MTK_SYSLOG_VERSION_2 */
...
#if defined(MTK_SYSLOG_VERSION_2) &&  !defined(MTK_DEBUG_PLAIN_LOG_ENABLE)
