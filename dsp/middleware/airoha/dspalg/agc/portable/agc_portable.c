...
uint32_t agc_library_load(void *code_address, void *data_address, uint32_t *dram_pic_usage)
{
   ...
   if(agc_lib_loaded_counter == 0)
   {
        ...
        if(PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_load_v1(p_agc_pisplit_library_handle, code_address, data_address)) {
#else
...
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
    preloader_pisplit_get_library_code_and_data_size(p_agc_pisplit_library_handle, NULL, dram_pic_usage);
#else
    UNUSED(code_address);
    UNUSED(data_address);
...
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
        if(PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_unload_v1(p_agc_pisplit_library_handle)) {
#else
