...
void ClkSkewMode_Selection(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    switch (msg.ccni_message[1]) {
        case CLK_SKEW_V1:
            DSP_MW_LOG_I("[CLK_SKEW]clkskew v1", 0);
            ClkSkewMode_g = CLK_SKEW_V1;
            break;
        case CLK_SKEW_V2:
            DSP_MW_LOG_I("[CLK_SKEW]clkskew v2", 0);
    ...
    if ((pClkSkewCtrl->IntrDownCnt > 0) && (pClkSkewCtrl->CompensatedPolarity != 0)) {
        ...
        if ((pClkSkewCtrl->AccumulatedSamples > 0) && (pClkSkewCtrl->CompensatedSamples == 0)) {
            ...
            if(!ClkSkewSetup->pcdc_en){
                DSP_MW_LOG_I("[CLK_SKEW][%d] acu:%d", 2, dir, pClkSkewCtrl->CompensatedPolarity);
            }
...
VOID clk_skew_finish_compensation(DSP_CLOCK_SKEW_PARAM_t* ClkSkewParam, S16 CompensatedSamples)
{
    uint32_t mask;
...
VOID clk_skew_finish_compensation_pcdc(DSP_CLOCK_SKEW_PARAM_t* ClkSkewParam, S16 CompensatedSamples)
{
    uint32_t mask;
    DSP_CLOCK_SKEW_CTRL_t* ClkSkewCtrl = &(ClkSkewParam->ClkSkewCtrl);
    DSP_CLOCK_SKEW_DEBUG_t* ClkSkewDebug = &(ClkSkewParam->ClkSkewDebug);
...
void Clock_Skew_Mem_Check(void *para)
{
    ...
    if((DSP_CLK_SKEW_MEMSIZE(Channels,FrameSize,ClkSkewSetup->pcdc_en)) > buffer_lenth){
        DSP_MW_LOG_E("[CLK_SKEW][%d]Addr:0x%x, Mem size isn't enough, need:%d, now size:%d",4, ClkSkewSetup->clk_skew_dir, ClkSkewParam,(DSP_CLK_SKEW_MEMSIZE(Channels,FrameSize,ClkSkewSetup->pcdc_en)),buffer_lenth);
        DSP_MW_LOG_E("[CLK_SKEW][%d]PARAM size:%d, frame size:%d, working buffer:%d, channel:%d",5, ClkSkewSetup->clk_skew_dir,sizeof(DSP_CLOCK_SKEW_PARAM_t),FrameSize,sizeof(skew_ctrl_t),Channels);
    }else{
        DSP_MW_LOG_I("[CLK_SKEW][%d]Addr:0x%x, Mem check pass, need:%d, now size:%d",4, ClkSkewSetup->clk_skew_dir, ClkSkewParam,(DSP_CLK_SKEW_MEMSIZE(Channels,FrameSize,ClkSkewSetup->pcdc_en)),buffer_lenth );
    }
}
...
ATTR_TEXT_IN_IRAM_LEVEL_2 VOID Clock_Skew_Offset_Update(BT_CLOCK_OFFSET_SCENARIO type, SOURCE source, SINK sink)
{
    ...
    if (ClkSkewRCDCCtrl->first_offset_flag == TRUE) {
        ...
        if(ClkSkewSetup->clk_skew_dir == CLK_SKEW_DL){
            ...
            DSP_MW_LOG_I("[CLK_SKEW]init offset:%d",1,ClkSkewCtrl->initial_offset);
        }
