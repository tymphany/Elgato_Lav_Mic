...
void *cvsd_dec_decoder_export_parameters[3] = {
    cvsd_dec_default_function_parameter,
    cvsd_dec_default_function_parameter,
    cvsd_dec_default_function_parameter
};
...
void cvsd_dec_load_library_callback(preloader_pisplit_handle_t handle)
{
    ...
    if (p_export_parameters == NULL) {
        ...
        cvsd_dec_decoder_export_parameters[2] = (void *)p_export_parameters[2];
#if PIC_LOGPRINT
...
uint32_t cvsd_dec_library_load(void *code_address, void *data_address, uint32_t *dram_pic_usage)
{
    ...
    if(PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_load_v1(p_cvsd_dec_pisplit_library_handle, code_address, data_address))
#else
...
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
    preloader_pisplit_get_library_code_and_data_size(p_cvsd_dec_pisplit_library_handle, NULL, dram_pic_usage);
#else
    UNUSED(code_address);
    UNUSED(data_address);
...
uint32_t cvsd_dec_library_unload()
{
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
    if(PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_unload_v1(p_cvsd_dec_pisplit_library_handle))
#else
    ...
    cvsd_dec_decoder_export_parameters[2] = cvsd_dec_default_function_parameter;
