	...
	ifneq ($(AIR_VOICE_NR_USE_PIC_ENABLE), y)
		...
		ifeq ($(AIR_ECNR_1MIC_INEAR_ENABLE), y)
			...
			ifeq ($(AIR_BTA_IC_PREMIUM_G2), y)
				...
				LIBS += $(strip $(LIBDIR2))/ec_nr/$(IC_CONFIG)/libec_rxnr_swb_init.a
			endif
			...
			ifeq ($(AIR_AI_NR_PREMIUM_ENABLE), y)
				ifeq ($(AIR_AI_NR_PREMIUM_200K_SHORT_BOOM_OO_ENABLE), y)
					INC += prebuilt/middleware/third_party/dspalg/igo_nr_short_boom_oo/inc
					PIC += $(strip $(LIBDIR3))/igo_nr_short_boom_oo/pisplit/pisplit_igo_txnr_short_boom_oo.o
				else ifeq ($(AIR_AI_NR_PREMIUM_INEAR_500K_BROADSIDE_ENABLE), y)
					INC += prebuilt/middleware/third_party/dspalg/igo_nr_pro_broadside/inc
					PIC += $(strip $(LIBDIR3))/igo_nr_pro_broadside/pisplit/pisplit_igo_plus_1_txnr_pro_broadside.o
				else
					ifeq ($(AIR_BTA_IC_PREMIUM_G2), y)
			...
			ifeq ($(AIR_AI_NR_PREMIUM_INEAR_ENABLE), y)
				ifeq ($(AIR_AI_NR_PREMIUM_INEAR_500K_PRO_DISTRACTOR_ENABLE), y)
					INC += prebuilt/middleware/third_party/dspalg/igo_nr_pro_distractor/inc
					PIC += $(strip $(LIBDIR3))/igo_nr_pro_distractor/pisplit/pisplit_igo_plus_1_txnr_pro_distractor.o
				else ifeq ($(AIR_AI_NR_PREMIUM_INEAR_500K_PRO_TWS_OO_ENABLE), y)
					INC += prebuilt/middleware/third_party/dspalg/igo_nr_pro_tws_oo/inc
					PIC += $(strip $(LIBDIR3))/igo_nr_pro_tws_oo/pisplit/pisplit_igo_plus_1_txnr_pro_tws_oo.o
				else
					INC += prebuilt/middleware/third_party/dspalg/igo_nr/inc
					PIC += $(strip $(LIBDIR3))/igo_nr/pisplit/pisplit_igo_plus_1_txnr.o
				endif
