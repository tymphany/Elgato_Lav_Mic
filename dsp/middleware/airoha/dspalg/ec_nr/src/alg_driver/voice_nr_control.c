...
extern DSP_PARA_AEC_NR_STRU *g_voice_ecnr_nvkey_aec;
...
void voice_nr_set_param(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    DSP_MW_LOG_I("[DSP][VOICE_NR] voice_nr_set_param type: %d done", 1, msg16);
}
#else
void voice_nr_set_param(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
...
static void AirDump_update_notify_count(uint32_t writeoffset)
{
    ...
    if (wo_past != wo_over) {
        ...
        DSP_MW_LOG_I("[DSP][VOICE_NR] [AirDump] buffer cnt: %d, wo:%d, wo_past:%d, wo_over:%d", 4, rAirDumpCtrl->notify_count, writeoffset, wo_past, wo_over);
        wo_past = wo_over;
...
void voice_avc_vol_send_update(int16_t pre_vol, int16_t post_vol)
{
