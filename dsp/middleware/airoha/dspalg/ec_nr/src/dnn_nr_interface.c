...
bool stream_function_dnn_nr_initialize(void *para)
{
    void *tx_nr_ptr = stream_function_get_working_buffer(para);

    if (tx_nr_ptr != NULL) {
        DNN_1MIC_NR_PARAMS nr_params;
        DSP_PARA_RESERVED_STRU ResKey;
        p_dnn_NvKey = (void *)&ResKey;
        nvkey_read_full_key(NVKEY_DSP_PARA_RESERVED, p_dnn_NvKey, sizeof(DSP_PARA_RESERVED_STRU));
        memcpy(&nr_params, p_dnn_NvKey, sizeof(DNN_1MIC_NR_PARAMS));

        DNN_NR_Init(tx_nr_ptr, &nr_params);
    }
    return false;
}


ATTR_TEXT_IN_IRAM_LEVEL_2 bool stream_function_dnn_nr_process(void *para)
{
...
#endif
    DNN_NR_Prcs(prInBuf, NrOutBuf, prWorkBuf);
