...
void *environment_detection_export_parameters[3] = {environment_detection_default_function_parameter,
...
{
    if (environment_detection_load_lib_xSemaphore == NULL) {
        environment_detection_load_lib_xSemaphore = xSemaphoreCreateBinary();

        if (environment_detection_load_lib_xSemaphore == NULL) {
            AUDIO_ASSERT(0);
        }
    }
    if (p_environment_detection_pisplit_library_handle == NULL) {
        if (PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_get_handle(&p_environment_detection_pisplit_library_handle, &pisplit_environment_detection_det, environment_detection_load_library_callback)) {
            DSP_MW_LOG_I("pisplit_environment_detection_det preloader_pisplit_get_handle() error!!!!", 0);
            AUDIO_ASSERT(0);
        }
    }
    if (environment_detection_lib_loaded_counter == 0) {
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
        if(PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_load_v1(p_environment_detection_pisplit_library_handle, code_address, data_address)) {
#else
        if (PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_load(p_environment_detection_pisplit_library_handle, PRELOADER_I_HIGH_PERFORMANCE, PRELOADER_D_HIGH_PERFORMANCE)) {
#endif
            DSP_MW_LOG_I("pisplit_environment_detection_det preloader_pisplit_load() error!!!!", 0);
            AUDIO_ASSERT(0);
        }
        environment_detection_lib_loaded_counter = 1;
        if (xSemaphoreTake(environment_detection_load_lib_xSemaphore, portMAX_DELAY) == pdTRUE) {
        } else {
            AUDIO_ASSERT(0);
        }
        DSP_MW_LOG_I("environment_detection_lib_loaded_counter: %d \r\n", 1, environment_detection_lib_loaded_counter);
    } else {
        environment_detection_lib_loaded_counter++;
        //printf("environment_detection_library_load preloader_pisplit_load() had been loaded!!!");
    }
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
    preloader_pisplit_get_library_code_and_data_size(environment_detection_lib_loaded_counter, NULL, dram_pic_usage);
#else
    UNUSED(code_address);
    UNUSED(data_address);
...
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
        if(PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_unload_v1(p_environment_detection_pisplit_library_handle)) {
#else
        if (PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_unload(p_environment_detection_pisplit_library_handle)) {
#endif
            DSP_MW_LOG_I("pisplit_environment_detection_det preloader_pisplit_unload() error!!!!", 0);
            AUDIO_ASSERT(0);
        }
        environment_detection_export_parameters[0] = environment_detection_default_function_parameter;
        environment_detection_export_parameters[1] = environment_detection_default_function_parameter;
        environment_detection_export_parameters[2] = environment_detection_default_function_parameter;
