...
#ifdef AIR_ANC_ADAPTIVE_CLOCK_CONTROL_ENABLE
    dsp_adapt_anc_send_unstable_status(AUDIO_ADAPTIVE_ANC_CLK_CTRL_TYPE_ED, (uint8_t)noise_level);
#endif
    return false;
}
...
ATTR_TEXT_IN_IRAM_LEVEL_2 bool stream_function_environment_detection_initialize(void *para)
{
    ...
    if (!environment_detection_check_memory()) {
        ...
        if (DSP_ENVIRONMENT_DETECTION_INSTANCE_MEMSIZE+scratch_memory_size > stream_function_get_working_buffer_length(para)) {
            DSP_ANC_ED_LOG_E("[ED]MEM SIZE is insufficient require:%d, allocated:%d", 2, DSP_ENVIRONMENT_DETECTION_INSTANCE_MEMSIZE+scratch_memory_size, stream_function_get_working_buffer_length(para));
            assert(false);
        }
