...
//        DSP_MW_LOG_I("[RECORD_LC]report_cnt:%d, report_res:%d", 2, report_cnt, report_res);
        ...
        if (ld_aecref1_power != 0) {
            ...
            if (mean_power > (double)(ld_NvKey.ld_thrd)/32768) {
                //PASS
                report_res++;
                DSP_MW_LOG_I("[RECORD_LC]frame %d res:%d ->PASS (ld_iwxe1_power:%d, ld_aecref1_power:%d, fe_vad_cntr:%d)", 5, report_cnt, (int)(mean_power*32768), ld_iwxe1_power, ld_aecref1_power, fe_vad_cntr);
            } else {
                //FAIL
                DSP_MW_LOG_I("[RECORD_LC]frame %d res:%d ->FAIL (ld_iwxe1_power:%d, ld_aecref1_power:%d, fe_vad_cntr:%d)", 5, report_cnt, (int)(mean_power*32768), ld_iwxe1_power, ld_aecref1_power, fe_vad_cntr);
            }

        } else {
            //FAIL
            DSP_MW_LOG_I("[RECORD_LC]frame %d FAIL (ld_iwxe1_power:%d, ld_aecref1_power:%d, fe_vad_cntr:%d)", 1, report_cnt, ld_iwxe1_power, ld_aecref1_power, fe_vad_cntr);
        }
