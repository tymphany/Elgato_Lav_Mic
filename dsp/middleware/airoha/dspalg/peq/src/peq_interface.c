...
static VOID adaptive_eq_notify_mcu_index(S16 eq_index)
{
    ...
    DSP_MW_LOG_I("[AEQ][adaptive_eq_notify_mcu_index]adaptive_eq index = %d", 1, eq_index);
    ...
    if (AUDIO_MSG_STATUS_OK != aud_msg_tx_handler(msg, 0, FALSE)) {
        ...
        DSP_MW_LOG_I("[AEQ][adaptive_eq_notify_mcu_index]adaptive_eq index Success!", 0);
    }
}
#endif
...
void Adaptive_Eq_Set_Param(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    for (i = 0; i < adaptive_eq_number; i++) {
        ...
        if (i == 0) {
            ...
            DSP_MW_LOG_E("[Adaptive_Peq_Set_Param] Un-supported adaptive eq number: %d\n", 1, i);
            return;
        }
        ...
        if (MACRO_CHECK_AVAILABLE_NVKEY(peq_param[i].peq_nvkey_id) /*&& (p_peq_ctrl->peq_nvkey_id != peq_param->peq_nvkey_id)*/) {
            ...
            if (peq_param[i].peq_nvkey_id != 0) {
                ...
                if (status != 0) {
                    DSP_MW_LOG_E("[Adaptive_Peq_Set_Param] Get PEQ param error: status:%d  nvkeyID:%d \n", 2, status, p_peq_ctrl->peq_nvkey_id);
                    return;
                }
            }
        }
        if (p_peq_ctrl->peq_nvkey_id != 0) {
            DSP_MW_LOG_I("[Adaptive_Peq_Set_Param] Adaptive PEQ nvkeyID:0x%x\n", 1, p_peq_ctrl->peq_nvkey_id);
        } else {
            DSP_MW_LOG_I("[Adaptive_Peq_Set_Param] NO Adaptive PEQ PARAM\n", 0);
        }
    }
}
