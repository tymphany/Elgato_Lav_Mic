...
void *g_src_fixed_ratio_export_parameters[] = {
    src_fixed_ratio_default_function_parameter, /* For get_sampler_by_n_version() */
    src_fixed_ratio_default_function_parameter, /* For updn_sampling_Init() */
    src_fixed_ratio_default_function_parameter, /* For updn_sampling_16bit_Proc() */
    src_fixed_ratio_default_function_parameter, /* For updn_sampling_32bit_Proc() */
};
...
static void src_fixed_ratio_default_function_parameter(void)
{
    DSP_MW_LOG_E("[src_fixed_ratio] codec library not load or had been unload!!!", 0);
    AUDIO_ASSERT(0);
}
...
static void src_fixed_ratio_load_library_callback(preloader_pisplit_handle_t handle)
{
    ...
    if (p_export_parameters == NULL) {
        DSP_MW_LOG_E("[src_fixed_ratio] preloader_pisplit_get_export_parameter() fail", 0);
        AUDIO_ASSERT(0);
        return;
    }
...
uint32_t src_fixed_ratio_library_load(void *code_address, void *data_address, uint32_t *dram_pic_usage)
{
    ...
    if (error_handling != PRELOADER_PISPLIT_XTLIB_NO_ERR) {
        DSP_MW_LOG_E("[src_fixed_ratio] preloader_pisplit_get_handle() fail %d", 1, error_handling);
        AUDIO_ASSERT(0);
        return g_src_fixed_ratio_lib_loaded_counter;
    }
...
#endif
    if (error_handling != PRELOADER_PISPLIT_XTLIB_NO_ERR) {
        DSP_MW_LOG_E("[src_fixed_ratio] preloader_pisplit_load() fail %d", 1, error_handling);
...
uint32_t src_fixed_ratio_library_unload(void)
{
    ...
    if (PRELOADER_PISPLIT_XTLIB_NO_ERR != error_handling) {
        DSP_MW_LOG_E("[src_fixed_ratio] preloader_pisplit_unload() fail %d", 1, error_handling);
        AUDIO_ASSERT(0);
        return g_src_fixed_ratio_lib_loaded_counter;
    }
    error_handling = preloader_pisplit_release_handle(g_src_fixed_ratio_pisplit_library_handle);
    if (PRELOADER_PISPLIT_XTLIB_NO_ERR != error_handling) {
        DSP_MW_LOG_E("[src_fixed_ratio] preloader_pisplit_release_handle() fail %d", 1, error_handling);
        AUDIO_ASSERT(0);
        return g_src_fixed_ratio_lib_loaded_counter;
    }
    for (i = 0; i < (sizeof(g_src_fixed_ratio_export_parameters)/sizeof(void *)); i++) {
