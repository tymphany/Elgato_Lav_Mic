...
ATTR_TEXT_IN_IRAM_LEVEL_2 bool stream_function_wind_initialize(void *para)
{
    ...
    if (!wind_check_memory()) {
        ...
        if (need_size > stream_function_get_working_buffer_length(para)) {
            DSP_MW_LOG_E("[Wind Detection]MEM SIZE is insufficient require:%d, allocated:%d", 2, need_size, stream_function_get_working_buffer_length(para));
            assert(false);
        }
    ...
    if (wind_instance->nvkey_ready) {
        ...
        DSP_MW_LOG_I("[Wind Detection] initialize, nvkey.ENABLE = %d, Headset_project %d", 2, wind_instance->nvkey.ENABLE, stream_function_wind_check_headset());
    }
