...
#else
#if defined(AIR_GAMING_MODE_DONGLE_ENABLE) || defined(AIR_BLE_AUDIO_DONGLE_ENABLE) || defined(AIR_ULL_AUDIO_V2_DONGLE_ENABLE) || defined(AIR_ADVANCED_PASSTHROUGH_ENABLE) || defined(AIR_WIRELESS_MIC_RX_ENABLE) || defined(AIR_HEARING_AID_ENABLE)
...
void aud_msg_rx_handler(hal_ccni_event_t event, void *msg)
{
    ...
    if(pMsg[0] & 0x40000000){ /* If ccni_message[0] bit 30 = 1, it stands for an IRQ Level CCNI */
        ...
        aud_msg_node_t *irq_rx = (aud_msg_node_t *)pvPortMalloc(sizeof(aud_msg_node_t));
        if (irq_rx == NULL) {
            DSP_AUD_MSG_LOG_E("[IRQ_LEVEL_RX] AUDIO CCNI MSG malloc failed !!\r\n", 0);
