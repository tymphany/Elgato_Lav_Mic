...
#if defined AIR_GAMING_MODE_DONGLE_I2S_IN_ENABLE || defined AIR_GAMING_MODE_DONGLE_V2_I2S_SLV_IN_ENABLE || defined AIR_BLE_AUDIO_DONGLE_I2S_IN_ENABLE
    if ((source->scenario_type == AUDIO_SCENARIO_TYPE_GAMING_MODE_MUSIC_DONGLE_I2S_IN) ||
        (source->scenario_type == AUDIO_SCENARIO_TYPE_ULL_AUDIO_V2_DONGLE_DL_I2S_SLV_IN_0) ||
        (source->scenario_type == AUDIO_SCENARIO_TYPE_BLE_AUDIO_DONGLE_MUSIC_I2S_IN)) {
        DSP_MW_LOG_I("[Dongle I2S VDMA Mode] vdma addr 0x%x size %d threshold %d, src1 in_addr 0x%x size %d, src1 out_addr 0x%x size %d, src2 in_addr 0x%x size %d, src2 out_addr 0x%x size %d", 11,
            vdma_setting.base_address, vdma_setting.size, vdma_setting.threshold,
            AFE_GET_REG(ASM_IBUF_SADR), AFE_GET_REG(ASM_IBUF_SIZE), AFE_GET_REG(ASM_OBUF_SADR), AFE_GET_REG(ASM_OBUF_SIZE),
            AFE_GET_REG(ASM2_IBUF_SADR), AFE_GET_REG(ASM2_IBUF_SIZE), AFE_GET_REG(ASM2_OBUF_SADR), AFE_GET_REG(ASM2_OBUF_SIZE));
    }
#else
    if (runtime->audio_interface == HAL_AUDIO_INTERFACE_1) {
