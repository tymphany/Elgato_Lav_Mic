...
void DSP_Set_ASRC_Configuration_Parameters(VOID *para)
{
    ...
    if (DSP_RsChange2SRCOutRs(stream_ptr->callback.Src.in_resolution) == 4) {
        src_configuration.input_buffer.format = (hal_audio_pcm_format_t)AFE_PCM_FORMAT_S32_LE;
        src_configuration.output_buffer.format  = (hal_audio_pcm_format_t)AFE_PCM_FORMAT_S32_LE;
    } else {
        src_configuration.input_buffer.format =  (hal_audio_pcm_format_t)AFE_PCM_FORMAT_S16_LE;
        src_configuration.output_buffer.format =  (hal_audio_pcm_format_t)AFE_PCM_FORMAT_S16_LE;
    }
...
BOOL DSP_Callback_ASRC_Handle(VOID *para, VOID *buf_ptr1, VOID *buf_ptr2, U32 in_size)
{
    ...
    // printf("1 src_feature_ptr->inSRC_count %d src_feature_ptr->inSRC_ro %d src_feature_ptr->inSRC_wo %d in_size %d,ORD:%x,OWR:%x\n",src_feature_ptr->inSRC_count,src_feature_ptr->inSRC_ro,src_feature_ptr->inSRC_wo,in_size,AFE_READ(ASM_CH01_OBUF_RDPNT),AFE_READ(ASM_CH01_OBUF_WRPNT));
    ...
    src_feature_ptr->outSRC_count = (src_feature_ptr->outSRC_wo > src_feature_ptr->outSRC_ro) ?
                                    src_feature_ptr->outSRC_wo - src_feature_ptr->outSRC_ro :
                                    src_feature_ptr->outSRC_wo + src_feature_ptr->outSRC_mem_size - src_feature_ptr->outSRC_ro;
    // DSP_MW_LOG_I("bug pass outSRC_wo %d,outSRC_ro %d inSRC_count %d outSRC_count %d out_frame_size %d\n",5,src_feature_ptr->outSRC_wo,src_feature_ptr->outSRC_ro,
    // src_feature_ptr->inSRC_count,src_feature_ptr->outSRC_count,stream_ptr->callback.Src.out_frame_size);
    if (src_feature_ptr->outSRC_count > stream_ptr->callback.Src.out_frame_size * nch)//(stream_ptr->callback.Src.out_frame_size * nch)
    {
        ...
        // printf("2 outSRC_wo %d,outSRC_ro %d outSRC_count %d out_frame_size %d nch %d\n ",src_feature_ptr->outSRC_wo,src_feature_ptr->outSRC_ro,src_feature_ptr->outSRC_count,stream_ptr->callback.Src.out_frame_size,nch);        
...
void afe_audio_init_input_channel(void *owner, uint32_t channel_number, DSP_STREAMING_PARA_PTR stream_ptr)
{
#ifdef AIR_AUDIO_HARDWARE_ENABLE
...
void afe_audio_deinit_input_channel(void *owner)
{
    uint32_t i, irq_mask;
...
bool stream_function_fixed_gain_tdm_process(void *para)
{
    ...
    for (i = 1 ; i <= channel_number ; i++) {
#if 0 // for debug
        static U32 print_time = 0;
        print_time ++;
        if (print_time % 60 == 0) {
            printf("[TDM] sw gain feature channel = %d, buffer[15] = 0x%x", i, ((U8 *)stream_function_get_inout_buffer(para, i))[15]);
        }
#endif
        if (i == 1) {
...
bool stream_copy_to_virtual_sourc_process(void *para)
{
    U32 i, in_ChNum;
    in_ChNum = 0;
    if((Source_blks[SOURCE_TYPE_DSP_VIRTUAL] == NULL)||Source_blks[SOURCE_TYPE_DSP_VIRTUAL]->transform == NULL){
        return 0;
    }
    void *virtual_para = (void *)&((DSP_Callback_Get(Source_blks[SOURCE_TYPE_DSP_VIRTUAL], Source_blks[SOURCE_TYPE_DSP_VIRTUAL]->transform->sink))->EntryPara);
    ...
    U32 output_size = stream_function_get_output_size(para) < Source_blks[SOURCE_TYPE_DSP_VIRTUAL]->param.virtual_para.mem_size ?
                        stream_function_get_output_size(para) : Source_blks[SOURCE_TYPE_DSP_VIRTUAL]->param.virtual_para.mem_size;
    for (i = 0 ; i < channel_num ; i++) {
        if(Source_blks[SOURCE_TYPE_DSP_VIRTUAL]->streamBuffer.BufferInfo.startaddr[i] != NULL){
            memcpy(Source_blks[SOURCE_TYPE_DSP_VIRTUAL]->streamBuffer.BufferInfo.startaddr[i],
    ...
    Source_blks[SOURCE_TYPE_DSP_VIRTUAL]->param.virtual_para.is_dummy_data = false;
