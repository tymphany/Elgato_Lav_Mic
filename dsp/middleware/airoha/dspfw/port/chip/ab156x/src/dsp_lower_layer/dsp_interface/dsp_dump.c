...
void add_cfg_dump_id(DSP_DATADUMP_MASK_BIT dumpID)
{
    U32 i;
...
void remove_cfg_dump_id(DSP_DATADUMP_MASK_BIT dumpID)
{
    U32 i;
    for(i = 0; i < AUDIO_DUMP_CONFIG_MAX_NUM; i++){
...
void LOG_AUDIO_DUMP(U8 *audio, U32 audio_size, DSP_DATADUMP_MASK_BIT dumpID)
{
    ...
    if (check_cfg_dump_id(dump_id) || (DSP_AUDIO_DUMP_TEST && (dump_id > DSP_DATADUMP_TEST_ID_MIN))){
#endif
        if(!DSP_AUDIO_DUMP_TEST && (!DumpIdCfgVersion || DumpIdCfgVersion != DUMP_ID_VERSION)){
            DSP_MW_LOG_I("[Audio Dump] ID version don't match! FW ID version = %d",DUMP_ID_VERSION);
            return;
        }
        if (DSP_AUDIO_DUMP_TEST || AudioDumpDevice == AUDIO_DUMP_DEVICE_LOGGING) {
            ...
            while(left_size) {
                ...
                if (curr_send_size != left_send_size) {
                    DSP_MW_LOG_E("[Audio Dump] audio dump drop happen, cur packet total %d, sent %d", 2, left_send_size, curr_send_size);
