...
void DSP_GC_SetInputVolume(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    if (input_index == HAL_AUDIO_INPUT_GAIN_SELECTION_D0_A0) {
        ...
        DSP_MW_LOG_I("DSP_GC_SetInputVolume index %2d: a = [%d] (0x%x), d = [%d] (0x%x)", 5, input_index, (int16_t)gain_index1, gain_index1,
            (int16_t)gain_index0, gain_index0);
    } else {
        hal_audio_set_stream_in_volume_for_multiple_microphone(gain_index0, gain_index1, input_index);
        DSP_MW_LOG_I("DSP_GC_SetInputVolume index %2d: gain_index0 = [%d] (0x%x), gain_index1 = [%d] (0x%x)", 5, input_index, (int16_t)gain_index0,
            gain_index0, (int16_t)gain_index1, gain_index1);
    }
...
void DSP_GC_MuteOutput(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    DSP_MW_LOG_I("mute enable = %d, hw_gain_index = 0x%x", 2, enable, hw_gain_index);
...
{
    DSP_MW_LOG_I("************* ATTENTION: POWER OFF DAC Immediately *****************", 0);
    ...
    hal_audio_mute_stream_out(true, HAL_AUDIO_STREAM_OUT3);
#else
    ...
    if (dac_device == HAL_AUDIO_CONTROL_NONE) {
        DSP_MW_LOG_W("[POWER OFF] dac is not open. type %d", 1, dac_device);
        return;
    }
    ...
    DSP_MW_LOG_I("[POWER OFF] dac rate:[%d] dl_sdm_setting:[%d] performance:[%d] dac_mode:[%d] dac_device[0x%x]", 5,
                 dev_handle.dac.rate,
