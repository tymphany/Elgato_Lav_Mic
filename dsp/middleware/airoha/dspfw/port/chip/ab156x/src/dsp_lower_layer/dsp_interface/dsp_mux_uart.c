...
void dsp_uart_rx(uart_type_t type, U8 * user_rx_buf, U32 buf_size)
{
    ...
    if (mux_status != MUX_STATUS_OK) {
        DSP_MW_LOG_E("[DCHS][uart callback] dsp uart rx fail,status=%d", 1, mux_status);
        AUDIO_ASSERT(0);
        return;
    }
    if(rx_size != buf_size){
        DSP_MW_LOG_E("[DCHS][uart callback] dsp uart rx fail,already_rx_size=%d,need_rx_size=%d", 2, rx_size, buf_size);
        AUDIO_ASSERT(0);
        return;
    }
