...
void dsp_vow_init(void)
{
    ...
    if (dsp_vow) {
        ...
        dsp_vow->bias_voltage[0] = HAL_AUDIO_BIAS_VOLTAGE_1_85V;
        ...
        DSP_MW_LOG_I("DSP VOW Malloc Fail ", 0);
        AUDIO_ASSERT(0);
    }
#endif
}
...
static void vow_source_device_set_para(hal_audio_device_parameter_t *device_handle, int device_num)
{
    if (record_if.source != NULL) {
        if(device_num == 0) {
            ...
            DSP_MW_LOG_E("DSP VOW get record_if device_num error", 0);
            OS_ASSERT(0);
        }
    } else {
        DSP_MW_LOG_I("Should never come here", 0);
        OS_ASSERT(0);
    }
}
...
int vow_enable(hal_audio_device_parameter_vow_t *vow, SOURCE source)
{
    ...
    if (!vow) {
        dsp_vow_init();
        DSP_MW_LOG_I("VOW set default parameters", 0);
    } else {
        dsp_vow = vow;
    }
#endif
    ...
    if ((!vow_mic_use_count) && (!vow_mic1_use_count) && dsp_vow->suspend_mic != true) {
        ...
        if (dsp_vow->suspend_mic) {
            ...
            DSP_MW_LOG_I("DSP VOW Suspend MIC mic_selection %d,device_interface %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
            ...
            DSP_MW_LOG_I("DSP VOW Suspend MIC mic_selection1 %d,device_interface1 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
            hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_OFF);
            vow_source_device_set_para(&device_handle_in, 2);
            DSP_MW_LOG_I("DSP VOW Suspend MIC mic_selection2 %d,device_interface2 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
            hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_OFF);
            vow_source_device_set_para(&device_handle_in, 3);
            DSP_MW_LOG_I("DSP VOW Suspend MIC mic_selection3 %d,device_interface3 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
            hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_OFF);
#ifdef MTK_AUDIO_HW_IO_CONFIG_ENHANCE
            vow_source_device_set_para(&device_handle_in, 4);
            DSP_MW_LOG_I("DSP VOW Suspend MIC mic_selection4 %d,device_interface4 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
            hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_OFF);
            ...
            DSP_MW_LOG_I("DSP VOW Suspend MIC mic_selection5 %d,device_interface5 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
            hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_OFF);
#endif
#endif
            //Enable VOW
            if ((dsp_vow->mic_selection != HAL_AUDIO_CONTROL_NONE) || (dsp_vow->mic1_selection != HAL_AUDIO_CONTROL_NONE)) {
                ...
                HAL_AUDIO_LOG_ERROR("DSP - Error Hal Audio VOW Wrong Device :%d , Dvice1: %d!", 1, dsp_vow->mic_selection, dsp_vow->mic1_selection);
                AUDIO_ASSERT(false);
            }
        } else {
            if ((dsp_vow->mic_selection)&HAL_AUDIO_CONTROL_DEVICE_ANALOG_MIC_DUAL) {
                ...
                HAL_AUDIO_LOG_ERROR("DSP - Error Hal Audio VOW concurrent mode only for AMIC :%d !", 1, dsp_vow->mic_selection);
                AUDIO_ASSERT(false);
            }
        }
    }
...
int vow_disable(hal_audio_device_parameter_vow_t *vow, SOURCE source)
{
    ...
    if (dsp_vow->suspend_mic) {
        if ((dsp_vow->mic_selection)&HAL_AUDIO_CONTROL_DEVICE_DIGITAL_MIC_DUAL) {
            ...
            HAL_AUDIO_LOG_ERROR("DSP - Error Hal Audio VOW Wrong Device :%d !", 1, dsp_vow->mic_selection);
            AUDIO_ASSERT(false);
        }
        // device_handle_in.common.audio_device = dsp_vow->mic_selection;
        // device_handle_in.common.device_interface = dsp_vow->mic_interface;
        ...
        vow_source_device_set_para(&device_handle_in, 0);
        //Resume MIC
        DSP_MW_LOG_I("DSP VOW Resume MIC mic_selection %d,device_interface %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
        hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_ON);
#ifdef AIR_AUDIO_SUPPORT_MULTIPLE_MICROPHONE
        vow_source_device_set_para(&device_handle_in, 1);
        DSP_MW_LOG_I("DSP VOW Resume MIC mic_selection1 %d,device_interface1 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
        hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_ON);
        vow_source_device_set_para(&device_handle_in, 2);
        DSP_MW_LOG_I("DSP VOW Resume MIC mic_selection2 %d,device_interface2 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
        hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_ON);
        vow_source_device_set_para(&device_handle_in, 3);
        DSP_MW_LOG_I("DSP VOW Resume MIC mic_selection3 %d,device_interface3 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
        hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_ON);
#ifdef MTK_AUDIO_HW_IO_CONFIG_ENHANCE
        vow_source_device_set_para(&device_handle_in, 4);
        DSP_MW_LOG_I("DSP VOW Resume MIC mic_selection4 %d,device_interface4 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
        hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_ON);
        /*to match the phase1's configuration in "mcu\driver\chip\AIR_BTA_IC_PREMIUM_G3\src\hal_audio.c\hal_audio_get_stream_in_setting_config()", the device number ends at 5*/
        vow_source_device_set_para(&device_handle_in, 5);
        DSP_MW_LOG_I("DSP VOW Resume MIC mic_selection5 %d,device_interface5 %d", 2, device_handle_in.common.audio_device, device_handle_in.common.device_interface);
        hal_audio_set_device(&device_handle_in, device_handle_in.common.audio_device, HAL_AUDIO_CONTROL_ON);
#endif
#endif
        //Suspend MEM
...
int vow_sink(hal_audio_device_parameter_vow_t *vow, SOURCE source)
{
    ...
    DSP_MW_LOG_I("DSP vow_sink set mem done", 0);
    ...
    DSP_MW_LOG_I("DSP vow_sink set device done", 0);
    ...
    DSP_MW_LOG_I("DSP vow_sink set path done", 0);
