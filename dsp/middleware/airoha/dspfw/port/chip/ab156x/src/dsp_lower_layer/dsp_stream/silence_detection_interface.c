...
void Sink_Audio_SilenceDetection(VOID* SrcBuf, U16 CopySize, SINK sink)
{
    ...
    if ((afe_adc_performance_mode[AFE_ANALOG_DAC] == AFE_PEROFRMANCE_NORMAL_MODE) &&
    #if defined(AIR_BTA_IC_PREMIUM_G2)
    (afe_analog_gain[AFE_HW_ANALOG_GAIN_OUTPUT].analog_mode == HAL_AUDIO_ANALOG_OUTPUT_CLASSG) && (!hal_audio_agent_user_count[HAL_AUDIO_AGENT_DEVICE_ANC])) {
    #else
    (afe_analog_gain[AFE_HW_ANALOG_GAIN_OUTPUT].analog_mode == HAL_AUDIO_ANALOG_OUTPUT_CLASSG2) && (!hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_ANC))) {
    #endif
        //if (!SD_Parameter.NvKey.NLE_isEnable) { //mark out because pop noise
           // hal_audio_dl_set_nle_enable(TRUE);
        //}
        SD_Parameter.NvKey.NLE_isEnable = TRUE;
    ...
    if (sink->scenario_type == AUDIO_SCENARIO_TYPE_VP) {
        ...
        if (sink->scenario_type == AUDIO_SCENARIO_TYPE_A2DP) {
    ...
    if((SD_Parameter.NvKey.APO_isEnable || SD_Parameter.NvKey.EAO_isEnable || SD_Parameter.NvKey.NLE_isEnable || SD_Parameter.NvKey.RSDM_isEnable)){
        ...
        if(SD_Parameter.NvKey.NLE_isEnable){
            ...
            #if defined(AIR_BTA_IC_PREMIUM_G2)
            if ((hal_audio_agent_user_count[HAL_AUDIO_AGENT_DEVICE_SIDETONE]>0)
#ifdef MTK_ANC_ENABLE
            || (hal_audio_agent_user_count[HAL_AUDIO_AGENT_DEVICE_ANC]>0)
#endif
            #else
            if (hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_SIDETONE)
#ifdef MTK_ANC_ENABLE
            || (hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_ANC)&& (!g_anc_nle_apply))
#endif
            #endif
            ){
                SD_Parameter.NLE_First_Silence = TRUE;
            ...
            } else if (((afe_digital_gain[AFE_HW_DIGITAL_GAIN1].register_value == 0) || (afe_digital_gain[AFE_HW_DIGITAL_GAIN1].register_value == 0xFFFFFFFF)) &&
        ...
        if (format == 2) {//16bits
            ...
            for (i = 0; i < SampleCnt; i++) {
                ...
                if(abs(*(Buf_16+i)) > abs(*NLE_Max)){
                    *NLE_Max = abs(*(Buf_16+i));
                }

                if((*(Buf_16+i)) > *RSDM_Max){
                    *RSDM_Max = (*(Buf_16+i));
                }
                ...
                if(SD_Parameter.NvKey.NLE_isEnable && (abs(*(Buf_16+i)) >= abs(NLE_TH)) && (NLE_MUTEX == FALSE)){
                    SD_Parameter.NLE_First_Silence = TRUE;
                    if ( SD_Parameter.NLE_SilenceFlag == TRUE) {
                        SD_Parameter.NLE_SilenceFlag = FALSE;
                        NLE_FunPtr(SD_Parameter.NLE_SilenceFlag);
                ...
                if(SD_Parameter.NvKey.RSDM_isEnable && (abs(*(Buf_16+i)) > 0) && (RSDM_MUTEX == FALSE)){
                    SD_Parameter.RSDM_First_Silence = TRUE;
                    if (SD_Parameter.RSDM_SilenceFlag == TRUE) {
                        SD_Parameter.RSDM_SilenceFlag = FALSE;
                    }
            ...
            for (i = 0; i < SampleCnt; i++) {
                ...
                if(abs(*(Buf_32+i)) > abs(*NLE_Max)){
                    *NLE_Max = abs(*(Buf_32+i));
                }

                if((*(Buf_32+i)) > *RSDM_Max){
                    *RSDM_Max = (*(Buf_32+i));
                }
                ...
                if(SD_Parameter.NvKey.NLE_isEnable && (abs(*(Buf_32+i)) >= abs(NLE_TH)) && (NLE_MUTEX == FALSE)){
                    SD_Parameter.NLE_First_Silence = TRUE;
                    if (SD_Parameter.NLE_SilenceFlag == TRUE) {
                        SD_Parameter.NLE_SilenceFlag = FALSE;
                        //DSP_MW_LOG_I("#[NLE DEBUG] 1# %d, %d, %d",3,abs(*(Buf_32+i)),abs(NLE_TH),i);
                        NLE_FunPtr(SD_Parameter.NLE_SilenceFlag);
                    }
                }
                if(SD_Parameter.NvKey.RSDM_isEnable && (abs(*(Buf_32+i)) > 0) && (RSDM_MUTEX == FALSE)){
                    SD_Parameter.RSDM_First_Silence = TRUE;
                    if (SD_Parameter.RSDM_SilenceFlag == TRUE) {
                        SD_Parameter.RSDM_SilenceFlag = FALSE;
                    }
        ...
        if(SD_Parameter.Mutex == FALSE){
            ...
            if((SD_Parameter.NvKey.NLE_isEnable) && (NLE_MUTEX == FALSE)){
                ...
                if((abs(SD_Parameter.NLE_MaxValue_32) < abs(SD_Parameter.NLE_TH_32)) && (abs(SD_Parameter.NLE_MaxValue_16) < abs(SD_Parameter.NLE_TH_16))){
            ...
            if((SD_Parameter.NvKey.RSDM_isEnable) && (RSDM_MUTEX == FALSE)){
                if((SD_Parameter.RSDM_MaxValue_32 == 0) && (SD_Parameter.RSDM_MaxValue_16 == 0)){
