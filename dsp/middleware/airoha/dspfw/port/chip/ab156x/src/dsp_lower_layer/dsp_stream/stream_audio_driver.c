...
ATTR_TEXT_IN_IRAM_LEVEL_1 VOID AudioCheckTransformHandle(TRANSFORM transform)
{
    if (transform != NULL) {
        ...
        if (transform->source->taskId == DHP_TASK_ID) {
            xTaskResumeFromISR(pDHP_TaskHandler);
        } else if (transform->source->taskId == DAV_TASK_ID) {
        ...
        else if (transform->source->taskId == DTDM_TASK_ID) {
            xTaskResumeFromISR(pDTDM_TaskHandler);
        }
#endif
#ifdef AIR_DCHS_MODE_ENABLE
        else if (transform->source->taskId == DDCHS_TASK_ID) {
            xTaskResumeFromISR(pDDCHS_TaskHandler);
        }
#endif
        else {
...
hal_audio_interconn_selection_t stream_audio_convert_control_to_interconn(hal_audio_control_t audio_control, hal_audio_path_port_parameter_t port_parameter, uint32_t connection_sequence, bool is_input)
{
    ...
    switch (audio_control) {
        ...
        case HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_L:
            if (audio_control == HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_L) {
...
VOID Sink_Audio_BufferInfo_Rst(SINK sink, U32 offset)
{
    ...
    if (sink->buftype == BUFFER_TYPE_CIRCULAR_BUFFER) {
        ...
        if (g_n9_ble_dl_open_flag == true) {
            ...
            U16 prefill_ms = (g_n9_ble_dl_gaming_flag == true) ? 5 : (g_n9_ble_dl_ull_flag == true) ? 2 : (sink->param.audio.period == 10) ? 10 : 15;
            if (sink->param.audio.AfeBlkControl.u4asrcflag) {
        ...
        if (sink->param.audio.mem_handle.scenario_type == AUDIO_SCENARIO_TYPE_WIRELESS_MIC_RX_UL_I2S_SLV_OUT_0) {
            if (sink->param.audio.AfeBlkControl.u4asrcflag) {
                afe_prefill = WIRELESS_MIC_RX_RESERVED_PROCESS_TIME * (sink->param.audio.src_rate * sink->param.audio.format_bytes * sink->param.audio.channel_num * sink->param.audio.period) / 1000;
            } else {
                afe_prefill = WIRELESS_MIC_RX_RESERVED_PROCESS_TIME * (sink->param.audio.rate * sink->param.audio.format_bytes * sink->param.audio.channel_num * sink->param.audio.period) / 1000;
            }
...
VOID Source_Audio_BufferInfo_Rst(SOURCE source, U32 offset)
{
    ...
    if (source->buftype == BUFFER_TYPE_CIRCULAR_BUFFER) {
        ...
        else if((source->transform->sink->type >= SINK_TYPE_AUDIO_TRANSMITTER_MIN)&&(source->transform->sink->type >= SINK_TYPE_AUDIO_TRANSMITTER_MAX)
            && (source->transform->sink->param.data_ul.scenario_type == AUDIO_TRANSMITTER_WIRED_AUDIO)
            && ((source->transform->sink->param.data_ul.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT)||(source->transform->sink->param.data_ul.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM))) {
                preFillSize = 2 * source->param.audio.channel_num * source->param.audio.format_bytes; //prefill 2 samples
        }
#endif
        source->streamBuffer.BufferInfo.WriteOffset = (offset) % source->streamBuffer.BufferInfo.length;
            ...
            while (length > copy_offset) {
                ...
                if (sink->param.audio.format_bytes == 4) {
                    ...
                    if(sink->scenario_type == AUDIO_SCENARIO_TYPE_DCHS_UART_DL && dchs_get_device_mode() == DCHS_MODE_RIGHT){
                        U8* silence_buf = (U8*)malloc(copy_size);
                        memset(silence_buf, 0, copy_size);
                ...
                if(dchs_get_device_mode() != DCHS_MODE_SINGLE){
                    if(transform->source->type == SOURCE_TYPE_A2DP || transform->source->type == SOURCE_TYPE_N9SCO
                        ...
                        || transform->source->param.n9ble.context_type == BLE_CONTEXT_CONVERSATIONAL
                            ...
                            U32 align_copy_size   = copy_size / 4 * 4;
                            U32 align_copy_offset = copy_offset / 4 * 4;
                            //DSP_MW_LOG_I("[DCHS DL][debug] copy_offset=%d,copy_size=%d,out_ptr[i]=0x%x,out_ptr[i + 1]=0x%x",4,copy_offset,copy_size,callback_ptr->EntryPara.out_ptr[i] + copy_offset,callback_ptr->EntryPara.out_ptr[i + 1] + copy_offset);
                            dsp_converter_16bit_to_32bit((int32_t *)L_ch, (int16_t *)(callback_ptr->EntryPara.out_ptr[i] + align_copy_offset),     align_copy_size / sizeof(int16_t));
                            dsp_converter_16bit_to_32bit((int32_t *)R_ch, (int16_t *)(callback_ptr->EntryPara.out_ptr[i + 1] + align_copy_offset), align_copy_size / sizeof(int16_t));
                            DSP_D2I_BufferCopy_32bit((U32 *)L_and_R_ch,
                                                (U32 *)L_ch,
                                                (U32 *)R_ch,
                                                (U32)(align_copy_size * 2) >> 2); //data double
                            U32 compensate_size = copy_size - align_copy_size;
                            dsp_uart_tx(UART_DL,(U8 *)L_and_R_ch, (align_copy_size + compensate_size) * 4); //uart relay data
                            LOG_AUDIO_DUMP((U8 *)L_and_R_ch, align_copy_size * 4, AUDIO_DCHS_DL_DATA_SOURCE);
                        }else{
