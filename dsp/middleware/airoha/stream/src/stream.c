 ...
 * @return The Sink ID associated with the audio hardware.
 */
SOURCE StreamVirtualSource(VOID *entry, U32 report_length, U8 channel_num)
{
    SOURCE source;
    U8 *mem_ptr;

    if (Source_blks[SOURCE_TYPE_DSP_VIRTUAL] != NULL) {
        return Source_blks[SOURCE_TYPE_DSP_VIRTUAL];
    }

    source = new_source(SOURCE_TYPE_DSP_VIRTUAL);
    if (source == NULL) {
        return NULL;
    }
    source->taskId = DAV_TASK_ID;
    source->buftype               = BUFFER_TYPE_CIRCULAR_BUFFER;
    source->type                  = SOURCE_TYPE_DSP_VIRTUAL;
    ...
    source->param.virtual_para.is_processed = true; //not process
...
SOURCE StreamN9BleSource(void *param)
{
    ...
    if (open_param->stream_in_param.ble.dual_cis_mode == DUAL_CIS_MODE) {
        ...
        source->param.n9ble.sub_share_info_base_addr = (n9_dsp_share_info_ptr)((source->param.n9ble.sub_share_info_base_addr->start_addr) | 0x60000000);
#else
        source->param.n9ble.sub_share_info_base_addr = (n9_dsp_share_info_ptr)hal_memview_cm4_to_dsp0((U32)open_param->stream_in_param.ble.p_sub_share_info->start_addr);
#endif
    ...
    if (open_param->stream_in_param.ble.codec_type == BT_BLE_CODEC_LC3)
    {
        ...
        {
            LC3I_Set_Dec_Param(1, source->param.n9ble.frame_interval / 100, plc_mode, source->streamBuffer.ShareBufferInfo.sampling_rate, (source->param.n9ble.frame_length * 8) * (1000 * 10) / (source->param.n9ble.frame_interval / 100));
        } else {
            LC3I_Set_Dec_Param(open_param->stream_in_param.ble.channel_num, source->param.n9ble.frame_interval / 100, plc_mode, source->streamBuffer.ShareBufferInfo.sampling_rate, (source->param.n9ble.frame_length * 8) * (1000 * 10) / (source->param.n9ble.frame_interval / 100));
        }
        #endif
    }
    #ifdef AIR_BT_LE_LC3PLUS_ENABLE
...
SOURCE StreamUartSource(void *param)
{
    ...
    source->param.audio.format_bytes = (open_param->stream_in_param.afe.format > AFE_PCM_FORMAT_U16_BE) ? 4 : 2;
