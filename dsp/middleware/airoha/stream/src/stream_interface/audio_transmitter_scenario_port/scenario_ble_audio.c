...
#ifdef AIR_SILENCE_DETECTION_ENABLE
static void ble_audio_dongle_dl_software_timer_handler(void *user_data)
{
    SOURCE source = NULL;
...
static void ble_audio_dongle_dl_software_timer_start(ble_audio_dongle_dl_handle_t *handle)
{
    uint32_t saved_mask;
    UNUSED(handle);
...
static void ble_audio_dongle_dl_software_timer_stop(ble_audio_dongle_dl_handle_t *handle)
{
    uint32_t saved_mask;
    UNUSED(handle);
                        ...
                        if (runtime->irq_exist == false) {
                            ...
                            if (source->scenario_type == AUDIO_SCENARIO_TYPE_BLE_AUDIO_DONGLE_MUSIC_I2S_IN) {
                                if ((buffer_info->WriteOffset & (~15)) == (buffer_info->ReadOffset & (~15))) {
                                    BLE_LOG_E("[ble audio dongle][dl][afe in] hwsrc start fail owo[%d] oro[%d]", buffer_info->WriteOffset, buffer_info->ReadOffset);
                                }
                            }
                        } else {
...
void ble_audio_dongle_dl_init(SOURCE source, SINK sink, audio_transmitter_open_param_p audio_transmitter_open_param, bt_common_open_param_p bt_common_open_param)
{
    ...
    src_fixed_ratio_config_t sw_src_fixed_ratio_config;
    ...
    if ((source->scenario_type == AUDIO_SCENARIO_TYPE_BLE_AUDIO_DONGLE_MUSIC_LINE_IN) ||
        ...
        sw_src_fixed_ratio_config.out_sampling_rate = bt_common_open_param->scenario_param.ble_audio_dongle_param.codec_param.lc3.sample_rate;
    }
...
ATTR_TEXT_IN_IRAM uint32_t ble_audio_dongle_dl_source_copy_payload(SOURCE source, uint8_t *src_buf, uint8_t *dst_buf, uint32_t length)
{
    ...
    if (source->param.data_dl.current_notification_index == 0) {
        ...
        while (current_frame < dongle_handle->usb_min_start_frames) {
            ...
            if (dongle_handle->usb_sample_format == AFE_PCM_FORMAT_S16_LE)
            {
                /* get samples */
            ...
            else if (dongle_handle->usb_sample_format == AFE_PCM_FORMAT_S24_LE)
            {
                /* get samples */
...
/******************************************************************************/
#ifdef AIR_SILENCE_DETECTION_ENABLE
void ble_dongle_silence_detection_init(ble_audio_dongle_dl_handle_t *dongle_handle, audio_scenario_type_t scenario)
{
    uint32_t saved_mask;
...
void ble_dongle_silence_detection_deinit(ble_audio_dongle_dl_handle_t *dongle_handle, audio_scenario_type_t scenario)
{
    uint32_t saved_mask;
    volume_estimator_port_t *port;
    ble_audio_dongle_silence_detection_handle_t *silence_detection_handle = NULL;
