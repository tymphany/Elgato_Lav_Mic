...
static SOURCE g_1st_usb_in_source = NULL;
sw_mixer_member_t *g_usb_in_out_mixer_member;
static SOURCE g_usb_in_out_source = NULL;
...
static uint32_t internal_fs_converter(stream_samplerate_t fs)
{
    switch (fs) {
        ...
        default:
            DSP_MW_LOG_E("[DSP][eSCO] sample rate is not supported!", 0);
            AUDIO_ASSERT(false);
            return fs;
    }
}
...
static wired_audio_handle_t *wired_audio_alloc_handle(void *owner)
{
    uint32_t saved_mask;
    int16_t count;
...
static void wired_audio_release_handle(wired_audio_handle_t *handle)
{
...
WIRED_AUDIO_ATTR_TEXT_IN_IRAM void wired_audio_usb_in_mixer_postcallback(sw_mixer_member_t *member, void *para, uint32_t *out_frame_size)
{
    int32_t i;
    wired_audio_handle_t *c_handle = NULL;
    SOURCE source = NULL;
    ...
    if ((*out_frame_size != 0) && (all_streams_in_mixer == true)) {
        ...
        c_handle = g_wired_audio_first_dl_handle;
        for (i = 0; i < g_wired_audio_first_dl_handle->total_number; i++) {
            // if (c_handle->mixer_status != WIRED_AUDIO_STREAM_MIXER_UNMIX)
            // {
...
WIRED_AUDIO_ATTR_TEXT_IN_IRAM void wired_audio_usb_in_out_mixer_postcallback(sw_mixer_member_t *member, void *para, uint32_t *out_frame_size)
{
    UNUSED(member);
    UNUSED(para);
    ...
    if ((*out_frame_size != 0)) {
        ...
        stream_function_sw_mixer_member_input_buffers_clean(g_usb_in_out_mixer_member, false);
    }
}
...
void wired_audio_usb_in_vol_balance_init(SOURCE source)
{
    ...
    if ((source->param.data_dl.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0)||(source->param.data_dl.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0_IEM)) {
...
static void wired_audio_usb_in_start(mcu2dsp_start_param_p start_param, SOURCE source, SINK sink)
{
    ...
    #ifdef AIR_DCHS_MODE_ENABLE
    if(dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_1].data_scenario_type == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_0){
...
void wired_audio_usb_in_source_init(SOURCE source, audio_transmitter_open_param_p open_param, AUDIO_TRANSMITTER_PARAMETER *source_param)
{
    ...
    if ((open_param->scenario_sub_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0) &&
        ...
        (open_param->scenario_sub_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0_IEM)) {
    ...
    if (codec_pcm->format == AFE_PCM_FORMAT_S24_LE) {
        sample_bits = 32;
        sample_byte = 4;
    ...
    if (g_1st_usb_in_source == NULL) {
        g_1st_usb_in_source = source;
    }

#if defined(AIR_WIRED_AUDIO_USB_IN_PASS_THROUGH_ENABLE)
    if (open_param->scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0) {
...
#ifndef AIR_WIRED_AUDIO_USB_IN_PASS_THROUGH_ENABLE
    /* SW src config */
    ...
    smp_config.out_sampling_rate = WIRED_AUDIO_MIXING_MODE_DL_SAMPLE_RATE;
    ...
    /* SW mixer config, fix to 96K or 48K depend on WIRED_AUDIO_MIXING_MODE_DL_SAMPLE_RATE */
    ...
    if ((open_param->scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0) ||
        ...
        src_dsp_frame_sample = (WIRED_AUDIO_MIXING_MODE_DL_SAMPLE_RATE * codec_pcm->frame_interval) / (1000 * 1000);

        stream_function_sw_mixer_init(SW_MIXER_PORT_0);

        callback_config.preprocess_callback = NULL;
        callback_config.postprocess_callback = wired_audio_usb_in_mixer_postcallback;//gaming_mode_dongle_mixer_postcallback;
        ...
        if (wired_audio_get_usb_audio_start_number() == 0) {
            ...
            stream_function_sw_mixer_member_register(SW_MIXER_PORT_0, handle->mixer_member, true);
            WIRED_AUDIO_LOG_MSGID_I("[config][start] mix first. \r\n", 0);
        } else {
            uint32_t saved_mask;
            ...
            /* mix stream */
            dongle_handle_0 = (wired_audio_handle_t *)(g_1st_usb_in_source->param.data_dl.scenario_param.usb_in_local_param.usb_in_param.handle);
            ...
            dongle_handle_1->mixer_status = WIRED_AUDIO_STREAM_MIXER_MIX;
    ...
    } else if(open_param->scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0_IEM) {
        sw_mixer_callback_config_t       callback_config;
        sw_mixer_input_channel_config_t  in_ch_config;
        sw_mixer_output_channel_config_t out_ch_config;
        src_dsp_frame_sample = (WIRED_AUDIO_MIXING_MODE_DL_SAMPLE_RATE * codec_pcm->frame_interval) / (1000 * 1000);
        ...
        WIRED_AUDIO_LOG_MSGID_I("[USB_IN][start] sw mixer init: %d, %d, %d, %d, %d, %d", 6,
                                ...
                                out_ch_config.total_channel_number,
                                out_ch_config.resolution);
        if ((wired_audio_get_usb_audio_start_number() == 0) && (g_usb_in_out_source == NULL)) {
            handle->mixer_member = stream_function_sw_mixer_member_create((void *)source,
                                                                                    SW_MIXER_MEMBER_MODE_NO_BYPASS,
                                                                                    &callback_config,
                                                                                    &in_ch_config,
            ...
            /* do connections */
            if(g_usb_in_out_source == NULL) {
                stream_function_sw_mixer_channel_connect(dongle_handle_0->mixer_member, 1, SW_MIXER_CHANNEL_ATTRIBUTE_MAIN, dongle_handle_0->mixer_member, 1);
                stream_function_sw_mixer_channel_connect(dongle_handle_0->mixer_member, 2, SW_MIXER_CHANNEL_ATTRIBUTE_MAIN, dongle_handle_0->mixer_member, 2);
                stream_function_sw_mixer_channel_connect(dongle_handle_1->mixer_member, 1, SW_MIXER_CHANNEL_ATTRIBUTE_NORMAL, dongle_handle_0->mixer_member, 1);
                stream_function_sw_mixer_channel_connect(dongle_handle_1->mixer_member, 2, SW_MIXER_CHANNEL_ATTRIBUTE_NORMAL, dongle_handle_0->mixer_member, 2);
            } else {
                stream_function_sw_mixer_channel_connect(g_usb_in_out_mixer_member, 1, SW_MIXER_CHANNEL_ATTRIBUTE_MAIN, dongle_handle_0->mixer_member, 1);
                stream_function_sw_mixer_channel_connect(g_usb_in_out_mixer_member, 2, SW_MIXER_CHANNEL_ATTRIBUTE_MAIN, dongle_handle_0->mixer_member, 2);
                stream_function_sw_mixer_channel_connect(dongle_handle_0->mixer_member, 1, SW_MIXER_CHANNEL_ATTRIBUTE_NORMAL, g_usb_in_out_mixer_member, 1);
                stream_function_sw_mixer_channel_connect(dongle_handle_0->mixer_member, 2, SW_MIXER_CHANNEL_ATTRIBUTE_NORMAL, g_usb_in_out_mixer_member, 2);
                // if (dongle_handle_0 != dongle_handle_1) {
                //     stream_function_sw_mixer_channel_connect(dongle_handle_1->mixer_member, 1, SW_MIXER_CHANNEL_ATTRIBUTE_NORMAL, dongle_handle_0->mixer_member, 1);
                //     stream_function_sw_mixer_channel_connect(dongle_handle_1->mixer_member, 2, SW_MIXER_CHANNEL_ATTRIBUTE_NORMAL, dongle_handle_0->mixer_member, 2);
                // }
...
void wired_audio_usb_in_out_source_init(SOURCE source)
{
    ...
    //     g_1st_usb_in_source = source;
    // } else {
    //     WIRED_AUDIO_LOG_MSGID_E("[USB_IN_OUT]: usb in(0x%x) coming before usb_in_out", 1, g_1st_usb_in_source);
    //     AUDIO_ASSERT(0);
    // }
    g_usb_in_out_source = source;

    uint32_t src_dsp_frame_sample = (WIRED_AUDIO_MIXING_MODE_DL_SAMPLE_RATE * WIRED_AUDIO_DL_PROCESS_PERIOD) / (1000);
    stream_function_sw_mixer_init(SW_MIXER_PORT_0);

    callback_config.preprocess_callback = NULL;
    callback_config.postprocess_callback = wired_audio_usb_in_out_mixer_postcallback;//gaming_mode_dongle_mixer_postcallback;
    ...
    WIRED_AUDIO_LOG_MSGID_I("[USB_IN_OUT][config][start] mix first. \r\n", 0);
}
        ...
        if ((avail_data_sample_by_channel >= (check_data_sample_by_channel + dsp_frame_samples_by_channel - usb_frame_samples_by_channel)) &&
            ...
            *avail_size = handle->copy_block_size;
            ...
            if (avail_data_sample_by_channel < (dsp_frame_samples_by_channel - usb_frame_samples_by_channel)) {
                /* Too less data */
                WIRED_AUDIO_LOG_MSGID_W("[USB_IN][source_get_avail_size][sub_id %d]: AVM buffer avail data is too less, actual avm sample: %d", 2,
                                            source->param.data_dl.scenario_sub_id, avail_data_sample_by_channel);
            } else {
                /* Too many data */
                WIRED_AUDIO_LOG_MSGID_W("[USB_IN][source_get_avail_size][sub_id %d]: AVM buffer avail data is too many, actual avm sample: %d", 2,
                                            source->param.data_dl.scenario_sub_id, avail_data_sample_by_channel);
            }
            source->param.data_dl.scenario_param.usb_in_local_param.is_dummy_data = true;
            handle->copy_block_size = 0;
...
#ifdef WIRED_AUDIO_DEBUG_LOG
    WIRED_AUDIO_DEBUG_LOG_MSGID_I("[USB_IN][source_get_avail_size][sub_id %d]: avail number %d, AVM process number %d, in size %d, read_offset %d, read_offset(fix) %d, write_offset = %d", 7,
                                source->param.data_dl.scenario_sub_id,
...
static void wired_audio_usb_in_source_clock_skew_check(SOURCE source)
{
    ...
    if (handle->compen_method == WIRED_AUDIO_COMPENSATORY_METHOD_SW_CLK_SKEW) {
        ...
        WIRED_AUDIO_LOG_MSGID_I("[USB_IN][source_clock_skew_check][sub_id %d]: do clkskew %d, %d", 4, source->param.data_dl.scenario_sub_id, source->param.data_dl.scenario_sub_id, handle->total_compen_samples, compensatory_samples);
...
WIRED_AUDIO_ATTR_TEXT_IN_IRAM uint32_t wired_audio_usb_in_source_copy_payload(SOURCE source, uint8_t *src_buf, uint8_t *dst_buf, uint32_t length)
{
        ...
        do {
            ...
            if (codec_pcm->channel_mode == 1) {
                ...
                if (codec_pcm->format == AFE_PCM_FORMAT_S24_LE) {
                    ...
                    DSP_I2D_BufferCopy_16bit((uint16_t *)(src_buf + sizeof(audio_transmitter_frame_header_t)),
    ...
    if ((source->param.data_dl.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0) ||
        (source->param.data_dl.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0_IEM)) {
        LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[0], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_I_1);
    } else {
        LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[0], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_2_I_1);
    }
    if (codec_pcm->channel_mode == 2) {
        if (source->param.data_dl.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_1) {
            LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[1], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_I_2);
        } else {
            LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[1], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_2_I_2);
        }
    } else if (codec_pcm->channel_mode == 8) {
        LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[1], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_I_2);
        LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[2], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_I_3);
        LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[3], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_I_4);
        LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[4], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_I_5);
        LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[5], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_I_6);
        LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[6], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_I_7);
        LOG_AUDIO_DUMP((uint8_t *)stream->callback.EntryPara.in_ptr[7], dsp_frame_size_by_channel, WIRED_AUDIO_USB_IN_I_8);
    }
#endif
...
static void wired_audio_usb_out_start(mcu2dsp_start_param_p start_param, SOURCE source, SINK sink)
{
    ...
    uint32_t i, total_channels, sample_byte, sample_bits, out_frame_sample, process_frame_sample, process_sample_rate;
    src_fixed_ratio_config_t smp_config;
    sw_gain_config_t default_config;
    ...
    handle->out_channel_num = stream_function_get_channel_number(&(ul_stream->callback.EntryPara));
...
static void wired_audio_usb_out_iem_start(mcu2dsp_start_param_p start_param, SOURCE source, SINK sink)
{
    ...
    if(sink->param.data_ul.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT){
        ...
        channel_num_sw_clk_skew = 1;
        channel_num_sw_buffer = 1;
#else
        channel_num_sw_clk_skew = handle->out_channel_num;
        channel_num_sw_buffer = handle->out_channel_num;
#endif
    } else if(sink->param.data_ul.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM){
        channel_num_sw_clk_skew = handle->out_channel_num;
        channel_num_sw_buffer = handle->out_channel_num;
    }
    ...
    sw_clk_skew_config.skew_compensation_mode = SW_CLK_SKEW_COMPENSATION_1_SAMPLE_IN_8_FRAME;
    sw_clk_skew_config.skew_work_mode = SW_CLK_SKEW_CONTINUOUS;
    sw_clk_skew_config.max_output_size = (sw_clk_skew_config.channel * codec_pcm->sample_rate * codec_pcm->frame_interval * sample_byte) / (1000 * 1000);    sw_clk_skew_config.continuous_frame_size = sw_clk_skew_config.max_output_size / sw_clk_skew_config.channel;
    sw_clk_skew_config.continuous_frame_size = sw_clk_skew_config.max_output_size / sw_clk_skew_config.channel;
    handle->clk_skew_port = stream_function_sw_clk_skew_get_port(sink);
    handle->clkskew_monitor_count = 0;
    WIRED_AUDIO_LOG_MSGID_I("[USB_OUT][start] sw clkskew init: 0x%08x, %d, %d, %d, %d, %d, %d, %d, %d", 9,
...
void wired_audio_usb_out_sink_init(SINK sink, audio_transmitter_open_param_p open_param, AUDIO_TRANSMITTER_PARAMETER *sink_param)
{
    ...
    if ((open_param->scenario_sub_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT) &&
        (open_param->scenario_sub_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM)) {
    ...
    WIRED_AUDIO_LOG_MSGID_I("[USB_OUT][init] open_param->pcm: %d, %d, %d, %d, %d", 5,
...
uint32_t wired_audio_usb_out_sink_copy_payload(SINK sink, uint8_t *src_buf, uint8_t *dst_buf, uint32_t length)
{
    ...
    if ((sink->param.data_ul.scenario_sub_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT) &&
        (sink->param.data_ul.scenario_sub_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM)) {
        AUDIO_ASSERT(0);
    }
...
bool wired_audio_usb_out_sink_query_notification(SINK sink, bool *notification_flag)
{
    ...
    if (avail_block_size > (p_ShareBufferInfo->sub_info.block_info.block_size * 2)) {
...
static line_in_local_param_t *g_wired_audio_line_in_param;
...
static void wired_audio_line_in_open(mcu2dsp_open_param_p open_param, SOURCE source, SINK sink)
{
    ...
    g_wired_audio_line_in_param = line_in_param;
}
...
static void wired_audio_line_in_start(mcu2dsp_start_param_p start_param, SOURCE source, SINK sink)
{
    ...
    line_in_param = g_wired_audio_line_in_param;
...
#if 0
    AUDIO_ASSERT(line_in_handle->src_port == NULL);
    sw_src_config_t sw_src_config;
    ...
    sw_src_config.in_frame_size_max = 240 * sizeof(int16_t);
    if (line_in_param->codec_param.pcm.format == AFE_PCM_FORMAT_S24_LE) {
        sw_src_config.out_res = RESOLUTION_32BIT;
    } else {
        sw_src_config.out_res = RESOLUTION_16BIT;
    }
    sw_src_config.out_sampling_rate = line_in_param->codec_param.pcm.sample_rate;
    sw_src_config.out_frame_size_max = line_in_param->codec_param.pcm.sample_rate / 16000 * 240 * sizeof(int16_t);
    line_in_handle->src_port = stream_function_sw_src_get_port(source);
    WIRED_AUDIO_LOG_MSGID_I("[LINE_IN] sw src init: 0x%08x, %d, %d, %d, %d, %d, %d, %d, %d\r\n", 9,
                                line_in_handle->src_port,
                                sw_src_config.mode,
                                sw_src_config.channel_num,
                                sw_src_config.in_res,
                                sw_src_config.in_sampling_rate,
                                ...
                                sw_src_config.out_sampling_rate,
                                sw_src_config.out_frame_size_max);
    stream_function_sw_src_init(line_in_handle->src_port, &sw_src_config);
#endif
}
...
static void wired_audio_line_in_close(SOURCE source, SINK sink)
{
...
#if 0
    stream_function_sw_src_deinit(line_in_handle->src_port);
    line_in_handle->src_port = NULL;
#endif
    ...
    g_wired_audio_line_in_param = NULL;
}
...
static void wired_audio_line_out_start(mcu2dsp_start_param_p start_param, SOURCE source, SINK sink)
{
    ...
    uint32_t process_frame_sample, process_sample_rate;
    src_fixed_ratio_config_t smp_config;
    sw_gain_config_t default_config;

    UNUSED(start_param);

    handle = g_wired_audio_line_out_param->handle;
    ul_stream = DSP_Streaming_Get(source, sink);
    handle->out_channel_num = ul_stream->callback.EntryPara.in_channel_num;
    if (gDspAlgParameter.EscoMode.Tx == VOICE_SWB) {
        process_sample_rate = 32000;
        process_frame_sample = 480;
    } else {
    ...
    smp_config.with_codec = false;
    handle->src_fix_port = stream_function_src_fixed_ratio_get_port(sink);
    WIRED_AUDIO_LOG_MSGID_I("[LINE_OUT][start] src_fixed_ratio init: channel_number %d, in_sampling_rate %d, out_sampling_rate %d, resolution %d", 4,
                                smp_config.channel_number,
                                smp_config.in_sampling_rate,
                                smp_config.out_sampling_rate,
                                smp_config.resolution);
    ...
    total_channels = handle->out_channel_num;
    handle->gain_port = stream_function_sw_gain_get_port(sink);
    WIRED_AUDIO_LOG_MSGID_I("[LINE_OUT][start] sw gain init: 0x%08x, %d, %d, %d, %d, %d, %d, %d", 8,
                                handle->gain_port,
                                total_channels,
                                default_config.resolution,
                                default_config.target_gain,
    ...
    for (i = 1; i <= total_channels; i++) {
        ...
        digital_gain_times_of_db = 0;
#endif
        WIRED_AUDIO_LOG_MSGID_I("[LINE_OUT][start] sw gain init: channel %d gain to %d * 0.01dB\r\n", 2, i, digital_gain_times_of_db);
        stream_function_sw_gain_configure_gain_target(handle->gain_port, i, digital_gain_times_of_db);
    }
}
...
static void wired_audio_usb_in_out_iem_open(mcu2dsp_open_param_p open_param, SOURCE source, SINK sink)
{
    UNUSED(open_param);
    UNUSED(source);
    UNUSED(sink);
    ...
    switch (p_config_param->config_operation) {
        ...
        case WIRED_AUDIO_CONFIG_OP_VOL_LEVEL_MUSIC_CH_8:
            if ((sub_id.wiredaudio_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0) &&
                ...
                (sub_id.wiredaudio_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0_IEM)) {
            ...
            if ((sub_id.wiredaudio_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0)||(sub_id.wiredaudio_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0_IEM)) {
        ...
        case WIRED_AUDIO_CONFIG_OP_VOL_LEVEL_VOICE_DUL:
            if ((sub_id.wiredaudio_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT) &&
                ...
                (sub_id.wiredaudio_id != AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM)) {
...
void wired_audio_configure_task(audio_transmitter_scenario_sub_id_t sub_id, SOURCE source, SINK sink)
{
    ...
    else if((sub_id.wiredaudio_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0_IEM) ||
        (sub_id.wiredaudio_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM) ||
...
void wired_audio_open(audio_transmitter_scenario_sub_id_t sub_id, mcu2dsp_open_param_p open_param, SOURCE source, SINK sink)
{
    if ((sub_id.wiredaudio_id == AUDIO_TRANSMITTER_WIRED_AUDIO_LINE_IN) ||
    ...
    } else if (sub_id.wiredaudio_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM) {
        ...
        wired_audio_usb_in_out_iem_open(open_param, source, sink);
    }
}
...
void wired_audio_para_setup(audio_transmitter_scenario_sub_id_wiredaudio_t sub_id, DSP_STREAMING_PARA_PTR stream, SOURCE source, SINK sink)
{
    ...
    if ((sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0) ||
        ...
        if (g_1st_usb_in_source != source) {
            /* As the non 1st USB card use virtual sink type, so we must set the codec_out_size here. */
            stream->callback.EntryPara.codec_out_size = (WIRED_AUDIO_MIXING_MODE_DL_SAMPLE_RATE * sizeof(uint32_t) * pcm_para->frame_interval) / (1000 * 1000);
    ...
    } else if ((sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT)||(sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM)) {
        ...
        if(sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM) {
...
void wired_audio_resolution_config(DSP_STREAMING_PARA_PTR stream)
{
    ...
    } else if (stream->sink->param.data_ul.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM) {
        pcm_para = &(stream->sink->param.data_ul.scenario_param.usb_out_local_param.codec_param.pcm);
        if (pcm_para->format == AFE_PCM_FORMAT_S24_LE) {
