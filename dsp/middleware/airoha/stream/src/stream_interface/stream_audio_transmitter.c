...
/* Public variables ----------------------------------------------------------*/
/* Functions -----------------------------------------------------------------*/
void audio_transmitter_register_isr_handler(uint32_t index, f_audio_transmitter_ccni_callback_t callback)
{
    uint32_t saved_mask;
...
void audio_transmitter_unregister_isr_handler(uint32_t index, f_audio_transmitter_ccni_callback_t callback)
{
    uint32_t saved_mask;
    ...
    switch (scenario_id) {
        ...
        case AUDIO_TRANSMITTER_GAMING_MODE:
            ...
            if (sub_id.gamingmode_id == AUDIO_TRANSMITTER_GAMING_MODE_VOICE_HEADSET) {
                src_fixed_ratio_config_t ull_ul_smp_config;
    ...
    switch (scenario_id) {
        ...
        case AUDIO_TRANSMITTER_DCHS:
            if (sub_id.dchs_id == AUDIO_TRANSMITTER_DCHS_UART_DL) {
                //set HWSRC driver
                U32 ch_type = 0;
                for(ch_type = 0; ch_type < CH_MAX; ch_type++){
                    if(ch_type != LOCAL_SCENARIO_1 && dchs_dl_ch_scenario_msg[ch_type].sample_rate && dchs_dl_ch_scenario_msg[ch_type].sample_rate != DCHS_DL_FIX_SAMPLE_RATE){
                        DSP_MW_LOG_I("[audio_transmitter][DCHS] set hwsrc driver, ch_type = %d", 1, ch_type);
                        dchs_dl_source_ch_hwsrc_driver_control(ch_type, true);
                    }
                }
            }
            break;
...
ATTR_TEXT_IN_IRAM_LEVEL_1 static void hw_semaphore_take_audio_transmitter(uint32_t *hw_semaphore_irq_mask)
{
    uint32_t take_times;
...
ATTR_TEXT_IN_IRAM_LEVEL_1 static void hw_semaphore_give_audio_transmitter(uint32_t *hw_semaphore_irq_mask)
{
    // uint32_t int_mask;
...
static uint32_t port_audio_transmitter_sink_copy_payload(SINK sink, uint8_t *src_buf, uint8_t *dst_buf, uint32_t length)
{
    if (sink->param.data_ul.scenario_type == AUDIO_TRANSMITTER_A2DP_SOURCE) {
        ...
        {
            if (sink->param.data_ul.scenario_param.advanced_record_param.codec_type == AUDIO_DSP_CODEC_TYPE_PCM)
            {
                if(sink->transform->source->param.audio.channel_num == 1) {
                    ...
                    length = length * 2;
                    if (sink->transform->source->param.audio.format <= AFE_PCM_FORMAT_U16_BE)
                    {
                        ...
                        DSP_D2I_BufferCopy_32bit((U32 *)dst_buf,
                                                stream->callback.EntryPara.out_ptr[0],
                                                stream->callback.EntryPara.out_ptr[1],
                                                length / 4);
                    }
