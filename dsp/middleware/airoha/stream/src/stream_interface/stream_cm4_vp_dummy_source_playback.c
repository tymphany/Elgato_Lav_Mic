...
static VOID cm4_vp_dummy_source_playback_hardware_semaphore_take(VOID) // Similar to StreamDSP_HWSemaphoreTake, may consider to combine
{
...
BOOL SourceReadBuf_CM4_vp_dummy_source_playback(SOURCE source, U8 * dst_addr, U32 length)
{
    ...
    if (transform != NULL) {
        ...
        if (callback_ptr == NULL) {
            //error handling
            DSP_MW_LOG_I("[CM4_VP_DUMMY] SourceReadBuf_CM4_vp_playback : Can not get DSP_Callback", 0);
            AUDIO_ASSERT(0);
        } else {
            dst_addr = (2 == source_channels) ? callback_ptr->EntryPara.out_ptr[0] : callback_ptr->EntryPara.in_ptr[0];
        }
    }
    //DSP_MW_LOG_I("[Rdebug]SourceReadBuf DSP_C2D_BufferCopy before size(%d)", 1, sizeof(DUMMY_tone_test));
    //DSP_MW_LOG_I("[Rdebug]SourceReadBuf DSP_C2D_BufferCopy ReadOff(%d) DUMMY_tone_test(0x%x) startaddr(0x%x) length(%d) sh_length(%d)", 5, share_buff_info->ReadOffset, DUMMY_tone_test, share_buff_info->startaddr, length, share_buff_info->length);
    //DSP_MW_LOG_I("[Rdebug]C2D DestBuf(0x%x), SrcBuf(0x%x), CopySize(%d), SrcCBufStart(0x%x), SrcCBufSize(%d)", 5, dst_addr, (share_buff_info->startaddr + share_buff_info->ReadOffset), length * source_channels, share_buff_info->startaddr, share_buff_info->length);
    ...
    //DSP_MW_LOG_I("[Rdebug]SourceReadBuf DSP_C2D_BufferCopy length(%d) Source_channel(%d)", 2, length , source_channels);

    if (AUDIO_BITS_PER_SAMPLING_24 == bit_type) {
        //DSP_MW_LOG_I("[Rdebug]SourceReadBuf DSP_C2D_BufferCopy 24bits. ToDO", 0);
        S32 *dst_addr1 = NULL;
        S32 *dst_addr0 = NULL;
        S32 *src_temp_addr = NULL;
        if (2 == source_channels) {

            /* Use channel 0 as temp buffer */
            src_temp_addr = (S32 *)dst_addr;
...
#ifdef AIR_AUDIO_DUMP_ENABLE
            if (length != 0) {
                LOG_AUDIO_DUMP((U8 *)dst_addr, (U32)length, PROMPT_RT_PATTERN);
            }
#endif
        }
    } else {
...
VOID SourceInit_CM4_vp_dummy_source_playback(SOURCE source)
{
    ...
    source->sif.SourceReadBuf     = SourceReadBuf_CM4_vp_dummy_source_playback;
}
