...
static bool dchs_dl_uart_copying = false;
...
dchs_dl_mix_type_t  dchs_dl_mix_type  = DCHS_DL_MIX_TYPE_NONE;
...
afe_asrc_id_t dchs_dl_get_hwsrc_id(buf_ch_type_t ch_type)
{
...
{
    afe_asrc_id_t hwsrc_id = dchs_dl_get_hwsrc_id(ch_type);
    ...
    hwsrc_cfg->sample_count_threshold = AUDIO_DURATION_TIME * DCHS_DL_FIX_SAMPLE_RATE / MUX_UART_BUF_SLICE / 1000;
    hwsrc_cfg->mode = AFE_SRC_NO_TRACKING;
    hwsrc_cfg->ul_mode = false;
    if(hwsrc_id == AFE_SRC_1 && !hwsrc1_buf_addr){
    ...
    }else if(hwsrc_id == AFE_SRC_2 && !hwsrc2_buf_addr){
    ...
    //output buffer
    hwsrc_cfg->output_buffer.addr   = (hwsrc_id == AFE_SRC_1) ? (U32)hwsrc1_buf_addr : (U32)hwsrc2_buf_addr;
    hwsrc_cfg->output_buffer.size   = HWSRC_BUF_LENGTH;
    hwsrc_cfg->output_buffer.rate   = DCHS_DL_FIX_SAMPLE_RATE;
    if(dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_1].data_scenario_type == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_0){
        hwsrc_cfg->output_buffer.rate = dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_1].sample_rate;//if usb in exist, vp hwsrc out rate = usb sample rate
    }
...
void SourceDCHSDLSwGainInit(SOURCE source)
{
    ...
    U32 ch_num = 0;
    //for(ch_num = 1; ch_num <= 6; ch_num ++){
        //stream_function_sw_gain_configure_gain_target(dchs_gain_port, 5, default_gain); //-120 = mute
        //stream_function_sw_gain_configure_gain_target(dchs_gain_port, 6, default_gain); //-120 = mute
    //}
    if(source->param.audio.channel_num == USB_IN_8_CHANNEL){
        stream_function_sw_gain_init(dchs_gain_port, 10, &default_config);
        stream_function_sw_gain_configure_gain_target(dchs_gain_port, 9, default_gain);  //-120 = mute
        stream_function_sw_gain_configure_gain_target(dchs_gain_port, 10, default_gain); //-120 = mute
        DCHS_DL_LOG_I("[SW Gain][Init]total channel %d, default gain = %ddB", 2, 10, default_gain);
    }else{
        stream_function_sw_gain_init(dchs_gain_port, 6, &default_config);
        // stream_function_sw_gain_configure_gain_target(dchs_gain_port, 5, default_gain); //-120 = mute
        // stream_function_sw_gain_configure_gain_target(dchs_gain_port, 6, default_gain); //-120 = mute
        for(ch_num = 1; ch_num <= 6; ch_num ++){
            stream_function_sw_gain_configure_gain_target(dchs_gain_port, ch_num, default_gain); //-120 = mute
        }
        DCHS_DL_LOG_I("[SW Gain][Init]total channel %d, default gain = %ddB", 2, 6, default_gain);
    }
...
void SourceDCHSDLSwMixerInit(SOURCE source)
{
    ...
    in_ch_config.buffer_size = source->param.audio.frame_size * ((source->param.audio.format >= AFE_PCM_FORMAT_U16_BE) ? 4 : 2);
...
void dchs_dl_i2d_buffer_copy(buf_ch_type_t ch_type, U8 *dst_addr_L, U8 *dst_addr_R, U32 length, bool hwsrc_enable, U32 format_bytes, U8 channel_sel)
{
    ...
    if(dchs_dl_ch_scenario_msg[ch_type].channel_num == 1){
        ...
        if(owo > oro){
            if(format_bytes == 4){
                DSP_I2D_BufferCopy_32bit((U32 *)src_addr,
                                            (U32 *)dst_addr_L,
                                            (U32 *)dst_addr_R,
                                            (length) >> 2 >> channel_shift);
            }else{
                DSP_I2D_BufferCopy_16bit((U16 *)src_addr,
                                            (U16 *)dst_addr_L,
                                            (U16 *)dst_addr_R,
            ...
            if(format_bytes == 4){
                ...
                if(wrap_size){
                    DSP_I2D_BufferCopy_32bit((U32 *)src_start,
                ...
                if(wrap_size){
                    DSP_I2D_BufferCopy_16bit((U16 *)src_start,
...
void dchs_dl_reset_dchs_dl_stream_status(void)
{
    ...
    g_dchs_dl_process_count = 1;//
    dchs_dl_mix_type = DCHS_DL_MIX_TYPE_NONE;
}

void dchs_dl_resume_dchs_task(void)
{
    volatile SOURCE dchs_dl_source = Source_blks[SOURCE_TYPE_UART];
    if(dchs_dl_source && dchs_dl_source->transform){
...
void dchs_dl_copy_source_ch_data_2_stream_buf(buf_ch_type_t ch_type, U8 *dst_addr_L, U8 *dst_addr_R, U32 length, bool hwsrc_enable)
{
    ...
    if(ch_type != LOCAL_SCENARIO_2){
        ...
        U32 drop_bytes = MIN(ch_data_size, dchs_dl_silence_count[ch_type]);
        if(drop_bytes){
            dchs_dl_source_ch_buf_drop(ch_type, drop_bytes, hwsrc_enable);
            dchs_dl_silence_count[ch_type] -= drop_bytes;
            DCHS_DL_LOG_W("[Source Drop] source ch: %d, drop_size: %d for silence before.", 2, ch_type, drop_bytes);
        }
    }
    ch_data_size = dchs_dl_get_ch_data_size(ch_type, hwsrc_enable);
...
{
    DCHS_DL_LOG_I("[timer callback]Play en check, :0x%x, enable :0x%x, AFE_AUDIO_BT_SYNC_CON0:0x%x, AFE_AUDIO_BT_SYNC_MON1:0x%x,", 4,*((volatile uint32_t*)(CONN_BT_TIMCON_BASE+0x0208)),*((volatile uint8_t*)(CONN_BT_TIMCON_BASE+0x0200)), AFE_READ(AFE_AUDIO_BT_SYNC_CON0), AFE_READ(AFE_AUDIO_BT_SYNC_MON1));// enable:1->0,play en work
...
void dchs_mcu2dsp_msg_sync_callback(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    if(sync_msg->sync_type == DCHS_DL_OTHER_CHIP_DL_EXIST){
        ...
        if(chip_role == DCHS_MODE_RIGHT){
            ...
            if(sync_msg->sync_msg.dual_chip_dl_sync.sample_rate != DCHS_DL_FIX_SAMPLE_RATE){
                dchs_dl_source_ch_hwsrc_cfg(UART_SCENARIO, HWSRC_2);
                if(source){
        ...
        if(other_chip_dl_exist){
            if(g_dchs_dl_data_mix_count){
                dchs_dl_mix_type = UART_SCEANRIO_WAITING_MIX;
            }else{
        ...
        DCHS_DL_LOG_I("[DSP msg sync] other_chip_dl_exist = %d,dchs_dl_mix_type=%d,format bytes=%d,rate=%d,channel_num=%d,chip_role=%d", 6, dchs_dl_check_scenario_exist(UART_SCENARIO), dchs_dl_mix_type, dchs_dl_ch_scenario_msg[UART_SCENARIO].format_bytes,sync_msg->sync_msg.dual_chip_dl_sync.sample_rate,dchs_dl_ch_scenario_msg[UART_SCENARIO].channel_num
        ...
        if(data_scenario_type == AUDIO_SCENARIO_TYPE_VP){
            dchs_dl_set_scenario_exist(LOCAL_SCENARIO_2, local_chip_dl_exist);
            ...
            if(sample_rate != DCHS_DL_FIX_SAMPLE_RATE){
            ...
            if(local_chip_dl_exist){
                ...
                //memset(&dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_1], 0, sizeof(dl_data_scenario_msg_t));
            ...
            if(sample_rate != DCHS_DL_FIX_SAMPLE_RATE){
                if(source){
                    //dchs_dl_source_ch_hwsrc_cfg(LOCAL_SCENARIO_1, HWSRC_3);
                    //dchs_dl_source_ch_hwsrc_driver_control(LOCAL_SCENARIO_1, local_chip_dl_exist);
                }
            }
            if(local_chip_dl_exist){
                if(g_dchs_dl_data_mix_count){
                    dchs_dl_mix_type = LOCAL_SCEANRIO_WAITING_MIX;
                }else{
        ...
        if(dchs_gain_port){
            ...
            if(operation >= DCHS_DL_CONFIG_OP_MAX){
                return;
            }
        ...
        if(data_scenario == AUDIO_SCENARIO_TYPE_VP || data_scenario == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_LINE_IN
            || data_scenario == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_0){
...
{
    //check fix point
    if(!dchs_dl_check_scenario_exist(UART_SCENARIO) && dchs_dl_mix_type == UART_SCEANRIO_WAITING_MIX){
        ...
        DCHS_DL_LOG_I("[Source Size] UART_SCENARIO exist:%d, mix_count:%d, mix_point:%d,mix_type:%d", 4, dchs_dl_check_scenario_exist(UART_SCENARIO), g_dchs_dl_data_mix_count,dchs_dl_ch_scenario_msg[UART_SCENARIO].mix_point,dchs_dl_mix_type);
    }
    if(!dchs_dl_check_scenario_exist(LOCAL_SCENARIO_1) && dchs_dl_mix_type == LOCAL_SCEANRIO_WAITING_MIX){
        ...
        DCHS_DL_LOG_I("[Source Size] SCENARIO_1 exist:%d, mix_count:%d, mix_point:%d,mix_type:%d", 4, dchs_dl_check_scenario_exist(LOCAL_SCENARIO_1), g_dchs_dl_data_mix_count,dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_1].mix_point,dchs_dl_mix_type);
    }
...
{ 
    ...
    if(dchs_dl_check_scenario_exist(LOCAL_SCENARIO_1) && dchs_dl_check_scenario_exist(UART_SCENARIO)){
        ...
        if(!dchs_dl_uart_copying){ //avoid ccni rx task sourcesize duplicated copy
            dchs_dl_copy_uart_data_2_source_buf();
        }
        ...
        if(((!dchs_dl_check_hwsrc_enable(LOCAL_SCENARIO_1) && dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_1].sink_buf_info->bBufferIsFull) || local_scenario_size >= audio_frame_size) //two size meet the stream frame size,then read the buf
            && ((!dchs_dl_check_hwsrc_enable(UART_SCENARIO) && dchs_dl_ch_scenario_msg[UART_SCENARIO].sink_buf_info->bBufferIsFull) || uart_scenario_size >= audio_frame_size)){
        ...
        if((!dchs_dl_check_hwsrc_enable(LOCAL_SCENARIO_1) && dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_1].sink_buf_info->bBufferIsFull) || local_scenario_1_size >= audio_frame_size){
            return audio_frame_size;
        }else if(dchs_dl_check_fill_silence()){
            return audio_frame_size;
        }
    }else if(dchs_dl_check_scenario_exist(UART_SCENARIO)){
        if(!dchs_dl_uart_copying){
            dchs_dl_copy_uart_data_2_source_buf();
        }
        ...
        if((!dchs_dl_check_hwsrc_enable(UART_SCENARIO) && dchs_dl_ch_scenario_msg[UART_SCENARIO].sink_buf_info->bBufferIsFull) || uart_scenario_size >= audio_frame_size){
            return audio_frame_size;
        }else if(dchs_dl_check_fill_silence()){
            return audio_frame_size;
        }
    ...
    if(dchs_dl_check_scenario_exist(LOCAL_SCENARIO_2)){
        ...
        if((!dchs_dl_check_hwsrc_enable(LOCAL_SCENARIO_2) && dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_2].sink_buf_info->bBufferIsFull) || local_scenario_2_size >= audio_frame_size){
...
BOOL SourceReadBuf_DCHS_DL(SOURCE source, U8 *dst_addr, U32 length)
{
    ...
    if(dchs_dl_check_scenario_exist(LOCAL_SCENARIO_1) && dchs_dl_check_scenario_exist(UART_SCENARIO)){
        ...
        // local ch buf
        dchs_dl_copy_source_ch_data_2_stream_buf(LOCAL_SCENARIO_1, local_scenario_1_L, local_scenario_1_R, length * (dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_1].channel_num >= 2 ? 2 : 1), dchs_dl_check_hwsrc_enable(LOCAL_SCENARIO_1));
...
VOID SourceDrop_DCHS_DL(SOURCE source, U32 amount)
{
    ...
    if(dchs_dl_check_scenario_exist(LOCAL_SCENARIO_1) && dchs_dl_check_scenario_exist(UART_SCENARIO)){
        ...
        if(!dchs_dl_silence_count[LOCAL_SCENARIO_1]){
            dchs_dl_source_ch_buf_drop(LOCAL_SCENARIO_1, amount * (dchs_dl_ch_scenario_msg[LOCAL_SCENARIO_1].channel_num >= 2 ? 2 : 1), dchs_dl_check_hwsrc_enable(LOCAL_SCENARIO_1));
        }
    }else if(dchs_dl_check_scenario_exist(UART_SCENARIO)){
        if(!dchs_dl_silence_count[UART_SCENARIO]){
            dchs_dl_source_ch_buf_drop(UART_SCENARIO, amount * dchs_dl_ch_scenario_msg[UART_SCENARIO].channel_num, dchs_dl_check_hwsrc_enable(UART_SCENARIO));
...
SOURCE SourceInit_DCHS_DL(SOURCE source)
{
    ...
    DCHS_DL_LOG_I("[DCHS DL]set uart frame size=%d", 1, 4096);
...
void dps_uart_relay_ul_mem_sync_info(uint32_t delay_time)
{
...
static void hal_audio_gpt_trigger_mem_vul(void)
{
    S32 cur_native_bt_clk = 0, cur_native_bt_phase = 0;
    MCE_GetBtClk((BTCLK *)&cur_native_bt_clk, (BTPHASE *)&cur_native_bt_phase, DCHS_CLK_Offset);
...
void ul_afe_set_mem_enable(S32 play_bt_clk, S32 play_bt_phase)
{
    ...
    DSP_MW_LOG_I("[DCHS UL]UL bt clk count play_bt_clk:%u, bt clk phase:%u, cur_native_bt_clk:%u,cur_native_bt_clk_phase:%u,bt_clk_diff:%d", 5, play_bt_clk, play_bt_phase,cur_native_bt_clk, cur_native_bt_phase, bt_clk_diff);
    dchs_hfp_absolute_parameter.absolute_time_count = count_1 + (uint32_t)bt_clk_diff;
