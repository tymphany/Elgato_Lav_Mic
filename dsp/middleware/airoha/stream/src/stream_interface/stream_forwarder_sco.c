...
void Sco_UL_Fix_Sample_Rate_Init(void)
{
    ...
    DSP_MW_LOG_I("[DSP][eSCO] UL src_fixed_ratio_init: channel_number %d, in_sampling_rate %d, out_sampling_rate %d, resolution %d", 4,
...
void Sco_DL_Fix_Sample_Rate_Init(void)
{
    ...
    if (gDspAlgParameter.EscoMode.Rx == VOICE_NB) {
        ...
        DSP_MW_LOG_I("[DSP][eSCO] DL src_fixed_ratio_init done", 0);
    }
}
...
{
    uint16_t pattern_framesize, play_en_intra_clk;
    uint32_t rx_forwarder_gpt_time, interval_gpt_time, spec_interval;
    uint32_t play_en_bt_clk, play_en_nclk;
    ...
    if ((source == NULL) || (sink == NULL) || (source->transform == NULL) || (sink->transform == NULL)) {
        DSP_MW_LOG_W("[RX FWD] source or sink == NULL || source->transform or sink->transform == NULL", 0);
    ...
    if(dchs_get_device_mode() != DCHS_MODE_SINGLE){
        UNUSED(play_en_bt_clk);
        UNUSED(play_en_nclk);
        UNUSED(play_en_intra_clk);
    ...
    if ((sink_param->irq_exist == false) && (src_param->rx_forwarder_en == true) &&
         ...
         */
        DSP_MW_LOG_I("[RX FWD] packet type %d, packet size %d, packet number %d", 3, g_sco_pkt_type, g_pattern_framesize, g_sco_frame_per_pkt_num);
        ...
        DSP_MW_LOG_I("[RX FWD] First Rx Forwarder IRQ resume DAVT", 0);
...
static void Sco_TX_IntrHandler(void)
{
    ...
    if (sink == NULL) {
        DSP_MW_LOG_W("[TX FWD] sink == NULL", 0);
        Forwarder_Tx_Intr_HW_Handler();
        return;
    }
...
static hal_nvic_status_t Sco_Audio_Fwd_Ctrl(fowarder_ctrl forwarder_en, fowarder_type forwarder_type)
{
    ...
    if (forwarder_type == RX_FORWARDER) {
        ...
        DSP_MW_LOG_W("[Tx FWD] No this kind of forwarder!", 0);
...
ATTR_TEXT_IN_IRAM_LEVEL_2 static BOOL SinkFlushSco(SINK sink, uint32_t amount)
{
    ...
    if ((SinkSlackSco(sink) == 0) || (amount != sink->param.n9sco.process_data_length)) {
        DSP_MW_LOG_W("[DSP][eSCO] sinkflush, amount:%d != process_data_length:%d", 2, amount, sink->param.n9sco.process_data_length);
        return false;
    }
...
ATTR_TEXT_IN_IRAM static uint32_t SourceSizeSco(SOURCE source)
{
    ...
    if ((dataRemainLen >= processFrameLen) || (source->param.n9sco.write_offset_advance != 0)) {
        ...
        DSP_MW_LOG_W("[DSP][eSCO] AVM buffer check: SourceSize = 0, dataRemainLen:%d, Ro:%d, Wo:%d", 3, dataRemainLen, readOffset, writeOffset);
        return 0;
    }
}
...
ATTR_TEXT_IN_IRAM_LEVEL_2 static void SourceDropSco(SOURCE source, uint32_t amount)
{
    ...
    if (amount != source->param.n9sco.process_data_length) {
        DSP_MW_LOG_I("[DSP][eSCO] Source Drop, false amount: %d, length:%d", 2, amount, source->param.n9sco.process_data_length);
        return;
    }
