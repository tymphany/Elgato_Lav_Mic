...
extern volatile uint32_t SW_DEBUG_SRAM_1_GPT_1M;
extern volatile uint32_t SW_DEBUG_SRAM_2_GPT_1M;
extern volatile uint32_t SW_DEBUG_SRAM_3_GPT_1M;
extern volatile uint32_t SW_DEBUG_SRAM_4_GPT_1M;
...
SOURCE StreamAudioAdaptAncSource(void *param)
{
    ...
    source->param.audio.channel_num = 4; //in*2, out*2
...
extern void hal_anc_set_dma_enable(bool enable);
...
void dsp_adpat_anc_close(hal_ccni_message_t msg, hal_ccni_message_t *ack)
{
    ...
    DSP_PIC_FeatureDeinit(full_adapt_ANC_if.pfeature_table);
#endif
