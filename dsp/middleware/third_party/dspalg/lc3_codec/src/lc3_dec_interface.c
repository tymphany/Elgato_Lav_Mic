...
void LC3_Dec_Deinit(void){
    ...
    memset(lc3_memory, 0, sizeof(LC3_INSTANCE));
    p_lc3i_tab_dec = NULL;
    if (g_lc3_user_cnt == 0) {
        /*Table common memory*/
...
bool stream_codec_decoder_lc3_initialize(void *para)
{
...
#endif
    {
        ...
        updn_sampling_Init((SMP_INSTANCE_t *)lc3_dec_smp_instance_ptr, &dsp_sampler_coef);
...
ATTR_TEXT_IN_IRAM bool stream_codec_decoder_lc3_process(void *para)
{
    ...
    for (i = 0; i < stream_ptr->source->param.n9ble.process_number; i++) {
        ...
        if ((ret != LC3_OK) && ((packet_lost_st != 0) && (ret != LC3_DECODE_ERROR))) {
            DSP_MW_LOG_E("[lc3][dec][fail] LC3_Dec_Prcs() %d PLC : %d", 2, ret, packet_lost_st);
    ...
    if (lc3_dec_src_enabled) {
        if (sample_rate == 32000) {/*32->16*/
            ...
            updn_sampling_16bit_Proc((SMP_INSTANCE_t *)lc3_dec_smp_instance_ptr, 0, 2, (int16_t *)left_buffer, (int16_t *)left_buffer, curr_frame_sample_count);
            ...
            updn_sampling_16bit_Proc((SMP_INSTANCE_t *)lc3_dec_smp_instance_ptr, 1, 2, (int16_t *)left_buffer, (int16_t *)right_buffer, curr_frame_sample_count);
            frame_sample_count *= 2;
