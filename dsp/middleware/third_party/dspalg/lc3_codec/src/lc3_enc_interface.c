...
static uint16_t lc3_enc_get_sample_rate(void *para)
{
    ...
    return stream_ptr->sink->param.n9ble.share_info_base_addr->sampling_rate;
}
...
VOID LC3_Tab_Enc_MemInit (VOID* para,LC3I_Param *param){
    ...
    if (!LC3_Enc_MemCheck())
    {
        ...
        if (p_lc3i_tab_common == NULL) {
            if (p_lc3i_tab_common != NULL) {
                DSP_MW_LOG_E("[lc3][fail] Something wrong", 0);
                assert(0);
            }
            /*Table Common*/
...
bool stream_codec_encoder_lc3_initialize (void *para)
{
    ...
    if (STREAM_BLE_UL_GET_PARAM(&sample_rate, &bit_rate)) {
        ...
        if (sample_rate_in == FS_RATE_16K && sample_rate == 32000) {
            ...
            updn_sampling_Init((SMP_INSTANCE_t *)lc3_enc_smp_instance_ptr, &dsp_sampler_coef);
        } else {
...
bool stream_codec_encoder_lc3_process(void *para)
{
    LC3_ERR_T ret;
    uint16_t pcm_resolution;
    uint8_t *input_buffer, *out_buffer;
    uint32_t out_size, in_frame_size;
    ...
    if (lc3_enc_sw_buf_enabled) {
        ...
        if (lc3_enc_src_enabled) {
            ...
            if (sample_rate_in == FS_RATE_16K && sample_rate == 32000) { /*32000 -> 16000*/
                ...
                updn_sampling_16bit_Proc((SMP_INSTANCE_t *)lc3_enc_smp_instance_ptr, 1, 2, (int16_t *)input_buffer, (int16_t *)temp_buf, 240);
            } else if (sample_rate_in == FS_RATE_32K && sample_rate == 16000) { /*16000 -> 32000*/
                updn_sampling_16bit_Proc((SMP_INSTANCE_t *)lc3_enc_smp_instance_ptr, 0, 2, (int16_t *)input_buffer, (int16_t *)temp_buf, 480);
            }
        } else {
            memcpy(temp_buf, input_buffer, lc3_enc_sw_buf_block_len);
        }

        out_buffer = stream_codec_get_1st_output_buffer(para);
        ...
        do {
...
#endif
            //ret = LC3I_Enc_Prcs (p_lc3_enc_ptr, p_lc3i_enc_tab, out_buffer, input_buffer, &this_out_size, &lc3_FFTx);
#ifdef MTK_BT_A2DP_LC3_USE_PIC
            if (xSemaphoreTake(g_lc3i_codec_process_xSemaphore, portMAX_DELAY) != pdTRUE) {
                DSP_MW_LOG_E("[lc3][enc][fail] LC3_Enc_Prcs() semaphore take fail ",0);
                configASSERT(0);
            }
#endif
            ...
            if (xSemaphoreGive(g_lc3i_codec_process_xSemaphore) == pdFALSE) {
                DSP_MW_LOG_E("[lc3][enc][fail] LC3_Enc_Prcs() semaphore give fail",0);
            }
#endif
