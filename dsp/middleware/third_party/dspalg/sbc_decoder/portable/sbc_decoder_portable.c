...
void *sbc_decoder_export_parameters[1] = {sbc_default_function_parameter};
#if PIC_LOGPRINT
...
void sbc_load_library_callback(preloader_pisplit_handle_t handle)
{
    ...
    {
        sbc_decoder_export_parameters[0] = (void *)p_export_parameters[0];
        #if PIC_LOGPRINT
...
uint32_t sbc_library_load(void *code_address, void *data_address, uint32_t *dram_pic_usage)
{
    ...
    if(PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_get_handle(&p_sbc_pisplit_library_handle,&xa_sbc_dec_library,sbc_load_library_callback) )
    {
        DSP_MW_LOG_E("xa_sbc_dec_library preloader_pisplit_get_handle() error!!!!", 0);
        AUDIO_ASSERT(0);
    }
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
    if(PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_load_v1(p_sbc_pisplit_library_handle, code_address, data_address))
#else
...
#endif
    {
        DSP_MW_LOG_E("xa_sbc_dec_library preloader_pisplit_load() error!!!!", 0);
...
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
    preloader_pisplit_get_library_code_and_data_size(p_sbc_pisplit_library_handle, NULL, dram_pic_usage);
#else
    UNUSED(code_address);
    UNUSED(data_address);
...
{
#ifdef AIR_DSP_MEMORY_REGION_ENABLE
    if(PRELOADER_PISPLIT_XTLIB_NO_ERR != preloader_pisplit_unload_v1(p_sbc_pisplit_library_handle))
#else
    ...
    sbc_decoder_export_parameters[0] = sbc_default_function_parameter;
