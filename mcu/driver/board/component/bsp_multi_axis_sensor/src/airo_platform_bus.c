...
static platform_bus_private_info_t                          priv_info[BSP_MULTI_AXIS_BUS_TYPE_MAX][MAX_PLATFORM_BUS_PORT];
...
static int32_t airo_bus_i2c_read_dma(hal_i2c_port_t port, platform_transfer_dma_config_t *config)
{
    ...
    s_air_irq_triggered    = false;
    pinfo = &priv_info[BSP_MULTI_AXIS_BUS_TYPE_I2C][port];
    s_air_tx_buf[0]        = config->reg_addr;
...
int airo_bus_spi_read_dma(hal_spi_master_port_t master_port, platform_transfer_dma_config_t *config)
{
    ...
    spi_master_send_and_receive_config.receive_length = 1 + config->size;
    priv_info[BSP_MULTI_AXIS_BUS_TYPE_SPI][master_port].call_back = config->call_back;
    priv_info[BSP_MULTI_AXIS_BUS_TYPE_SPI][master_port].user_data = config->user_data;
    hal_spi_master_register_callback(master_port, airo_bus_spi_callback, &priv_info[BSP_MULTI_AXIS_BUS_TYPE_SPI][master_port]);
...
int platform_bus_init(uint32_t handle, platform_bus_init_config_t *config)
{
    ...
    switch (BSP_SENSOR_GET_BUS_TYPE(handle)) {
        case BSP_MULTI_AXIS_BUS_TYPE_I2C:
            status = airo_bus_i2c_init(port, &config->i2c_config);
            break;
        case BSP_MULTI_AXIS_BUS_TYPE_SPI:
