...
///////////////////////////Porting layer///////////////////////////////////////
#define     qmi8658_log       log_sensor_info
...
static int  platform_write(uint32_t handle, uint8_t reg, uint8_t *data, uint16_t size)
{
    platform_transfer_config_t config;

    config.dev_addr = QMI8658A_SLV_ADDR;
    config.reg_addr = reg;
    config.buff     = data;
    config.size     = size;
...
static int  platform_read(uint32_t handle, uint8_t reg, uint8_t *data, uint16_t size)
{
    platform_transfer_config_t config;
...
unsigned char qmi8658_write_reg(unsigned char reg, unsigned char value)
{
    ...
    ret = (unsigned char) platform_write(qmi_handle, reg, &temp, 1);
...
unsigned char qmi8658_write_regs(unsigned char reg, unsigned char *value, unsigned char len)
{
    ...
    ret = (unsigned char) platform_read(qmi_handle, reg, value, len);
	return ret;
}
...
unsigned char qmi8658_read_reg(unsigned char reg, unsigned char* buf, unsigned short len)
{
    ...
    ret = (unsigned char) platform_read(qmi_handle, reg, buf, len);
...
unsigned short qmi8658_read_fifo(unsigned char* data, uint32_t len)
{
    ...
    //uint32_t  tick0, tick1, tick2, tick_dur0,tick_dur1;
	...
	if((g_imu.cfg.fifo_ctrl&0x03)!=qmi8658_Fifo_Bypass)
	{
		...
		fifo_bytes = len;//(unsigned short)(((fifo_status[1]&0x03)<<8)|fifo_status[0]);
    ...
    //hal_gpt_get_duration_count(tick0, tick1, &tick_dur0);
    //hal_gpt_get_duration_count(tick1, tick2, &tick_dur1);
...
unsigned char _qmi8658_init(void)
{
...
int qmi8658_get_acc_range(bsp_multi_axis_accel_range_t range ,enum qmi8658_AccRange *qmi_acc)
{
    uint8_t temp = 0;

    switch(range){
        case BSP_MULTI_AXIS_ACCEL_RANGE_2G:  temp = Qmi8658AccRange_2g; break;
        case BSP_MULTI_AXIS_ACCEL_RANGE_4G:  temp = Qmi8658AccRange_4g; break;
        case BSP_MULTI_AXIS_ACCEL_RANGE_8G:  temp = Qmi8658AccRange_8g; break;
        case BSP_MULTI_AXIS_ACCEL_RANGE_16G: temp = Qmi8658AccRange_16g; break;
    }
    if(qmi_acc) {
        *qmi_acc = temp;
    }
...
int qmi8658_get_gyr_range(bsp_multi_axis_gyro_range_t range ,enum qmi8658_GyrRange *qmi_gyr)
{
    uint8_t temp = 0;

    switch(range){
        case BSP_MULTI_AXIS_GYRO_RANGE_125_DEG_SEC:  temp = Qmi8658GyrRange_128dps; break;
        case BSP_MULTI_AXIS_GYRO_RANGE_250_DEG_SEC:  temp = Qmi8658GyrRange_256dps; break;
        case BSP_MULTI_AXIS_GYRO_RANGE_500_DEG_SEC:  temp = Qmi8658GyrRange_512dps; break;
        case BSP_MULTI_AXIS_GYRO_RANGE_1000_DEG_SEC: temp = Qmi8658GyrRange_1024dps; break;
        case BSP_MULTI_AXIS_GYRO_RANGE_2000_DEG_SEC: temp = Qmi8658GyrRange_2048dps; break;
    }
    if(qmi_gyr) {
        *qmi_gyr = temp;
    }
...
int qmi8658_get_odr(bsp_multi_axis_output_data_rate_t odr, uint8_t *acce_odr, uint8_t *gyro_odr)
{
    uint8_t acc_odr = 0x08, gyr_odr = 0x08;
    switch(odr) {
        case BSP_MULTI_AXIS_OUTPUT_DATA_RATE_25HZ: {
                acc_odr = Qmi8658AccOdr_31_25Hz; 
                gyr_odr = Qmi8658GyrOdr_31_25Hz;
            }break;
        case BSP_MULTI_AXIS_OUTPUT_DATA_RATE_50HZ: {
                acc_odr = Qmi8658AccOdr_62_5Hz; 
                gyr_odr = Qmi8658GyrOdr_62_5Hz;
            }break;
        case BSP_MULTI_AXIS_OUTPUT_DATA_RATE_100HZ: {
                acc_odr = Qmi8658AccOdr_125Hz; 
                gyr_odr = Qmi8658GyrOdr_125Hz;
            }break;
        case BSP_MULTI_AXIS_OUTPUT_DATA_RATE_200HZ: {
                acc_odr = Qmi8658AccOdr_250Hz; 
                gyr_odr = Qmi8658GyrOdr_250Hz;
            }break;
        case BSP_MULTI_AXIS_OUTPUT_DATA_RATE_400HZ: {
                acc_odr = Qmi8658AccOdr_500Hz; 
                gyr_odr = Qmi8658GyrOdr_500Hz;
            }break;
        case BSP_MULTI_AXIS_OUTPUT_DATA_RATE_800HZ: {
                acc_odr = Qmi8658AccOdr_1000Hz; 
                gyr_odr = Qmi8658GyrOdr_1000Hz;
            }break;
        case BSP_MULTI_AXIS_OUTPUT_DATA_RATE_1600HZ: {
                acc_odr = Qmi8658AccOdr_2000Hz; 
                gyr_odr = Qmi8658GyrOdr_2000Hz;
            }break;
        case BSP_MULTI_AXIS_OUTPUT_DATA_RATE_3200HZ: {
                acc_odr = Qmi8658AccOdr_4000Hz; 
                gyr_odr = Qmi8658GyrOdr_4000Hz;
            }break;
    }
    if (acce_odr) {
        *acce_odr = acc_odr;
...
//////////////////////////////////////////Public API for User///////////////////////////////////////////////////
int    qmi8658_sensor_init(bsp_multi_axis_handle_t handle, bsp_multi_axis_init_config_t *config)
{
    qmi_handle = handle;

    platform_init(handle, (platform_bus_init_config_t *)&config->bus_config);
    if(_qmi8658_init() == 1){        
        qmi8658_write_reg(Qmi8658Register_Ctrl7, 0);
        return 0;
    } else {
        return -1;
    }
...
int    qmi8658_sensor_deinit(bsp_multi_axis_handle_t handle)
{
    qmi8658_write_reg(Qmi8658Register_Ctrl7, 0x00);
    platform_deinit((uint32_t)handle);
    return BSP_MULTI_AXIS_OK;
}


int     qmi8658_sensor_config(bsp_multi_axis_handle_t handle, bsp_multi_axis_config_t *config)
{
	//unsigned char qmi8658_revision_id = 0x00;
    //unsigned char firmware_id[3];
    enum qmi8658_AccOdr   acc_odr;
    enum qmi8658_GyrOdr   gyr_odr;
    enum qmi8658_AccRange acc_range;
    enum qmi8658_GyrRange gyr_range;

#if 0
    qmi8658_read_reg(Qmi8658Register_Revision, &qmi8658_revision_id, 1);
    qmi_log("Qmi8658_init slave=0x%x   0x%x\r\n", QMI8658A_SLV_ADDR, qmi8658_revision_id);
    qmi8658_read_reg(0x49, firmware_id, 3);
    qmi_log("Old Firmware ID[0x%x 0x%x 0x%x]\r\n", firmware_id[0],firmware_id[1],firmware_id[2]);
#endif
    qmi8658_write_reg(Qmi8658Register_Ctrl1, 0x60 | QMI8658_INT2_ENABLE | QMI8658_INT1_ENABLE);

    /*convert paremeter to QMI*/
    qmi8658_get_odr(config->sensor_odr, &acc_odr, &gyr_odr);
    qmi8658_get_acc_range(config->accel_range, &acc_range);
    qmi8658_get_gyr_range(config->gyro_range,  &gyr_range);
    ...
    qmi8658_write_reg(Qmi8658Register_Ctrl7, 0x00);
    qmi8658_write_reg(Qmi8658Register_Ctrl8, 0x00);

    /* config acc & gyr */
    g_imu.cfg.enSensors = QMI8658_ACCGYR_ENABLE;
    g_imu.cfg.accRange  = acc_range;
    g_imu.cfg.accOdr    = acc_odr;
    g_imu.cfg.gyrRange  = gyr_range;
    g_imu.cfg.gyrOdr    = gyr_odr;
    qmi8658_config_acc(g_imu.cfg.accRange, g_imu.cfg.accOdr,  Qmi8658Lpf_Disable, Qmi8658St_Disable);
    qmi8658_config_gyro(g_imu.cfg.gyrRange, g_imu.cfg.gyrOdr, Qmi8658Lpf_Disable, Qmi8658St_Disable);

    /* config fifo */
    qmi8658_config_fifo(config->frame_threshold * 2, qmi8658_Fifo_128,qmi8658_Fifo_Stream, qmi8658_Int2);
...
uint32_t qmi8658_sensor_available_byte()
{
    uint16_t num;
    uint8_t  fifo_status[2];

    qmi8658_read_reg(Qmi8658Register_FifoCount, fifo_status, 2);
    num = (unsigned short)(((fifo_status[1]&0x03)<<8) | fifo_status[0]);
    return num;
}

int  qmi8658_sensor_read_fifo_data(uint8_t *buffer, uint32_t len)
{
    //qmi8658_read_reg(Qmi8658Register_FifoData, buffer, len);
    qmi8658_read_fifo(buffer, len);
    return len;
}

uint32_t     qmi8658_sensor_format(uint8_t *buffer, uint32_t buff_len, bsp_multi_axis_data_t *user_buff, uint32_t user_buff_len)
{
    qmi_data_format_t  *pformat_data = (qmi_data_format_t *)buffer;
    uint32_t            avail_frame, i;  
    float               temper = 0.0;

    avail_frame = buff_len / 12;
    avail_frame = user_buff_len > avail_frame? avail_frame : user_buff_len;
    temper      = qmi8658_readTemp();

    for (i = 0; i < avail_frame; i++) {
        user_buff[i].head.temperature = temper;
        user_buff[i].axis_sensor_data_t.accel_data.x = (int16_t)((pformat_data->x[1] << 8) | pformat_data->x[0]) / (float) g_imu.ssvt_a;        
        user_buff[i].axis_sensor_data_t.accel_data.y = (int16_t)((pformat_data->y[1] << 8) | pformat_data->y[0]) / (float) g_imu.ssvt_a;
        user_buff[i].axis_sensor_data_t.accel_data.z = (int16_t)((pformat_data->z[1] << 8) | pformat_data->z[0]) / (float) g_imu.ssvt_a;
        pformat_data++;
        user_buff[i].axis_sensor_data_t.gyro_data.x  = (int16_t)((pformat_data->x[1] << 8) | pformat_data->x[0]) / (float) g_imu.ssvt_a;        
        user_buff[i].axis_sensor_data_t.gyro_data.y  = (int16_t)((pformat_data->y[1] << 8) | pformat_data->y[0]) / (float) g_imu.ssvt_a;        
        user_buff[i].axis_sensor_data_t.gyro_data.z  = (int16_t)((pformat_data->z[1] << 8) | pformat_data->z[0]) / (float) g_imu.ssvt_a;        
        pformat_data++;
    }
    return avail_frame;
}
