...
ATTR_TEXT_IN_TCM hal_eint_status_t hal_eint_mask(hal_eint_number_t eint_number)
{
  //  uint32_t mask;
   ...
   // hal_nvic_save_and_set_interrupt_mask(&mask);
	...
	if (eint_number < EINT_GROUP_MAX_NUMBER) {
		EINT_REGISTER->DOMAIN[0].CLR[0] = (1 << eint_number);
		EINT_REGISTER->EINT_MASK_SET = (1 << eint_number);
	} else {
		EINT_REGISTER->DOMAIN[0].CLR[1] = (1 << (eint_number & (EINT_GROUP_MAX_NUMBER - 1)));
...
ATTR_TEXT_IN_TCM hal_eint_status_t hal_eint_unmask(hal_eint_number_t eint_number)
{
 //   uint32_t mask;
  ...
  //  hal_nvic_save_and_set_interrupt_mask(&mask);
	...
	if (eint_number < EINT_GROUP_MAX_NUMBER) {
        EINT_REGISTER->DOMAIN[0].SET[0] = (1 << eint_number);
		EINT_REGISTER->EINT_MASK_CLR = (1 << eint_number);
	} else {
		EINT_REGISTER->DOMAIN[0].SET[1] = (1 << (eint_number & (EINT_GROUP_MAX_NUMBER - 1)));
