...
extern volatile i2c_transfer_info_t i2c_transfer_info[HAL_I2C_MASTER_MAX];

const hal_clock_cg_id   g_i2c_pdn_table[(uint32_t)HAL_I2C_MASTER_MAX] = {HAL_CLOCK_CG_I2C0, HAL_CLOCK_CG_I2C1, HAL_CLOCK_CG_I2C2, HAL_CLOCK_CG_I2C_AO};
...
uint32_t                g_i2c_source_clock_table[(uint32_t)HAL_I2C_MASTER_MAX] = {HAL_I2C_SOURCE_CLOCK_TABLE};
...
#endif
hal_i2c_sw_fifo_base    i2c_sw_fifo_space[HAL_I2C_MASTER_MAX][HAL_I2C_SW_FIFO_LEN];
ATTR_RODATA_IN_SYSRAM  I2C_REGISTER_T    *i2c_master_register[(uint32_t)HAL_I2C_MASTER_MAX] = {(I2C_REGISTER_T *)(I2C_0_BASE), (I2C_REGISTER_T *)(I2C_1_BASE), \
...
void i2c_sw_fifo_init(uint32_t i2c_port)
{
    ...
    i2c_sw_fifo[i2c_port].i2c_sw_fifo_start = &i2c_sw_fifo_space[i2c_port][0];
...
void i2c_set_frequency(uint32_t i2c_port, uint32_t transfer_frequency)
{
    ...
    switch (transfer_frequency) {
        ...
        case (HAL_I2C_FREQUENCY_2M) :
            frequency = I2C_FREQUENCY_2M;
            break;
        case (HAL_I2C_FREQUENCY_3M) :
            frequency = I2C_FREQUENCY_3M;
            break;
#endif
        default :
    ...
    ************************************************************/
    ...
    i2c_master_register[i2c_port]->DELAY_LEN = 0x4;
}
 ...
 ************************************************************/
ATTR_TEXT_IN_TCM void i2c_config_hardware(uint32_t i2c_port, i2c_transaction_type_t transaction_type, uint8_t slave_address, i2c_transaction_size_t *config_size)
{
    /* set slave address */
    ...
    switch (transaction_type) {
        ...
        case I2C_TRANSACTION_OPEN_DRAIN_SEND_TO_RECEIVE_POLLING:
            /* bit24: len_chg, bit16:dir_chg, bit8:dma_en, bit0:rs_stop*/
            i2c_master_register[i2c_port]->CONTROL_UNION.CONTROL = (1<<24) | (1<<16) | (0x1);
            i2c_master_register[i2c_port]->CONTROL2_UNION.CONTROL2_CELLS.ACKERR_DET_EN = 1;
            i2c_master_register[i2c_port]->TRANSFER_LEN = config_size->send_size;
        ...
        case I2C_TRANSACTION_OPEN_DRAIN_SEND_TO_RECEIVE_DMA:
            i2c_master_register[i2c_port]->CONTROL_UNION.CONTROL = (1<<24) | (1<<16) | (1<<8) | (0x1);
            i2c_master_register[i2c_port]->CONTROL2_UNION.CONTROL2_CELLS.ACKERR_DET_EN = 1;
            i2c_master_register[i2c_port]->TRANSFER_LEN = config_size->send_size;
        ...
        case I2C_TRANSACTION_OPEN_DRAIN_SEND_DMA_EXTEND:
            /* bit24: len_chg, bit16:dir_chg, bit8:dma_en, bit0:rs_stop*/
            i2c_master_register[i2c_port]->CONTROL_UNION.CONTROL = (1<<8) | (0x1);
            i2c_master_register[i2c_port]->CONTROL2_UNION.CONTROL2_CELLS.ACKERR_DET_EN = 1;
...
void i2c_master_wait_transaction_finish(hal_i2c_port_t i2c_port, i2c_transaction_type_t transaction_type, volatile uint32_t *flag)
{
    ...
    while (1) {
        ...
        if (I2C_POLLING_TIMEOUT_VALUE < gpt_duration_count) {
            log_hal_msgid_error("[I2C%d][type=%d]:timeout!\r\n", 2, i2c_port, transaction_type);
            log_hal_msgid_error("hal_i2c_master_wait_transaction_finish,[type] =%d\r\n", 1, transaction_type);
            log_hal_msgid_error("START:0x%x\r\n", 1, (unsigned int)(i2c_master_register[i2c_port]->START));
            log_hal_msgid_error("INTR_STAT:0x%x\r\n", 1, (unsigned int)(i2c_master_register[i2c_port]->INTR_STAT));
            log_hal_msgid_error("ACK_ERR_REC:0x%x\r\n", 1, (unsigned int)(i2c_master_register[i2c_port]->ACKERR_FLAG_UNION.ACKERR_FLAG));
            log_hal_msgid_error("FIFO_STAT:0x%x\r\n", 1, (unsigned int)(i2c_master_register[i2c_port]->FIFO_STAT_UNION.FIFO_STAT));
            /* get iic transcation status */
            hal_nvic_restore_interrupt_mask(saved_mask);
            //assert(0);
            break;
        }
...
void i2c_master_dump_register(hal_i2c_port_t i2c_port)
{
    log_hal_msgid_error("--------- i2c%d dump ---------\r\n", 1, i2c_port);
    log_hal_msgid_error("START:%x, CONTROL:%x, CONTROL2:%x, IO_CNF:%x\r\n", 4,
                            i2c_master_register[i2c_port]->START,
                            i2c_master_register[i2c_port]->CONTROL_UNION.CONTROL,
                            i2c_master_register[i2c_port]->CONTROL2_UNION.CONTROL2,
                            i2c_master_register[i2c_port]->IO_CONFIG
                        );
    log_hal_msgid_error("TIMING:%x, DELAY_LEN:%x, HS:%x, CLOCK_DIV:%x\r\n", 4,
                            i2c_master_register[i2c_port]->TIMING_UNION.TIMING,
                            i2c_master_register[i2c_port]->DELAY_LEN,
                            i2c_master_register[i2c_port]->HS_UNION.HS,
                            i2c_master_register[i2c_port]->CLOCK_DIV
                        );
    log_hal_msgid_error("TRANSF_LEN:%x, TRANSF_LEN_AUX:%x, TRANSAC_LEN:%x\r\n", 3,
                            i2c_master_register[i2c_port]->TRANSFER_LEN,
                            i2c_master_register[i2c_port]->TRANSFER_LEN_AUX,
                            i2c_master_register[i2c_port]->TRANSAC_LEN
                        );
    log_hal_msgid_error("INTR_STA:%x, FIFO_STAT:%x, DEBUG_STA:%x, DEBUG_CTRL:%x\r\n", 4,
                            i2c_master_register[i2c_port]->INTR_STAT,
                            i2c_master_register[i2c_port]->FIFO_STAT_UNION.FIFO_STAT,
                            i2c_master_register[i2c_port]->DEBUGSTAT,
                            i2c_master_register[i2c_port]->DEBUGCTRL
                        );
    log_hal_msgid_error("SLV_ADDR:%x, INTR_MASK:%x, ACK_ERR:%x, PD_CG:%x, AO_CG:%x\r\n", 5,
                            i2c_master_register[i2c_port]->SLAVE_ADDR,
                            i2c_master_register[i2c_port]->INTR_MASK,
                            i2c_master_register[i2c_port]->ACKERR_FLAG_UNION.ACKERR_FLAG,
...
i2c_transaction_status_t i2c_get_transaction_status(uint32_t i2c_port, volatile uint32_t  *flag)
{
    ...
    if (return_status != I2C_TRANSACTION_SUCCESS) {
        ...
        while (1) {
            hal_nvic_save_and_set_interrupt_mask(&saved_mask);
            if (*flag == TRUE) {
                if (i2c_master_register[i2c_port]->INTR_STAT & (0x1 << I2C_COMPLETE_STATUS_BIT)) {
                    *flag = FALSE;
                    I2C_CLEAR_INTERRUPT_STATUS(i2c_port);
                    hal_nvic_restore_interrupt_mask(saved_mask);
                    return return_status;
                ...
                if (I2C_INTERRUPT_TIMEOUT_VALUE < gpt_duration_count) {
                    log_hal_msgid_error("[I2C%d][isr]:trans_comp timeout!\r\n", 1, i2c_port);
                    log_hal_msgid_error("i2c_get_transaction_status\r\n" , 1);
                    log_hal_msgid_error("START:0x%x\r\n", 1, (unsigned int)(i2c_master_register[i2c_port]->START));
                    log_hal_msgid_error("INTR_STAT:0x%x\r\n", 1, (unsigned int)(i2c_master_register[i2c_port]->INTR_STAT));
                    log_hal_msgid_error("ACK_ERR_REC:0x%x\r\n", 1, (unsigned int)(i2c_master_register[i2c_port]->ACKERR_FLAG_UNION.ACKERR_FLAG));
                    log_hal_msgid_error("FIFO_STAT:0x%x\r\n", 1, (unsigned int)(i2c_master_register[i2c_port]->FIFO_STAT_UNION.FIFO_STAT));
                    ...
                    I2C_CLEAR_INTERRUPT_STATUS(i2c_port);
                    return I2C_TRANSACTION_UNKNOWN_ERROR;
                }
            } else {
                hal_nvic_restore_interrupt_mask(saved_mask);
