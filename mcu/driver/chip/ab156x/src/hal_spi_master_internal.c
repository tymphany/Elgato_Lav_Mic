...
{
    static hal_spi_sw_fifo_base spi_sw_fifo_space[HAL_SPI_SW_FIFO_LEN];

    spi_sw_fifo[spi_port].spi_sw_fifo_len = HAL_SPI_SW_FIFO_LEN;
    spi_sw_fifo[spi_port].spi_sw_fifo_start = &spi_sw_fifo_space[0];
...
uint32_t spi_push_sw_fifo(uint32_t spi_port, hal_spi_sw_fifo_node_t *tansfer_config, uint32_t transfer_cnt)
{
    ...
    if (spi_sw_fifo[spi_port].spi_sw_fifo_r_index > spi_sw_fifo[spi_port].spi_sw_fifo_w_index) {
        ...
        if (temp_cnt1 < transfer_cnt) {
            ...
            memcpy(spi_sw_fifo_p, &tansfer_config[temp_cnt2] , temp_cnt2 * sizeof(hal_spi_sw_fifo_node_t));
            spi_sw_fifo[spi_port].spi_sw_fifo_w_index += temp_cnt0;
            temp_cnt0 =  temp_cnt1 + temp_cnt2;
        } else {
            spi_sw_fifo[spi_port].spi_sw_fifo_w_index = (spi_sw_fifo[spi_port].spi_sw_fifo_w_index + temp_cnt1) % spi_sw_fifo[spi_port].spi_sw_fifo_len ;
            temp_cnt0 =  temp_cnt1;
        }
    }
...
uint32_t  spi_pop_sw_fifo(uint32_t spi_port, hal_spi_sw_fifo_node_t *transfer_config, uint32_t transfer_cnt)
{
    ...
    if (spi_sw_fifo[spi_port].spi_sw_fifo_r_index ==  spi_sw_fifo[spi_port].spi_sw_fifo_w_index) {
        ...
        if (temp_cnt1 < transfer_cnt) {
            ...
            memcpy(transfer_config, spi_sw_fifo[spi_port].spi_sw_fifo_start , temp_cnt2 * sizeof(hal_spi_sw_fifo_node_t));
            spi_sw_fifo[spi_port].spi_sw_fifo_r_index += temp_cnt0;
            temp_cnt0 =  temp_cnt1 + temp_cnt2;
        } else {
            spi_sw_fifo[spi_port].spi_sw_fifo_r_index = (spi_sw_fifo[spi_port].spi_sw_fifo_r_index + temp_cnt1) %  spi_sw_fifo[spi_port].spi_sw_fifo_len ;
            temp_cnt0 =  temp_cnt1;
        }
    }
    hal_nvic_restore_interrupt_mask(saved_mask);
