...
typedef struct {
    ...
    uint8_t mavg_r[4];                                     /**< Touch control moving avergae times. */
    uint8_t avg_s[4];                                      /**< Touch control ADC in average times. */
    uint8_t coarse_cap[4];                                 /**< The channel coarse cap value. */
    int16_t high_thr[4];                                   /**< The channel high threshold. */
    int16_t low_thr[4];                                    /**< The channel low threshold. */
    int16_t high_nthr[4];                                  /**< The channel high Nthreshold. */
    int16_t low_nthr[4];                                   /**< The channel low Nthreshold. */
    int8_t  fine_cap[4];                                   /**< The channel cfine value. */
    uint8_t swtune_en;                                     /**< The channel coarsecap tune enable. */
    uint8_t swDebounceTime;                                /**< The channel debounce time. */
...
typedef struct {
    ...
    int16_t  mavg_dbg;                                      /**< Specify the current moving average adc value. */
    int16_t  man;                                           /**< Specify the current fine cap vavlue of the manual mode. */
    int8_t   fine_cap;                                      /**< Specify the calibration value. */
    int16_t  ear_detect_base;                               /**< Specify the ear detect base value. */
    int16_t  adc_debounce;                                  /**< Specify the adc debounce state. */
    int16_t  ear_detect_data;                               /**< Specify the current ear detect data */
    uint8_t  noise_cnt_race;                                /**< Specify the noise count of press and releas */
...
/** @brief This structure defines coarse cap command tune result. */
typedef struct {
    uint8_t result;                                    /**< Specify the coarse cap tune result. */
    uint8_t coarse_value;                       /**< Specify the coarese cap tune value. */
}hal_captouch_cal_result_t;

/** @brief This structure defines coarse cap command tune result. */
typedef struct {
    hal_captouch_cal_result_t coarse_cal[4];                                    /**< Specify the coarese cap command tune result for all channel. */
...
typedef enum {
    ...
    HAL_CAPTOUCH_CHANNEL_MAX                                /**< Specifies the maximum channel number. */
...
/**
 * @brief     This function change the moving average and adc average.
 * @param[in] channel is the channel number. For more details, please refer to #hal_captouch_channel_t.
 * @param[in] mavg_val is the number of samples to be averaged when calibrating channel. The range of the value should be 0<=mavg_val<=15.
 * @param[in] avg_val is the weight of new sample when computing adc output average. The range of the value should be 0<=avg_val<=7.
 * @return    #HAL_CAPTOUCH_STATUS_OK, if the operation is successful.\n
 */
...
/**
 * @brief     This function tunes the coarse cap value for all channels.
 * @param[in] coarse_cal_result is for the tuning result and coarse cap value #hal_captouch_coarse_cal_t.
 * @param[out] coarse_cal_result is for the tuning result and coarse cap value #hal_captouch_coarse_cal_t.
 * @return    #HAL_CAPTOUCH_STATUS_OK, if the operation is successful.\n
 */
