 ...
 *
 * - Use EINT. \n
 *  - Step 1: Call #hal_gpio_init() to init the pins, if EPT tool hasn't been used to configure the related pinmux. If EPT tool has been used, please skip step 1 and step 2.
 *  - Step 2: Call #hal_pinmux_set_function() to set the @ref GPIO pinmux. For more information about #hal_pinmux_set_function(), please refer to GPIO API reference.
 *  - Step 3: Call #hal_eint_mask() mask to select an EINT source.
 *  - Step 4: Initialize the selected EINT source.
 ...
 *  - Step 1: Call #hal_gpio_init() to init the pins, if EPT tool hasn't been used to configure the related pinmux. If EPT tool has been used, please skip step 1 and step 2.
 *  - Step 2: Call #hal_pinmux_set_function() to set the @ref GPIO pinmux. For more information about #hal_pinmux_set_function(), please refer to GPIO API reference.
 *  - Step 3: initialize the selected EINT source.
 ...
 *  - Step 4: regist a callback function. Call #hal_eint_register_callback() to register a user callback function.
 *  - Sample code:
 *    @code
 *       //Init GPIO, set GPIO pinmux(if EPT tool hasn't been used to configure the related pinmux).
 *       hal_gpio_init(HAL_GPIO_4);
...
/**
 * @brief This function sets the EINT debounce count.
 * @param[in] eint_number is the EINT number, the value is from HAL_EINT_0 to HAL_EINT_MAX-1, please refer to #hal_eint_number_t.
 * @param[in] count is the EINT number's hardware debounce count in 32768 Hz. EINT debounce is disabled when count is set to zero.
 * @return    To indicate whether this function call  is successful.
 *            If the return value is #HAL_EINT_STATUS_OK, the operation completed successfully;
 *            If the return value is #HAL_EINT_STATUS_INVALID_PARAMETER, a wrong parameter is given, the parameter must be verified.
 @par         Example
...
/**
 * @brief This function sets the mapping of the EINT to GPIO.
 * @param[in] eint_number is the EINT number, the value is from HAL_EINT_0 to HAL_EINT_MAX-1, please refer to #hal_eint_number_t.
 * @param[in] gpio_number specifies the pin number to map.
 * @return    To indicate whether this function call  is successful.
 *            If the return value is #HAL_EINT_STATUS_OK, the operation completed successfully;
 *            If the return value is #HAL_EINT_STATUS_INVALID_PARAMETER, a wrong parameter is given, the parameter must be verified.
 * @note This API only support on AB155x.
 */
hal_eint_status_t hal_eint_set_mux_mapping(hal_eint_number_t eint_number, hal_gpio_pin_t gpio_number);
/**
 * @brief This function gets the mapping of the EINT to GPIO.
 * @param[in] eint_number is the EINT number, the value is from HAL_EINT_0 to HAL_EINT_MAX-1, please refer to #hal_eint_number_t.
 * @param[out] gpio_number is the mapping of the GPIO pin.
 * @return    To indicate whether this function call  is successful.
 *            If the return value is #HAL_EINT_STATUS_OK, the operation completed successfully;
 *            If the return value is #HAL_EINT_STATUS_INVALID_PARAMETER, a wrong parameter is given, the parameter must be verified.
 * @note This API only support on AB155x.
