 ...
 *       const uint8_t send_data[8] = {0x00, 0x00, 0xFF, 0xAA, 0x14, 0x15, 0x16, 0x17}; //the data that used to send
 ...
 *
 *       //Setting gpio capacitance and slew-rate for i2c timing
 *       #ifdef HAL_GPIO_FEATURE_SET_CAPACITANCE
 *       hal_gpio_set_capacitance(HAL_GPIO_27, HAL_GPIO_CAPACITANCE_3);
 *       hal_gpio_set_capacitance(HAL_GPIO_28, HAL_GPIO_CAPACITANCE_3);
 *       #ifdef HAL_GPIO_FEATURE_SET_SLEW_RATE
 *       hal_gpio_set_slew_rate(HAL_GPIO_27);
 *       hal_gpio_set_slew_rate(HAL_GPIO_28);
 *       #endif
 *       #endif
 *
 ...
 *       hal_i2c_config_t i2c_config;
 *       uint8_t slave_address = 0X50;
 *       ATTR_RWDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN uint8_t send_data[8] = {0x00, 0x00, 0xFF, 0xAA, 0x14, 0x15, 0x16, 0x17}; //the data that used to send
 *       ATTR_RWDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN uint8_t receive_data[8] = {0};
 *       volatile uint32_t size = 8;
 *       i2c_config.frequency = HAL_I2C_FREQUENCY_400K;
 *
 *       //Init GPIO, set GPIO pinmux(if EPT tool hasn't been used to configure the related pinmux).
 ...
 *       hal_pinmux_set_function(HAL_GPIO_28, function_index);
 *
 *       //Setting gpio capacitance and slew-rate if HAL_GPIO_FEATURE_SET_CAPACITANCE option enabled.
 *       #ifdef HAL_GPIO_FEATURE_SET_CAPACITANCE
 *       hal_gpio_set_capacitance(HAL_GPIO_27, HAL_GPIO_CAPACITANCE_3);
 *       hal_gpio_set_capacitance(HAL_GPIO_28, HAL_GPIO_CAPACITANCE_3);
 *       #ifdef HAL_GPIO_FEATURE_SET_SLEW_RATE
 ...
 *       //Setting gpio capacitance and slew-rate if HAL_GPIO_FEATURE_SET_CAPACITANCE option enabled.
 *       #ifdef HAL_GPIO_FEATURE_SET_CAPACITANCE
 *       hal_gpio_set_capacitance(gpio_pin, HAL_GPIO_CAPACITANCE_3);
 *       #ifdef HAL_GPIO_FEATURE_SET_SLEW_RATE
 *       hal_gpio_set_slew_rate(gpio_pin);
 *       #endif
 *       #endif
 *
 *       if(HAL_I2C_STATUS_OK == hal_i2c_master_init(HAL_I2C_MASTER_0, &i2c_config)) {
 ...
 *       ATTR_RWDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN static uint8_t i2c_send_data[8] = {0x00, 0x00, 0xFF, 0xAA, 0x14, 0x15, 0x16, 0x17}; //the data that used to send
 *       ATTR_RWDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN static uint8_t i2c_receive_data[8] = {0};
 *
 ...
 *       //Setting gpio capacitance and slew-rate if HAL_GPIO_FEATURE_SET_CAPACITANCE option enabled.
 *       #ifdef HAL_GPIO_FEATURE_SET_CAPACITANCE
 *       hal_gpio_set_capacitance(HAL_GPIO_36, HAL_GPIO_CAPACITANCE_3);
 *       hal_gpio_set_capacitance(HAL_GPIO_37, HAL_GPIO_CAPACITANCE_3);
 *       #ifdef HAL_GPIO_FEATURE_SET_SLEW_RATE
 *       hal_gpio_set_slew_rate(HAL_GPIO_36);
 *       hal_gpio_set_slew_rate(HAL_GPIO_37);
 *       #endif
 *       #endif
 *
 *       if(HAL_I2C_STATUS_OK == hal_i2c_master_init(HAL_I2C_MASTER_0, &i2c_config)) {
 ...
 * - \b Using \b I2C \b in \b extended \b DMA \b mode \b with \b sw \b fifo. \n
 *  To use I2C driver in extended DMA mode, configure the GPIO pins to pinmux to SCL and SDA, then call
 *  #hal_pinmux_set_function to select the pinmux function. After setting the pinmux, call #hal_i2c_master_send_to_receive_dma_ex_none_blocking
 *  to send and receive data. The steps are shown below:
 *  - Step1: Call #hal_gpio_init() to initialize the pin. For mode details about hal_gpio_init please refer to GPIO module in HAL.
 *  - Step2: Call #hal_pinmux_set_function() to set the GPIO Pinmux or use the EPT tool to apply the pinmux settings.
 *  - Step3: Call #hal_i2c_master_send_to_receive_dma_ex_none_blocking() to send and receive data.
 *  - Sample code:
 *    @code
 *       hal_i2c_send_to_receive_config_ex_no_busy_t  hal_i2c_send_to_receive_config_ex_no_busy;
 *       ATTR_RWDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN static uint8_t i2c_send_data[8] = {0x00, 0x00, 0xFF, 0xAA, 0x14, 0x15, 0x16, 0x17}; //the data that used to send
 *       ATTR_RWDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN static uint8_t i2c_receive_data[8] = {0};
 *
 *       i2c_send_to_receive_config_no_busy_ex.i2c_config.frequency = HAL_I2C_FREQUENCY_400K;
 *       i2c_send_to_receive_config_no_busy_ex.i2c_callback = i2c_user_callback;
 *       i2c_send_to_receive_config_no_busy_ex.user_data = user_data;
 *       i2c_send_to_receive_config_no_busy_ex.i2c_send_to_receive_config_ex.slave_address = I2C_EEPROM_SLAVE_ADDRESS;
 *       i2c_send_to_receive_config_no_busy_ex.i2c_send_to_receive_config_ex.send_data = i2c_send_data;
 *       i2c_send_to_receive_config_no_busy_ex.i2c_send_to_receive_config_ex.send_packet_length = 1;
 *       i2c_send_to_receive_config_no_busy_ex.i2c_send_to_receive_config_ex.send_bytes_in_one_packet = 1;
 *
 *       i2c_send_to_receive_config_no_busy_ex.i2c_send_to_receive_config_ex.receive_buffer = i2c_receive_data;
 *       i2c_send_to_receive_config_no_busy_ex.i2c_send_to_receive_config_ex.receive_packet_length = 1;
 *       i2c_send_to_receive_config_no_busy_ex.i2c_send_to_receive_config_ex.receive_bytes_in_one_packet = 5;
 ...
 *       #endif
 *
 *       if (HAL_I2C_STATUS_OK != hal_i2c_master_send_to_receive_dma_ex_none_blocking(i2c_port, &i2c_send_to_receive_config_no_busy_ex)) {
 *               //Error handler;
 *           }
 *
 *    @endcode
...
/** @brief This enum defines the transaction result event. These result is passed through a callback function.
  ...
  */
typedef enum {
    HAL_I2C_NON_BLOCKING_FUNCTION_ERROR = -4,    /**<  The I2C is using by non-block transaction. */
...
typedef enum {
    HAL_I2C_STATUS_FULL_BY_NON_BLOCKING_FUNCTION = -7,/**<  An error occurred when sw fifo is full. */
    HAL_I2C_STATUS_ERROR_USED_BY_NORMAL_USER = -6,    /**<  An error occurred when HW is using by normal api. */
    HAL_I2C_STATUS_BUSY_BY_NON_BLOCKING_FUNCTION = -5,/**<  An error occurred when HW is using by non-blocking api. */
...
typedef enum {
    ...
    HAL_I2C_FREQUENCY_722K = 5,          /**<  722kbps. */
    HAL_I2C_FREQUENCY_1M   = 6,          /**<  1mbps. */
#ifdef HAL_I2C_MASTER_FEATURE_HIGH_SPEED
    HAL_I2C_FREQUENCY_2M   = 7,          /**<  2mbps. */
    HAL_I2C_FREQUENCY_3M   = 8,          /**<  3.25mbps. */
    HAL_I2C_FREQUENCY_2M5  = 9,          /**<  2.5mbps. */
    HAL_I2C_FREQUENCY_2M8  = 10,         /**<  2.8mbps. */
#endif
...
/** @brief This enum defines the I2C IO mode.  */
typedef enum {
    HAL_I2C_IO_PUSH_PULL  = 0,          /**<  push-pull. */
    HAL_I2C_IO_OPEN_DRAIN = 3,          /**<  open-drain. */
    HAL_I2C_IO_MAX                /**<  The total number of supported I2C IO config.*/
} hal_i2c_io_config_t;
...
/** @brief This structure defines the configuration parameters for extended DMA with sw fifo send to receive transaction format.
 *   For more details about i2c_config, i2c_callback, user_data and i2c_send_to_receive_config_ex, see @ref HAL_I2C_Transaction_Pattern_Chapter chapter.
 */
typedef struct {
    hal_i2c_config_t i2c_config;            /**<  The I2C master setting. */
    hal_i2c_callback_t i2c_callback;        /**<  The user callback. */
    void *user_data;                        /**<  The user data. */
    hal_i2c_send_to_receive_config_ex_t i2c_send_to_receive_config_ex; /**<  The I2C master transaction setting. */
} hal_i2c_send_to_receive_config_ex_no_busy_t;
 ...
 * @par       Example
 * @code
 *       ret = hal_i2c_master_set_frequency(HAL_I2C_MASTER_1,HAL_I2C_FREQUENCY_400K);
 * @endcode
 * @sa #hal_i2c_master_init()
 */
...
#ifdef HAL_I2C_MASTER_FEATURE_CONFIG_IO
/**
 * @brief This function sets the SCL/SDA IO config.
 * @param[in] i2c_port is the I2C master port number. The value is defined in #hal_i2c_port_t.
 * @param[in] io is an enum value defined in #hal_i2c_io_config_t. Only value of type #hal_i2c_io_config_t is accepted.
 * @return   #HAL_I2C_STATUS_OK, the operation completed successfully.
 *           #HAL_I2C_STATUS_ERROR, the operation is failed.
 * @par       Example
 * @code
 *       ret = hal_i2c_master_set_io_config(HAL_I2C_MASTER_1,HAL_I2C_IO_PUSH_PULL);
 * @endcode
 * @sa #hal_i2c_master_init()
 */
hal_i2c_status_t hal_i2c_master_set_io_config(hal_i2c_port_t i2c_port, hal_i2c_io_config_t io);
#endif /* #ifdef HAL_I2C_MASTER_FEATURE_CONFIG_IO */
...
#ifdef HAL_I2C_MASTER_FRATURE_NONE_BLOCKING
/**
 * @brief This function sends data to and then receives data from I2C slave in a extended DMA mode with software fifo.
 *  This function returns immediately after configuring the hardware registers. And this function can be used separately without call hal_i2c_master_init().For more details of extended DMA mode, see
 *  extended DMA mode in @ref HAL_I2C_Features_Chapter chapter. For details about transaction length and transaction waveform pattern, see @ref HAL_I2C_Transaction_Pattern_Chapter chapter.
 *  Note, not all I2C masters support DMA mode. For more details about DMA mode, see #hal_i2c_port_t.
 * @param[in] i2c_port is the I2C master port number. The value is defined in #hal_i2c_port_t.
 * @param[in,out] i2c_send_to_receive_config_no_busy_ex is the configuration parameter of this API for both send and receive.
 * @return    #HAL_I2C_STATUS_OK, the operation completed successfully.\n
