 ...
 *
 * - \b Support \b RC5 \b \b NEC \b \b RC6 \b \b RCMM \b and \b PWD \b modes.
 *   The legacy design supports two hardware decoding methods(RC5,pwd),and the new design supports four decoding methods(NEC,RC5,RC6,RCMM):
 *  - \b NEC \b mode:
 *  The NEC protocol uses pulse distance encoding of the bits.
 *  @image html nec_potocol.png
 *  In this mode, the NEC code can be received and decoded directly.The first bit IRRX sample would be the bit that the arrow point. The bit order would be auto adjust by hardware.
 *  - \b RC5 \b mode:
 *  The Philips RC5 IR transmission protocol uses Manchester encoding of the message bits.
 *  @image html rc5_potocol.png
 *  In this mode, the RC5 code can be received and decoded directly.The first bit IRRX sample would be the bit that the arrow point. The bit order would be auto adjust by hardware.
 *  - \b RC6 \b mode:
 *  The Philips RC6 IR transmission protocol uses Manchester encoding of the message bits.
 *  In this mode, the RC6 code can be received and decoded directly.The RC6 potocol is similar with RC5.
 *  - \b RCMM \b mode:
 *  The Philips RCMM IR transmission protocol uses Manchester encoding of the message bits.
 *  @image html rcmm_potocol.png
 *  In this mode, the RCMM code can be received and decoded directly.The first bit IRRX sample would be the bit that the arrow point. The bit order would be auto adjust by hardware.
 *  - \b PWD \b mode:
 ...
 *  - Step4: Call hal_irrx_receive_nec_start() to start the hardware.
 *  - Step5: Call hal_irrx_receive_nec() to receive the NEC code.
 ...
 *       hal_irrx_init();
 *
 *       hal_irrx_receive_nec_start(user_callback,NULL);     //A callback function for the hardware will be triggered when the received code is decoded.
 *    @endcode
 *    @code
 *       // Callback function. This function should be registered with #hal_irrx_receive_nec_start().
 *       void user_callback (void *user_data)
 *       {
 *           hal_irrx_receive_nec(data,bit_length);
 *           hal_irrx_deinit();
 *           // user's handler
 *       }
 *
 ...
 *
 *       hal_irrx_init();
 *       #ifdef HAL_IRRX_DESIGN_LEGACY
 *       //legacy using function
 ...
 *       #else
 *       //other
 *       hal_irrx_receive_rc5_start(user_callback,NULL);     //A callback function for the hardware will be triggered when the received code is decoded.
 *       #endif
 *    @endcode
 *    @code
 ...
 *       void user_callback (void *user_data)
 *       {
 *           hal_irrx_receive_rc5(data,bit_length);
 *           hal_irrx_deinit();
 *           // user's handler
 *       }
 *
 *    @endcode
 *
 *  - Use IRRX in the RC6 mode. \n
 *  - Step1: Call #hal_gpio_init() to init the pins, if EPT tool hasn't been used to configure the related pinmux.
 *  - Step2: Set GPIO34 to RX function to enable the IRRX mode.
 *  - Step3: Call hal_irrx_init() to initialize the IRRX.
 *  - Step4: Call hal_irrx_receive_rc6_start() to start the hardware.
 *  - Step5: Call hal_irrx_receive_rc6() to receive the RC5 code.
 *  - Step6: Call hal_irrx_deinit() to deinitialize the IRRX.
 *  - sample code:
 *    @code
 *       //Init GPIO, set GPIO pinmux(if EPT tool hasn't been used to configure the related pinmux).
 ...
 *       hal_irrx_init();
 *
 *       hal_irrx_receive_rc6_start(user_callback,NULL);     //A callback function for the hardware will be triggered when the received code is decoded.
 *    @endcode
 *    @code
 *       // Callback function. This function should be registered with #hal_irrx_receive_rc5_start().
 *       void user_callback (void *user_data)
 *       {
 *           hal_irrx_receive_rc6(data,bit_length);
 *           hal_irrx_deinit();
 *           // user's handler
 *       }
 *
 *    @endcode
 *
 *  - Use IRRX in the RCMM mode. \n
 *  - Step1: Call #hal_gpio_init() to init the pins, if EPT tool hasn't been used to configure the related pinmux.
 *  - Step2: Set GPIO34 to RX function to enable the IRRX mode.
 *  - Step3: Call hal_irrx_init() to initialize the IRRX.
 *  - Step4: Call hal_irrx_receive_rcmm_start() to start the hardware.
 *  - Step5: Call hal_irrx_receive_rcmm() to receive the RC5 code.
 *  - Step6: Call hal_irrx_deinit() to deinitialize the IRRX.
 *  - sample code:
 *    @code
 *       //Init GPIO, set GPIO pinmux(if EPT tool hasn't been used to configure the related pinmux).
 ...
 *       hal_irrx_init();
 *
 *       hal_irrx_receive_rcmm_start(user_callback,NULL);     //A callback function for the hardware will be triggered when the received code is decoded.
 *    @endcode
 *    @code
 *       // Callback function. This function should be registered with #hal_irrx_receive_rc5_start().
 *       void user_callback (void *user_data)
 *       {
 *           hal_irrx_receive_rcmm(data,bit_length);
 *           hal_irrx_deinit();
 *           // user's handler
 *       }
 *
...
/**
 * @brief     This function receives the infrared radiation RC5 code.
 * @param[in] data the infrared radiation RC5 data format. The fist bit is the toggle bit in RC5 spec,the S1 S2 pulse bit would be ingnored.
 * @param[in] receive_bits_length is the RC5 receive pulse bit number.
 * @return   Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 *                If the return value is #HAL_IRRX_STATUS_INVALID_PARAM, it means a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
...
/**
 * @brief     This function starts to receive the infrared radiation NEC code.
 * @param[in] callback the callback function to be called when the receive operation is complete.
 * @param[in] parameter the parameter to the caller when return.
 * @return    Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 ...
 * @par       Example
 * Sample code, please refer to @ref HAL_IRRX_Driver_Usage_Chapter.
 * @sa  hal_irrx_receive_nec()
 */
hal_irrx_status_t hal_irrx_receive_nec_start(hal_irrx_callback_t callback, void *parameter);

/**
 * @brief     This function receives the infrared radiation NEC code.
 * @param[in] data the infrared radiation NEC data format. The fist bit is the address bit in NEC spec,the first 9ms pulse period would be ingnored.
 * @param[in] receive_bits_length is the NEC receive pulse bit number.
 * @return   Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 *                If the return value is #HAL_IRRX_STATUS_INVALID_PARAM, it means a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
 * Sample code, please refer to @ref HAL_IRRX_Driver_Usage_Chapter.
 * @sa  hal_irrx_receive_nec_start()
 */
hal_irrx_status_t hal_irrx_receive_nec(uint8_t *data, uint8_t *receive_bits_length);

/**
 * @brief     This function starts to receive the infrared radiation RC6 code.
 * @param[in] callback the callback function to be called when the receive operation is complete.
 * @param[in] parameter the parameter to the caller when return.
 * @return    Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 ...
 * @par       Example
 * Sample code, please refer to @ref HAL_IRRX_Driver_Usage_Chapter.
 * @sa  hal_irrx_receive_rc6()
 */
hal_irrx_status_t hal_irrx_receive_rc6_start(hal_irrx_callback_t callback, void *parameter);

/**
 * @brief     This function receives the infrared radiation RC6 code.
 * @param[in] data the infrared radiation RC6 data format. The fist bit is the header bit in RC6 spec,the 2.66ms leader pulse period would be ingnored.
 * @param[in] receive_bits_length is the RC6 receive pulse bit number.
 * @return   Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 *                If the return value is #HAL_IRRX_STATUS_INVALID_PARAM, it means a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
 * Sample code, please refer to @ref HAL_IRRX_Driver_Usage_Chapter.
 * @sa  hal_irrx_receive_rc6_start()
 */
hal_irrx_status_t hal_irrx_receive_rc6(uint8_t *data, uint8_t *receive_bits_length);

/**
 * @brief     This function starts to receive the infrared radiation RCMM code.
 * @param[in] callback the callback function to be called when the receive operation is complete.
 * @param[in] parameter the parameter to the caller when return.
 * @return    Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 ...
 * @par       Example
 * Sample code, please refer to @ref HAL_IRRX_Driver_Usage_Chapter.
 * @sa  hal_irrx_receive_rcmm()
 */
hal_irrx_status_t hal_irrx_receive_rcmm_start(hal_irrx_callback_t callback, void *parameter);

/**
 * @brief     This function receives the infrared radiation RCMM code.
 * @param[in] data the infrared radiation RCMM data format. The fist bit is the data pulse in RCMM spec(In the RCMM potocol,one pulse represent two  bit data),the head pulse period would be ingnored.
 * @param[in] receive_bits_length is the RCMM receive pulse bit number.
 * @return   Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 *                If the return value is #HAL_IRRX_STATUS_INVALID_PARAM, it means a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
 * Sample code, please refer to @ref HAL_IRRX_Driver_Usage_Chapter.
 * @sa  hal_irrx_receive_rcmm_start()
 */
hal_irrx_status_t hal_irrx_receive_rcmm(uint8_t *data, uint8_t *receive_bits_length);

/**
 * @brief     This function starts to receive the infrared radiation SIRC(support 12/15/20 bit mode).
 * @param[in] callback the callback function to be called when the receive operation is complete.
 * @param[in] parameter the parameter to the caller when return.
 * @return    Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 ...
 * @par       Example
 * Sample code, please refer to @ref HAL_IRRX_Driver_Usage_Chapter.
 * @sa  hal_irrx_receive_sirc()
 */
hal_irrx_status_t hal_irrx_receive_sirc_start(hal_irrx_callback_t callback, void *parameter);

/**
 * @brief     This function receives the infrared radiation SIRC(support 12/15/20 bit mode).
 * @param[in] data the infrared radiation SIRC data format. The first 2.4ms pulse period would be ingnored.
 * @param[in] receive_bits_length is the SIRC receive pulse bit number.
 * @return   Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 *                If the return value is #HAL_IRRX_STATUS_INVALID_PARAM, it means a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
 * Sample code, please refer to @ref HAL_IRRX_Driver_Usage_Chapter.
 * @sa  hal_irrx_receive_sirc_start()
 */
 ...
 *             #HAL_IRRX_BUSY means the IRRX status is busy.
 *             #HAL_IRRX_IDLE means the IRRX status is idle, user can use it to transfer data now.
 * @return   Indicates whether this function call is successful or not.
 *                If the return value is #HAL_IRRX_STATUS_OK, the operation completed successfully.
 * @par       Example
 * Sample code, please refer to @ref HAL_IRRX_Driver_Usage_Chapter.
