 ...
 * Users can use this API to adjust different current output settings through ISINK registers. For example, to adjust the backlight brightness.
 *
 *
 ...
 * -The ISINK breath mode configure parameter show as below:
 *  @image html hal_isink_breath_mode_image.png
 * -The ISINK pwm mode configure parameter show as below:
 *  @image html hal_isink_pwm_mode_image.png
 *
 * - The user can set ISINK to operate in a different mode. The steps are shown below:\n
 *
 *  - Step1: Call #hal_isink_init() to initialize the ISINK module.
 *  - Step2: Call #hal_isink_configure() to configure ISINK mode and setting.
 *  - Step3: Call #hal_isink_start() to trigger ISINK HW start output signal.
 *  - Step4: Call #hal_isink_stop() to trigger ISINK HW stop output signal.
 *  - Step5: Call #hal_isink_deinit() to de-initialize ISINK module.
 *  - Breath mode sample code:
 *    @code
 *       hal_isink_config_t  isink;
 *
 *       isink.mode = HAL_ISINK_MODE_BREATH;
 *       isink.config.breath_mode.darker_to_lighter_time1 = HAL_ISINK_TIME_0_123_SECOND;
 *       isink.config.breath_mode.darker_to_lighter_time2 = HAL_ISINK_TIME_0_123_SECOND;
 *       isink.config.breath_mode.lightest_time           = HAL_ISINK_TIME_0_123_SECOND;
 *       isink.config.breath_mode.lighter_to_darker_time1 = HAL_ISINK_TIME_0_123_SECOND;
 *       isink.config.breath_mode.lighter_to_darker_time2 = HAL_ISINK_TIME_0_123_SECOND;
 *       isink.config.breath_mode.darkest_time            = HAL_ISINK_TIME_0_246_SECOND;
 *
 *       if(HAL_ISINK_STATUS_OK != hal_isink_init(HAL_ISINK_CHANNEL_0)) {  //Initialize the ISINK.
 *             //Error handler
 *       }
 *       if(HAL_ISINK_STATUS_OK != hal_isink_configure(HAL_ISINK_CHANNEL_0,&isink)) { //Configure ISINK mode and setting.
 *             //Error handler
 *       }
 *       if(HAL_ISINK_STATUS_OK != hal_isink_start(HAL_ISINK_CHANNEL_0)) {  //Trigger ISINK execution.
 *             //Error handler
 *       }
 *       if(HAL_ISINK_STATUS_OK != hal_isink_stop(HAL_ISINK_CHANNEL_0)) {  //Stop ISINK.
 *             //Error handler
 *       }
 ...
 *       }
 *    @endcode
 *  - Pwm mode sample code:
 *    @code
 *       hal_isink_config_t  isink;
 *
 *       isink.mode = HAL_ISINK_MODE_PWM;
 *       isink.config.pwm_mode.hi_level_time = 500; //unit:ms
 *       isink.config.pwm_mode.lo_level_time = 500; //unit:ms
 *       isink.config.pwm_mode.blink_nums    = 2;
 *       isink.config.pwm_mode.idle_time     = 2000; //unit:ms
 *
 *       if(HAL_ISINK_STATUS_OK != hal_isink_init(HAL_ISINK_CHANNEL_0)) {  //Initialize the ISINK.
 *             //Error handler
 *       }
 ...
 *       }
 *    @endcode
 *  - Register mode sample code:
 *    @code
 *       hal_isink_config_t  isink;
 *
 *       isink.mode = HAL_ISINK_MODE_REGISTER;
 *       isink.config.register_mode.current = HAL_ISINK_OUTPUT_CURRENT_4_MA;
 *       isink.config.register_mode.enable_double = false; //Disable double current
 *
 *       if(HAL_ISINK_STATUS_OK != hal_isink_init(HAL_ISINK_CHANNEL_0)) {  //Initialize the ISINK.
 *             //Error handler
 *       }
 ...
 * @{
 * @section ISINK_Driver_Usage_Chapter How to use this module
 * The ISINK register mode configure parameter show as below:
...
/** @brief isink breath mode config */
typedef struct {
    hal_isink_breath_adjust_on_t  darker_to_lighter_time1;         /**<The ISINK breath mode timing adjustment setting 1 corresponding to the brightness level from darker to lighter. */
    hal_isink_breath_adjust_on_t  darker_to_lighter_time2;         /**<The ISINK breath mode timing adjustment setting 2 corresponding to the brightness level from darker to lighter. */
    hal_isink_breath_adjust_on_t  lightest_time;                   /**< Duration of the brightest level.  */
    hal_isink_breath_adjust_on_t  lighter_to_darker_time1;         /**<The ISINK breath mode timing adjustment setting 1 corresponding to the brightness level from lighter  to darker. */
    hal_isink_breath_adjust_on_t  lighter_to_darker_time2;         /**<The ISINK breath mode timing adjustment setting 2 corresponding to the brightness level from lighter   to darker.*/
...
}hal_isink_pwm_mode_t;

/** @brief isink register mode config */
typedef struct {
    hal_isink_current_t     current;       /**< The current that ISINK output. */
    bool                    enable_double; /**< Enable or disable double current. */
}hal_isink_register_mode_t;

/** @brief isink config */
typedef struct{
    hal_isink_mode_t                mode;          /**< The mode of ISINK. */
    union{
        hal_isink_breath_mode_t     breath_mode;   /**< The config of breath mode when the mode is breath mode. */
        hal_isink_pwm_mode_t        pwm_mode;      /**< The config of pwm mode when the mode is pwm mode. */
        hal_isink_register_mode_t   register_mode; /**< The config of register mode when the mode is register mode */
    }config;                                       /**< The ISINK configure setting. */
}hal_isink_config_t;

#ifdef HAL_ISINK_FEATURE_HW_PMIC2562
/** @brief isink timing config */
typedef struct {
    uint16_t  t0;                   /**<The isink delay start time(unit ms)*/
    struct {
        uint16_t  t_rising;         /**<The duration time from off to lightest(unit ms). */
        uint16_t  t_lightest;       /**<The duration time of the brightest level(unit ms). */
    }t1;                            /**<The duration time of the isink on(unit ms). */
    struct {
        uint16_t  t_falling;        /**<The duration time from lightest to darkest(unit ms). */
        uint16_t  t_darkest;        /**<The duration time of the darkest time(unit ms). */
    }t2;                            /**<The duration time of the isink off(unit ms). */
    uint16_t  t3;                   /**<The isink idle time for next ext-loop(unit ms). */
}hal_isink_timing_config_t;

/** @brief isink extend config */
typedef struct{
    hal_isink_timing_config_t       timing;        /**< The timing of isink. */
    uint8_t                         brightness;    /**< The brightness of isink. */
    uint8_t                         blink_nums;    /**< The blink number of led(the period is t1 + t2). */
}hal_isink_config_ext_t;
...
#ifdef HAL_ISINK_FEATURE_ADVANCED_CONFIG
/**
 * @brief  This function is to configure ISINK HW.
 * @param[in]  channel is the ISINK channel number. For more details about the parameter, please refer to #hal_isink_channel_t.
 * @param[in]  config is the ISINK mode and mode setting.For more details about the parameter, please refer to #hal_isink_config_t.
 * @return     Indicates whether this function call is successful or not.
 *            If the return value is #HAL_ISINK_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_ISINK_STATUS_ERROR_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
 */
hal_isink_status_t  hal_isink_configure(hal_isink_channel_t channel, hal_isink_config_t *config);

#ifdef HAL_ISINK_FEATURE_HW_PMIC2562
/**
 * @brief  This function is to configure ISINK with new hardware feature.
 * @param[in]  channel is the ISINK channel number. For more details about the parameter, please refer to #hal_isink_channel_t.
 * @param[in]  config is the ISINK timing, brightness and blink numbers. For more details about the parameter, please refer to #hal_isink_config_ext_t.
 * @return     Indicates whether this function call is successful or not.
 *            If the return value is #HAL_ISINK_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_ISINK_STATUS_ERROR_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
 */
hal_isink_status_t  hal_isink_configure_ext(hal_isink_channel_t channel, hal_isink_config_ext_t *config);
#endif

/**
 * @brief  This function start ISINK hardware.
 * @param[in]  channel is the ISINK channel number. For more details about the parameter, please refer to #hal_isink_channel_t.
 * @return     Indicates whether this function call is successful or not.
 *            If the return value is #HAL_ISINK_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_ISINK_STATUS_ERROR_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
...
/**
 * @brief  This function stop ISINK hardware.
 * @param[in]  channel is the ISINK channel number. For more details about the parameter, please refer to #hal_isink_channel_t.
 * @return     Indicates whether this function call is successful or not.
 *            If the return value is #HAL_ISINK_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_ISINK_STATUS_ERROR_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
