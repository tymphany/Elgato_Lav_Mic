 ...
 * @{
 * This section describes the programming interfaces of Memory Protection Unit (MPU) HAL driver.
 * The Armv8-M architecture provides a system address map and permits implementations to include an optional MPU.
 * The optional MPU uses the Protected Memory System Architecture (PMSAv8) and contains improved flexibility in the MPU region definition.
 * The MPU supports:
 ...
 * |Terms                   |Details                                                                 |
 * |------------------------------|------------------------------------------------------------------------|
 * |\b MPU                        | Memory Protection Unit. For more information, please refer to <a href="https://developer.arm.com/documentation/ddi0553/bo">ARMv8-M Architecture Reference Manual</a>.|
 *
 *
 * @section HAL_MPU_Features_Chapter Supported features
 * The MPU HAL driver enables configuration of location, size and access permissions for each region. The configuration of memory attributes is not supported. Call the MPU APIs to configure the MPU, as shown below:
 * - \b Configure \b each \b region \b for \b different \b access \b permissions. \n
 *   Call hal_mpu_region_configure(), to configure the specified region. The first parameter is of #hal_mpu_region_t type, to indicate which region is to be configured. The second parameter is of #hal_mpu_region_config_t type, and defines the location, size and access permissions of the region. The sub region feature of MPU is also supported by defining mpu_subregion_mask field of type #hal_mpu_region_config_t.
 * - \b Enable \b the \b MPU \b feature \b inside \b the \b MCU. \n
 *   To enable the MPU settings for the configured region(s), user has to follow these two steps:
 ...
 * 	 memset(region_config, 0, sizeof(hal_mpu_region_config_t));//Note, call memset() to make sure all memory regions are set to 0.
 * 	 region_config.mpu_region_address = 0x10000000;//The start address of the region that is configured.
 * 	 region_config.mpu_region_end_address = 0x10001000;//The limit address of the region that is configured.
 * 	 region_config.mpu_region_access_permission = HAL_MPU_RO_BY_PRIVILEGED; //Read-only by privileged code only.
 * 	 region_config.mpu_xn = FALSE;//Enables the instruction access.
 *
 * 	 hal_mpu_region_configure(region, &region_config);
 *
...
#ifdef CORE_CM33
    HAL_MPU_REGION_8 = 8,                       /**< MPU region 8 */
    HAL_MPU_REGION_9 = 9,                       /**< MPU region 9 */
    HAL_MPU_REGION_10 = 10,                     /**< MPU region 10 */
    HAL_MPU_REGION_11 = 11,                     /**< MPU region 11 */
    HAL_MPU_REGION_12 = 12,                     /**< MPU region 12 */
    HAL_MPU_REGION_13 = 13,                     /**< MPU region 13 */
    HAL_MPU_REGION_14 = 14,                     /**< MPU region 14 */
    HAL_MPU_REGION_15 = 15,                     /**< MPU region 15 */
#endif
...
typedef enum {
    ...
    HAL_MPU_RW_BY_PRIVILEGED = 0,               /**< Read/write by privileged code only. */
    HAL_MPU_RW_BY_ANY = 1,                      /**< Read/write by any privilege level. */
    HAL_MPU_RO_BY_PRIVILEGED = 2,               /**< Read-only by privileged code only. */
    HAL_MPU_RO_BY_ANY = 3                       /**< Read-only by any privilege level. */
#else
    #error "Unsupported hardware architecture( MPU )."
#endif
