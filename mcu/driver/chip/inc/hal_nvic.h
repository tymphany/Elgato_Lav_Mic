 ...
 *           function_task();
 *       } else {
 *           // Cortex-M4 is handling the exception and the exception number is HAL_NVIC_QUERY_EXCEPTION_NUMBER.
 *           function_isr();
 *       }
 *
 *    @endcode
 ...
 *  If the value is 0, the Cortex-M4 is in thread mode, otherwise an exception occured and the exception number is the value of the macro.
 *  The value of the IRQn is euqal to the exception number minus 16 (IRQn = exception_number - 16).
 *
 *  If the IRQn is less than zero, it is listed as below.
 *
 * | exception number|exception types  |IRQ number|description |
 * |-----------------|-----------------|----------|----------------------------------------------|
 * |1                |Reset            | -15      | Two levels of reset: power on and local rset. |
 * |2                |NonMaskableInt   | -14      | None Maskable interrupt.|
 * |3                |HardFault        | -13      | Hard Fault exception: fault than can not be handled by any exception mechanisms.|
 * |4                |MemoryManagement | -12      | Memory management exception determined by MPU or fixed memory constrains. |
 * |5                |BusFault         | -11      | Memory-related faults other than those handled by MemManage. Can be synchronous and asynchronous.|
 * |6                |UsageFault       | -10      | Non memory-related faults. Undef/invalid state/error on exception return/attempt access to a disable and unavailable coprocessor.|
 * |11               |SVC              | -5       | Supervisor Call. |
 * |12               |DebugMonitor     | -4       | Debug Monitor exception. |
 * |13               |PendSV           | -2       | PendSV. |
 * |15               |SysTick          | -1       | SysTick. |
 * |16~n             |Interrupts       | 0~n      | Interrupts.|
 *
 *  If the IRQn is greater than or equal to zero, it is the peripheral IRQ.
 *  If you want to know what module each IRQn represents.\n
 *  Please refer to the \<chip_name\>.h in the path driver\\CMSIS\\Device\\airoha\\\<chip_name\>\\Include.
 *
 * @code
 ...
 *  uint32_t irq_number;
 *
 *  irq_number = HAL_NVIC_QUERY_EXCEPTION_NUMBER - 16;
 *
 *  @endcode
*/

/** @brief  This macro is used to query current exception number.
  */
