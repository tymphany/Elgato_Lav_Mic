 ...
 *   Call hal_pwm_init() function to initialize the PWM source clock, then call hal_pwm_set_frequency() function to set the PWM frequency and get the total counter of the hardware. Total counter is PWM counter internal value at specified frequency. The duty cycle is calculated as the product of the application's duty ratio and total counter.
...
#ifdef HAL_PWM_FEATURE_FREQUENCY_DUTY
/**
 * @addtogroup HAL
 * @{
 * @addtogroup PWM
 ...
 *  and the application needs a waveform at 200Hz frequency and 50% duty ratio. The calculations of division clock,
 *  total count and duty cycle are based on the formulas as shown below.
 *
 ...
 *   - Step4: Call hal_pwm_set_frequency_and_duty() set output frequency and duty cycle.
 *   - Step5: Call hal_pwm_start() to trigger PWM at a specified frequency and duty count values.
 *   - Step6: Call hal_pwm_get_frequency() to get the current frequency.
 ...
 *   - Sample code:
 *    @code
 *      float duty_ratio = 0.5; //The range from 0 to 1.
 ...
 *      //function index = HAL_GPIO_35_PWM5; for more information about pinmux please reference hal_pinmux_define.h
 ...
 *             //error handle
 *       }
 *       if(HAL_PWM_STATUS_OK != hal_pwm_set_frequency_and_duty(HAL_PWM_5, source_clock, frequency, duty_ratio)) {
 *             //error handle
 *       }
...
#endif /* HAL_PWM_FEATURE_FREQUENCY_DUTY */
...
typedef enum {
    HAL_PWM_CLOCK_DIVISION_1 = 0,            /**< Specify the PWM source clock 1 division. */
    ...
    HAL_PWM_CLOCK_DIVISION_MAX,              /**< MAX number of source clock division. */
...
#endif /* HAL_PWM_FEATURE_ADVANCED_CONFIG */
...
/**
 * @brief   This function sets the PWM output frequency and hardware duty cycle.
 * @param[in]  pwm_channel is the PWM channel number. For more details about the parameter, please refer to #hal_pwm_channel_t.
 * @param[in]  clk  is PWM source clock, For more details about this parameter, please refer to #hal_pwm_source_clock_t.
 * @param[in]  frequency is the PWM output frequency.
 * @param[in]  duty_ratio is the PWM hardware duty cycle.
 * @return    To indicate whether this function call is successful or not.
 *            If the return value is #HAL_PWM_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_PWM_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
 * @par       Example
