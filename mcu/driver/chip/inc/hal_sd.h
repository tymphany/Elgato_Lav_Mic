 ...
 * @addtogroup SD
 * @{
 * This section introduces the Secure Digital Memory Dard/Embedded Multimedia Card(SD/eMMC) APIs including terms and acronyms,
 ...
 *     Easy Pinmux Tool (EPT) is used to configure the SD card detection pin for the EINT mode. Follow the steps below to configure the SD card detection pin in the EPT.\n
 *     - Step 1. Select the correct GPIO pin for the EINT mode and PU-47K_ohms in EPT
 *               tool's GPIO Setting window.
 *     - Step 2. Select the corresponding EINT pin's name for 'HAL_MSDC_EINT' in
 *               EPT tool's EINT Setting window.
 *     - Step 3. Switch on the corresponding feature option (HAL_SD_CARD_DETECTION).
 *
 ...
 *       }
 *    @endcode
 * - Read from or write to the SD/eMMC in the DMA blocking mode. \n
 *  - Step 1. Call #hal_gpio_init() to initialize the pins, if EPT tool hasn't been used to configure the related pinmux.
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if EPT tool hasn't been used to configure the related pinmux.
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API only once.
 ...
 *       }
 *
 *       if (HAL_SD_STATUS_OK != hal_sd_write_blocks_dma_blcoking(sd_port, &read_buffer, start_address, block_number)) {
 *           // Error handler.
 *       }
 *    @endcode
 * - Read from or write to the SD/eMMC in the DMA interrupt mode. \n
 ...
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API only once.
 *  - Step 4. Call #hal_sd_register_callback() to register the transfer result callback.
 *  - Step 5. Call #hal_sd_read_blocks_dma() to read the SD/eMMC card data.
 *  - Step 6. Call #hal_sd_write_blocks_dma() to write data to the SD/eMMC card.
 *  - Sample code:
 *      @code
 *       hal_sd_port_t sd_port;
 *       hal_sd_config_t sd_config;
 ...
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API only once.
 *  - Step 4. Call #hal_sd_get_erase_sector_size() to get the erase sector unit size.
 *  - Step 5. Call #hal_sd_erase_sectors() to erase the SD/eMMC card.
 *  - Sample code:
 *      @code
 *       hal_sd_port_t sd_port;
 *       hal_sd_config_t sd_config;
 ...
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API only once.
 *  - Step 4. Call #hal_sd_get_capacity() to get the SD/eMMC card capacity.
 *  - Sample code:
 *      @code
 *       hal_sd_port_t sd_port;
 *       hal_sd_config_t sd_config;
 ...
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API only once.
 *  - Step 4. Call #hal_sd_get_csd() to get the SD/eMMC card CSD register value.
 *  - Sample code:
 *      @code
 *       hal_sd_port_t sd_port;
 *       hal_sd_config_t sd_config;
 ...
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API only once.
 *  - Step 4. Call #hal_sd_get_cid() to get the SD/eMMC card CID register value.
 *  - Sample code:
 *      @code
 *       hal_sd_port_t sd_port;
 *       hal_sd_config_t sd_config;
 ...
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API only once.
 *  - Step 4. Call #hal_sd_get_ocr() to get the SD/eMMC card OCR register value.
 *  - Sample code:
 *      @code
 *       hal_sd_port_t sd_port;
 *       hal_sd_config_t sd_config;
 ...
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API only once.
 *  - Step 4. Call #hal_sd_get_card_type() to get the SD/eMMC card type.
 *  - Sample code:
 *      @code
 *       hal_sd_port_t sd_port;
 *       hal_sd_config_t sd_config;
 ...
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API only once.
 *  - Step 4. Call #hal_sd_get_erase_sector_size() to get the SD/eMMC card erase sector size.
 *  - Sample code:
 *      @code
 *       hal_sd_port_t sd_port;
 *       hal_sd_config_t sd_config;
 ...
 *  - Step 1. Call #hal_gpio_init() to init the pins, if EPT tool hasn't been used to configure the related pinmux.
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 *  - Step 3. Call #hal_sd_init() to initialize the MSDC and the SD/eMMC card transfer states. Call this API again if init fail and the retry times is less than 3.
 *  - Step 4. Call #hal_sd_read_blocks_dma() to read the SD/eMMC card data. Call this API again if read fail and the retry times is less than 3.
 *  - Step 5. Call #hal_sd_write_blocks_dma() to write data to the SD/eMMC card. Call this API again if write fail and the retry times is less than 3.
 *  - Sample code:
 *      @code
 *       hal_sd_port_t sd_port;
 *       hal_sd_config_t sd_config;
...
/** @brief This enum defines the card DMA transfer result event. The event is passed to the user defined callback function.
  *  For more details, please refer to function #hal_sd_register_callback().
  */
typedef enum {
    HAL_SD_EVENT_SUCCESS = 0,                  /**<  The transaction completed without any error. */
    HAL_SD_EVENT_TRANSFER_ERROR = -1,          /**<  An error occurred during transaction. */
    HAL_SD_EVENT_CRC_ERROR = -2,               /**<  An CRC error occurred during transaction. */
    HAL_SD_EVENT_DATA_TIMEOUT = -3,            /**<  An data timeout occurred during transaction. */
 ...
 *  @param [in] user_data is the user defined parameter obtained from the #hal_sd_register_card_detection_callback() function.
 */
 ...
 *         This function is called after the SD/EMMC data transfer is complete.
 *           For more details about the callback, please refer to #hal_sd_register_callback().
 *  @param [in] event is the DMA interrupt transfer event. This parameter specifies whether the transfer is success or not.
 *              For more details about the event type, please refer to #hal_sd_callback_event_t.
 *  @param [in] user_data is the user defined parameter obtained from the #hal_sd_register_callback() function.
 */
 ...
 *            The MSDC output clock is recommended to set to 45000kHz, no need to modify.
 ...
 *            If the return value is #HAL_SD_STATUS_OK,the call succeeded, else the initialization has failed.
 * @sa  #hal_sd_deinit()
 */
 ...
 * @param[in] sd_callback is a pointer to the callback function.
...
/**
 * @brief     This function gets the sector size of the SD/eMMC card to erase.
 * @param[in] sd_port is the MSDC port to get the sector size.
...
/**
 * @brief      This function reads data from the SD/eMMC card in the DMA blocking mode. This API may block the application task.
 *                This API is recommended when each transmission is greater than 2 blocks.
 * @param[in] sd_port is the MSDC port to read.
...
/**
 * @brief     This function writes data to the SD/eMMC card in the DMA blocking mode. This API may block the application task.
 *               This API is recommended when each transmission is greater than 2 blocks.
 * @param[in] sd_port is the MSDC port to write.
...
/**
 * @brief This function registers a callback function to transfer data with DMA.
 * @param[in] sd_port is the MSDC port  to transfer data
 * @param[in] sd_callback is the function pointer of the callback.
 *            The callback function is called once the SD/eMMC data transfer done.
 * @param[in] user_data is the callback parameter.
 * @return    #HAL_SD_STATUS_OK, if the operation completed successfully.
 */
...
/**
 * @brief      This function reads data from the SD/eMMC card in the DMA interrupt mode. This API will not block the application task.
 *                This API is recommended when each transmission is greater than 2 blocks.
 * @param[in] sd_port is the MSDC port to read.
 * @param[out] read_buffer is the address to store data read from the card. The address must be a noncacheable and 4 bytes aligned address.
 * @param[in] read_address is the read start address of the SD/eMMC card.
 * @param[in] block_number is the block number on the SD/eMMC card to read.
 * @return    #HAL_SD_STATUS_OK, if the operation completed successfully.
 *            #HAL_SD_STATUS_ERROR, an error occurred, such as a wrong parameter is given.
 *            #HAL_SD_STATUS_BUSY, the MSDC is busy.
...
/**
 * @brief     This function writes data to the SD/eMMC card in the DMA interrupt mode. This API will not block the application task.
 *               This API is recommended when each transmission is greater than 2 blocks.
 * @param[in] sd_port is the MSDC port to write.
 * @param[in] write_buffer is the address to store the data that will be written, the address must be a noncacheable and 4 bytes aligned address.
 * @param[in] write_address is the start address on the SD/eMMC card to write into.
 * @param[in] block_number is the block number of the SD/eMMC card to write.
 * @return    #HAL_SD_STATUS_OK, if the operation completed successfully.
 *            #HAL_SD_STATUS_ERROR, an error occurred, such as a wrong parameter is given.
 *            #HAL_SD_STATUS_BUSY, the MSDC is busy.
