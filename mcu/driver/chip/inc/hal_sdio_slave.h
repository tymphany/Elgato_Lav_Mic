...
* Then user can call #hal_sdio_slave_send_mailbox() to write data to the SDIO host or call #hal_sdio_slave_receive_mailbox()  to read data from the SDIO host.
* And call #hal_sdio_slave_trigger_d2h_interrupt() to trigger a software interrupt after call #hal_sdio_slave_send_mailbox() to inform host to read mailbox.
...
*  - Step 3. Call #hal_sdio_slave_init() to initialize the SDIO slave controller. Call this API only once.
*  - Step 4. Call #hal_sdio_slave_receive_mailbox() to read mailbox after SDIO host write mailbox to slave.
*  - Sample code:
*    @code
*        uint32_t mailbox_value = 0;
*
*        // Initialize the GPIO, set the GPIO pinmux, if it wasn't configured with the EPT.
...
*        // Call the hal_pinmux_set_function() to set the GPIO pinmux.For more information, please refer to hal_pinmux_define.h 
...
*  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
*  - Step 3. Call #hal_sdio_slave_init() to initialize the SDIO slave controller. Call this API only once.
*  - Step 4. Call #hal_sdio_slave_send_mailbox() to write mailbox to SDIO host.
*  - Step 5. Call #hal_sdio_slave_trigger_d2h_interrupt() to inform SDIO host that slave has write the mailbox to host.
*  - Sample code:
*       @code
*        uint32_t mailbox_value = 0;
*
...
*  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
*  - Step 3. Call #hal_sdio_slave_init() to initialize the SDIO slave controller. Call this API only once.
*  - Step 4. Call #hal_sdio_slave_register_callback() register a user callback.
*  - Step 5. Call #hal_sdio_slave_send_dma() to send data to the SDIO host.
*  - Sample code:
*       @code
*        uint32_t mailbox_value = 0;
...
*  - Step 3. Call #hal_sdio_slave_init() to initialize the SDIO slave controller. Call this API only once.
*  - Step 4. Call #hal_sdio_slave_register_callback() register a user callback.
*  - Step 5. Call #hal_sdio_slave_receive_dma() to receive data from the SDIO host.
*  - Sample code:
*       @code
*        uint32_t mailbox_value = 0;
...
typedef enum {
    HAL_SDIO_SLAVE_EVENT_TO_ERROR = -5,                /**< Indicates if an Timeout error occured. */
    HAL_SDIO_SLAVE_EVENT_TX0_ERROR = -4,               /**< Indicates if an TX0  error occured. */
    HAL_SDIO_SLAVE_EVENT_RX0_ERROR = -3,               /**< Indicates if an RX0 error occured. */
    HAL_SDIO_SLAVE_EVENT_RX1_ERROR = -2,               /**< Indicates if an RX1 error occured. */
    HAL_SDIO_SLAVE_EVENT_ERROR = -1,                   /**< Indicates if an CRC error occured. */
    ...
    HAL_SDIO_SLAVE_EVENT_NONE     = 4                   /**< SDIO slave default event. */
