 ...
 * @addtogroup SPI_SLAVE
 * @{
 * - \b Support \b the \b bypass \b mode. \n
 *  Bypass mode is mutually exclusive with SPI slave command mode. In bypass mode, the user can choose one of the master ports to bypass the signal,
 *  The signal can be bypassed from master to slave, from slave to master, or bi-directionally.
 *
 *  - Bypass from master to slave \n
 *    An external SPI master connects to the SPI slave. The external SPI master receives data from the SPI master through the SPI slave.
 *  - Bypass from slave to master \n
 *    An external SPI master connects to the SPI slave. The external SPI master sends data to the SPI master through the SPI slave.
 *  - Bypass bi-directionally \n
 *    An external SPI master connects to the SPI slave. The external SPI master sends data to the SPI master or receives data from the SPI master through the SPI slave.
 *
 * @}
 * @}
 */
 ...
 * @addtogroup SPI_SLAVE
 * @{
 * - \b Support \b the \b direct \b mode. \n
 *  In direct mode, SPI slave is not driven by command. It is completely full duplex.
 *
 * @}
 * @}
 */
 ...
 *   SPI slave driver supports locking or unlocking sleep to save power, if sleep feature is turn on.
 ...
 * @addtogroup SPI_SLAVE
 * @{
 * - \b Using \b the \b SPI \b slave \b in \b the \b bypass \b mode.
 *  Call #hal_pinmux_set_function() to pinmux GPIO pins to the SPI pins (SS, SCLK, MOSI, MISO, SIO2 and SIO3) of SPI slave and SPI master based on the user's hardware platform design.
 *  Then call #hal_spi_slave_bypass_init() to apply basic settings for SPI slave hardware.The user should call #hal_spi_slave_bypass_deinit() to release the SPI slave resource to make it available for other users.
 *  The steps are shown below:
 *  - Step 1. Call #hal_gpio_init() to init the pins, if EPT tool has not been used to configure the related pinmux.
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT tool has not been used to configure the related pinmux.
 *           For more details about #hal_pinmux_set_function(), please refer to @ref GPIO.
 *  - Step 3. Call #hal_spi_slave_bypass_init() to initialize SPI slave bypass mode.
 *  - Step 4. Call #hal_spi_slave_bypass_deinit() to deinitialize the SPI slave bypass mode if it is no longer in use.
 *  - sample code: *
 *    @code
 *      hal_spi_slave_bypass_config_t bypass_config;
 *      hal_spi_slave_status_t status;
 *       // Initialize the GPIO, set GPIO pinmux if it was not configured with EPT tool.
 *      hal_gpio_init(SPIS_PIN_NUMBER_CS);
 *      hal_gpio_init(SPIS_PIN_NUMBER_SLK);
 *      hal_gpio_init(SPIS_PIN_NUMBER_MOSI);
 *      hal_gpio_init(SPIS_PIN_NUMBER_MISO);
 *      hal_gpio_init(SPIM_PIN_NUMBER_CS);
 *      hal_gpio_init(SPIM_PIN_NUMBER_SLK);
 *      hal_gpio_init(SPIM_PIN_NUMBER_MOSI);
 *      hal_gpio_init(SPIM_PIN_NUMBER_MISO);
 *      // Call hal_pinmux_set_function() to set the GPIO pinmux. For more information, please refer to hal_pinmux_define.h.
 *      // It is not necessary to configure the pinmux if the EPT tool is used.
 *      //function_index = SPIS_PIN_FUNC_CS.
 *      hal_pinmux_set_function(SPIS_PIN_NUMBER_CS, function_index);
 *      //function_index = SPIS_PIN_FUNC_SLK.
 *      hal_pinmux_set_function(SPIS_PIN_NUMBER_SLK, function_index);
 ...
 *      //function_index = SPIS_PIN_FUNC_MISO.
 *      hal_pinmux_set_function(SPIS_PIN_NUMBER_MISO, function_index);
 *      //function_index = SPIM_PIN_FUNC_CS.
 *      hal_pinmux_set_function(SPIM_PIN_NUMBER_CS, function_index);
 *      //function_index = SPIM_PIN_FUNC_SLK.
 *      hal_pinmux_set_function(SPIM_PIN_NUMBER_SLK, function_index);
 *      //function_index = SPIM_PIN_FUNC_MOSI.
 *      hal_pinmux_set_function(SPIM_PIN_NUMBER_MOSI, function_index);
 *      //function_index = SPIM_PIN_FUNC_MISO.
 *      hal_pinmux_set_function(SPIM_PIN_NUMBER_MISO, function_index);
 *
 *      bypass_config.port = HAL_SPI_SLAVE_BYPASS_MASTER_0;
 *      bypass_config.cs = HAL_SPI_SLAVE_BYPASS_MASTER_CS_0;
 *      bypass_config.mode = HAL_SPI_SLAVE_BYPASS_MODE_RW;
 *
 *      status = hal_spi_slave_bypass_init(HAL_SPI_SLAVE_0, &bypass_config);
 *      if (HAL_SPI_SLAVE_STATUS_OK != status) {
 *         // Error handler;
 *      }
 *    @endcode
 *
 ...
 * @addtogroup SPI_SLAVE
 * @{
 * - \b Using \b the \b SPI \b slave \b in \b the \b direct \b mode.
 *  Call #hal_pinmux_set_function() to pinmux GPIO pins to four SPI pins (SS, SCLK, MOSI, and MISO) based on the user's hardware platform design.
 *  Then call #hal_spi_slave_init() in the callback function to apply basic settings for SPI slave hardware. call #hal_spi_slave_set_vfifo() to set
 *  the SPI slave to direct mode. And call #hal_spi_slave_register_callback() to register a callback function. Since the SPI slave's behavior is passive,
 *  most of the operations are carried through a callback function.
 *  The user should call #hal_spi_slave_deinit() in the callback function to release the SPI slave resource to make it available for other users.
 *  The steps are shown below:
 *  - Step 1. Call #hal_gpio_init() to init the pins if EPT tool hasn't been used to configure the related pinmux.
 *  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT tool hasn't been used to configure the related pinmux.
 *           For more details about #hal_pinmux_set_function(), please refer to @ref GPIO.
 *  - Step 3. Call #hal_spi_slave_init() in the callback function to initialize a single SPI slave controller if POWER ON interrupt occurred.
 *  - Step 4. Call #hal_spi_slave_set_vfifo() to set the SPI slave to direct mode and set transmission buffer.
 *  - Step 5. Call #hal_spi_slave_register_callback() to register a user callback.
 *  - Step 6. Call #hal_spi_slave_send_vfifo() in the callback function  to send data in DMA mode if a TX empty interrupt occurred.
 *  - Step 7. Call #hal_spi_slave_receive_vfifo() in the callback functionto receive data in the DMA mode if RX full or timeout interrupt occurred.
 *  - Step 8. Call #hal_spi_slave_deinit() to deinitialize the SPI slave that is no longer in use.
 *  - sample code:
 *    @code
 *       uint16_t slave_status;
 *       hal_spi_slave_config_t spi_configure;
 *       hal_spi_slave_vfifo_config_t vfifo_config;
 *       // Initialize the GPIO and set GPIO pinmux if it was not configured with the EPT tool.
 *       hal_gpio_init(SPIS_PIN_NUMBER_CS);
 *       hal_gpio_init(SPIS_PIN_NUMBER_SLK);
 *       hal_gpio_init(SPIS_PIN_NUMBER_MOSI);
 *       hal_gpio_init(SPIS_PIN_NUMBER_MISO);
 *
 *       // Call hal_pinmux_set_function() to set the GPIO pinmux. For more information, please refer to hal_pinmux_define.h.
 *       // No need to configure the pinmux if the EPT tool is used.
 *       //function_index = SPIS_PIN_FUNC_CS.
 *       hal_pinmux_set_function(SPIS_PIN_NUMBER_CS, function_index);
 *       //function_index = SPIS_PIN_FUNC_SLK.
 *       hal_pinmux_set_function(SPIS_PIN_NUMBER_SLK, function_index);
 ...
 *       spi_configure.phase = HAL_SPI_SLAVE_CLOCK_PHASE0;
 *       spi_configure.polarity = HAL_SPI_SLAVE_CLOCK_POLARITY0;
 *       spi_configure.timeout_threshold = 100;
 *       if (HAL_SPI_SLAVE_STATUS_OK != hal_spi_slave_init(HAL_SPI_SLAVE_0, &spi_configure)) {
 *            // Error handler;
 *       }
 *       vfifo_config.tx_buffer = &slave_tx_dma_buffer;
 *       vfifo_config.tx_buffer_size = tx_vfifo_length;
 *       vfifo_config.tx_threshold = tx_threshold;
 *       vfifo_config.rx_buffer = &slave_rx_dma_buffer;
 *       vfifo_config.rx_buffer_size = rx_vfifo_length;
 *       vfifo_config.rx_threshold = rx_threshold;
 *       if (HAL_SPI_SLAVE_STATUS_OK != hal_spi_slave_set_vfifo(HAL_SPI_SLAVE_0, &vfifo_config)) {
 *            // Error handler;
 *       }
 *       hal_spi_slave_register_callback(slave_port,spi_slave_callback,NULL)// Register a user callback.
 *    @endcode
 *    @code
 ...
 *              recv_size = hal_spi_slave_get_vfifo_available_data_bytes(HAL_SPI_SLAVE_0);
 *              if (recv_size != 0) {
 *                  hal_spi_slave_receive_vfifo(HAL_SPI_SLAVE_0, slave_user_rx_buffer, recv_size);
 *              }
 *
 *              break;
 *          case HAL_SPI_SLAVE_EVENT_TX_DMA_EMPTY  :
 *              // The user should call hal_spi_slave_send_vfifo to send data to the VFIFO buffer.
 *              break;
 *          case HAL_SPI_SLAVE_EVENT_RX_DMA_FULL   :
 *              recv_size = hal_spi_slave_receive_vfifo(HAL_SPI_SLAVE_0, slave_user_rx_buffer, hal_spi_slave_get_vfifo_available_data_bytes(HAL_SPI_SLAVE_0));
 *              break;
...
*    // Then call #hal_gpio_set_driving_current() to set the proper driving current for this SPI pin.
...
#ifdef HAL_SPI_SLAVE_FEATURE_BYPASS
/** @brief SPI slave bypass mode configure. */
typedef struct
{
    hal_spi_slave_bypass_master_port_t port;         /**< The master port to which the slave bypasses. */
    hal_spi_slave_bypass_master_cs_t cs;             /**< The CS pin of the master port to which the slave bypasses. */
    hal_spi_slave_bypass_mode_t mode;                /**< The spi slave bypass mode. */
} hal_spi_slave_bypass_config_t;
#endif

#ifdef HAL_SPI_SLAVE_FEATURE_DIRECT_MODE
/** @brief SPI slave direct mode DMA transfer type. */
typedef enum {
    HAL_SPI_SLAVE_DMA_TRANSFER_TYPE_AUTO_CONTROL = 0,     /**< SPI slave direct mode auto control transfer. */
    HAL_SPI_SLAVE_DMA_TRANSFER_TYPE_SINGLE_BYTE  = 1,     /**< SPI slave direct mode DMA move data by byte. */
    HAL_SPI_SLAVE_DMA_TRANSFER_TYPE_SINGLE_WORD  = 2,     /**< SPI slave direct mode DMA move data by word. */
    HAL_SPI_SLAVE_DMA_TRANSFER_TYPE_BURST4_WORD  = 3,     /**< SPI slave direct mode DMA move data by 4 words.*/
    HAL_SPI_SLAVE_DMA_TRANSFER_TYPE_MAX          = 4,     /**< SPI slave direct mode transfer mode max number. */
} hal_spi_slave_dma_transfer_type_t;
...
typedef struct {
    uint8_t *tx_buffer;                /**< This field represents the transmitting user allocated VFIFO buffer. It will only be used by the SPI slave driver to send data. It must be non-cacheable and is 16-byte aligned. */
    uint32_t tx_buffer_size;           /**< This field represents the size of the transmitting VFIFO buffer. */
    uint32_t tx_threshold;             /**< This field represents the threshold of the transmitting VFIFO buffer. VFIFO DMA will trigger an interrupt when the available bytes in VFIFO buffer are lower than the threshold. */
    uint8_t *rx_buffer;                /**< This field represents the receiving user allocated VFIFO buffer. It will only be used by the SPI slave driver for receiving data, and must be non-cacheable and 16 bytes align. */
    uint32_t rx_buffer_size;           /**< This field represents size of the receiving VFIFO buffer. */
    uint32_t rx_threshold;             /**< This field represents the threshold of the receiving VFIFO buffer. VFIFO DMA will trigger receive interrupt when available bytes in VFIFO buffer are more than the threshold. */
    hal_spi_slave_dma_transfer_type_t transfer_type;    /**< This field represents the SPI slave DMA transfer type. */
} hal_spi_slave_vfifo_config_t;
#endif
...
#ifdef HAL_SPI_SLAVE_FEATURE_BYPASS
/**
 * @brief     This function initializes the SPI slave bypass mode and sets user defined common parameters .
 *
 * @param[in] spi_port is the SPI slave port number, the value is defined at #hal_spi_slave_port_t.
 * @param[in] bypass_config is the SPI slave configure parameters. Details are described at #hal_spi_slave_bypass_config_t.
 * @return #HAL_SPI_SLAVE_STATUS_ERROR_PORT, if the SPI slave port is invalid. \n
 *         #HAL_SPI_SLAVE_STATUS_INVALID_PARAMETER, if an invalid parameter is given by user. \n
 *         #HAL_SPI_SLAVE_STATUS_OK, if this function returned successfully.
 */
...
/**
 * @brief This function deinitializes the SPI slave bypass function.
 *
 * @param[in] spi_port is the SPI slave port number, the value is defined at #hal_spi_slave_port_t.
 * @return #HAL_SPI_SLAVE_STATUS_ERROR_PORT, if the SPI slave port is invalid. \n
 *         #HAL_SPI_SLAVE_STATUS_OK, if this function returned successfully.
...
#ifdef HAL_SPI_SLAVE_FEATURE_DIRECT_MODE
/**
 * @brief     This function sets the VFIFO DMA hardware relative to the SPI slave.
 *                With the help of the VFIFO DMA hardware, MCU does not need to copy data between the user buffer and the SPI slave FIFO byte by byte.
 *                The MCU will be open to other tasks while the VFIFO DMA is copying data.
 *                The user must set threshold, address and size of the VFIFO TX/RX buffer for its proper operation.
 *
 * @param[in] spi_port is the SPI slave port number, the value is defined at #hal_spi_slave_port_t.
 * @param[in] config specifies a user defined VFIFO DMA configuration parameter.
 * @return    #HAL_SPI_SLAVE_STATUS_ERROR_PORT, if the SPI slave port is invalid. \n
 *            #HAL_SPI_SLAVE_STATUS_INVALID_PARAMETER, if an invalid parameter is given by user. \n
 *            #HAL_SPI_SLAVE_STATUS_OK, if this function returned successfully.
 */
 ...
 *               Then driver receives user data from the VFIFO RX buffer and puts it in the available free space.
 ...
 * @return the data length have received from vfifo.
 */
 ...
 * @param[in] data   specifies pointer to the user's data buffer.
 * @param[in] size   specifies size of the user's data buffer.
 * @return the data length have been send to vfifo.
 */
uint32_t hal_spi_slave_send_vfifo(hal_spi_slave_port_t spi_port, const uint8_t *data, uint32_t size);

/**
 * @brief To get the avaliable space in the VFIFO TX buffer
 *
 * @param[in] spi_port is the SPI slave port number, the value is defined at #hal_spi_slave_port_t.
 * @return  Available space of the VFIFO TX buffer.
 */
uint32_t hal_spi_slave_get_vfifo_available_send_space(hal_spi_slave_port_t spi_port);

/**
 * @brief  To get the avaliable data in the RX VFIF buffer
 *
 * @param[in] spi_port is the SPI slave port number, the value is defined at #hal_spi_slave_port_t.
 * @return  Available data of the VFIFO RX buffer.
 */
