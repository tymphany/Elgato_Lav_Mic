 ...
 *  - \b Polling \b mode: In this mode, whether the data has been received or sent is determined by polling the UART status register.
 *                    To receive data, it reads the data from the data register. To send the data, it writes to the data register.
 *                    When a task calls the UART driver API in the polling mode, it blocks the other tasks from running, until the API in the polling mode returns.
 *  - \b DMA \b mode: In this mode, a separate VFIFO DMA hardware is enabled to transmit or receive data between the VFIFO DMA buffer and the UART hardware.
 ...
 *                    In both receive or transmit operations, UART raises an interrupt if requests to send (RTS) or clear to send (CTS) are active.
 *   \n \n
 ...
 *  - \b DMA \b mode: \n
 *                    Unlike polling mode, VFIFO DMA hardware and transmit (TX) or receive (RX) buffers are required to operate in the DMA mode.
 ...
 *     If the hardware flow control is enabled, RTS and CTS should be enabled, too.
 *     Concerning issues about how to configure UART pins, please refer to @ref GPIO of driver document.
 *  - sample code:
 *    @code
 *       void uart_pinmux_init(void)
 *       {
 *           //mt2523_hdk is used as a hardware platform, to use another board, query the related GPIO port and pinmux function in hal_pinmux_define.h.
 *
 *           //Configure RXD pin.
 ...
 *           //function_index = HAL_GPIO_0_U2RXD, no need to configure the pinmux, if EPT is used.
 ...
 *           //function_index = HAL_GPIO_1_U2TXD, no need to configure the pinmux, if EPT is used.
 *           hal_pinmux_set_function(HAL_GPIO_1, function_index);
 *
 *           //Configure RTS pin.
 *           hal_gpio_init(HAL_GPIO_3);
 *           //Call hal_pinmux_set_function() to set GPIO pinmux, for more information, please refer to hal_pinmux_define.h
 *           //function_index = HAL_GPIO_3_U0RTS, no need to configure the pinmux, if EPT is used.
 *           hal_pinmux_set_function(HAL_GPIO_3, function_index);
 *
 *           //Configure CTS pin.
 *           hal_gpio_init(HAL_GPIO_2);
 *           //Call hal_pinmux_set_function() to set GPIO pinmux, for more information, please refer to hal_pinmux_define.h
 *           //function_index = HAL_GPIO_2_U0CTS, no need to configure the pinmux, if EPT is used.
...
typedef enum {
    ...
    HAL_UART_BAUDRATE_2000000   = 13,   /**< Defines UART baudrate as 2000000 bps. */
#ifdef HAL_UART_FEATURE_3M_BAUDRATE
    HAL_UART_BAUDRATE_3000000   = 14,   /**< Defines UART baudrate as 3000000 bps. */
#endif
    HAL_UART_BAUDRATE_3200000   = 15,   /**< Defines UART baudrate as 3200000 bps. */
#ifdef HAL_UART_FEATURE_6M_BAUDRATE
    HAL_UART_BAUDRATE_6000000   = 16,   /**< Defines UART baudrate as 6000000 bps. */
#endif
#ifdef HAL_UART_FEATURE_8_6M_BAUDRATE
    HAL_UART_BAUDRATE_8666000   = 17,   /**< Defines UART baudrate as 8666000 bps. */
#endif
...
typedef enum {
    ...
    HAL_UART_EVENT_WAKEUP_SLEEP = 3,                /**< Indicates if there happened UART RX wakeup sleep. */
#ifdef HAL_UART_FEATURE_FLOWCONTROL_CALLBACK
    HAL_UART_EVENT_SW_FLOW_CTRL = 4,                /**< Indicates if there software flowcontrol is working. ex. received xoff*/
    HAL_UART_EVENT_HW_FLOW_CTRL = 5,                /**< Indicates if there hardware flowcontrol is working. ex. cts pull high*/
#endif
    HAL_UART_EVENT_TRANSMISSION_DONE = 6,           /**< Indicates if there is data transfer completed. ex. UART THR empty*/
    HAL_UART_EVENT_RECEIVE_DEAD = 7,
 ...
 *               In this function, the driver first poll the status of UART transmit in a loop until it is ready.
 *               Then the driver places the character in the UART transmit register and returns from this function.
 * @param[in] uart_port   initializes the specified UART port number.
 ...
 *               In this function, driver first polls the status of a UART receive operation in a loop until it is ready.
 *               Then the driver gets the character from the UART receive register and return it.
 * @param[in] uart_port   initializes the specified UART port number.
 * @return
 ...
 *               Then the driver puts user data that fits into the free space into the VFIFO TX buffer.
 *               If driver cannot put all user data into VFIFO TX buffer, it will turn the VFIFO DMA TX interrupt on so that user is notified
 *               when VFIFO TX buffer becomes available again.
 *               Finally, the driver returns the bytes that are placed in the VFIFO TX buffer.
 ...
 *               Then driver receives user data from the VFIFO RX buffer and places in the available free space.
 *               If the driver cannot get all user data from VFIFO RX buffer, it turns the VFIFO DMA RX interrupt on so that user is notified
 *               when VFIFO RX buffer becomes available again.
 *               Finally, the driver returns the bytes that are received from the VFIFO RX buffer.
 ...
 * @param[in] uart_port   initializes the specified UART port number.
 * @param[in] timeout   specifies time out value for UART VFIFO DMA. The unit is ms. This value
 *                      must not larger than #HAL_UART_TIMEOUT_VALUE_MAX.
 * @return
 *                #HAL_UART_STATUS_OK, if the operation completed successfully. \n
 *                #HAL_UART_STATUS_ERROR_PARAMETER, if parameter is invalid. \n
 *                #HAL_UART_STATUS_ERROR_UNINITIALIZED, if the UART has not been initialized. \n
 * @par       Example
 * @code
 *       hal_uart_set_dma_timeout(HAL_UART_0, 100); // set time out of VFIFO DMA to 100ms.
 * @endcode
 */
...
/**
 * @brief     This function sets auto baud rate for the specific
 *            UART port.
 * @param[in] uart_port   initializes the specified UART port number.
 * @param[in] is_enable   specifies that if auto baud rate
 ...
 * @par       Example
 * @code
 *       // Enable auto baud rate feature for UART_0.
 *       hal_uart_set_auto_baudrate(HAL_UART_0, true);
 * @endcode
 */
hal_uart_status_t hal_uart_set_auto_baudrate(hal_uart_port_t uart_port, bool is_enable);

/**
 * @brief     This function queries whether the current status of the UART is idle and whether there is data in the RX buffer.
 * @param[in] uart_port   initializes the specified UART port number.
 * @return
 *                if FIFO mode: return true means that the uart current status is idle; if DMA mode: return true means that the uart current status is idle and RX buffer is empty. \n
 *                if FIFO mode: return false means that the uart current status not idle; if DMA mode: return false means that the uart current status not idle or RX buffer not empty. \n
 * @par       Example
 * @code
 *       // query whether the current status of the UART0 is idle and whether there is data in the RX buffer.
 *       hal_uart_query_idle(HAL_UART_0);
 * @endcode
 */
