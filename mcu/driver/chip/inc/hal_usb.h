...
#define HAL_USB_MAX_PACKET_SIZE_ENDPOINT_INTERRUPT_FULL_SPEED	16
/**
  * @}
  */
...
/** @brief Endpoint type defined in USB 2.0 Specification. BULK, INTR, ISO endpoint */
typedef enum {
    HAL_USB_EP_TRANSFER_ILLEGAL = 0,  /**<  The transfer type defined in USB 2.0 Specification, illegal pipe. */
...
/**
 * @brief This function sets up high speed mode.
 * @return #HAL_USB_STATUS_OK, if the operation completed successfully.
 * @par Example
 * Please refer to "Initial USB hardware" in @ref HAL_USB_Driver_Usage_Chapter.
 */
...
/**
 * @brief This function sets up high band-width mode and packets.
 * @param[in] hb_enable is selected to enable high band-width mode.
 * @param[in] packet is set up for maximum payload size for endpoint.
 * @return #HAL_USB_STATUS_OK, if the operation completed successfully.
 * @par Example
 * Please refer to "Initial USB hardware" in @ref HAL_USB_Driver_Usage_Chapter.
 */
...
/**
 * @brief This function initializes the USB endpoint in RX direction.
 * @param[in] ep_num is the endpoint to initialize.
 * @param[in] data_size is the maximum packet size at this endpoint. For more details about maximum packet size, please refer to USB 2.0 Specification.
 * @param[in] type is the endpoint type defined in USB 2.0 Specification, such as CTRL, BULK, INTR, ISO endpoint.
...
/**
 * @brief This function toggle the USB endpoint TX.
 * @param[in] ep_num is the endpoint number to disable.
 * @return #HAL_USB_STATUS_OK, if the operation completed successfully.
 * @par Example
 * Sample code:
 * @code
 * void USB_Set_endpoint_tx_ready_toggle (void)
 * {
 *   int ep_num = 1;
 *   hal_usb_set_endpoint_tx_ready_toggle(ep_num);
 * }
 * @endcode
 */
 ...
 * For more information on the number of DMA channels on the Airoha IoT SDK, please refer to #HAL_USB_MAX_NUMBER_ENDPOINT.
...
/**
 * @brief This function queries the DMA ready status.
 * @param[in] ep_num is the endpoint number.
 * @param[in] direction is the USB endpoint direction.
 * @return HAL_USB_STATUS_OK, if the DMA is ready.
 * @par Example
 * An example implementation can be found in "USB DMA Usage." in @ref HAL_USB_Driver_Usage_Chapter.
 */
