...
// clk:0~(0xFFFFFFE), 28-bit, unit is 312.5us
// phase from 0 to 2499, add 1, then, phase=0, clk[27:2]+=1
//
ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ uint16_t DCHS_bt_get_native_phase(void)
{
...
ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ uint32_t DCHS_bt_get_native_clock(void)
{
    uint32_t NativeClk = 0;
    uint16_t NativePhase = 0;
    DCHS_bt_pka_get_native_clk(&NativeClk, &NativePhase);
...
ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ uint32_t DCHS_bt_get_native_clock_us(void)
{
    ...
    return NATIVE_CLK_PHASE_2_US(NativeClk, NativePhase); // us, max = (0xFFFFFFE * 625 + 2499) >> 1 = 0x8000270
}



ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ uint32_t DCHS_bt_get_native_duration(uint32_t max_val, uint32_t start_count, uint32_t end_count)
{
    ...
    if (end_count > start_count) {
        ...
        duration_count = (max_val - (start_count - end_count)) + 1;
    }

    return duration_count;
}

ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ int16_t DCHS_bt_get_native_phase_duration(uint16_t start_count, uint16_t end_count)
{
    return (uint16_t)DCHS_bt_get_native_duration(2499, start_count, end_count);
}

ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ int32_t DCHS_bt_get_native_clock_duration(uint32_t start_count, uint32_t end_count)
{
    int32_t duration;
    duration = DCHS_bt_get_native_duration(0x0FFFFFFE, start_count, end_count);
...
ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ int16_t DCHS_bt_get_native_phase_elapse(uint16_t start_count)
{
    return DCHS_bt_get_native_phase_duration(start_count, DCHS_bt_get_native_phase());
}

ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ int32_t DCHS_bt_get_native_clock_elapse(uint32_t start_count)
{
    return DCHS_bt_get_native_clock_duration(start_count, DCHS_bt_get_native_clock());
}
ATTR_TEXT_IN_FAST_MEM static __FORCE_INLINE__ int32_t DCHS_bt_get_native_clock_us_elapse(uint32_t start_count)
{
    return DCHS_bt_get_native_clock_us_duration(start_count, DCHS_bt_get_native_clock_us());
}

#define MUX_LL_UART_PKT_HEAD 0x55
#define MUX_GET_DEST_CORE_ID(id) ((id) & 0xF)
#define MUX_GET_DEST_USER_ID(id) (((id) >> 4) & 0xF)
#define MUX_ID(scid, dcid, uid)  ((((dcid) & 0xF)) | (((uid) & 0xF) << 4))

#ifdef AIR_LL_MUX_WAKEUP_ENABLE
#define MUX_UART_WAKEUP_CMD            (0xF511)
#define MUX_UART_WAKEUP_CMD_RESPONSE   (0xF522)
#endif
...
#define MAX_MUX_LL_USER_RX_DATA_CNT 8
typedef struct {
    uint32_t wptr;
    uint32_t rptr;
    uint32_t is_empty;
    uint8_t *p_data[MAX_MUX_LL_USER_RX_DATA_CNT];
    uint32_t data_size[MAX_MUX_LL_USER_RX_DATA_CNT];
    uint32_t drop_count;
} buffer_fifo_t;
...
} mux_ll_user_callback_timestamp_t;

typedef struct {
    mux_user_tag_t utag;

    uint32_t tx_timestamp;
    uint32_t tx_local_timestamp;
...
#define MUX_LL_CALLBACK_TIMESTAMP_BUFFER_MAX 8
typedef struct {
    ...
    void *user_data;
    mux_ll_user_callback_timestamp_t callback_timestamp[MUX_LL_CALLBACK_TIMESTAMP_BUFFER_MAX];
    uint8_t scid;
    uint8_t dcid;
