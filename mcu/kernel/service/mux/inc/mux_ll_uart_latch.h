...
#define DCHS_LOCK_UART_TX()            do {\
    RB_SET_FLAG(&g_port_config.txbuf, MUX_RB_FLAG_UART_LATCH);\
    LOG_MSGID_I(common, "[uart latch] lock uart tx", 0);\
} while (0)

#define DCHS_UNLOCK_UART_TX()          do {\
    RB_CLR_FLAG(&g_port_config.txbuf, MUX_RB_FLAG_UART_LATCH);\
    LOG_MSGID_I(common, "[uart latch] unlock uart tx", 0);\
} while (0)

#define DCHS_IS_UART_TX_LOCKED()  RB_CHK_FLAG(&g_port_config.txbuf, MUX_RB_FLAG_UART_LATCH)

typedef void (*mux_ll_uart_race_decode_callback_t)(uint16_t race_id, uint8_t *buff, uint32_t len);

extern void mux_ll_uart_race_decode(mux_ringbuffer_t *rb, mux_ll_uart_race_decode_callback_t callback);
