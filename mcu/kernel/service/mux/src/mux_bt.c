...
static mux_bt_port_context_t* mux_bt_get_connected_port_context(mux_bt_port_t type)
{
    uint32_t i = 0;
    ...
    for (i = port_group->index_start; i <= port_group->index_end; i++) {
        if (mux_port_context[i].connection_handle != MUX_BT_INVAILD_HANDLE) {
            LOG_MSGID_I(MUX_BT, "[MUX][BT] get_port_connect_handle = %02x, type = %02x", 2, mux_port_context[i].connection_handle, type);
            return &mux_port_context[i];
        }
    }
    LOG_MSGID_I(MUX_BT, "[MUX][BT] get_port_connect_handle = %02x", 1, type);
    return NULL;
}
        ...
        case BLE_AIR_EVENT_CONNECT_IND: {
            ...
            if (context != NULL) {
                ...
                if (context->is_open) {
                    ...
                    LOG_MSGID_I(MUX_BT, "[MUX][BT] le port connect handle = %02x ,port_index =%02x", 2, conn_ind->conn_handle ,context->port_index);
...
static void mux_bt_port_spp_event_callback(spp_air_event_t event_id, void *param)
{
    switch (event_id) {
        case SPP_AIR_CONNECT_IND: {
            ...
            mux_bt_port_context_t *context1 = mux_bt_get_connected_port_context(MUX_BT_PORT_TYPE_SPP);
            if (NULL != context1) {
                if (bt_utils_memcmp(&context1->peer_address, &conn_ind->address, sizeof(bt_bd_addr_t)) == 0) {
                    if (context1->connection_handle != conn_ind->handle) {
                        mux_bt_reset_port_context_conn_info(context1);
                    }
                }
            }
...
void mux_bt_phase2_send(uint8_t port_index)
{
    uint32_t per_cpu_irq_mask = 0;
...
void mux_bt_phase1_send(uint8_t port_index)
{
    PORT_MUX_UNUSED(port_index);
    return;
}
