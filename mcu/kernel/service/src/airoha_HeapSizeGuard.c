...
TASK_QUEUE_StatisticsInfo_t xTaskSizeGuardArr[MAX_TASK_SIZE_GUARD_NUM];
TASK_QUEUE_StatisticsInfo_t xQueueSizeGuardArr[MAX_Queue_SIZE_GUARD_NUM];
uint32_t xQueueIndex = 0;
uint32_t xTaskIndex = 0;
...
//#ifdef HEAP_SYS_PEAK_PROFILING
    if (xIsFirstMalloc) {
        ...
        vHeapSizeGuard_Register_Dump_Callback();
        xIsFirstMalloc = 0U;
    }
//#endif /* HEAP_SYS_PEAK_PROFILING */
...
void vHeapSizeGuard_Dump_Task_Queue(void)
{
    uint32_t index;
    uint32_t xCount = 0, xStrLen = 0;

    /* allocate a buffer to save statistic infomation in order to reduce syslog channel usage*/
#define BUFF_SIZE 256
#define SINGLE_STR_SIZE 64
    char *pxResultStr = pvPortMalloc((size_t)BUFF_SIZE);

    exception_printf("task create info: %d\r\n", xTaskIndex);
    for (index = 0; index < xTaskIndex; index++) {
        snprintf(pxResultStr + xStrLen, SINGLE_STR_SIZE, "lr=0x%x, addr=0x%x, size=%d\r\n",
                 xTaskSizeGuardArr[index].lr,
                 xTaskSizeGuardArr[index].addr,
                 xTaskSizeGuardArr[index].size);
        xCount++;
        xStrLen = strlen(pxResultStr);

        /* output by syslog */
        if (xCount == (BUFF_SIZE / SINGLE_STR_SIZE)) {
            exception_printf("%s", pxResultStr);
            memset(pxResultStr, 0, BUFF_SIZE);
            xCount = 0;
            xStrLen = 0;
        }
    ...
    xStrLen = 0;
    memset(pxResultStr, 0, BUFF_SIZE);
    exception_printf("queue create info: %d\r\n", xQueueIndex);
    for (index = 0; index < xQueueIndex; index++) {
        snprintf(pxResultStr + xStrLen, SINGLE_STR_SIZE, "lr=0x%x, addr=0x%x, size=%d\r\n",
                 xQueueSizeGuardArr[index].lr,
                 xQueueSizeGuardArr[index].addr,
                 xQueueSizeGuardArr[index].size);
        xCount++;
        xStrLen = strlen(pxResultStr);
...
void vHeapSizeGuard_Register_Dump_Callback(void)
{
    uint32_t ret;
    exception_config_type callback_config;

    /* register callback to exception flow to dump task and queue size */
    callback_config.init_cb = vHeapSizeGuard_Dump_Task_Queue;
