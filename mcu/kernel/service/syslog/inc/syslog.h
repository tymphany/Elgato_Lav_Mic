 ...
 *        such as mcu/out/{board_name}/freertos_create_thread/cm4_log_str.bin for freertos_create_thread project.
 ...
 *             status = nvdm_write_data_item(NVDM_GROUP_SYSLOG,
 *                                           NVDM_SYSLOG_ITEM_MODULE_FILTER,
 ...
 *             syslog_filter_buf = (char *)pvPortMalloc(SYSLOG_FILTER_LEN);
 *             configASSERT(syslog_filter_buf != NULL);
 *             if (nvdm_read_data_item(NVDM_GROUP_SYSLOG, NVDM_SYSLOG_ITEM_MODULE_FILTER, (uint8_t *)syslog_filter_buf, &sz) == NVDM_STATUS_OK) {
...
do { \
    ...
    print_module_log(&log_control_block_##module, \
                                    __FUNCTION__, \
                                    __LINE__, \
                                    PRINT_LEVEL_DEBUG, \
                                    (message), \
...
do { \
    extern log_control_block_t log_control_block_##module; \
    dump_module_buffer(&log_control_block_##module, \
                                            __FUNCTION__, \
                                            __LINE__, \
                                            PRINT_LEVEL_DEBUG, \
                                            (data), \
...
*/
#if defined(MTK_SYSLOG_SUB_FEATURE_MSGID_TO_STRING_LOG_SUPPORT) || defined(MTK_DEBUG_PLAIN_LOG_ENABLE)
...
do { \
    extern log_control_block_t log_control_block_##module; \
    ATTR_LOG_STRING msg_id_string[] = message; \
    (void)(log_control_block_##module); \
    syslog_port_dummy(msg_id_string, ##__VA_ARGS__); \
} while (0)
#define LOG_HEXDUMP_D(module, message, data, len, ...) \
do { \
    extern log_control_block_t log_control_block_##module; \
