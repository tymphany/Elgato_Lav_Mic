...
TaskHandle_t atci_cmd_hdlr_bt_testbox_TaskHdlr = NULL;
QueueHandle_t atci_cmd_hdlr_bt_testbox_TaskQueue = NULL;
...
uint8_t atci_cmd_hdlr_bt_testbox_HciEventParser(uint8_t *param, uint16_t paramLen, atci_bt_hci_event *hci_event)
{
    ...
    switch (hci_event->event) {
            ...
            switch (atci_cmd_hdlr_bt_testbox_GetTextboxType()) {
                ...
                case TextboxType_SetLocalBtNameToController:
                    if (TextboxType_hcicmd_ResetBT == cmdOpcode ||
                        ...
                        snprintf((char *)output.response_buf, sizeof(output.response_buf), "+EBTTB:%d,%d\r\n",
                ...
                case TextboxType_Methodd_DUTRX:
                    if (TextboxType_hcicmd_TestCommand == cmdOpcode) {
                        ...
                        snprintf((char *)output.response_buf, sizeof(output.response_buf), "+EBTTB:%d,%ld\r\n",
                ...
                case TextboxType_GetLocalBtAddressFromController:
                    if (TextboxType_hcicmd_GetBtAddr == cmdOpcode) {
                        snprintf((char *)output.response_buf, sizeof(output.response_buf), "+EBTTB:%d,%x:%x:%x:%x:%x:%x\r\n",
                ...
                case TextboxType_GetLocalBtNameFromController:
                    if (TextboxType_hcicmd_GetBtName == cmdOpcode) {
                        ...
                        snprintf((char *)output.response_buf, sizeof(output.response_buf), "+EBTTB:%d,%s\r\n",
        ...
        case ATCI_HCE_INQUIRY_RESULT_WITH_EIR:
            ...
            LOG_MSGID_I(atci_bt, "bt addr :%x:%x:%x:%x:%x:%x,RSSI=%ddbm", 7,
                        hci_event->parms[6], hci_event->parms[5], hci_event->parms[4],
                        hci_event->parms[3], hci_event->parms[2], hci_event->parms[1],
                        hci_event->parms[14] - 256);
            snprintf((char *)output.response_buf, sizeof(output.response_buf), "+EBTTB:%d,%d,%x:%x:%x:%x:%x:%x\r\n",
                     atci_cmd_hdlr_bt_testbox_GetTextboxType(),
                     hci_event->parms[14] - 256,
                     hci_event->parms[6], hci_event->parms[5], hci_event->parms[4],
                     hci_event->parms[3], hci_event->parms[2], hci_event->parms[1]);
...
{
    if (atci_cmd_hdlr_bt_testbox_TaskHdlr) {
        vTaskDelete(atci_cmd_hdlr_bt_testbox_TaskHdlr);
        atci_cmd_hdlr_bt_testbox_TaskHdlr = NULL;
    }

    if (atci_cmd_hdlr_bt_testbox_TaskQueue) {
        vQueueDelete(atci_cmd_hdlr_bt_testbox_TaskQueue);
        atci_cmd_hdlr_bt_testbox_TaskQueue = NULL;
    }
}
...
static int atci_cmd_hdlr_bt_testbox_HciCmd_SetNameToController()
{
    ...
    hci_cmd.cmd_len = 248;
    strncpy((char *)(&(hci_cmd.cmd)), atci_cmd_hdlr_bt_testbox_LocalName, sizeof(hci_cmd.cmd));
    bt_driver_send_hci_command(hci_cmd.op_code, hci_cmd.cmd_len, hci_cmd.cmd);
...
static int atci_cmd_hdlr_bt_testbox_SetAddrToNvdm(char *req)
{
    ...
    if (0 != atci_cmd_hdlr_bt_testbox_CheckBtaddrValid(req)) {
        LOG_MSGID_I(atci_bt, "[BT]invalid address.\n", 0);
        output.response_len = 0;
        output.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
    } else {
        for (i = 0; i < 6; ++i) {
            tmp_buf[0] = req[2 * i];
            tmp_buf[1] = req[2 * i + 1];
            addr[5 - i] = (uint8_t)strtoul((char *)tmp_buf, NULL, 16);
        }
        bt_device_manager_store_local_address(&addr);
        LOG_MSGID_I(atci_bt, "[BT]Successfully store address to NVDM [%02X:%02X:%02X:%02X:%02X:%02X]\n", 6, addr[0],
...
static int atci_cmd_hdlr_bt_testbox_GetAddrFromNvdm()
{
    ...
    LOG_MSGID_I(atci_bt, "[BT]Read address from NVDM [%02X:%02X:%02X:%02X:%02X:%02X]", 6, (*addr)[5],
                (*addr)[4], (*addr)[3], (*addr)[2], (*addr)[1], (*addr)[0]);
    memcpy(atci_cmd_hdlr_bt_testbox_LocalAddr, addr, sizeof(bt_bd_addr_t));
    snprintf((char *)output.response_buf, sizeof(output.response_buf), "+EBTTB:%d,[%02X:%02X:%02X:%02X:%02X:%02X]\r\n",
             atci_cmd_hdlr_bt_testbox_GetTextboxType(), (*addr)[5],
             (*addr)[4], (*addr)[3], (*addr)[2], (*addr)[1], (*addr)[0]);
    output.response_len = strlen((char *)output.response_buf);
    output.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
...
static int atci_cmd_hdlr_bt_testbox_SetNameToNvdm(char *req)
{
    ...
    if (NVDM_STATUS_OK != status) {
        ...
        strncpy(atci_cmd_hdlr_bt_testbox_LocalName, req, sizeof(atci_cmd_hdlr_bt_testbox_LocalName));
...
static int atci_cmd_hdlr_bt_testbox_EnterBT()
{
    ...
    if (false == atci_cmd_hdlr_bt_testbox_PowerStatus) {
        if (true == atci_cmd_hdlr_bt_testbox_NormalBtPowerOn) {
            LOG_MSGID_I(atci_bt, "BT has powered on, need to power off", 0);
            bt_power_off();
...
int atci_cmd_hdlr_bt_testbox_doing(atci_cmd_hdlr_bt_testbox_type type, char *req)
{
    ...
    if (NULL != req) {
        ...
        LOG_MSGID_I(atci_bt, "type:%d, req:NULL", 1, type);
    }
...
atci_status_t atci_cmd_hdlr_bt_testbox_btaddr(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    if (parse_cmd->mode == ATCI_CMD_MODE_EXECUTION) {/*AT+EBTADDR=<op>*/
        if (strstr((char *)parse_cmd->string_ptr, "AT+EBTADDR=0") != NULL) {
            ...
            snprintf((char *)output.response_buf, sizeof(output.response_buf), "+EBTADDR:%02X%02X%02X%02X%02X%02X\r\n",
            ...
            if (parse_cmd->string_ptr[12]  !=  ',') {
                LOG_MSGID_I(atci_bt, "input is not valid: %c", 1, parse_cmd->string_ptr[11]);
                output.response_flag = 0 | ATCI_RESPONSE_FLAG_APPEND_ERROR;
                goto END;
            }
            ...
            if (is_invalid != 0) {
                LOG_MSGID_I(atci_bt, "bt address is invalid: %d", 1, is_invalid);
                output.response_flag = 0 | ATCI_RESPONSE_FLAG_APPEND_ERROR;
                goto END;
            }
            for (i = 0; i < 6; ++i) {
...
atci_status_t atci_cmd_hdlr_bt_testbox_CW_tone_readRSSI(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_TESTING:/**< Test mode command, such as "AT+CMD=?". AT+EBBTRXRSSI=?*/
            strncpy((char *)resonse.response_buf, "+EBBTRXRSSI=99\r\nOK\r\n", sizeof(resonse.response_buf));
            resonse.response_len = strlen((char *)resonse.response_buf);
            atci_send_response(&resonse);
            break;

        case ATCI_CMD_MODE_EXECUTION:    //
            strncpy((char *)resonse.response_buf, "OK\r\n", sizeof(resonse.response_buf));//here response infor : OK
            resonse.response_len = strlen((char *)resonse.response_buf);
            atci_send_response(&resonse);

            LOG_I(atci_bt, "parse_cmd->string_ptr=%s\r\n", parse_cmd->string_ptr);
            if (strncmp(parse_cmd->string_ptr, "AT+EBBTRXRSSI=", strlen("AT+EBBTRXRSSI=")) == 0) {
                param1 = (char *)strtok(parse_cmd->string_ptr, "=");//char *strtok(char s[], const char *delim); ��??a��?��?��??a��?������?��?��??��s?a��a��??a��?��?��?��?��?delim?a��???��?��?��?��??��
                param1 = (char *)strtok(NULL, ",");
                things = atoi(param1);
                LOG_MSGID_I(atci_bt, "things=%d\r\n", 1, things);
                if (0 == things) {
                    p = (uint8_t *)(RSSI_BASE);
                    for (length = 0; length < 8 ; length++) {
                        //LOGI("RSSI = %2x \n",*p);
                        LOG_MSGID_I(atci_bt, "things=%d,current RSSI = %2x \r\n", 2, things, *p);
                        if (length == 2) {
                            result = *p;
                        }
                        p++;
                    }
                    resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;//Auto append "OK\r\n" at the end of the response string
                    //add end
                }
            }
            snprintf((char *)resonse.response_buf, sizeof(resonse.response_buf), "+EBBTRXRSSI:%d,%d\r\n", things, result);
            resonse.response_len = strlen((char *)resonse.response_buf);
            atci_send_response(&resonse);
            break;
