...
static uint8_t aes_cbc_iv[HAL_AES_CBC_IV_LENGTH] = {
...
bool aes_cbc_ecb_test(int bits)
{
    ...
    if (bits == 128) {
        key.buffer = hardware_id_128;
        key.length = sizeof(hardware_id_128);
    } else if (bits == 192) {
        key.buffer = hardware_id_192;
        key.length = sizeof(hardware_id_192);
    } else if (bits == 256) {
        key.buffer = hardware_id_256;
