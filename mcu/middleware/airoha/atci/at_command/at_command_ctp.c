...
#define LOGMSGIDE(fmt,cnt,arg...)   LOG_MSGID_E(atci_ctp, "[CTP]"fmt,cnt,##arg)
#define LOGMSGIDW(fmt,cnt,arg...)   LOG_MSGID_W(atci_ctp, "[CTP]"fmt,cnt,##arg)
#define LOGMSGIDI(fmt,cnt,arg...)   LOG_MSGID_I(atci_ctp ,"[CTP]"fmt,cnt,##arg)
...
void atci_ctp_handler(void)
{
    ...
    LOGMSGIDW("[user]**enter ctp hanlder get event in at command\r\n", 0);
    while (1) {
        ret = bsp_ctp_get_event_data(&touch_event);
        ctp_get_buf_roomleft(&left_size);

        if (ret == BSP_CTP_EVENT_EMPTY) {
            LOGMSGIDW("[DATA] all event data has been get out\r\n", 0);
            break;
        }

        if (touch_event.model > 5) {
            LOGMSGIDW("[DATA] model =%d overflow\r\n", 1, touch_event.model);
            break;
        }
        ...
        if ((ret !=  BSP_CTP_EVENT_EMPTY) && (ret !=  BSP_CTP_OK)) {
            LOGMSGIDW("[DATA] some error happend when get event\r\n", 0);
        }
    }
