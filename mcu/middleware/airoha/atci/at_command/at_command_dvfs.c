            ...
            if (param != NULL) {
                ...
                if (op == 0) {
                    ...
                    for (idx = 0; idx < list_num; idx++) {
                        if (idx == list_num - 1) {
                            snprintf((char *) response.response_buf, sizeof(tmp), "%d\r\n", cpufreq_list[idx]);
                        } else {
                            snprintf((char *) response.response_buf, sizeof(tmp), "%d,", cpufreq_list[idx]);
                        }
                    }
        ...
        case ATCI_CMD_MODE_EXECUTION:
            ...
            if (!param || !param1) {
                ...
                if (freq >= HAL_DVFS_OPP_NUM) {
                    ...
                    freq = (unsigned int)hal_dvfs_get_cpu_frequency();
                    snprintf((char *)response.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, "dvfs lock on =%d\r\n", freq);
                    response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;
                }
            }
            break;
