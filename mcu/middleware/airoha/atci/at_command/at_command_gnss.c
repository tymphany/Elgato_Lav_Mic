...
//GNSS Data
#ifndef GNSS_DEMO_TASK_NAME
#define GNSS_DEMO_TASK_NAME "gnss_t"
#endif
#ifndef GNSS_DEMO_TASK_STACK_SIZE
#define GNSS_DEMO_TASK_STACK_SIZE 4000
#endif
#ifndef GNSS_DEMO_TASK_PRIO
...
#else
    hal_sleep_manager_release_sleeplock(sleep_handler, HAL_SLEEP_LOCK_ALL);
#endif
...
#else
    hal_sleep_manager_acquire_sleeplock(sleep_handler, HAL_SLEEP_LOCK_ALL);
#endif
...
atci_status_t gnss_power_control_at_handler(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    output.cmd_id = parse_cmd->cmd_id;
#endif

    //if (strstr("AT+EGPSC", (char *)parse_cmd->string_ptr), strlen("AT+EGPSC"))
    //{
...
#ifdef ATCI_APB_PROXY_ADAPTER_ENABLE
    pmtk_command_len = strlen(parse_cmd->string_ptr) - strlen("AT+EGPSS=");
#else
