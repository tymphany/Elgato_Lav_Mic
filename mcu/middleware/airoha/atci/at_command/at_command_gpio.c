...
atci_status_t atci_cmd_hdlr_gpio(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
...
#if (PRODUCT_VERSION == 7687 || PRODUCT_VERSION == 7697)
            strncpy((char *)resonse.response_buf, "Not support GPIO AT command", sizeof("Not support GPIO AT command"));
            resonse.response_len = strlen((char *)resonse.response_buf);
            resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
            atci_send_response(&resonse);
#else
            strncpy((char *)resonse.response_buf, "ATCI_CMD_MODE_TESTING", sizeof("ATCI_CMD_MODE_TESTING"));
            resonse.response_len = strlen((char *)resonse.response_buf);
        ...
        case ATCI_CMD_MODE_EXECUTION:
            ...
            break;
#else
            strncpy((char *)resonse.response_buf, "ATCI_CMD_MODE_EXECUTION", sizeof("ATCI_CMD_MODE_EXECUTION"));
            resonse.response_len = strlen((char *)resonse.response_buf);
            resonse.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
            atci_send_response(&resonse);
            ...
            if (strncmp(parse_cmd->string_ptr, "AT+EGPIO=GPIO_GET:", strlen("AT+EGPIO=GPIO_GET:")) == 0) {
                ...
                for (i = pin_index[0]; i <= pin_index[1]; i++) {
                    ...
                    ret_len1 = snprintf((char *)&resonse.response_buf[resonse.response_len], ATCI_UART_TX_FIFO_BUFFER_SIZE - resonse.response_len, \
                    ...
                    if ((ATCI_UART_TX_FIFO_BUFFER_SIZE - (resonse.response_len + ret_len1 + 1)) <= 0) {
                        resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                        atci_send_response(&resonse);
                        resonse.response_len = 0;
                    }

                    resonse.response_len += (uint16_t)(ret_len1);
                }
            ...
            /*  set GPIO current driving. */
                ...
                if (config_data[0] >= sizeof(current_state) / sizeof(current_state[0])) {
                    ...
                    strncpy((char *)resonse.response_buf, str5, ret_len1);
                    for (uint32_t i = 0; i < sizeof(current_state) / sizeof(current_state[0]); i++) {
                        ret_len1 += snprintf((char *)(resonse.response_buf + ret_len1), 16, "%d : %dma\r\n", (int)i, (int)current_state[i]);
                    }
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
            ...
            /* Set serveral configurations to one GPIO. */
                ...
                resonse.response_len = snprintf((char *)(resonse.response_buf), ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                ...
                if (gpio_in_data != HAL_GPIO_DATA_HIGH) {
                    strncpy((char *)resonse.response_buf, "GPIO_0 and GPIO5 Output High Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_0 and GPIO5 Output High Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_0 and GPIO5 Output High Fail, AT+GPIO_FT FAIL\r\n");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                if (gpio_in_data != HAL_GPIO_DATA_LOW) {
                    strncpy((char *)resonse.response_buf, "GPIO_0 and GPIO5 Output Low Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_0 and GPIO5 Output Low Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_0 and GPIO5 Output Low Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                if (gpio_in_data != HAL_GPIO_DATA_HIGH) {
                    strncpy((char *)resonse.response_buf, "GPIO_2 and GPIO3 Output High Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_2 and GPIO3 Output High Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_2 and GPIO3 Output High Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                hal_gpio_get_input(HAL_GPIO_3, &gpio_in_data);
                if (gpio_in_data != HAL_GPIO_DATA_LOW) {
                    strncpy((char *)resonse.response_buf, "GPIO_2 and GPIO3 Output Low Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_2 and GPIO3 Output Low Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_2 and GPIO3 Output Low Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                if (gpio_in_data != HAL_GPIO_DATA_HIGH) {
                    strncpy((char *)resonse.response_buf, "GPIO_20 and GPIO21 Output High Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_20 and GPIO21 Output High Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_20 and GPIO21 Output High Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                hal_gpio_get_input(HAL_GPIO_21, &gpio_in_data);
                if (gpio_in_data != HAL_GPIO_DATA_LOW) {
                    strncpy((char *)resonse.response_buf, "GPIO_20 and GPIO21 Output Low Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_20 and GPIO21 Output Low Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_20 and GPIO21 Output Low Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                if (gpio_in_data != HAL_GPIO_DATA_HIGH) {
                    strncpy((char *)resonse.response_buf, "GPIO_13 and GPIO14 Output High Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_13 and GPIO14 Output High Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_13 and GPIO14 Output High Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                hal_gpio_get_input(HAL_GPIO_14, &gpio_in_data);
                if (gpio_in_data != HAL_GPIO_DATA_LOW) {
                    strncpy((char *)resonse.response_buf, "GPIO_13 and GPIO14 Output Low Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_13 and GPIO14 Output Low Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_13 and GPIO14 Output Low Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                if (gpio_in_data != HAL_GPIO_DATA_HIGH) {
                    strncpy((char *)resonse.response_buf, "GPIO_10 and GPIO4 Output High Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_10 and GPIO4 Output High Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_10 and GPIO4 Output High Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                hal_gpio_get_input(HAL_GPIO_4, &gpio_in_data);
                if (gpio_in_data != HAL_GPIO_DATA_LOW) {
                    strncpy((char *)resonse.response_buf, "GPIO_10 and GPIO4 Output Low Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_10 and GPIO4 Output Low Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_10 and GPIO4 Output Low Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                if (gpio_in_data != HAL_GPIO_DATA_HIGH) {
                    strncpy((char *)resonse.response_buf, "GPIO_1 and GPIO11 Output High Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_1 and GPIO11 Output High Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_1 and GPIO11 Output High Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                hal_gpio_get_input(HAL_GPIO_11, &gpio_in_data);
                if (gpio_in_data != HAL_GPIO_DATA_LOW) {
                    strncpy((char *)resonse.response_buf, "GPIO_1 and GPIO11 Output Low Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_1 and GPIO11 Output Low Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_1 and GPIO11 Output Low Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                if (gpio_in_data != HAL_GPIO_DATA_HIGH) {
                    strncpy((char *)resonse.response_buf, "GPIO_22 Read High Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_22 Read High Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_22 Read High Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
                ...
                if (gpio_in_data != HAL_GPIO_DATA_HIGH) {
                    strncpy((char *)resonse.response_buf, "GPIO_15 Read High Fail, AT+GPIO_FT FAIL\r\n", strlen("GPIO_15 Read High Fail, AT+GPIO_FT FAIL\r\n"));
                    LOGE("GPIO_15 Read High Fail, AT+GPIO_FT FAIL");
                    test_fail++;
                    goto gpio_ft_fail;
                }
...
gpio_ft_fail:
                if (test_fail == 0) {
                    strncpy((char *)resonse.response_buf, "AT+GPIO_FT PASS\r\n", strlen("AT+GPIO_FT PASS\r\n\r\n"));
                    LOGI("AT+GPIO_FT PASS");
                }
                resonse.response_len = strlen((char *)resonse.response_buf);
                resonse.response_flag = (test_fail == 0) ? ATCI_RESPONSE_FLAG_APPEND_OK : ATCI_RESPONSE_FLAG_APPEND_ERROR;
                atci_send_response(&resonse);
                break;
                ...
                if (HAL_EINT_STATUS_OK !=  hal_eint_init(HAL_EINT_NUMBER_4, &eint_config)) {
                    test_fail = 1;
                    strncpy((char *)resonse.response_buf, "AT+GPIO_PPS FAIL\r\n", strlen("AT+GPIO_PPS FAIL\r\n"));
                    LOGE("AT+GPIO_PPS FAIL, hal_eint_init fail\r\n");
                    goto gpio_pps_test_fail;
                }
                if (HAL_EINT_STATUS_OK !=  hal_eint_register_callback(HAL_EINT_NUMBER_4, pps_test_callback, NULL)) {
                    test_fail = 1;
                    strncpy((char *)resonse.response_buf, "AT+GPIO_PPS FAIL\r\n", strlen("AT+GPIO_PPS FAIL\r\n"));
                    LOGE("AT+GPIO_PPS FAIL, hal_eint_register_callback fail\r\n");
                    goto gpio_pps_test_fail;
                }
...
gpio_pps_test_fail:
                if (test_fail == 0) {
                    strncpy((char *)resonse.response_buf, "AT+GPIO_PPS PASS\r\n", strlen("AT+GPIO_PPS PASS\r\n\r\n"));
                    LOGI("AT+GPIO_PPS PASS");
                }
                resonse.response_len = strlen((char *)resonse.response_buf);
                resonse.response_flag = (test_fail == 0) ? ATCI_RESPONSE_FLAG_APPEND_OK : ATCI_RESPONSE_FLAG_APPEND_ERROR;
                atci_send_response(&resonse);
