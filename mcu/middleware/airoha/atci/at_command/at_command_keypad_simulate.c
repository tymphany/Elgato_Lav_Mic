...
do{ \
    ...
    if(at_keypad_lock_sleep_handle[i] == INVALID_SLEEP_HANDLE ) { \
        LOGMSGIDE("[keypad]:get sleep handle failed\r\n", 0); \
...
#define AT_KEYPAD_LOCK_SLEEP(i) \
do{ \
    hal_sleep_manager_acquire_sleeplock(at_keypad_lock_sleep_handle[i], HAL_SLEEP_LOCK_ALL); \
}while(0)

#define AT_KEYPAD_UNLOCK_SLEEP(i) \
do{ \
    hal_sleep_manager_release_sleeplock(at_keypad_lock_sleep_handle[i], HAL_SLEEP_LOCK_ALL); \
}while(0)
#endif
...
atci_status_t atci_cmd_hdlr_keypad_simulate(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_EXECUTION:
            ...
            if (param2 == NULL) {
                LOGMSGIDE("param is null after assign\r\n", 0);
