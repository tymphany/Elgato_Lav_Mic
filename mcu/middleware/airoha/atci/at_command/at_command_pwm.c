...
/*compatible for other chip, new chip should be defined below Macro in hal_platform.h*/
#if ((PRODUCT_VERSION == 1552) || (PRODUCT_VERSION == 2811) || (defined(AM255X)))
#define  HAL_PWM_CLOCK_32K_SUPPORTED
#define  HAL_PWM_CLOCK_13M_SUPPORTED
#define  HAL_PWM_CLOCK_48M_SUPPORTED
#endif
#if (PRODUCT_VERSION == 7698)
#define  HAL_PWM_CLOCK_32K_SUPPORTED
#define  HAL_PWM_CLOCK_13M_SUPPORTED
#define  HAL_PWM_CLOCK_40M_SUPPORTED
#endif

#if defined DAIR_BTA_IC_PREMIUM_G3
#define HAL_PWM_CLOCK_32K_SUPPORTED
#define HAL_PWM_CLOCK_26M_SUPPORTED
#define HAL_PWM_CLOCK_39M_SUPPORTED
#define HAL_PWM_CLOCK_86M_SUPPORTED
#endif
...
atci_status_t atci_cmd_hdlr_pwm(atci_parse_cmd_param_t *parse_cmd)
{
#if ((PRODUCT_VERSION == 7687) || (PRODUCT_VERSION == 7697) || (PRODUCT_VERSION == 5932))
    ...
    if (arg_valid == 0) {
        strncpy((char *)(char *)response.response_buf, "argument error\r\n", strlen("argument error\r\n"));
    }
    if (para_valid == 0) {
        strncpy((char *)(char *)response.response_buf, "parameter invalid\r\n", strlen("parameter invalid\r\n"));
    }
    if (error == 1) {
