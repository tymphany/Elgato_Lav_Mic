...
#else
#define LOGE(fmt,arg...)   printf("[REG]"fmt,##arg)
#define LOGW(fmt,arg...)   printf("[REG]"fmt,##arg)
#define LOGI(fmt,arg...)   printf("[REG]"fmt,##arg)
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_EXECUTION: // rec: AT+EREG=<op>  the handler need to parse the parameters
            if (strncmp(cmd, "AT+EREG=0,", strlen("AT+EREG=0,")) == 0) {
                ...
                snprintf((char *)response.response_buf, sizeof(response.response_buf), "+EREG:0x%x,0x%x\r\n", (input_addr - 4), read_value);
                ...
                snprintf((char *)response.response_buf, sizeof(response.response_buf), "+ERRFEG:0x%x,0x%x\r\n", input_addr, read_value);
                /* ATCI will help append "OK" at the end of resonse buffer */
                response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;
                response.response_len = strlen((char *)response.response_buf);
            ...
            } else if (strncmp(cmd, "AT+EREG=CID", strlen("AT+EREG=CID")) == 0) {
                /*command: AT+EREG=CID */
                extern uint32_t sub_chip_version_get();
                uint32_t uid = sub_chip_version_get();
                LOGMSGIDI("Chip Version: 0x%x \r\n", 1, uid);
                response.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                snprintf((char *)response.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, "CID: %d \r\n", (unsigned int)uid);
                response.response_len = strlen((char *)response.response_buf);
            } else if (strncmp(cmd, "AT+EREG=5,", strlen("AT+EREG=5,")) == 0) {
                /*command: AT+EREG=5,0xABCDABCD
                  read with 4byte */
                end_pos = strchr(cmd, ',');
                end_pos ++;
