...
void    _atci_rtc_handle_rtc_mode(char *cmd_string, atci_response_t *response)
{
    uint32_t second = 0;
    char    *end_pos = NULL;
...
void    _atci_rtc_handle_32k_to_gpio(char *cmd_string, atci_response_t *response)
{
    char    *end_pos = NULL;
    ...
    end_pos = strchr(end_pos, ',');
    end_pos++;
    pinfunc = atoi(end_pos);

    /* ctrl value */
    end_pos = strchr(end_pos, ',');
    end_pos++;
    ctrl_val = mt_htoi(end_pos);
...
void    _atci_rtc_handle_execution(char *cmd_string, atci_response_t *response)
{
    ...
    switch (mode) {
        ...
        case 1: {   /* AT+ERTC=TEST,1,<sec> for enter rtc mode*/
            _atci_rtc_handle_rtc_mode(end_pos, response);
        }
        break;
        ...
        break;
        case 6: { /* AT+ERTC=TEST,6,<second> for set rtc alarm*/
            _atci_rtc_handle_eosc_test(end_pos, response);
        }
        break;

        case 7: {
            _atci_rtc_handle_32k_to_gpio(end_pos, response);
        }
        break;
...
void    _atci_rtc_handle_rtc_gpio(char *cmd_string, atci_response_t *response)
{
    uint8_t  mode, pin;
    char    *end_pos = NULL;
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_EXECUTION: {
            if (strncmp(parse_cmd->string_ptr, "AT+ERTC=REG,", strlen("AT+ERTC=REG,")) == 0) {
                ...
                _atci_rtc_handle_rtc_gpio(parse_cmd->string_ptr, &response);
            }
        }
        break;
#endif
