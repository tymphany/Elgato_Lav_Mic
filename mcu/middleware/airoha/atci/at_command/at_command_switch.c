...
atci_status_t atci_cmd_hdlr_switch(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    LOGMSGIDI("atci_cmd_hdlr_switch no support MUX", 0);
    ...
    LOGMSGIDI("atci_cmd_hdlr_switch\r\n", 0);
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_EXECUTION: // rec: AT+SWITCH=<p1>  the handler need to parse the parameters
            ...
            if (param != NULL && strlen(param) > 0) {
                if (param[0] == '1' || param[0] == '2') {
                    ...
                    if (param_len < cmp_len) {
                        isSwitchRace = false;
                        LOGMSGIDI("param len is short,no need to switch", 0);
                    } else {
            ...
            if (isSwitchRace) {
                strncpy((char *)(response.response_buf), AT_COMMAND_SWITCH_INFO, strlen("AT switch to RACE command mode!\r\n"));
                response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;
                response.response_len = strlen((char *)response.response_buf);
                atci_send_response(&response);

                if (isSwitchRace_no_atci) {
                    atci_status_t sta = atci_deinit_keep_table(g_atci_uart_port);
                    LOGMSGIDI("AT Switch call atci_deinit_keep_table. result=%d\r\n", 1, sta);
                }

                //TODO: call race command init
                if (g_atci_uart_port == g_atci_uart_port_default) {
                    race_status_t stu = race_uart_deinit();
                    ...
                    if (serial_port_config_read_dev_number("atci", &port) != SERIAL_PORT_STATUS_OK) {
                        ...
                        LOGMSGIDI("serial_port_config_write_dev_number setting uart1", 0);
