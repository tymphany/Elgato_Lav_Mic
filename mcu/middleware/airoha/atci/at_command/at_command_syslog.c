...
#if defined(MTK_SAVE_LOG_TO_FLASH_ENABLE) && !defined(MTK_MUX_ENABLE)
    pos += snprintf((char *)(buf + pos),
                    ATCI_UART_TX_FIFO_BUFFER_SIZE - pos,
...
void log_show_filter_config(uint8_t *buf, uint32_t size)
{
    ...
    for (i = 0; i < cpu_number; i++) {
        if (log_get_module_filter_number(i, &module_number) == true) {
            ...
            snprintf((char *)(buf + pos), size - pos, "\r\n");
        }
    }
}
...
void syslog_at_excution_mode(atci_parse_cmd_param_t *parse_cmd, atci_response_t *response)
{
    bool ret;
    uint8_t type;
...
atci_status_t atci_cmd_hdlr_syslog(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_EXECUTION:
                syslog_at_excution_mode(parse_cmd, &response);
            break;
...
#define SYSLOG_CFG_TX_FIFO_SIZE     (1024 * 3)

atci_status_t atci_cmd_hdlr_syslog(atci_parse_cmd_param_t *parse_cmd)
{
    atci_status_t status;
    atci_response_t response = {{0}, 0};
    atci_response_heavy_data_t response_cfg;
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_READ:
            response_cfg.response_buf = (uint8_t *)pvPortMalloc(SYSLOG_CFG_TX_FIFO_SIZE);
            if (response_cfg.response_buf != NULL) {
                log_show_filter_config(response_cfg.response_buf, SYSLOG_CFG_TX_FIFO_SIZE);
                response_cfg.response_len = strlen((char *)response_cfg.response_buf);
                response_cfg.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
                status = atci_send_heavy_response(&response_cfg);
                vPortFree(response_cfg.response_buf);
                return status;
            } else {
                response.response_flag = ATCI_RESPONSE_FLAG_APPEND_ERROR;
            }
