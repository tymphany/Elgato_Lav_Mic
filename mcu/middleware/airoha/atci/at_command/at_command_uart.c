...
#if defined(HAL_UART_MODULE_ENABLED) && defined(AIR_BTA_IC_STEREO_HIGH_G3)
...
atci_status_t atci_cmd_hdlr_uart(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    atci_response_t * resonse  = (atci_response_t *)pvPortMalloc(sizeof(atci_response_t));
    ...
    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_TESTING:   /* rec: AT+EUART=?   */
            strncpy((char *)resonse->response_buf, "+EUART:<p1>,<p2>", strlen("+EUART:<p1>,<p2>")+1);
