...
#define LOGMSGIDE(fmt,cnt,arg...)   LOG_MSGID_E(atci, "ATCI hdl: "fmt,cnt,##arg)
#define LOGMSGIDW(fmt,cnt,arg...)   LOG_MSGID_W(atci, "ATCI hdl: "fmt,cnt,##arg)
#define LOGMSGIDI(fmt,cnt,arg...)   LOG_MSGID_I(atci ,"ATCI hdl: "fmt,cnt,##arg)

#define ATCI_HANDLER "atci handler"
#define ATCI_SEND_RESPONSE "atci_send_response"
#define ATCI_INPUT_COMMAND_HANDLER "atci_input_command_handler"
...
extern uint32_t g_atcmd_table_semaphore;
...
atci_status_t atci_register_handler(atci_cmd_hdlr_item_t *hdlr_table, int32_t hdlr_number)
{
    ...
    if (1) {
        ...
        if (ret == ATCI_STATUS_OK) {
            ...
            atci_semaphore_take(g_atcmd_table_semaphore);
        ...
        if (ret == ATCI_STATUS_OK) {
            ...
            LOGMSGIDI("atci handler: register success \r\n", 0);
        } else {
            LOGMSGIDE("atci handler: register fail \r\n", 0);
            ret = ATCI_STATUS_REGISTRATION_FAILURE;
        }
    } else {

        LOGMSGIDE("atci handler: register fail, please call atci_init() frist \r\n", 0);
    }
...
atci_status_t atci_send_response(atci_response_t *response)
{
    ...
    if (g_atci_local_startup_flag != 1) {
        LOGMSGIDW("atci_send_response: fail, atci does not ready!\r\n", 0);
        return ret;
    }

    notify_queue_msg_num = atci_queue_get_item_num(g_atci_resonse_command_queue);

    if (notify_queue_msg_num >= ATCI_LOCAL_QUEUE_LENGTH) {
        atci_process_flag((void *)response->response_buf, (uint16_t *) & (response->response_len),
        ...
        if (ATCI_RESPONSE_FLAG_URC_FORMAT == (response->response_flag & ATCI_RESPONSE_FLAG_URC_FORMAT)) {
            LOGMSGIDW("atci_send_response, atci response queue full(%d), drop this urc data!\r\n", 1, notify_queue_msg_num);
        } else {

            if (atci_input_commad_in_processing == 2) {
                atci_input_commad_in_processing = 3;
            }

            LOGMSGIDW("atci_send_response, atci response queue full(%d), drop this rsp data!\r\n", 1, notify_queue_msg_num);
        }

//      LOGW("atci_send_response(), atci response queue full(%d), drop this rsp data!\r\n", notify_queue_msg_num);
        return ret;
    }


    /* send ATCI_SEND_CMD msg to ATCI queue */
    ret = atci_process_flag((void *)response->response_buf, (uint16_t *) & (response->response_len),
                            ATCI_UART_TX_FIFO_BUFFER_SIZE, response->response_flag);

    if (ret == ATCI_STATUS_ERROR) {
        LOGMSGIDW("atci_send_response, len(%d) flag(%x), too long, drop!\r\n", 2, response->response_len, response->response_flag);
        return ret;
    }

    msg_data = (uint8_t *)atci_mem_alloc(response->response_len + sizeof(response->response_len));

    if (msg_data == NULL) {
        LOGMSGIDW("atci_send_response, atci_mem_alloc fail \r\n", 0);
        return ATCI_STATUS_ERROR;
    }

    memcpy((void *)(msg_data), (uint8_t *)(&(response->response_len)), sizeof(response->response_len));
    memcpy((void *)(msg_data + sizeof(response->response_len)), (uint8_t *)response->response_buf, response->response_len);
...
atci_status_t atci_send_heavy_response(atci_response_heavy_data_t *response)
{
    ...
    notify_queue_msg_num = atci_queue_get_item_num(g_atci_resonse_command_queue);
    LOGMSGIDW("atci_send_response, msg_num: %x\r\n", 1, notify_queue_msg_num);
    ...
    //LOGW("atci_send_response, flag = %x,  rsp = %s\r\n", response->response_flag, (char *)(response->response_buf));

    tail_len = atci_process_tail_len(response->response_flag);

    LOGMSGIDW("atci_send_response, tail_len = %x, len = %x, len_byte = %x\r\n", 3, tail_len, response->response_len, sizeof(response->response_len));

    data_len = response->response_len + tail_len;

    /* send ATCI_SEND_CMD msg to ATCI queue */

    LOGMSGIDW("atci_send_response, data_len = %x\r\n", 1, data_len);
    msg_data = (uint8_t *)atci_mem_alloc(data_len + sizeof(data_len));
    ...
    memcpy((void *)(msg_data), (uint8_t *)(&(data_len)), sizeof(data_len));


    memcpy((void *)(msg_data + sizeof(data_len)), (uint8_t *)response->response_buf, response->response_len);

    atci_process_flag((void *)(msg_data + sizeof(data_len)), (uint16_t *) & (response->response_len),
                      data_len, response->response_flag);
...
static atci_status_t atci_init_hdlr_tbl_hash_value(atci_cmd_hdlr_item_t *hdlr_table, uint16_t hdlr_number)
{
    ...
    if (memcmp(atcmd_table, hdlr_table, sizeof(atci_cmd_hdlr_item_t)) == 0) {
        ...
        LOGMSGIDW("atci_init_hdlr_tbl_hash_value, exclude atcmd_table success\r\n", 0);
    } else {
...
static atci_status_t atci_add_to_cm4_hdlr_tbls(atci_cmd_hdlr_item_t *hdlr_table, uint16_t hdlr_number)
{
    if (g_atci_registered_table_number >= ATCI_MAX_GNENERAL_TABLE_NUM) {
        ...
        LOGMSGIDE("atci handler register fail, hdlr table extend \r\n", 0);
...
atci_status_t atci_input_command_handler(atci_send_input_cmd_msg_t *cmd_msg)
{
    ...
    if (cmd_msg == NULL) {
        LOGMSGIDW("atci_input_command_handler:input buf is null\r\n", 0);
        return ATCI_STATUS_ERROR;
    }

    //LOGI("atci_input_command_handler, input_buf: %s\n", cmd_msg->input_buf);
...
static atci_status_t atci_local_parse_input_at_cmd(atci_parse_cmd_param_ex_t *parse_cmd)
{
    ...
    //LOGI("atci_local_parse_input_at_cmd, string: %s\n", parse_cmd->string_ptr);
...
static atci_status_t atci_local_dispatch_cm4_general_handler(atci_parse_cmd_param_ex_t *parse_cmd)
{
    ...
    for (i = 0; i < g_atci_registered_table_number; i++) {
        ...
        for (j = 0; j < item_table_size; j++) {
            handler_item = &(g_atci_cm4_general_hdlr_tables[i].item_table[j]);
            //printf("%s,%d,%d\n",handler_item->command_head,handler_item->hash_value1,handler_item->hash_value2);
            if (parse_cmd->hash_value1 == handler_item->hash_value1 && parse_cmd->hash_value2 == handler_item->hash_value2 &&
    ...
    if ((NULL != handler_item) && (ret == ATCI_STATUS_OK)) {
        ...
        LOGMSGIDE("find command handler fail \r\n", 0);
    }
