...
static bool audio_mixer_adapt_resample_open(audio_mixer_track_id_t track_id)
{
    ...
    if (is_need_bli_src) {
        ...
        if (Audio_Mixer.track[track_id].bli_allocate_buffer_alloc == NULL) {
            AUD_LOG_E("[Mixer] alloc fail: bli_allocate_buffer %d \n", 1, buf_size);
            return false;
        }
...
static void audio_mixer_process_track(track_info_t *p_track)
{
    ...
    if (p_track->bli_handle || p_track->channel_convert) {
        ...
        if (SRC_free_count) {
            ...
            AUD_LOG_I("[Mixer] UDF1 t %x, c %d\n", 2, p_track, SRC_free_count);
        }
    } else {
        // Need not SRC
        for (i = 0; i < 2; i++) {
    ...
    if (p_track->eof_state == AUDIO_MIXER_EOF_STATE_FLUSH_TRACK) {
        ...
        AUD_LOG_I("[Mixer] Track EOF. Remaining %d \n", 1, temp_count);
        if (temp_count < 4) {
            ...
            // Enlarge EOF time
            if (Audio_Mixer.hal_is_pcm_route || Audio_Mixer.hal_is_p2w) {
                p_track->eof_remaining_size += Audio_Mixer.mixer_block_size * 3;
            }

            AUD_LOG_I("[Mixer] enter mixer EOF. Remaining %d \n", 1, p_track->eof_remaining_size);
        }
        ...
        if (Audio_Mixer.track[i].state == AUDIO_MIXER_STATE_MACHINE_RUNNING) {
            ...
            if (Audio_Mixer.track[i].eof_state == AUDIO_MIXER_EOF_STATE_FLUSH_HAL) {
                if (Audio_Mixer.hal_is_pcm_route ||
                    ...
                    Audio_Mixer.track[i].eof_state = AUDIO_MIXER_EOF_STATE_NOTIFIED;
                    Audio_Mixer.track[i].callback(HAL_AUDIO_EVENT_END, Audio_Mixer.track[i].callback_user_data);
                } else {
                    AUD_LOG_I("mixer set eof to HAL\r\n", 0);
...
static void audio_mixer_process(audio_mixer_queue_event_id_t id, void *data)
{
    ...
    if (id == AUDIO_MIXER_QUEUE_EVENT_END) {
        for (i = 0; i < AUDIO_MIXER_MAX_TRACK_NUMBER; i++) {
            if (Audio_Mixer.track[i].state == AUDIO_MIXER_STATE_MACHINE_RUNNING) {
                Audio_Mixer.track[i].waiting = true;
                if (Audio_Mixer.track[i].eof_state != AUDIO_MIXER_EOF_STATE_NOTIFIED) {
                    Audio_Mixer.track[i].eof_state = AUDIO_MIXER_EOF_STATE_NOTIFIED;
                    Audio_Mixer.track[i].callback(HAL_AUDIO_EVENT_END, Audio_Mixer.track[i].callback_user_data);
                }
            }
...
{
    AUD_LOG_I("[Mixer] task_main create\n", 0);
...
static void auddio_mixer_send_event(audio_mixer_queue_event_id_t id, void *parameter)
{
    ...
    if (Audio_Mixer.queue_handle != NULL) {
        if (xQueueSendFromISR(Audio_Mixer.queue_handle, &event, 0) != pdPASS) {
            AUD_LOG_I("[Mixer] send event fail\n", 0);
        }
    }
...
static void audio_mixer_apply_gain(uint8_t *p_src_buf, uint32_t size)
{
    ...
    // Ramp up the original sound
    if (Audio_Mixer.track[AUDIO_MIXER_TRACK_ROLE_SIDE].eof_state >= AUDIO_MIXER_EOF_STATE_FLUSH_MIXER) {
        target_gain = AUDIO_MIXER_UNIT_GAIN;
    }
    ...
    audio_mixer_track_id_t          track_id)
{
    ...
    if (Audio_Mixer.hal_is_runnung) {
        AUD_LOG_E("[Mixer] start re-entry\n", 0);
        return true;
    }
    ...
    AUD_LOG_I("[Mixer] Start HAL: DSP path %d, s %d, c %d, b %d\n", 4, Audio_Mixer.hal_is_pcm_route, Audio_Mixer.hal_sampling_rate, Audio_Mixer.hal_channel_number, Audio_Mixer.mixer_block_size);
    ...
    if (Audio_Mixer.mixer_buffer.base_address_alloc == NULL) {
        AUD_LOG_E("[Mixer] alloc fail: mixer_buffer \n", 0);
        return false;
    }
    ...
    if (Audio_Mixer.mixer_temp_block_alloc == NULL) {
        AUD_LOG_E("[Mixer] alloc fail: mixer_temp_block \n", 0);
        return false;
    }
    ...
    if (Audio_Mixer.resample_temp_block_alloc == NULL) {
        AUD_LOG_E("[Mixer] alloc fail: resample_temp_block \n", 0);
        return false;
    }
    ...
    if (Audio_Mixer.hal_is_p2w) {
        ...
        if (Audio_Mixer.pcm_route_buffer_alloc == NULL) {
            AUD_LOG_E("[Mixer] alloc fail: pcm_route_buffer \n", 0);
            return false;
        }
        Audio_Mixer.pcm_route_buffer = (uint8_t *)FOUR_BYTE_ALIGNED_ADDR(Audio_Mixer.pcm_route_buffer_alloc);

        Audio_Mixer.pcm_route_current_gain = AUDIO_MIXER_UNIT_GAIN;
        ...
        if (Audio_Mixer.hal_pcm_route_id < 0) {
            AUD_LOG_E("[Mixer] register PCM route fail: %d\r\n", 1, Audio_Mixer.hal_pcm_route_id);
        } else {
        ...
        if (Audio_Mixer.hal_memory_alloc == NULL) {
            AUD_LOG_E("[Mixer] alloc fail: hal_memory %d\n", 1, Audio_Mixer.hal_memory_size);
            return false;
        }
...
{
    AUD_LOG_I("[Mixer] Stop HAL: DSP path %d\n", 1, Audio_Mixer.hal_is_pcm_route);

    if (!Audio_Mixer.hal_is_runnung) {
        AUD_LOG_E("[Mixer] stop re-entry\n", 0);
        return;
    }
    ...
    audio_mixer_role_t              role)
{
    ...
    AUD_LOG_I("[Mixer] Get_id: s %d, c %d, r %d \n", 3, sampling_rate_enum, channel_number_enum, role);

    // Check role
    if (role >= AUDIO_MIXER_TRACK_ROLE_UNKNOW) {
        AUD_LOG_E("[Mixer] Get_id: Unknown track role %d\n", 1, role);
        return AUDIO_MIXER_TRACK_ID_ERROR;
    }

    // Check whether track is available or not
    if (Audio_Mixer.track_flag & (1 << role)) {
        AUD_LOG_E("[Mixer] Get_id: track %d is occupied\n", 1, role);
        return AUDIO_MIXER_TRACK_ID_ERROR;
    }
    ...
    if ((sampling_rate == 0) || (channel_number == 0)) {
        AUD_LOG_E("[Mixer] Get_id: Unknown format s=%d, c=%d\n", 2, sampling_rate_enum, channel_number_enum);
        return AUDIO_MIXER_TRACK_ID_ERROR;
    }
    ...
    if (p_track->source_buffer.base_address_alloc == NULL) {
        AUD_LOG_E("[Mixer] alloc fail: source_buffer\n", 0);
        Audio_Mixer.track_flag &= ~(1 << role);
        return AUDIO_MIXER_TRACK_ID_ERROR;
    }
    ...
    AUD_LOG_I("[Mixer] Get_id success %d \n", 1, role);
    ...
    audio_mixer_track_id_t          track_id)
{
    ...
    AUD_LOG_I("[Mixer] Start_track: %d \n", 1, track_id);

    // Check ID
    if ((track_id < 0) || (!(Audio_Mixer.track_flag & (1 << track_id)))) {
        AUD_LOG_E("[Mixer] start: track %d isn't valid\n", 1, track_id);
        return AUDIO_MIXER_STATUS_ERROR;
    }
    if (Audio_Mixer.track[track_id].state != AUDIO_MIXER_STATE_MACHINE_CONFIGURED) {
        AUD_LOG_E("[Mixer] start: track %d wrong state\n", 1, track_id);
        return AUDIO_MIXER_STATUS_ERROR;
    }
    ...
    if (total_active_track == 0) {
        ...
        if (Audio_Mixer.track[track_id].role == AUDIO_MIXER_TRACK_ROLE_MAIN) {
            AUD_LOG_E("[Mixer] start: concurrency forbidden\n", 0);
            return AUDIO_MIXER_STATUS_ERROR;
        }
#endif
    ...
    audio_mixer_track_id_t          track_id)
{
    ...
    AUD_LOG_I("[Mixer] Stop_track: %d \n", 1, track_id);

    // Check ID
    if ((track_id < 0) || (!(Audio_Mixer.track_flag & (1 << track_id)))) {
        AUD_LOG_E("[Mixer] stop: track %d isn't valid\n", 1, track_id);
        return AUDIO_MIXER_STATUS_ERROR;
    }
    if (Audio_Mixer.track[track_id].state != AUDIO_MIXER_STATE_MACHINE_RUNNING) {
        AUD_LOG_E("[Mixer] stop: track %d wrong state\n", 1, track_id);
        return AUDIO_MIXER_STATUS_ERROR;
    }
    ...
    uint32_t                        *free_space_count)
{
    *free_space_count = 0;
    ...
    // Check ID
    if ((track_id < 0) || (!(Audio_Mixer.track_flag & (1 << track_id)))) {
        AUD_LOG_E("[Mixer] query: track %d isn't valid\n", 1, track_id);
        return AUDIO_MIXER_STATUS_ERROR;
    }
...
//    log_hal_info("[Mixer] track %d, size %d\n", track_id, size);
    ...
    if (size > free_count) {
        AUD_LOG_E("[Mixer] write error. track %d, free %d, write %d\n", 3, track_id, free_count, size);
        return AUDIO_MIXER_STATUS_ERROR;
    }
...
)
{
    if (Audio_Mixer.track[track_id].eof_state == AUDIO_MIXER_EOF_STATE_NONE) {
        ...
        AUD_LOG_I("[Mixer] set EOF. Remaining %d \n", 1, Audio_Mixer.track[track_id].eof_remaining_size);
    }
}
