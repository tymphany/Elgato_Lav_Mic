...
audio_transmitter_status_t audio_transmitter_playback_open(uint16_t scenario_and_id)
{
    ...
    if (clock_setting_type != AUDIO_SCENARIO_TYPE_COMMON) {
        ...
        TRANSMITTER_LOG_E("transmitter playback open scenario_and_id=0x%x did not find running flag\r\n", 1, scenario_and_id);
        AUDIO_ASSERT(0);
    }
...
audio_transmitter_status_t audio_transmitter_playback_stop(uint16_t scenario_and_id)
{
    ...
    uint16_t scenario_sub_id = scenario_and_id & 0xFF;
    if(scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_LINE_IN){
        dchs_cosys_ctrl_cmd_relay(AUDIO_UART_COSYS_DL_STOP, AUDIO_SCENARIO_TYPE_WIRED_AUDIO_LINE_IN , NULL, NULL);
    }else if(scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_IN_0){
        dchs_cosys_ctrl_cmd_relay(AUDIO_UART_COSYS_DL_STOP, AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_0 , NULL, NULL);
    }   
#endif
...
audio_transmitter_status_t audio_transmitter_playback_close(uint16_t scenario_and_id)
{
    ...
    if (clock_setting_type != AUDIO_SCENARIO_TYPE_COMMON) {
        ...
        TRANSMITTER_LOG_E("transmitter playback close scenario_and_id=0x%x did not find running flag\r\n", 1, scenario_and_id);
        AUDIO_ASSERT(0);
    }
