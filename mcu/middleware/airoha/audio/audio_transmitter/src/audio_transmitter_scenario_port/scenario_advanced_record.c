    ...
    switch (config->scenario_sub_id) {
        case AUDIO_TRANSMITTER_ADVANCED_RECORD_N_MIC:
        {
            ...
            if(n_mic_config->input_codec_type == AUDIO_DSP_CODEC_TYPE_PCM) {
                ...
                open_param->stream_out_param.data_ul.max_payload_size = open_param->stream_in_param.afe.frame_size * n_mic_config->output_codec_param.pcm.channel_mode * 4; //MAX format
