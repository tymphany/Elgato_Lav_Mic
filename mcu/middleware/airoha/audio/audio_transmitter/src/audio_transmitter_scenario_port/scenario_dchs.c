...
static dchs_dl_status_t dchs_dl_status = {false, AUDIO_SCENARIO_TYPE_COMMON};
...
static uint32_t dchs_dl_agent_gain[DL_AGENT_MAX] = {NO_GAIN};
...
void mcu_uart_rx(mux_handle_t handle,U8 * user_rx_buf,U32 buf_size)
{
    if(!audio_cmd_uart_handle){
        TRANSMITTER_LOG_I("[DCHS DL][MUX UART][OPEN] audio cmd uart don't opened!",0);
...
void dchs_mcu2dsp_msg_sync(dchs_mcu2dsp_sync_type_t sync_type, dchs_dl_chip_role_t chip_role, void *msg)
{
    ...
    hal_audio_dsp_controller_send_message(MSG_MCU2DSP_DCHS_COSYS_SYNC, 0, (uint32_t)&dchs_cosys_sync, sync_type == DCHS_DL_SET_GAIN_VALUE ? false : true);
}

void dchs_dl_mcu2dsp_send_volume(audio_scenario_type_t scenario_type)
{
    //sync volume to dsp
    if(dchs_get_device_mode() == DCHS_MODE_LEFT){
        if(dchs_dl_agent_gain[DL_AGENT_1] != NO_GAIN){
            ...
            dl_gain_cosys_ctrl_param.vol_gain  = dchs_dl_agent_gain[DL_AGENT_1];
            ...
            TRANSMITTER_LOG_I("[DCHS DL]scenario_type=%d, uart tx gain:0x%x, operation=%d", 3, scenario_type, dl_gain_cosys_ctrl_param.vol_gain, dl_gain_cosys_ctrl_param.operation);
        }
    }else if(dchs_get_device_mode() == DCHS_MODE_RIGHT){
        dual_chip_dl_sync_msg_t dl_mcu2dsp_sync_msg;
        if(dchs_dl_agent_gain[DL_AGENT_2] != NO_GAIN || dchs_dl_agent_gain[DL_AGENT_1] != NO_GAIN || dchs_dl_agent_gain[DL_AGENT_3] != NO_GAIN)
        {
            if(dchs_dl_agent_gain[DL_AGENT_2] != NO_GAIN){
                dl_mcu2dsp_sync_msg.operation  = DCHS_DL_CONFIG_OP_SET_LOCAL_SCENARIO_2_VOL_INFO;
                dl_mcu2dsp_sync_msg.vol_gain   = dchs_dl_agent_gain[DL_AGENT_2];
                dchs_mcu2dsp_msg_sync(DCHS_DL_SET_GAIN_VALUE, LOCAL_CHIP, &dl_mcu2dsp_sync_msg);//send ccni sync msg to dsp
                dchs_dl_agent_gain[DL_AGENT_2] = NO_GAIN;
                TRANSMITTER_LOG_I("[DCHS DL]scenario_type=%d, send gain:0x%x, operation=%d", 3, scenario_type, dl_mcu2dsp_sync_msg.vol_gain, dl_mcu2dsp_sync_msg.operation);
                return;
            }
            if(dchs_dl_agent_gain[DL_AGENT_1] != NO_GAIN){
                dl_mcu2dsp_sync_msg.operation  = DCHS_DL_CONFIG_OP_SET_LOCAL_SCENARIO_1_VOL_INFO;
                dl_mcu2dsp_sync_msg.vol_gain   = dchs_dl_agent_gain[DL_AGENT_1];
                dchs_dl_agent_gain[DL_AGENT_1] = NO_GAIN;
            }else if(dchs_dl_agent_gain[DL_AGENT_3] != NO_GAIN){
                dl_mcu2dsp_sync_msg.operation  = DCHS_DL_CONFIG_OP_SET_LOCAL_SCENARIO_1_VOL_INFO;
                dl_mcu2dsp_sync_msg.vol_gain   = dchs_dl_agent_gain[DL_AGENT_3];
                dchs_dl_agent_gain[DL_AGENT_3] = NO_GAIN;
            }
            dchs_mcu2dsp_msg_sync(DCHS_DL_SET_GAIN_VALUE, LOCAL_CHIP, &dl_mcu2dsp_sync_msg);//send ccni sync msg to dsp
            TRANSMITTER_LOG_I("[DCHS DL]scenario_type=%d, send gain:0x%x, operation=%d", 3, scenario_type, dl_mcu2dsp_sync_msg.vol_gain, dl_mcu2dsp_sync_msg.operation);
        }   
                ...
                while (query_uart_buf_data_size())
                {
                    if (cur_uart_cmd_status.cmd_status == READY_READ){
                        ...
                        if(uart_data_size >= sizeof(audio_uart_cmd_header_t)){
                            ...
                            mcu_uart_rx(audio_cmd_uart_handle, (U8 *)&cmd_header, sizeof(audio_uart_cmd_header_t));
                    ...
                    if (cur_uart_cmd_status.cmd_status == WAITING_READ){
                        ...
                        if(uart_data_size >= cur_uart_cmd_status.param_size){
                            ...
                            if(cur_uart_cmd_status.ctrl_type == AUDIO_UART_COSYS_DL_OPEN){
                                ...
                                TRANSMITTER_LOG_I("[DCHS DL][rx cmd] chip role=%d,scenario_type=%d,context type=0x%x, ctrl type = DL_OPEN,", 3 , feature_param.feature_param.cosys_ctrl_dl.ctrl_param.dchs_dl_open_param.chip_role, dl_open_ctrl_param.scenario_type,dl_open_ctrl_param.context_type);
                            ...
                            }else if(cur_uart_cmd_status.ctrl_type == AUDIO_UART_COSYS_DL_STOP){
                                ...
                                TRANSMITTER_LOG_I("[DCHS DL][rx cmd] scenario_type=%d,context type=0x%x, ctrl type = DL_STOP,other chip scneario exist:0x%x", 3 , dl_close_ctrl_param.scenario_type, ble_context_type[OTHER_CHIP],dchs_dl_check_scenario_exist_flag(OTHER_CHIP));
                                feature_param.type_mask = AM_UART_COSYS_CONTROL_DL;
                                ...
                                feature_param.feature_param.cosys_ctrl_dl.ctrl_type = AUDIO_UART_COSYS_DL_STOP;
                            }else if(cur_uart_cmd_status.ctrl_type == AUDIO_UART_COSYS_DL_SET_GAIN){
                                audio_dchs_dl_gain_param_t dl_gain_ctrl_param;
                                mcu_uart_rx(audio_cmd_uart_handle, (uint8_t*)&dl_gain_ctrl_param + header_size, sizeof(dl_gain_ctrl_param) - header_size);
                                dual_chip_dl_sync_msg_t dl_mcu2dsp_sync_msg;
                                dl_mcu2dsp_sync_msg.operation  = dl_gain_ctrl_param.operation;
                                dl_mcu2dsp_sync_msg.vol_gain   = dl_gain_ctrl_param.vol_gain;
                                TRANSMITTER_LOG_I("[DCHS DL][rx cmd] get gain:0x%x, operation=%d", 2, dl_mcu2dsp_sync_msg.vol_gain, dl_mcu2dsp_sync_msg.operation);
                                dchs_mcu2dsp_msg_sync(DCHS_DL_SET_GAIN_VALUE, OTHER_CHIP, &dl_mcu2dsp_sync_msg);//send ccni sync msg to dsp
                                ...
                                mcu_uart_rx(audio_cmd_uart_handle, (uint8_t*)&ul_open_ctrl_param + header_size, sizeof(audio_dchs_ul_open_param_t) - header_size);
                                ...
                                mcu_uart_rx(audio_cmd_uart_handle, (uint8_t*)&ul_start_ctrl_param + header_size, sizeof(audio_dchs_ul_start_param_t) - header_size);
                                ...
                                mcu_uart_rx(audio_cmd_uart_handle, (uint8_t*)&ul_close_ctrl_param + header_size, sizeof(audio_dchs_ul_close_param_t) - header_size);
                                ...
                                mcu_uart_rx(audio_cmd_uart_handle, (uint8_t*)&g_ul_volume_ctrl_param + header_size, sizeof(audio_dchs_ul_volume_param_t) - header_size);
                                ...
                                mcu_uart_rx(audio_cmd_uart_handle, (uint8_t*)&calibration_param + header_size, sizeof(audio_dchs_relay_cmd_param_t) - header_size);
                                ...
                                mcu_uart_rx(audio_cmd_uart_handle, (uint8_t*)&anc_ctrl_param + header_size, sizeof(audio_dchs_anc_param_t) - header_size);
...
void audio_transmitter_dchs_open_playback(audio_transmitter_config_t *config, mcu2dsp_open_param_t *open_param)
{
    if(config->scenario_sub_id == AUDIO_TRANSMITTER_DCHS_UART_DL){
        ...
        open_param->stream_in_param.afe.frame_size            = AUDIO_DURATION_TIME * DCHS_DL_FIX_SAMPLE_RATE / MUX_UART_BUF_SLICE / 1000;
        ...
        open_param->stream_in_param.afe.sampling_rate         = DCHS_DL_FIX_SAMPLE_RATE;
        ...
        open_param->stream_out_param.afe.sampling_rate   = DCHS_DL_FIX_SAMPLE_RATE;//48k or 96k
        open_param->stream_out_param.afe.irq_period      = 0;//config->scenario_config.dchs_config.irq_period;
        open_param->stream_out_param.afe.frame_size      = AUDIO_DURATION_TIME * DCHS_DL_FIX_SAMPLE_RATE / MUX_UART_BUF_SLICE / 1000;//open_param->stream_in_param.afe.frame_size;
        ...
        if(config->scenario_config.dchs_config.scenario_type == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_0){
...
void dchs_dl_get_stream_out_volume(hal_audio_hw_stream_out_index_t hw_gain_index, uint32_t digital_volume_index)
{
    ...
    if(hw_gain_index == HAL_AUDIO_STREAM_OUT1){
        dchs_dl_agent_gain[DL_AGENT_1]  = digital_volume_index;
        if(dchs_get_device_mode() == DCHS_MODE_LEFT){
            dl_gain_cosys_ctrl_param.operation = DCHS_DL_CONFIG_OP_SET_UART_SCENARIO_VOL_INFO;
        }else if(dchs_get_device_mode() == DCHS_MODE_RIGHT){
            dl_mcu2dsp_sync_msg.operation = DCHS_DL_CONFIG_OP_SET_LOCAL_SCENARIO_1_VOL_INFO;
        ...
        dchs_dl_agent_gain[DL_AGENT_2]  = digital_volume_index;
        ...
        dchs_dl_agent_gain[DL_AGENT_3]  = digital_volume_index;
        if(dchs_get_device_mode() == DCHS_MODE_LEFT){
            dl_gain_cosys_ctrl_param.operation = DCHS_DL_CONFIG_OP_SET_UART_SCENARIO_VOL_INFO;
        }else if(dchs_get_device_mode() == DCHS_MODE_RIGHT){
            dl_mcu2dsp_sync_msg.operation = DCHS_DL_CONFIG_OP_SET_LOCAL_SCENARIO_1_VOL_INFO;
    ...
    if(dchs_get_device_mode() == DCHS_MODE_LEFT){
        ...
        //relay dchs dl open param to the other chip
        mcu_uart_tx(UART_AUDIO_CMD, (U8 *)&dl_gain_cosys_ctrl_param, sizeof(audio_dchs_dl_gain_param_t));
        TRANSMITTER_LOG_I("[DCHS DL][cmd tx] got and relay gain:0x%x, operation=%d", 3, dl_gain_cosys_ctrl_param.vol_gain, dl_gain_cosys_ctrl_param.operation);
    }else if(dchs_get_device_mode() == DCHS_MODE_RIGHT){
...
void mcu_uart_tx(uart_type_t type, uint8_t *param_buf, uint32_t buf_size)
{
    if (type == UART_AUDIO_CMD) {
        ...
        TRANSMITTER_LOG_I("[DCHS DL][MUX UART][Tx] audio cmd uart tx success! data_size=%d",1,tx_size);
    ...
    if(ctrl){
        if(chip_role == LOCAL_CHIP){
            ...
            if(dchs_get_device_mode() == DCHS_MODE_RIGHT){
                ...
                dchs_mcu2dsp_msg_sync(DCHS_DL_LOCAL_CHIP_DL_EXIST, LOCAL_CHIP,  &dchs_dl_sync_msg[LOCAL_CHIP]);
            }else if(dchs_get_device_mode() == DCHS_MODE_LEFT){
                dchs_dl_sync_msg[LOCAL_CHIP].other_chip_dl_exist = true;
                dchs_mcu2dsp_msg_sync(DCHS_DL_OTHER_CHIP_DL_EXIST, LOCAL_CHIP,  &dchs_dl_sync_msg[LOCAL_CHIP]);
            }
        }else{
            dchs_dl_sync_msg[OTHER_CHIP].other_chip_dl_exist = true;
            dchs_dl_sync_msg[OTHER_CHIP].chip_role = chip_role;
            dchs_mcu2dsp_msg_sync(DCHS_DL_OTHER_CHIP_DL_EXIST,  LOCAL_CHIP, &dchs_dl_sync_msg[OTHER_CHIP]);//send ccni sync msg to dsp
        }
    }else{
        dual_chip_dl_sync_msg_t dl_mcu2dsp_sync_msg;
        ...
        if(chip_role == LOCAL_CHIP){
            if(dchs_get_device_mode() == DCHS_MODE_RIGHT){
                ...
                dchs_mcu2dsp_msg_sync(DCHS_DL_LOCAL_CHIP_DL_EXIST, LOCAL_CHIP, &dl_mcu2dsp_sync_msg);
                ...
                dchs_mcu2dsp_msg_sync(DCHS_DL_OTHER_CHIP_DL_EXIST, LOCAL_CHIP, &dl_mcu2dsp_sync_msg);
            }
        }else{
            dl_mcu2dsp_sync_msg.other_chip_dl_exist = false;
            if(dchs_get_device_mode() == DCHS_MODE_RIGHT){
                dchs_mcu2dsp_msg_sync(DCHS_DL_OTHER_CHIP_DL_EXIST, LOCAL_CHIP,  &dl_mcu2dsp_sync_msg);//send ccni sync msg to dsp
...
{
    TRANSMITTER_LOG_I("[DCHS DL][MCU CMd Execute]:ctrl_type:%d,scenario_type:%d", 2, cosys_ctrl->ctrl_type,cosys_ctrl->ctrl_param.dchs_dl_open_param.scenario_type);
    if(cosys_ctrl->ctrl_type == AUDIO_UART_COSYS_DL_OPEN){
        ...
        if(dchs_get_device_mode() == DCHS_MODE_LEFT){  // master latch slave clock
            if(!latch_timer_start_flag){
                ...
                TRANSMITTER_LOG_I("[DCHS DL] mcu dl latch timer started",0);
            }
        }
        dual_chip_dl_sync_msg_t dl_mcu2dsp_sync_msg;
        dl_mcu2dsp_sync_msg.sample_rate  = transmitter_config_dl.scenario_config.dchs_config.sampling_rate;
        dl_mcu2dsp_sync_msg.format_bytes = transmitter_config_dl.scenario_config.dchs_config.format >= AFE_PCM_FORMAT_S24_LE ? 4 : 2;
        dl_mcu2dsp_sync_msg.data_scenario_type  = scenario_type;
        ...
        if(dchs_dl_check_scenario_exist_flag(LOCAL_CHIP) || dchs_dl_check_scenario_exist_flag(OTHER_CHIP)){
            ...
            TRANSMITTER_LOG_I("[DCHS DL] scenario type:%d,context type:0x%x is running, ignore open scenario_type:%d,context type=0x%x", 3, dchs_dl_status.cur_running_scenario_type, dchs_dl_status.cur_ble_context_type, scenario_type,cosys_ctrl->ctrl_param.dchs_dl_open_param.context_type);
            return;
        }
        dchs_dl_set_scenario_exist_flag(chip_role, scenario_type, true);
        ...
        //send volume
        dchs_dl_mcu2dsp_send_volume(scenario_type);
        ...
        if(set_play_en){
            ...
            if(chip_role == LOCAL_CHIP && (scenario_type == AUDIO_SCENARIO_TYPE_VP || scenario_type == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_LINE_IN
                ...
                dual_chip_dl_sync_msg_t dl_mcu2dsp_sync_msg;
                dl_mcu2dsp_sync_msg.data_scenario_type = scenario_type;
                dchs_mcu2dsp_msg_sync(DCHS_DL_SET_PLAY_EN, chip_role, &dl_mcu2dsp_sync_msg);
            }
        }else{
            TRANSMITTER_LOG_I("[DCHS DL] scenario type:%d,context type:0x%x is running, ignore start scenario_type:%d,context type=0x%x", 4, dchs_dl_status.cur_running_scenario_type, dchs_dl_status.cur_ble_context_type, scenario_type,cosys_ctrl->ctrl_param.dchs_dl_start_param.context_type);
        }
    }else if(cosys_ctrl->ctrl_type == AUDIO_UART_COSYS_DL_STOP){
        ...
        if(dchs_dl_check_scenario_exist_flag(LOCAL_CHIP) || dchs_dl_check_scenario_exist_flag(OTHER_CHIP)){
            ...
            TRANSMITTER_LOG_W("[DCHS DL] the scenario:%d,context type:0x%x exist, ignore stop,scenario_type=%d, context type=0x%x,other:%d,local=%d", 6, dchs_dl_status.cur_running_scenario_type,dchs_dl_status.cur_ble_context_type, scenario_type,cosys_ctrl->ctrl_param.dchs_dl_close_param.context_type, dchs_dl_check_scenario_exist_flag(OTHER_CHIP),dchs_dl_check_scenario_exist_flag(LOCAL_CHIP));
            return;
        }
        if(dchs_get_device_mode() == DCHS_MODE_LEFT){
            if(latch_timer_start_flag){
                ...
                TRANSMITTER_LOG_I("[DCHS DL] mcu dl latch timer stoped",0);
            }
        }
        //stop
        uint16_t scenario_and_id = ((AUDIO_TRANSMITTER_DCHS) << 8) + AUDIO_TRANSMITTER_DCHS_UART_DL;
...
void dchs_cosys_ctrl_cmd_relay(uart_cmd_type_t ctrl_type, audio_scenario_type_t scenario_type, mcu2dsp_open_param_t *open_param, mcu2dsp_start_param_t * start_param)
{
    ...
    }else if(ctrl_type == AUDIO_UART_COSYS_DL_STOP){
        if (scenario_type == AUDIO_SCENARIO_TYPE_HFP_DL || scenario_type == AUDIO_SCENARIO_TYPE_A2DP || scenario_type == AUDIO_SCENARIO_TYPE_BLE_DL || scenario_type == AUDIO_SCENARIO_TYPE_VP
            || scenario_type == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_LINE_IN || scenario_type == AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_0){ // [hard code]
