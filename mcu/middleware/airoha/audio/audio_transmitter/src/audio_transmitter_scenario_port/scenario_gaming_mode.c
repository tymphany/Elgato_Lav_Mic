...
static dvfs_frequency_t g_audio_transmitter_dvfs_clk = HAL_DVFS_OPP_LOW;
#else
...
void gaming_mode_open_playback(audio_transmitter_config_t *config, mcu2dsp_open_param_t *open_param)
{
    ...
    configASSERT(0);
        ...
        open_param->stream_in_param.afe.format              = AFE_PCM_FORMAT_S32_LE; //config->scenario_config.gaming_mode_config.i2s_in_dongle_config.codec_param.pcm.format; // AFE_PCM_FORMAT_S16_LE;
        ...
        //hal_audio_dsp_hwsrc_clkmux_control(96000, true);
            ...
            open_param->param.stream_out = STREAM_OUT_AFE;
            open_param->stream_out_param.afe.memory          = HAL_AUDIO_MEM1;
            open_param->stream_out_param.afe.format          = AFE_PCM_FORMAT_S32_LE; //config->scenario_config.gaming_mode_config.i2s_in_dongle_config.codec_param.pcm.format;
...
audio_transmitter_status_t gaming_mode_set_runtime_config_playback(audio_transmitter_config_t *config, audio_transmitter_runtime_config_type_t runtime_config_type, audio_transmitter_runtime_config_t *runtime_config, mcu2dsp_audio_transmitter_runtime_config_param_t *runtime_config_param)
{
    ...
    else if (config->scenario_sub_id == AUDIO_TRANSMITTER_GAMING_MODE_MUSIC_DONGLE_LINE_IN)
    {
        ...
        || (operation == GAMING_MODE_CONFIG_OP_VOL_LEVEL_MUSIC_DUL)){
            ...
            uint8_t volume_ratio;
            volume_ratio = runtime_config->gaming_mode_runtime_config.vol_level.vol_ratio;
            TRANSMITTER_LOG_I("[ULL][Line In] default d_gain %d", 1, g_ull_line_in_default_d_gain);
            if(volume_ratio > 100) {
                TRANSMITTER_LOG_E("Volume ratio should between 0 and 100, volume_ratio = \r\n", 1,volume_ratio);
                volume_ratio = 100;
            }
            ...
            if((operation == GAMING_MODE_CONFIG_OP_VOL_LEVEL_MUSIC_L)|| (operation == GAMING_MODE_CONFIG_OP_VOL_LEVEL_MUSIC_DUL)){
                ...
                gain.vol_gain_1  = audio_get_gain_out_in_dB(volume_level, GAIN_DIGITAL, VOL_LINE_IN);
                gain.vol_gain_1 += gain_compensation_table[volume_ratio/GAIN_COMPENSATION_STEP];
            ...
            if((operation == GAMING_MODE_CONFIG_OP_VOL_LEVEL_MUSIC_R)|| (operation == GAMING_MODE_CONFIG_OP_VOL_LEVEL_MUSIC_DUL)){
                ...
                gain.vol_gain_2  = audio_get_gain_out_in_dB(volume_level, GAIN_DIGITAL, VOL_LINE_IN);
                gain.vol_gain_2 += gain_compensation_table[volume_ratio/GAIN_COMPENSATION_STEP];
            ...
            TRANSMITTER_LOG_E("operation %d can not do in AUDIO_TRANSMITTER_GAMING_MODE_MUSIC_DONGLE_LINE_IN. ",1,operation);
            configASSERT(0);
        }
    }
#endif
        ...
        || (operation == GAMING_MODE_CONFIG_OP_VOL_LEVEL_VOICE_DUL)){
            ...
            TRANSMITTER_LOG_E("operation %d can not do in AUDIO_TRANSMITTER_GAMING_MODE_VOICE_DONGLE_LINE_OUT. ",1,operation);
            configASSERT(0);
        }
    }
    #endif /* AIR_GAMING_MODE_DONGLE_LINE_OUT_ENABLE */
    ...
    else if (config->scenario_sub_id == AUDIO_TRANSMITTER_GAMING_MODE_MUSIC_DONGLE_I2S_IN)
    {
        ...
        || (operation == GAMING_MODE_CONFIG_OP_VOL_LEVEL_MUSIC_DUL)){
            ...
            TRANSMITTER_LOG_E("operation %d can not do in AUDIO_TRANSMITTER_GAMING_MODE_MUSIC_DONGLE_I2S_IN. ",1,operation);
            configASSERT(0);
        }
    }
#endif /* AIR_GAMING_MODE_DONGLE_I2S_IN_ENABLE */
...
#if defined(AIR_3RD_PARTY_NR_ENABLE)
        #ifdef AIR_BTA_IC_PREMIUM_G3
        if (g_audio_transmitter_dvfs_clk < HAL_DVFS_OPP_HIGH) {
            g_audio_transmitter_dvfs_clk = HAL_DVFS_OPP_HIGH;
        }
        #else
        if (g_audio_transmitter_dvfs_clk < HAL_DVFS_HALF_SPEED_208M) {
            g_audio_transmitter_dvfs_clk = HAL_DVFS_HALF_SPEED_208M;
        }
        #endif
...
audio_transmitter_status_t gaming_mode_stop_handler(uint16_t scenario_sub_id)
{
    if (scenario_sub_id == AUDIO_TRANSMITTER_GAMING_MODE_VOICE_HEADSET) {
        #ifdef AIR_BTA_IC_PREMIUM_G3
        if (g_audio_transmitter_dvfs_clk != HAL_DVFS_OPP_LOW) {
        #else
