...
#define ULL_BLE_DL_DVFS_SPEED HAL_DVFS_OPP_MID
    ...
    switch (status) {
        ...
        case AUDIO_NVDM_STATUS_POST_CHANGE:
            if (g_ble_feature_mode_1st_notify == false) {
                ...
                BLE_LOG_MSGID_I("ble feature mode: g_xSemaphore_ble_nvkey_change_notify give",0);
            } else {
...
bool ble_set_feature_mode(uint32_t mode)
{
    ...
    if (mode >= BLE_FEATURE_MODE_MAX) {
        BLE_LOG_MSGID_I("ble feature mode: invalid mode: %d", 1, mode);
        return false;
    }
    if (mode == g_ble_feature_mode) {
        BLE_LOG_MSGID_I("ble feature mode: no mode cahnge: %d", 1, mode);
        return false;
    }

    /* Bypass case 2: Check whether the BLE start up has been completed. */
    if (g_ble_feature_mode_1st_notify == true) {
        BLE_LOG_MSGID_I("ble feature mode: busy with start up, mode %d", 1, mode);
        return false;
    }

    /* Bypass case 4: Check whether previous user is busy on setting the mode */
    hal_nvic_save_and_set_interrupt_mask(&irq_status);
    if (g_ble_feature_mode_busy_flg == true) {
        hal_nvic_restore_interrupt_mask(irq_status);
        BLE_LOG_MSGID_I("ble feature mode: busy with previous mode change, mode %d", 1, g_ble_feature_mode);
        return false;
    }
    ...
    BLE_LOG_MSGID_I("ble feature mode: set mode to %d", 1, g_ble_feature_mode);
    ...
    if (am_status != AUD_EXECUTION_SUCCESS) {
        BLE_LOG_MSGID_I("ble feature mode: resume fail", 0);
        g_ble_feature_mode_busy_flg = false;
        return false;
    }
    ...
    g_ble_feature_mode_busy_flg = false;
    BLE_LOG_MSGID_I("ble feature mode: set mode done", 0);
...
#endif
        BLE_LOG_MSGID_I("ble feature mode: PSAP %d", 1, ble_get_feature_mode());
    }
#endif
...
#endif
        BLE_LOG_MSGID_I("ble feature mode: boom mic %d", 1, mic_cur_type);
    }
#endif
}
...
static bt_codec_media_status_t bt_ble_stop(bt_media_handle_t *handle)
{
    ...
    if (internal_handle->codec_info.channel_mode != CHANNEL_MODE_UL_ONLY) {
        ...
        #ifdef AIR_DCHS_MODE_ENABLE
        dchs_cosys_ctrl_cmd_relay(AUDIO_UART_COSYS_DL_STOP, AUDIO_SCENARIO_TYPE_BLE_DL , NULL, NULL);
        #endif

        #if defined(HAL_DVFS_MODULE_ENABLED)
#if defined(AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
        if (internal_handle->codec_info.dl_param.codec == BT_CODEC_TYPE_LE_AUDIO_LC3PULS || internal_handle->codec_info.dl_param.codec == BT_CODEC_TYPE_LE_AUDIO_VENDOR) {
    ...
    if (internal_handle->codec_info.channel_mode != CHANNEL_MODE_DL_ONLY) {
        ...
        #if defined(HAL_DVFS_MODULE_ENABLED)
        #if defined(AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
        if (internal_handle->codec_info.ul_param.codec == BT_CODEC_TYPE_LE_AUDIO_LC3PULS || internal_handle->codec_info.dl_param.codec == BT_CODEC_TYPE_LE_AUDIO_VENDOR) {
...
static bt_codec_media_status_t bt_ble_dl_internal_open(const bt_codec_le_audio_dl_t *param)
{
    ...
    #if defined(AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
    if (codec_type == BT_CODEC_TYPE_LE_AUDIO_LC3PULS || codec_type == BT_CODEC_TYPE_LE_AUDIO_VENDOR) {
...
static bt_codec_media_status_t bt_ble_ul_internal_open(const bt_codec_le_audio_ul_t *param)
{
    ...
    {
        open_param.stream_in_param.afe.format          = AFE_PCM_FORMAT_S32_LE; //for wireless mic 24bit, put 0 to low 8bit
...
void bt_ull_le_set_audio_manager_priority(audio_src_srv_priority_t priority)
{
    ...
    if ((device) && (device->type == AUDIO_SRC_SRV_PSEUDO_DEVICE_ULL_BLE)) {
        ...
        if (device->priority > priority) {
            ...
            if (waiting_psedev && (waiting_psedev->priority > priority)) {
