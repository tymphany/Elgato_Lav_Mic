...
mp3_codec_media_handle_t *mp3_codec_open(mp3_codec_callback_t mp3_codec_callback)
{
    ...
    if (i2s_tx_buffer == NULL) {
        MP3_LOG_E("[MP3 Codec] i2s_tx_buffer cannot allocate memory\r\n", 0);
        return NULL;
    } else {
        MP3_LOG_I("[MP3 Codec] i2s_tx_buffer = 0x%X\r\n", 1, i2s_tx_buffer);
    }
    mp3_decode_buffer = pvPortMalloc(sizeof(uint8_t) * MP3_DECODE_BUFFER_LENGTH);
    if (mp3_decode_buffer == NULL) {
        MP3_LOG_E("[MP3 Codec] mp3_decode_buffer cannot allocate memory\r\n", 0);
        return NULL;
    } else {
        MP3_LOG_I("[MP3 Codec] mp3_decode_buffer = 0x%X\r\n", 1, mp3_decode_buffer);
    }
