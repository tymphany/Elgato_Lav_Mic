...
#define I2S_TX_BUFFER_LENGTH 4608
#define __STATIC_i2s_tx_buffer__
#if defined(__STATIC_i2s_tx_buffer__)
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN static uint32_t i2s_tx_buffer[I2S_TX_BUFFER_LENGTH];
#else
...
#if 0
    MP3_LOG_I("[MP3 Codec ]mp3_codec_share_buffer_write_data_done:: handle->share_buff.write=%d\r\n", 1, handle->share_buff.write);
#endif
}
...
static mp3_codec_function_return_state_t mp3_codec_request_data_to_share_buffer(mp3_codec_media_handle_t *handle)
{
    ...
    if (!handle->waiting) {
        ...
        MP3_LOG_I("[MP3 Codec] mp3_codec_request_data_to_share_buffer: already request and waiting feed back\r\n", 0);
...
static void mp3_codec_event_send_from_isr(mp3_codec_queue_event_id_t id, void *parameter)
{
    ...
    if (xQueueSendFromISR(mp3_codec_queue_handle, &event, 0) != pdPASS) {
        MP3_LOG_I("[MP3 Codec]queue not pass %d\r\n", 1, id);
        return;
    }
...
static void mp3_codec_i2s_tx_isr_callback(hal_i2s_event_t event, void *user_data)
{
    ...
    // printf("mp3_codec_i2s_tx_isr_callback() +\r\n");
    hal_i2s_disable_tx_dma_interrupt_ex(i2s_port);
    switch (event) {
        ...
        case HAL_I2S_EVENT_DATA_REQUEST:
            ...
            for (loop_idx = 0; loop_idx < 2; loop_idx++) {
                ...
                hal_i2s_get_tx_sample_count_ex(i2s_port, &i2s_amount_space_tones);
                ...
                if (stream_out_pcm_buffer_data_tone_amount > 0) {
                    ...
                    for (i = 0; i < stream_out_tone_amount; i++) {
                        ...
                        if ((internal_handle->mp3_handle->CHNumber == 1)) { // mono
                            ...
                            i2s_tone_data = i2s_tone_data | ((uint32_t) * (out_buf_ptr)) << 16;
                            out_buf_ptr++;
                        ...
                        hal_i2s_tx_write_ex(i2s_port, i2s_tone_data);
                    }
                    ...
                    MP3_LOG_I("stream_out_pcm_buff no data\r\n", 0);

                    //MP3_LOG_I("7682 mp3_codec_i2s_tx_isr_callback amount %d\r\n", 1,stream_out_pcm_buffer_data_tone_amount);//modify for audio_ref_design
                    // printf("i2s_amount_space_tones = %d\r\n", (int)i2s_amount_space_tones);
                    if (internal_handle->is_set_eof_called == 0) {//internal_handle->is_set_eof_called == 0
                        for (uint32_t i = 0; i < i2s_amount_space_tones; i++) {
                            hal_i2s_tx_write_ex(i2s_port, 0);
                        }
                    }
        ...
        case HAL_AUDIO_EVENT_DATA_REQUEST:
            ...
            for (loop_idx = 0; loop_idx < 2; loop_idx++) {
                ...
                if (stream_out_pcm_buffer_data_count > 0) {
                    ...
                    // stream_out_pcm_buff no data
                    MP3_LOG_I("stream_out_pcm_buff no data\r\n", 0);
                    MP3_LOG_I("7682 mp3_codec_pcm_out_isr_callback\r\n", 0);//modify for audio_ref_design
...
static void mp3_codec_deocde_hisr_handler(void *data)
{
    ...
    if (1 == internal_handle->media_bitstream_end) {
        MP3_LOG_I("[MP3 Codec] handle->handler(handle, MP3_CODEC_MEDIA_BITSTREAM_END);\r\n", 0);
    ...
    if ((1 == handle->flush_data_flag) && (share_data_amount <= MPEG_AUDIO_FRAME_HEADER_LENGTH)) {
        ...
        uint32_t stream_out_pcm_buffer_data_count = 0;
        ring_buffer_get_read_information(&internal_handle->stream_out_pcm_buff, &out_buf_ptr, &stream_out_pcm_buffer_data_count);
        MP3_LOG_I("[MP3 Codec] mp3_codec_deocde_hisr_handler flush_data_flag(%d) share_data_amount %d pcm_buffer_data_count %d\r\n", 2, handle->flush_data_flag, share_data_amount, stream_out_pcm_buffer_data_count);
        //if (0 == stream_out_pcm_buffer_data_count) {
        MP3_LOG_I("[MP3 Codec] hal_i2s_set_eof_ex(%d)\r\n", 1, i2s_port);
    ...
    if (consumeBS < 0) {
        ...
        MP3_LOG_I("[MP3 Codec] never hear\r\n", 0);
    }
...
static mp3_codec_function_return_state_t mp3_codec_play_internal(mp3_codec_media_handle_t *handle)
{
    ...
    //MP3_LOG_I("CHNumber = %d\r\n", 1, internal_handle->mp3_handle->CHNumber);
    MP3_LOG_I("mp3_sampling_rate=%d, bclk=%d\r\n", 2, mp3_sampling_rate, bclk);
// 24.576/22.5792MHz
    switch (mp3_sampling_rate) {
        case eSR48KHz:
        case eSR32KHz:
        case eSR16KHz:
        case eSR8KHz:
        case eSR24KHz:
        case eSR12KHz:
            aucodec_set_dai_sysclk(mp3_sampling_rate, eSLAVE, bclk, eMCLK8KBASE, ePLLEnable);
            break;

        case eSR44K1Hz:
        case eSR22_05KHz:
        case eSR11_025KHz:
            aucodec_set_dai_sysclk(mp3_sampling_rate, eSLAVE, bclk, eMCLK11_025KBASE, ePLLEnable);
            break;
    }
    // aucodec_set_output(eSpkOut);//Input: DACIN, Output:  speaker out
    aucodec_set_output(eLineOut);//Input: DACIN, Output:  aux out
    MP3_LOG_I("[MP3 Codec] hal_i2s_sample_rate = %d\r\n", 1, hal_i2s_sample_rate);
    if (i2s_configure(HAL_I2S_STEREO, hal_i2s_sample_rate) < 0) { // Keep HAL_I2S_STEREO
        MP3_LOG_I("[MP3 Codec] i2s_configure failed\r\n", 0);
    }
    result = hal_i2s_setup_tx_vfifo_ex(i2s_port, i2s_tx_buffer, I2S_TX_BUFFER_LENGTH / 2, I2S_TX_BUFFER_LENGTH);
    if (HAL_I2S_STATUS_OK != result) {
        MP3_LOG_I("[MP3 Codec] hal_i2s_setup_tx_vfifo failed\r\n", 0);
        return MP3_CODEC_RETURN_ERROR;
    }


    hal_i2s_register_tx_vfifo_callback_ex(i2s_port, mp3_codec_i2s_tx_isr_callback, NULL);
...
static mp3_codec_function_return_state_t mp3_codec_pause(mp3_codec_media_handle_t *handle)
{
    ...
    MP3_LOG_I("[MP3 Codec] pause++: stream out pcm data=%d, share buffer data=%d\r\n", 2, (int)ring_buffer_get_data_byte_count(&mp3_codec_internal_handle->stream_out_pcm_buff), (int)mp3_codec_get_share_buffer_data_count(handle));
...
mp3_codec_function_return_state_t mp3_codec_close(mp3_codec_media_handle_t *handle)
{
    ...
    MP3_LOG_I("[MP3 Codec]Close codec\r\n", 0);
...
mp3_codec_media_handle_t *mp3_codec_open(mp3_codec_callback_t mp3_codec_callback)
{
    ...
    if (NULL == internal_handle) {
        MP3_LOG_E("[MP3 Codec] Cannot alloc internal_handle\r\n", 0);
    ...
    if (NULL == mp3_decode_buffer) {
        MP3_LOG_E("[MP3 Codec] Cannot alloc mp3_decode_buffer\r\n", 0);
