...
#define LOGMSGIDE(fmt,arg...)   LOG_MSGID_E(audio_nvdm, "[audio_nvdm]"fmt,##arg)
#define LOGMSGIDW(fmt,arg...)   LOG_MSGID_W(audio_nvdm, "[audio_nvdm]"fmt,##arg)
#define LOGMSGIDI(fmt,arg...)   LOG_MSGID_I(audio_nvdm, "[audio_nvdm]"fmt,##arg)
#endif
...
static const audio_eaps_t audio_eaps = {
...
void audio_nvdm_init()
{
    ...
    LOGMSGIDI("audio nvdm init ++\n", 0);

#ifndef MTK_AUDIO_TUNING_ENABLED
        // if not enable audio tuning tool. we just use the global audio_eaps which create with default value as audio_default_coefficien.h
        // this method can also support fota.
        LOGMSGIDI("not enable audio tuning, just use audio_default_coefficeint.h parameters\n", 0);
        LOGMSGIDI("audio nvdm init --\n", 0);
        UNUSED(size);
        return;
#else
...
static void audio_nvdm_callback_eaps_data_is_changed_callback_function(void)
{
    ...
    for (i = 0; i < MAX_CALLBACK_NUMBER; i++) {
        if (audio_nvdm_eaps_data_is_changed_callback[i].callback_id != NULL) {
            LOGMSGIDI("eaps data is changed. callback the registered callback funtion\n", 0);
...
int audio_nvdm_unregister_eaps_is_changed_callback(audio_nvdm_callback_id *callback_id)
{
    ...
    for (i = 0; i < MAX_CALLBACK_NUMBER; i++) {
        if (audio_nvdm_eaps_data_is_changed_callback[i].callback_id == callback_id) {
            ...
            LOGMSGIDI("unregister success\n", 0);
            return 0;
        }
    }

    LOGMSGIDI("didn't register before\n", 0);
...
int audio_nvdm_register_eaps_is_changed_callback(audio_nvdm_callback_id *callback_id, audio_nvdm_callback_funtion_t func, void *data)
{
    ...
    for (i = 0; i < MAX_CALLBACK_NUMBER; i++) {
        if (audio_nvdm_eaps_data_is_changed_callback[i].callback_id == callback_id) {
            LOGMSGIDI("already register. please unregister first\n", 0);
            return -1;
        }
    }
    ...
    for (i = 0; i < MAX_CALLBACK_NUMBER; i++) {
        if (audio_nvdm_eaps_data_is_changed_callback[i].callback_id == NULL) {
            ...
            LOGMSGIDI("register success i=%d\n",1, i);
            return 0;
        }
    }


    LOGMSGIDI("over max eaps data is changed callback function can be registered\n", 0);
...
int16_t audio_nvdm_save_dc_data_to_nvdm(void)
{
    ...
    LOGMSGIDI("Save HS DC compensation sta=%d, value =%x.\n",2, audio_nvdm_dc_calibration.calibration_sta, audio_nvdm_dc_calibration.hs_dc_compensate_value);
    return ret;
}
#endif
