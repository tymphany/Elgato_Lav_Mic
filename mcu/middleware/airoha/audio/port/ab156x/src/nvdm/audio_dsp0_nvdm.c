...
uint16_t nat_table_nvkey_id_replace(uint16_t nvkey_id)
{
    uint16_t repalce_nvkey_id;

    repalce_nvkey_id = nvkey_id;
#ifdef AIR_HFP_FEATURE_MODE_ENABLE
    //replace_nvdm_id = hfp_restore_feature_mode_nvkey_id(nvkey_id);
#endif
    ...
    if (repalce_nvkey_id != nvkey_id) {
        AUD_LOG_I("nat_table_nvkey_id_replace, from 0x%x to 0x%x", 2, nvkey_id, repalce_nvkey_id);
    }
