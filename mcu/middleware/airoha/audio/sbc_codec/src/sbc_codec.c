 ...
 * Log level control
 */
#define TASK_LOG_MSGID_E(fmt,arg...)      AUD_LOG_E("[SBC_CODEC]: "fmt,##arg)
#define TASK_LOG_MSGID_W(fmt,arg...)      AUD_LOG_W("[SBC_CODEC]: "fmt,##arg)
#define TASK_LOG_MSGID_I(fmt,arg...)      AUD_LOG_I("[SBC_CODEC]: "fmt,##arg)
#define TASK_LOG_MSGID_D(fmt,arg...)      AUD_LOG_D("[SBC_CODEC]: "fmt,##arg)
...
typedef uint8_t (*sbc_codec_internal_callback_t)(sbc_codec_media_handle_t *handle, void *parameter);

static QueueHandle_t sbc_codec_queue_handle = NULL;
static TaskHandle_t sbc_codec_task_handle = NULL;
static SemaphoreHandle_t sbc_codec_semaphore = NULL;
...
static void sbc_codec_dump_to_file(uint8_t *buffer, uint32_t size, const TCHAR *path)
{
    ...
    if (res) {
        TASK_LOG_MSGID_E("fopen error %d \r\n", 1, res);
    }
    res = f_write(&sbc_codec_fdst, buffer, size, &sbc_codec_length_write);
    if (res != FR_OK) {
        TASK_LOG_MSGID_E("f_write fail. %d", 1, res);
    }
    res = f_close(&sbc_codec_fdst);
    if (res) {
        TASK_LOG_MSGID_I("fclose error %d \r\n", 1, res);
    }
}
...
static int32_t sbc_codec_event_send_from_isr(sbc_codec_media_handle_t *handle, void *parameter)
{
    ...
    if (internal_handle->status & SBC_CODEC_STATUS_IN_TASK_LIST || handle->state != SBC_CODEC_STATE_PLAY) {
        if (handle->state != SBC_CODEC_STATE_PLAY) {
            TASK_LOG_MSGID_I("handle->state != SBC_CODEC_STATE_PLAY %d \r\n", 1, handle->state);
        }
        return 0;
    }

    TASK_LOG_MSGID_I("[sbc_codec_event_send_from_isr] codec queue handle: %x", 1, sbc_codec_queue_handle);

    sbc_codec_queue_event_t event;
    event.handle    = handle;
    event.parameter = parameter;
    internal_handle->status |= SBC_CODEC_STATUS_IN_TASK_LIST;
    if (sbc_codec_queue_handle == NULL || xQueueSendFromISR(sbc_codec_queue_handle, &event, 0) != pdPASS) {
        TASK_LOG_MSGID_E("send fail.", 0);
        internal_handle->status &= ~SBC_CODEC_STATUS_IN_TASK_LIST;
        return -1;
    }
...
{
    TASK_LOG_MSGID_I("sbc_codec_task_main,code queue handle: %x", 1, sbc_codec_queue_handle);
    sbc_codec_queue_event_t event;
...
uint32_t sbc_codec_get_buffer_size()
{
    // maximum size
    // sbc_codec_internal_handle_t *internal_handle = (sbc_codec_internal_handle_t *)handle;
    ...
    TASK_LOG_MSGID_I("sbc codec memory : total_size:%d(bytes) encoder_size %d sbc_max_frame_size %d bli_size %d SBC_ENCODE_BUFFER_SIZE %d SBC_FRAME_RING_SIZE %d PCM_DATA_RING_SIZE %d", 7,
...
static void sbc_codec_set_buffer(sbc_codec_media_handle_t *handle, uint8_t  *buffer, uint32_t length)
{
    ...
    if (internal_handle->total_size > length) {
        TASK_LOG_MSGID_E("buffer size too small.", 0);
    }
    /* Buffer assignment. */
...
static void sbc_codec_write_to_dst_buffer(ring_buffer_information_t *p_src_info, uint8_t *p_dst, uint32_t buf_len)
{
    ...
    if (data_cnt >= dst_cnt) {   // enough source check
        ...
        TASK_LOG_MSGID_E("source not enough. src:%d dst:%d \r\n", 2, data_cnt, dst_cnt);
    }
}
/**
...
static uint32_t sbc_codec_get_payload(sbc_codec_media_handle_t *handle, uint8_t *buffer, uint32_t size, uint32_t *sample_cnt)
{
    if (!(handle->state == SBC_CODEC_STATE_PLAY || handle->state == SBC_CODEC_STATE_WAIT_DRAIN)) {
        TASK_LOG_MSGID_E("cannot get payload when state is %d \r\n", 1, handle->state);
        sample_cnt = 0;
        return 0;
    }
    /**
    ...
    if (internal_handle->fragment_flag) {
        ...
        if (buf_len >= frame_len) {
            ...
            if (internal_handle->fragment_left == 1) {   // last fragment
                if (internal_handle->fragment_left_bytes != 0) {
                    TASK_LOG_MSGID_E("last fragment. should not have left bytes.", 0);
                }
            ...
            TASK_LOG_MSGID_E("frag number not right. could be payload size change.", 0);
        }
    } else {
        ...
        while (1) {
            ...
            if (buf_len >= frame_len) {
                ...
                if (*p_src != 0x9c) {
                    TASK_LOG_MSGID_E("sync word wrong.frame_len %d ", 1, frame_len);
                }
                ...
                if (allow_fragment == true) {
                    ...
                    if ((buf_len * 15) >= frame_len) {
                        ...
                        *sample_cnt = 0;
                        ...
                        TASK_LOG_MSGID_E("fragment too much", 0);
                    }
                } else {
...
static uint32_t sbc_codec_set_bit_rate(sbc_codec_media_handle_t *handle, uint32_t bit_rate)
{
    ...
    TASK_LOG_MSGID_I("sbc info : sample_rate %d block %d channel %d(%d) subband %d alloc %d bitpool %d frame length %d \r\n", 8,
...
static void sbc_codec_set_info(sbc_codec_media_handle_t *handle, uint32_t sample_rate, uint32_t channel)
{
    ...
    if (internal_handle->source_sample_rate != sample_rate || internal_handle->source_channel != channel) {
        TASK_LOG_MSGID_I("sample_rate %d->%d channel %d->%d \r\n", 4,
        ...
        case HAL_AUDIO_EVENT_DATA_NOTIFICATION: {
            ...
            if (data->buffer_size != 0) {
                ...
                if (spce_cnt >= src_cnt) {   // enough dst check
                    ...
                    TASK_LOG_MSGID_E("space not enough. %d < %d", 2, spce_cnt, src_cnt);
                }
            }
...
static void sbc_bli_reconfig(sbc_codec_internal_handle_t *internal_handle)
{
    ...
    if (internal_handle->bli_working_buffer_size < buffer_size) {
        ...
        TASK_LOG_MSGID_E("buffer size too large. buffer_size %d > %d internal_handle->bli_working_buffer_size", 2,
    ...
    TASK_LOG_MSGID_I("Config BLI sr:%d->%d ch:%d->%d \r\n", 4,
                     internal_handle->source_sample_rate, pFrame->sampling_frequency,
                     internal_handle->source_channel, pFrame->channels);
}

static uint8_t sbc_codec_encode_process(sbc_codec_media_handle_t *handle, void *parameter)
{
    ...
    xSemaphoreTake(sbc_codec_semaphore, portMAX_DELAY);
    ...
    if (internal_handle->source_sample_rate != internal_handle->bli_in_sr
        ...
        TASK_LOG_MSGID_I("BLI(sr,ch) src(%d, %d) now(%d, %d)", 4,
        ...
        while (data_count) {
            {   /* Check if p_sbc_frame_ring have enough space for a sbc frame + 1 byte data. */
                ...
                if (dst_cnt < (internal_handle->current_frame_size + 1)) {
                    TASK_LOG_MSGID_I("dst space not enough %d < %d", 2, dst_cnt, internal_handle->current_frame_size + 1);
                    break;
                }
                /* Check state. */
                if (handle->state == SBC_CODEC_STATE_STOP) {
                    TASK_LOG_MSGID_I("state not correct. %d", 1, handle->state);
                    break;
                }
            }
            ...
            if (subband_size == 0) {
                ...
                if (status < 0) {
                    TASK_LOG_MSGID_E("encode fail.", 0);
                }
                ...
                if (output_byte_count != internal_handle->current_frame_size || *(internal_handle->sbc_output) != 0x9c) {
                    TASK_LOG_MSGID_E("output size(%d) != frame size(%d). or sync word wrong 0x%x. bitpool %d in %d(0x%x) out %d(0x%x) consume %d ", 7,
                                     output_byte_count, internal_handle->current_frame_size, *(internal_handle->sbc_output),
                                     pFrame->bitpool, input_size, (uint32_t)p_src, internal_handle->sbc_output_size, (uint32_t)p_dst, subband_size);
                }
                /* reset buffer info. */
                ...
                {   /* write to mcu sbc frame ring buffer. */
                    ...
                    if (dst_cnt >= output_byte_count + 1) {
                        ...
                        *p_dst = (uint8_t)internal_handle->current_frame_size;
                        ...
                        TASK_LOG_MSGID_E("sbc frame ring overflow. dst_cnt %d output_byte_count+1=%d", 2, dst_cnt, output_byte_count + 1);
                    }
                }
            } /*end sbc encode*/
        } /*end while loop*/
    ...
    if (sbc_byte_count > sbc_requirement) {
        ...
        handle->waiting = true;
        xSemaphoreGiveFromISR(sbc_codec_semaphore, NULL);
...
{
    TASK_LOG_MSGID_I("play sbc codec ++", 0);
    sbc_codec_internal_handle_t *internal_handle = (sbc_codec_internal_handle_t *)handle;
    ...
    if (internal_handle->bli_working_buffer == NULL) {
        TASK_LOG_MSGID_E("buffer not set.", 0);
        return -1;
    }
    ...
    if (sbc_encoder_init(&(internal_handle->sbc_encoder_handle),
        ...
        TASK_LOG_MSGID_E("encoder init fail", 0);
        return -1;
    }
...
{
    TASK_LOG_MSGID_I("stop sbc codec ++", 0);
    sbc_codec_internal_handle_t *internal_handle = (sbc_codec_internal_handle_t *)handle;
    //uint32_t sbc_byte_count = 0;
    ...
    if( internal_handle->status&SBC_CODEC_STATUS_IN_TASK_LIST ) {
        TASK_LOG_MSGID_I("waiting for encode finish processing ++\n", 0);
        if ( internal_handle->status&SBC_CODEC_STATUS_IN_CALLBACK )
            TASK_LOG_MSGID_W("GET_PAYLOAD callback not finished. \r\n", 0);
    ...
    if ( sbc_byte_count != 0 || pcm_byte_count != 0) {
        do {
            TASK_LOG_MSGID_I("sbc_frame:%d pcm_data:%d",2, sbc_byte_count, pcm_byte_count);
            sbc_codec_encode_process ( handle, NULL ); //force drain
            sbc_byte_count = ring_buffer_get_data_byte_count(&internal_handle->p_sbc_frame_ring);
            pcm_byte_count = ring_buffer_get_data_byte_count(&internal_handle->p_pcm_ring);
        } while( sbc_byte_count != 0 || pcm_byte_count != 0 );
    }
    TASK_LOG_MSGID_I("flush sbc codec --", 0);
    */
    /*Stop.*/
...
{
    TASK_LOG_MSGID_I("close sbc codec ++ %d \n", 1, handle->state);
    sbc_codec_internal_handle_t *internal_handle = (sbc_codec_internal_handle_t *)handle;
    ...
    xSemaphoreGive(sbc_codec_semaphore);
    ...
    TASK_LOG_MSGID_I("close sbc codec --", 0);
    return 0;
}
...
{
    TASK_LOG_MSGID_I("open sbc codec ++", 0);
    ...
    if (internal_handle == NULL) {
        TASK_LOG_MSGID_I("internal handle create fail\r\n", 0);
        return 0;
    }
    ...
    switch (channel_mode) {
        ...
        case 1:
            ...
            break;
        default:
            TASK_LOG_MSGID_I("unknown channel mode %d\r\n", 1, channel_mode);
            vPortFree(internal_handle);
            return 0;
    }
    ...
    switch (sample_rate) {
        ...
        case 1:
            ...
            break;
        default:
            TASK_LOG_MSGID_I("unknown sample rate %d\r\n", 1, sample_rate);
            vPortFree(internal_handle);
            return 0;
    }
    ...
    switch (block_number) {
        ...
        case 1:
            ...
            break;
        default:
            TASK_LOG_MSGID_I("unknown block %d\r\n", 1, block_number);
            vPortFree(internal_handle);
            return 0;
    }
    ...
    if (sbc_codec_semaphore == NULL) {
        vSemaphoreCreateBinary(sbc_codec_semaphore);
        TASK_LOG_MSGID_I("create sbc_semaphore. %0x", 1, sbc_codec_semaphore);
    }

    if (sbc_codec_queue_handle == NULL) {
        TASK_LOG_MSGID_I("create sbc_codec_queue.", 0);
        sbc_codec_queue_handle = xQueueCreate(20, sizeof(sbc_codec_queue_event_t));
        TASK_LOG_MSGID_I("create sbc_codec_queue. %0x", 1, sbc_codec_queue_handle);
    }

    if (sbc_codec_task_handle == NULL) {
        TASK_LOG_MSGID_I("create sbc_codec_task.", 0);
        xTaskCreate(sbc_codec_task_main,
                    "sbc_codec_task", 1024 / sizeof(StackType_t), NULL, 7, &sbc_codec_task_handle);
        TASK_LOG_MSGID_I("create sbc_codec_task. %0x", 1, sbc_codec_task_handle);
    }
    ...
    if (sbc_codec_task_handle == NULL || sbc_codec_queue_handle == NULL) {
        TASK_LOG_MSGID_I("task(0x%x) or queue(0x%x) create fail.", 2, (uint32_t)sbc_codec_task_handle, (uint32_t)sbc_codec_queue_handle);
        return 0;
    }
    ...
    TASK_LOG_MSGID_I("open sbc codec -- codec queue handle: %x", 1, sbc_codec_queue_handle);
    return handle;
}
