...
//#define LOGW(fmt,arg...)   {}
//#define LOGI(fmt,arg...)   {}
#define LOGMSGIDE(fmt,arg...)   {}
#define LOGMSGIDW(fmt,arg...)   {}
...
#define LOGMSGIDE(fmt,arg...)   LOG_MSGID_E(gain_value_mapping, "[gain_value_mapping]"fmt,##arg)
#define LOGMSGIDW(fmt,arg...)   LOG_MSGID_W(gain_value_mapping, "[gain_value_mapping]"fmt,##arg)
#define LOGMSGIDI(fmt,arg...)   LOG_MSGID_I(gain_value_mapping, "[gain_value_mapping]"fmt,##arg)
#endif
...
static uint32_t a_value_linear_transfer_to_b_value(int32_t input_a, int32_t maximum_a_value, int32_t minimum_a_value, int32_t a_step_value_in_absolute, int32_t maximum_b_value, int32_t minimum_b_value, int32_t minimum_a_to_minimum_b_value)
{
    ...
    //LOGMSGIDI("intput_a=%d, total_dB_steps=%d, b_value_step_in_absolute=%d, intput_a_to_a_step=%d, b_value=%d\n", 5, input_a, total_dB_steps, b_value_step_in_absolute, intput_a_to_a_step, b_value);
...
void voice_uplink_dmic_db_transfer_to_analog_gain_and_digital_gain(int32_t input_db, int32_t *analog_gain_in_db, int32_t *digital_gain_in_db)
{
    int32_t db_max = 0;
    int32_t db_min = 0;
    int32_t db_step_in_absolute = VOICE_UPLINK_DMIC_DB_STEP;
    ...
    truncate_out_of_range_value(&input_db, VOICE_UPLINK_DMIC_DB_MINIMUM, VOICE_UPLINK_DMIC_DB_MAXIMUM);

    if (input_db <= VOICE_UPLINK_DMIC_DB_MAXIMUM && input_db >= VOICE_UPLINK_DMIC_DB_MINIMUM) {
        *analog_gain_in_db = 0;

        db_max = VOICE_UPLINK_DMIC_DB_MAXIMUM;
        db_min = VOICE_UPLINK_DMIC_DB_MINIMUM;
    ...
    LOGMSGIDI("voice uplink DMIC input_db=%d, analog gain=%d, digital gain=%d\n", 3, input_db, *analog_gain_in_db, *digital_gain_in_db);
}
...
void voice_uplink_amp_db_transfer_to_analog_gain_and_digital_gain(int32_t input_db, int32_t *analog_gain_in_db, int32_t *digital_gain_in_db)
{
    ...
    LOGMSGIDI("voice uplink amp input_db=%d, analog gain=%d, digital gain=%d\n", 3, input_db, *analog_gain_in_db, *digital_gain_in_db);
}
...
void voice_downlink_amp_db_transfer_to_analog_gain_and_digital_gain(int32_t input_db, int32_t *analog_gain_in_db, int32_t *digital_gain_in_db)
{
    ...
    if (input_db <= VOICE_DOWNLINK_AMP_DB_MAXIMUM && input_db >= -1) {
    ...
    } else if (input_db <= -2 && input_db >= VOICE_DOWNLINK_AMP_DB_MINIMUM) {
        *analog_gain_in_db = 0;
        ...
        *digital_gain_in_db = a_value_linear_transfer_to_b_value(input_db, db_max, db_min, db_step_in_absolute, b_value_max, b_value_min, minimum_db_to_minimum_b_value);
        ...
        // never here
        LOGMSGIDI("voice downlink amp, Warning! it must not be here, input_db(%d)out of range([%d, %d]),", 3, input_db, VOICE_DOWNLINK_AMP_DB_MINIMUM, VOICE_DOWNLINK_AMP_DB_MAXIMUM);
    }

    LOGMSGIDI("voice downlink amp input_db=%d, analog gain=%d, digital gain=%d\n", 3, input_db, *analog_gain_in_db, *digital_gain_in_db);
}
...
void audio_downlink_amp_db_transfer_to_analog_gain_and_digital_gain(int32_t input_db, int32_t *analog_gain_in_db, int32_t *digital_gain_in_db)
{
    ...
    if (input_db <= AUDIO_DOWNLINK_AMP_DB_MAXIMUM && input_db >= -13) {
        *digital_gain_in_db = 0;
        ...
        *analog_gain_in_db = a_value_linear_transfer_to_b_value(input_db, db_max, db_min, db_step_in_absolute, b_value_max, b_value_min, minimum_db_to_minimum_b_value);
    } else if (input_db <= -14 && input_db >= AUDIO_DOWNLINK_AMP_DB_MINIMUM) {
        *analog_gain_in_db = 0;
        ...
        *digital_gain_in_db = a_value_linear_transfer_to_b_value(input_db, db_max, db_min, db_step_in_absolute, b_value_max, b_value_min, minimum_db_to_minimum_b_value);
        ...
        // never here
        LOGMSGIDI("audio downlink amp, Warning! it must not be here, input_db(%d)out of range([%d, %d]),", 3, input_db, AUDIO_DOWNLINK_AMP_DB_MINIMUM, AUDIO_DOWNLINK_AMP_DB_MAXIMUM);
    }

    LOGMSGIDI("audio downlink amp input_db=%d, analog gain=%d, digital gain=%d\n", 3, input_db, *analog_gain_in_db, *digital_gain_in_db);
}
