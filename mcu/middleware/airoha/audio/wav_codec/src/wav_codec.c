...
static void wav_codec_send_event_from_isr(audio_codec_media_handle_t *handle)
{
    ...
    if (!(internal_handle->status & WAV_CODEC_STATUS_IN_TASK_LIST) /* && handle->state == AUDIO_CODEC_STATE_PLAY */) {
        ...
        if (audio_codec_event_send_from_isr(handle, NULL) == AUDIO_CODEC_RETURN_OK) {
            TASK_LOGMSGID_D("AUDIO_CODEC_RETURN_OK\n", 0);
        } else {
            internal_handle->status &= ~WAV_CODEC_STATUS_IN_TASK_LIST;
        }
    }
...
static audio_codec_status_t wav_codec_fill_decoder_bs(audio_codec_media_handle_t *handle, unsigned int *bs_buf_size, unsigned int remain_data)
{
    ...
    TASK_LOGMSGID_D("remain_data %d, data_cnt %d, internal_handle->decoder_input_buf_offset %d\n", 3, remain_data, data_cnt, (internal_handle->decoder_input_buf_size - internal_handle->decoder_input_buf_offset));
    ...
    if (data_cnt != 0) {
        ...
        *bs_buf_size += (internal_handle->decoder_input_buf_offset + data_cnt - dst_cnt);
        ...
        TASK_LOGMSGID_D("share ring buffer underflow or no remain bs, data byte %d remain_data %d \n", 2, data_cnt, remain_data);
            ...
            while (dst_cnt > 0) {
                ...
                if (dst_cnt > 0 && data_cnt > 0) {
                    ...
                    for (loop_idx = 0; loop_idx < 2; loop_idx++) {
                        ...
                        read_cnt = MINIMUM(dst_cnt, src_cnt);
                        TASK_LOGMSGID_D("src_cnt = %d, dst_cnt = %d, read_cnt = %d\n", 3, src_cnt, dst_cnt, read_cnt);
#if defined(MTK_AVM_DIRECT)
                        hal_audio_write_stream_out_by_type(internal_handle->message_type, p_src, read_cnt);
#elif defined(MTK_AUDIO_MIXER_SUPPORT)
                    ...
                    if (internal_handle->remain_bs == 0 && data_cnt == 0) {
                        TASK_LOGMSGID_D("[WAV Codec]No bs & pcm data. Set decode_end_flag true\n", 0);
                        internal_handle->decode_end_flag = true;
                        return AUDIO_CODEC_RETURN_OK;
                    } else if (internal_handle->remain_bs == 0) {
                        TASK_LOGMSGID_D("[WAV Codec]wav_codec_fill_pcm_data return Error\n", 0);
                        return AUDIO_CODEC_RETURN_ERROR;
                    }
                }
...
#endif
            data_cnt = ring_buffer_get_data_byte_count(p_ring); // source
            TASK_LOGMSGID_D("dst_cnt = %d, data_cnt = %d\n", 2, dst_cnt, data_cnt);
...
static void wav_codec_deocde_event_handler(audio_codec_media_handle_t *handle, void *data)
{
    ...
    if (status == AUDIO_CODEC_RETURN_OK && internal_handle->decoder_output_buf_offset == 0) {
        ...
        TASK_LOGMSGID_D("WAV_Decode : in %d out %d consumedBytes %d offset %d pcm_out_size %d \n", 5, bs_buf_size, internal_handle->decode_size, consumedBytes, internal_handle->decoder_input_buf_offset, pcm_out_size);
    } else {
        TASK_LOGMSGID_I("bs not enough.\n", 0);
        status = AUDIO_CODEC_RETURN_ERROR;
    }
    ...
    if (spce_cnt > 0 && (internal_handle->decoder_output_buf_offset != internal_handle->decode_size)) {
        ...
        for (loop_idx = 0; loop_idx < 2; loop_idx++) {
            ...
            TASK_LOGMSGID_D("write_cnt %d dst_cnt %d dec_src_cnt %d\n", 3, write_cnt, dst_cnt, src_cnt);
        ...
        TASK_LOGMSGID_I("pcm ring overflow.\n", 0);
    }
    ...
    TASK_LOGMSGID_D("internal_handle->status %d WAV_CODEC_STATUS_DSP_UNDERFLOW %d .\n", 2,
    ...
    if (internal_handle->remain_bs && (internal_handle->status & WAV_CODEC_STATUS_DSP_UNDERFLOW)) {
        internal_handle->status &= ~WAV_CODEC_STATUS_DSP_UNDERFLOW;
        TASK_LOGMSGID_I("dsp underflow case .\n", 0);
...
static void wav_codec_pcm_isr_handler(hal_audio_event_t event, void *data)
{
    ...
    TASK_LOGMSGID_D("internal_handle->status = %d\n", internal_handle->status 1,);
    ...
    switch (event) {
        ...
        case HAL_AUDIO_EVENT_DATA_REQUEST: {
            if (internal_handle->linear_data == 1) {
...
#if defined(MTK_AVM_DIRECT)
                    mcu2dsp_audio_msg_t config_msg = MSG_MCU2DSP_PLAYBACK_CONFIG;
                    if (internal_handle->message_type == AUDIO_MESSAGE_TYPE_PROMPT) {
                        config_msg = MSG_MCU2DSP_PROMPT_CONFIG;
                    }
                    hal_audio_dsp_controller_send_message(config_msg, AUDIO_PLAYBACK_CONFIG_EOF, 1, false);
#elif defined(MTK_AUDIO_MIXER_SUPPORT)
                ...
                if (handle->state == AUDIO_CODEC_STATE_PLAY) {
                    if (wav_codec_fill_pcm_data(handle) != AUDIO_CODEC_RETURN_OK) {
                        internal_handle->status |= WAV_CODEC_STATUS_DSP_UNDERFLOW;
                        TASK_LOGMSGID_D("dsp underflow, remain_bs %d\n", 1, internal_handle->remain_bs);
                    }
                }
                if (internal_handle->decode_end_flag == true) {
                    if (internal_handle->eof_flag == false) {
                        TASK_LOGMSGID_D("audio_pcm_set_eof\r\n", 0);
                        internal_handle->eof_flag = true;

#if defined(MTK_AVM_DIRECT)
                        dsp2mcu_audio_msg_t config_msg = MSG_MCU2DSP_PLAYBACK_CONFIG;
#if defined(MTK_AUDIO_MIXER_SUPPORT)
        ...
        case HAL_AUDIO_EVENT_END:
            if (internal_handle->linear_data == 1) {
                TASK_LOGMSGID_I("linear_data == 1, get event end\r\n", 0);
                internal_handle->decode_end_flag = false;
                handle->handler(handle, AUDIO_CODEC_MEDIA_OK);
            } else {
                TASK_LOGMSGID_I("linear_data == 0, get event end\r\n", 0);
                internal_handle->decode_end_flag = false;
        ...
        case HAL_AUDIO_EVENT_DATA_NOTIFICATION:
            TASK_LOGMSGID_W("error/none/data notify.\n", 0);
            break;
        default:
            TASK_LOGMSGID_W("un-support event.\n", 0);
            break;
    }
...
#if defined(MTK_AVM_DIRECT)
static void wav_codec_play_avm(audio_codec_media_handle_t *handle)
{
    wav_codec_internal_handle_t *internal_handle = (wav_codec_internal_handle_t *) handle;
    ...
    open_param.stream_out_param.afe.stream_channel  = HAL_AUDIO_DIRECT; //HAL_AUDIO_STEREO;
    ...
    hal_audio_service_hook_callback(msg_type, wav_codec_pcm_isr_handler, handle);
...
static audio_codec_status_t wav_codec_resume(audio_codec_media_handle_t *handle)
{
    ...
    if (handle->state != AUDIO_CODEC_STATE_PAUSE) {
        TASK_LOGMSGID_W("unexpected flow. calling codec_resume before codec_pause.\n", 0);
        return AUDIO_CODEC_RETURN_ERROR;
    }
...
#elif defined(MTK_AUDIO_MIXER_SUPPORT)
    {
        ...
        if (AUDIO_MIXER_STATUS_OK != audio_mixer_status) {
            ...
            TASK_LOGMSGID_E("start track fail\n", 0);
...
#if defined(MTK_AVM_DIRECT)
static void wav_codec_stop_avm(audio_codec_media_handle_t *handle)
{
...
static audio_codec_status_t wav_codec_pause(audio_codec_media_handle_t *handle)
{
    ...
    if (handle->state != AUDIO_CODEC_STATE_PLAY) {
        TASK_LOGMSGID_W("unexpected flow. calling codec_pause before codec_play.\n", 0);
        return AUDIO_CODEC_RETURN_ERROR;
    }
...
static audio_codec_status_t wav_codec_play_internal(audio_codec_media_handle_t *handle)
{
    ...
    {   /* parse header, reach data chuck */
        ...
        do { /* parse header */
            ...
            if (status != AUDIO_CODEC_RETURN_OK) {
                TASK_LOGMSGID_E("bs not enough.\n", 0);
                return AUDIO_CODEC_RETURN_ERROR;
            }
            TASK_LOGMSGID_D("bs_buf_size %d.\n", 1, bs_buf_size);
            ...
            if (decode_status == WAV_SUCCESS) {
                ...
                TASK_LOGMSGID_E("Unsupported File or Bad File.\n", 0);
                return AUDIO_CODEC_RETURN_ERROR;
            }
        } while (1);
    ...
    // dump audio info
    TASK_LOGMSGID_I("fomat %d data_offset %d data_chunk_length %d bit_rate %d dutation %d sample_rate %d channel_num %d \n", 7,
...
#if defined(MTK_AVM_DIRECT)
    internal_handle->sampling_rate = sample_rate;
    internal_handle->channel_number = channel_num;

    wav_codec_play_avm(handle);

    TASK_LOGMSGID_I("start stream out\n", 0);
    handle->state = AUDIO_CODEC_STATE_PLAY;
#elif defined(MTK_AUDIO_MIXER_SUPPORT)
    // if ( internal_handle->linear_data == 1 ) {
    ...
    if (handle->mixer_track_id < 0) {
        TASK_LOGMSGID_E("get track id fail\n", 0);
        return AUDIO_CODEC_RETURN_ERROR;
    }
...
static audio_codec_status_t wav_codec_play(audio_codec_media_handle_t *handle)
{
    ...
    if (handle->state != AUDIO_CODEC_STATE_READY && handle->state != AUDIO_CODEC_STATE_STOP) {
        TASK_LOGMSGID_W("unexpected flow. calling codec_play before codec_open or codec_stop.\n", 0);
        return AUDIO_CODEC_RETURN_ERROR;
    }
    ...
    TASK_LOGMSGID_I("wav_codec_play ++\n", 0);
...
static audio_codec_status_t wav_codec_stop(audio_codec_media_handle_t *handle)
{
    ...
    if (handle->state != AUDIO_CODEC_STATE_PLAY && handle->state != AUDIO_CODEC_STATE_PAUSE) {
        TASK_LOGMSGID_W("unexpected flow. stop before play.\n", 0);
        return AUDIO_CODEC_RETURN_ERROR;
    }
...
{
    FUNCTION_IN;
    TASK_LOGMSGID_I("Close codec\n", 0);

    wav_codec_internal_handle_t *internal_handle = (wav_codec_internal_handle_t *) handle;
    if (handle->state != AUDIO_CODEC_STATE_STOP && handle->state != AUDIO_CODEC_STATE_READY) {
        TASK_LOGMSGID_W("unexpected flow. calling codec_close before codec_stop or codec_open.\n", 0);
        return AUDIO_CODEC_RETURN_ERROR;
    }
...
audio_codec_status_t wav_codec_buffer_and_codec_init(audio_codec_media_handle_t *handle)
{
    ...
    TASK_LOGMSGID_I("memory usage : bs_ring_size %d int_buf_size %d bs_buf_size %d pcm_buf_size %d decode_pcm_ring %d\n", 5,
...

    {
        ...
        if (bs_ring == NULL) {
            TASK_LOGMSGID_E("PortMalloc fail, bs_ring\n", 0);
            return AUDIO_CODEC_RETURN_ERROR;
        }
    ...
    if (int_buffer == NULL || bs_buffer == NULL || pcm_buffer == NULL) {
        TASK_LOGMSGID_E("PortMalloc fail, int_buffer %d, bs_buffer %d, pcm_buffer %d\n", 3, int_buffer, bs_buffer, pcm_buffer);
        return AUDIO_CODEC_RETURN_ERROR;
    }
...
#else
    {
        ...
        TASK_LOGMSGID_I("UT memory usage : pcm_ring_size %d \n", 1, pcm_ring_size);
    }
#endif /*WAV_CODEC_UT_FLOW*/
...
audio_codec_media_handle_t *audio_codec_wav_codec_open(audio_codec_callback_t wav_codec_callback, void *param)
{
    ...
    if (audio_id > MAX_AUDIO_FUNCTIONS) {
        TASK_LOGMSGID_I("Audio ID > MAX AUDIO FUNCTIONS\n", 0);
        return 0;
    }
#endif
