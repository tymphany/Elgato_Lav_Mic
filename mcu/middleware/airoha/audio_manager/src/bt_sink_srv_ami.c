...
static bt_sink_srv_am_result_t am_audio_side_tone_get_nvdm_param(sidetone_nvdm_param_t *sidetone_param)
{
...
static sidetone_mechanism_t am_audio_side_tone_query_method(void)
{
...
#if defined(MTK_ANC_ENABLE) && (defined(MTK_ANC_V2) || defined(AIR_ANC_V3))
            if ((bt_sink_srv_ami_get_current_scenario() != HFP) && (scenario == SIDETONE_SCENARIO_RESERVE)) {
                                             ...
                                             bt_sink_srv_am_feature_t *feature_param)
{
    ...
    do {
        ...
        switch (feature_type) {
            ...
            case AM_PSAP: {
                memcpy(&local_feature->feature_param.psap_param, &feature_param->feature_param.psap_param, sizeof(bt_sink_srv_am_psap_param_t));
                break;
            }
#endif
...
void ami_hal_audio_status_set_running_flag(audio_scenario_type_t type, mcu2dsp_open_param_t *param, bool is_running)
{
    ...
    #ifdef AIR_DCHS_MODE_ENABLE
    dchs_cosys_ctrl_cmd_relay(is_running ? AUDIO_UART_COSYS_DL_OPEN : AUDIO_UART_COSYS_CMD_MIN,  type, param, NULL);
            ...
            if (is_am_task) {
                ...
                if (msg_id == MSG_ID_STREAM_OPEN_REQ || \
                    ...
                    if (xQueueSend(dest_queue, (void *) &g_prAmm_current, (TickType_t) 0) != pdPASS) {
                        audio_src_srv_report("[AudM]error. AM queue full when AM send own queue.", 0);
                        AUDIO_ASSERT(0);
                    }
                }
#else
        ...
        if (dest_queue != 0) {
            ...
            if (is_am_task) {
                ...
                if (xQueueSendToFront(dest_queue, (void *) &g_prAmm_current, (TickType_t) 0) != pdPASS) {
                    audio_src_srv_report("[AudM]front, error. AM queue full when AM send own queue.", 0);
                    AUDIO_ASSERT(0);
                }
            } else {
...
static bt_sink_srv_am_result_t ami_sidetone_enable_user_config(int32_t user_volume)
{
...
static bt_sink_srv_am_result_t ami_sidetone_disable_user_config(void)
{
