...
void bt_sink_srv_audio_tunning_atci_init(void)
{
    ...
    if (ret != ATCI_STATUS_OK) {
        audio_src_srv_report("bt sink audio tunnning register fail", 0);
    }
}
...
int16_t bt_sink_srv_audio_tuning_command_entry(char *string)
{
    ...
    LOG_I("[Sink][Audio Tunning]command: %s", string);
    ...
    if (am_speech_eaps) {
        ...
        audio_src_srv_report("[Sink][Audio Tunning]command_entry no memory--ply: 0x%x, type: %d", 2, g_prCurrent_player, g_prCurrent_player->type);
        ret = -200;
    }
    ...
    if (0 == strncmp(string, "VOL", 3)) {
        if (g_prCurrent_player && (g_prCurrent_player->type == A2DP ||
            ...
            ret = -100;
            audio_src_srv_report("[Sink][Audio Tunning]error--ply: 0x%x, type: %d", 2, g_prCurrent_player, g_prCurrent_player->type);
        }
#if 0
        /* tunning volume */
    ...
    audio_src_srv_report("[Sink][Audio Tunning]ret: %d, in: %d, out: %d", 3,
...
hal_audio_device_t bt_sink_srv_get_tunning_dev(int32_t index, uint8_t type)
{
    ...
    audio_src_srv_report("[Sink][Audio Tunning]get_dev--dev: 0x%x, index: %d, type: %d", 3, dev, index, type);
...
void bt_sink_srv_audio_tunning_update_callback(void *data)
{
    ...
    if (am_speech_eaps) {
        ...
        audio_src_srv_report("[Sink][Audio Tunning]update_callback no memory--ply: 0x%x, type: %d", 2, g_prCurrent_player, g_prCurrent_player->type);
        ret = -200;
    }
    ...
    audio_src_srv_report("[Sink][Audio Tunning]update_callback--ret: %d", 1, ret);
#ifndef MTK_DEBUG_LEVEL_INFO
    g_test_fix_warning = ret;
#endif
}
...
void bt_sink_srv_audio_tunning_update(uint8_t scen, uint8_t type)
{
    ...
    switch (scen) {
        case TUNNING_SCENARIO_A2DP: {
            if (TUNNING_TYPE_VOL == type) {
                ret1 = hal_audio_set_stream_out_volume(HAL_AUDIO_STREAM_OUT1, aud_tunning_p->a2dp.vol_out.digital, aud_tunning_p->a2dp.vol_out.analog);
        ...
        case TUNNING_SCENARIO_HF: {
            if (TUNNING_TYPE_VOL == type) {
                ...
                ret2 = hal_audio_set_stream_out_volume(HAL_AUDIO_STREAM_OUT1, aud_tunning_p->hf.vol_out.digital, aud_tunning_p->hf.vol_out.analog);
            } else if (TUNNING_TYPE_DEV == type) {
    ...
    audio_src_srv_report("[Sink][Audio Tunning]update--ret1: %d, ret2: %d, scen: %d, type: %d", 4, ret1, ret2, scen, type);
#ifndef MTK_DEBUG_LEVEL_INFO
...
void tunning_stream_out_db_transfer(uint16_t gain, uint32_t *digital, uint32_t *analog)
{
    ...
    *analog = (uint32_t)analog_gain_in_db;
    ...
    audio_src_srv_report("[Sink][Audio Tunning]stream_db--gain: %d, digital: %d, analog: %d", 3, gain, digital, analog);
}
...
void tunning_voice_uplink_db_transfer(uint16_t gain, uint32_t *digital, uint32_t *analog)
{
    ...
    *analog = (uint32_t)analog_gain_in_db;
    ...
    audio_src_srv_report("[Sink][Audio Tunning]voice_db--gain: %d, digital: %d, analog: %d", 3, gain, digital, analog);
}
...
static int16_t bt_sink_hf_audio_tuning_command_entry(char *string)
{
    ...
    LOG_I("[Sink][HF][TUNING] command:%s", string);
...
void bt_sink_hf_audio_tuning_atci_init(void)
{
    ...
    if (ret != ATCI_STATUS_OK) {
        audio_src_srv_report("at_cmd_init register fail", 0);
    }
}
