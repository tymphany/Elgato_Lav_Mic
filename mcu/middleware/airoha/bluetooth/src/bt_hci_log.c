...
#pragma comment(linker, "/alternatename:_hci_log_iso_out=_default_hci_log_iso_out")
#pragma comment(linker, "/alternatename:_hci_log_iso_in=_default_hci_log_iso_in")
...
#pragma weak hci_log_iso_out = default_hci_log_iso_out
#pragma weak hci_log_iso_in = default_hci_log_iso_in
#else
...
extern int32_t hci_log_iso_out(unsigned char *buf, int32_t length);
extern int32_t hci_log_iso_in(unsigned char *buf, int32_t length);
...
int32_t default_hci_log_iso_in(unsigned char *buf, int32_t length)
{
    return 0;
}

int32_t default_hci_log_iso_out(unsigned char *buf, int32_t length)
{
    return 0;
}
...
void bt_hci_log(const uint8_t in, const void *data, const uint32_t data_len)
{
    ...
    if (in == 2) {
        ...
        if (h4type == 2) {
            logtype = BT_HCI_LOG_ACL_IN;
        } else if (h4type == 3) {
            logtype = BT_HCI_LOG_SCO_IN;
        }
#ifdef AIR_LE_AUDIO_ENABLE
        else if (h4type == 5) {
            logtype = BT_HCI_LOG_ISO_IN;
        }
#endif
        else {
            logtype = BT_HCI_LOG_EVENT;
        }
    } else {
        if (h4type == 2) {
            logtype = BT_HCI_LOG_ACL_OUT;
        } else if (h4type == 3) {
            logtype = BT_HCI_LOG_SCO_OUT;
        }
#ifdef AIR_LE_AUDIO_ENABLE
        else if (h4type == 5) {
            logtype = BT_HCI_LOG_ISO_OUT;
        }
#endif
        else {
            logtype = BT_HCI_LOG_CMD;
        }
    }
    ...
    switch (logtype) {
        case BT_HCI_LOG_CMD:
            ...
            break;
        case BT_HCI_LOG_EVENT:
            hci_log_event((unsigned char *)p, data_len - 1);
            break;
        case BT_HCI_LOG_ACL_IN:
            hci_log_acl_in((unsigned char *)p, data_len - 1);
            break;
        case BT_HCI_LOG_ACL_OUT:
            hci_log_acl_out((unsigned char *)p, data_len - 1);
            break;
#ifdef AIR_LE_AUDIO_ENABLE
        case BT_HCI_LOG_ISO_IN:
            hci_log_iso_in((unsigned char *)p, data_len - 1);
            break;
        case BT_HCI_LOG_ISO_OUT:
            hci_log_iso_out((unsigned char *)p, data_len - 1);
            break;
#endif
        default:
            break;
