...
ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN uint8_t bt_hal_plain[32];
ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN uint8_t bt_hal_key[16];
ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN uint8_t bt_hal_coded[32];

ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN hal_aes_buffer_t bt_hal_plain_text;
ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN hal_aes_buffer_t bt_hal_key_text;
ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN hal_aes_buffer_t bt_hal_coded_text;

#ifdef HAL_SHA_MODULE_ENABLED
ATTR_ZIDATA_IN_NONCACHED_SYSRAM_4BYTE_ALIGN hal_sha256_context_t sha256_ctx;
#endif
...
void bt_os_layer_aes_encrypt(bt_os_layer_aes_buffer_t *encrypted_data, bt_os_layer_aes_buffer_t *plain_text, bt_os_layer_aes_buffer_t *key)
{
    ...
    hal_aes_status_t aes_status = HAL_AES_STATUS_OK;
    ...
    LOG_MSGID_I(BT_OS, "bt_os_layer_aes_encrypt before plain data 0x%x, key 0x%x, coded 0x%x", 3, *(uint32_t *)bt_hal_plain_text.buffer, *(uint32_t *)bt_hal_key_text.buffer, *(uint32_t *)bt_hal_coded_text.buffer);
    uint8_t retry_times = 5;
    while (retry_times-- &&
           (HAL_AES_STATUS_BUSY == (aes_status = hal_aes_ecb_encrypt(&bt_hal_coded_text, &bt_hal_plain_text, &bt_hal_key_text)))) {
        vTaskDelay(1);
    }
    LOG_MSGID_I(BT_OS, "bt_os_layer_aes_encrypt after plain data 0x%x, key 0x%x, coded 0x%x", 3, *(uint32_t *)bt_hal_plain_text.buffer, *(uint32_t *)bt_hal_key_text.buffer, *(uint32_t *)bt_hal_coded_text.buffer);
...
void bt_os_layer_aes_decrypt(bt_os_layer_aes_buffer_t *decrypted_data, bt_os_layer_aes_buffer_t *plain_text, bt_os_layer_aes_buffer_t *key)
{
    ...
    uint8_t retry_times = 5;
    while (retry_times-- &&
           (HAL_AES_STATUS_BUSY == (aes_status = hal_aes_ecb_decrypt(&bt_hal_coded_text, &bt_hal_plain_text, &bt_hal_key_text)))) {
        vTaskDelay(1);
    }
...
void bt_os_layer_sha256(uint8_t hashed_data[32], bt_os_layer_aes_buffer_t *plain_text)
{
    if (NULL != plain_text) {
        ...
        LOG_MSGID_E(BT_OS, "plain text is null !!!", 0);
    }
}
#endif
...
{
    /* Use uECC, */
    LOG_MSGID_I(BT_OS, "bt_os_layer_ecdh256 use uECC", 0);
    const struct uECC_Curve_t *curve = uECC_secp256r1();
    if (!uECC_shared_secret(public_key, private_key, dh_key, curve)) {
        LOG_MSGID_W(BT_OS, "uECC ecdh256 fail", 0);
    }
    /*
...
extern uint32_t bt_os_get_stack_mutex(void);
...
void bt_os_layer_take_mutex(uint32_t mutex_id)
{
    ...
    vTaskSuspendAll();
    if (uxSemaphoreGetCount(mutex_id) == 1 && bt_os_get_stack_mutex() == mutex_id) {
        LinkRegAddr = (uint32_t)__builtin_return_address(0);
    }
    xTaskResumeAll();
...
{
    return (uint8_t)xSemaphoreTake((SemaphoreHandle_t)semaphore_id, ticks_to_wait);
}
...
{
    if (0 == assert_enable) {
        light_assert(exptr, file, line);
    }
}

void bt_os_layer_lock_dvfs_156m(void)
{
#if defined (HAL_DVFS_MODULE_ENABLED) && defined (__MTK_AVM_DIRECT__) && defined(HAL_DVFS_312M_SOURCE)
    LOG_MSGID_I(BT_OS, "lock dvfs to 156m start %d\r\n", 1, pmu_get_vcore_voltage_mt6388());
    dvfs_lock_control("bt_stack", DVFS_156M_SPEED, DVFS_LOCK);
    LOG_MSGID_I(BT_OS, "lock dvfs to 156m end %d\r\n", 1, pmu_get_vcore_voltage_mt6388());
#endif
    return;
}
...
{
#if defined (HAL_DVFS_MODULE_ENABLED) && defined (__MTK_AVM_DIRECT__) && defined(HAL_DVFS_312M_SOURCE)
    LOG_MSGID_I(BT_OS, " unlock dvfs from 156m start %d\r\n", 1, pmu_get_vcore_voltage_mt6388());
    dvfs_lock_control("bt_stack", DVFS_156M_SPEED, DVFS_UNLOCK);
    LOG_MSGID_I(BT_OS, " unlock dvfs from 156m end %d\r\n", 1, pmu_get_vcore_voltage_mt6388());
#endif
    return;
}

#ifdef BT_DVFS_ENABLE
void bt_os_layer_lock_dvfs(bt_os_layer_dvfs_frequency_t frequency)
{
#if defined (HAL_DVFS_MODULE_ENABLED) && defined (__MTK_AVM_DIRECT__) && defined(HAL_DVFS_312M_SOURCE)
    LOG_MSGID_I(BT_OS, "lock dvfs to frequency = %d start, vcore =  %d\r\n", 1, frequency, pmu_get_vcore_voltage_mt6388());
    hal_dvfs_lock_control(frequency, BT_OS_LAYER_DVFS_UNLOCK);
    LOG_MSGID_I(BT_OS, "lock dvfs to frequency = %d end, vcore =  %d\r\n", 1, frequency, pmu_get_vcore_voltage_mt6388());
#elif defined (HAL_DVFS_MODULE_ENABLED) && defined (__MTK_AVM_DIRECT__) && defined(HAL_DVFS_416M_SOURCE)
    LOG_MSGID_I(BT_OS, "lock dvfs to frequency = %d start, vcore =  %d\r\n", 1, frequency, pmu_get_vcore_voltage());
    hal_dvfs_lock_control(frequency, BT_OS_LAYER_DVFS_LOCK);
    LOG_MSGID_I(BT_OS, "lock dvfs to frequency = %d end, vcore =  %d\r\n", 1, frequency, pmu_get_vcore_voltage());
#endif
}
...
{
#if defined (HAL_DVFS_MODULE_ENABLED) && defined (__MTK_AVM_DIRECT__) && defined(HAL_DVFS_312M_SOURCE)
    LOG_MSGID_I(BT_OS, "unlock dvfs to frequency = %d start, vcore =  %d\r\n", 1, frequency, pmu_get_vcore_voltage_mt6388());
    hal_dvfs_lock_control(frequency, BT_OS_LAYER_DVFS_UNLOCK);
    LOG_MSGID_I(BT_OS, "unlock dvfs to frequency = %d end, vcore =  %d\r\n", 1, frequency, pmu_get_vcore_voltage_mt6388());
#elif defined (HAL_DVFS_MODULE_ENABLED) && defined (__MTK_AVM_DIRECT__) && defined(HAL_DVFS_416M_SOURCE)
    LOG_MSGID_I(BT_OS, "unlock dvfs to frequency = %d start, vcore =  %d\r\n", 1, frequency, pmu_get_vcore_voltage());
    hal_dvfs_lock_control(frequency, BT_OS_LAYER_DVFS_UNLOCK);
    LOG_MSGID_I(BT_OS, "unlock dvfs to frequency = %d end, vcore =  %d\r\n", 1, frequency, pmu_get_vcore_voltage());
...
#if  (defined(AIR_BTA_IC_PREMIUM_G2))
    pmu_enable_power(PMU_BUCK_VRF, PMU_ON);
...
#if  (defined(AIR_BTA_IC_PREMIUM_G2))
    pmu_enable_power(PMU_BUCK_VRF, PMU_OFF);
#endif
...
#if defined (HAL_CCNI_MODULE_ENABLED) && defined (__MTK_AVM_DIRECT__) && ((PRODUCT_VERSION == 1552) || defined(AM255X))
    switch (ccni_type) {
        ...
        case BT_OS_LAYER_CCNI_CM4_TO_N9_HCI_CTRL_EVENT0:
            ...
            break;
        case BT_OS_LAYER_CCNI_CM4_TO_N9_FEC_EVENT:
            ccni_event_value = CCNI_CM4_TO_N9_EVENT2;
            break;
        case BT_OS_LAYER_CCNI_ACK_TO_CONTROLLER:
            ccni_event_value = CCNI_CM4_TO_N9_EVENT3;
            break;
        default:
...
#if (PRODUCT_VERSION == 7698)
uint32_t bt_os_layer_get_nvkey_start_address()
{
    return NVKEY_BASE;
}
#endif
