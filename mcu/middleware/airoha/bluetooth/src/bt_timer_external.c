...
#define BT_TIMER_EXT_MUTEX_LOCK()   bt_os_take_stack_mutex()
#define BT_TIMER_EXT_MUTEX_UNLOCK() bt_os_give_stack_mutex()
...
static bt_timer_ext_t *bt_timer_ext_find_internal(uint32_t timer_id)
{
...
bt_timer_ext_status_t bt_timer_ext_start(uint32_t timer_id, uint32_t data, uint32_t time_ms, bt_timer_ext_timeout_callback_t cb)
{
    ...
    BT_TIMER_EXT_MUTEX_LOCK();
    if (NULL != bt_timer_ext_find_internal(timer_id)) {
        LOG_MSGID_I(BT_TIMER_EXT, "bt_timer_ext_start(%x)fail, timer is existed!\r\n", 1, timer_id);
        BT_TIMER_EXT_MUTEX_UNLOCK();
        return BT_TIMER_EXT_STATUS_FAIL;
    }
    ...
    bt_timer_ext = bt_timer_ext_find_internal(0);
...
bt_timer_ext_status_t bt_timer_ext_stop(uint32_t timer_id)
{
    ...
    BT_TIMER_EXT_MUTEX_LOCK();
    bt_timer_ext_t *temp = bt_timer_ext_find_internal(timer_id);
    if (temp == NULL) {
...
{
    BT_TIMER_EXT_MUTEX_LOCK();
    bt_timer_ext_t *timer = bt_timer_ext_find_internal(timer_id);
    BT_TIMER_EXT_MUTEX_UNLOCK();
    return timer;
}
