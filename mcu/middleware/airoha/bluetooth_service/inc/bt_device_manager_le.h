...
/**
 *  @brief LE device manager sync info structure.
 */
BT_PACKED(
typedef struct {
    bt_device_manager_le_bonded_info_t  bonded_info;        /**< LE device bonded info in the packet of AWS sync. */
    bt_gap_le_srv_link_t                link_type;          /**< LE device link type in the packet of AWS sync. */
}) bt_device_manager_le_sync_packet_t;
 ...
 * @param[in] pairing_config         is a pointer to the #bt_gap_le_smp_pairing_config_t structure.
...
/**
 * @brief     This function gets the bonded information by connection handle.
 * @param[in] connection_handle   is the connection handle of the remote LE device.
 * @return    The LE bonding info of the remote device.
 */
...
/**
 * @brief     This function updates all devices to the resolving list again.
