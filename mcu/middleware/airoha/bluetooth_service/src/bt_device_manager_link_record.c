...
static void         bt_device_manager_link_record_sync_info_partner()
{
    ...
    if (NULL == dm_report) {
        bt_dmgr_report_id("[BT_DM][LINK_RECORD][E] Send AWS packet failed can't allocat buffer.", 0);
        return;
    }
...
static void         bt_device_manager_link_record_delete(bt_device_manager_link_t link_type, bt_bd_addr_t *address)
{
    ...
    if (0xFF == found_index) {
        bt_dmgr_report_id("[BT_DM][LINK_RECORD][E] Can't find disconnected device ", 0);
        return;
    }
    ...
    g_dm_link_record_cnt.connected_num--;
    bt_dmgr_report_id("[BT_DM][LINK_RECORD][I] Device:0x%x type:0x%x deleted, now num:%d", 3, *(uint32_t*)address, link_type, g_dm_link_record_cnt.connected_num);
}
...
static bt_status_t  bt_device_manager_link_record_gap_event_callback(bt_msg_type_t msg, bt_status_t status, void *buff)
{
    ...
    if (NULL == conn_info || conn_info->link_type != BT_GAP_LE_SRV_LINK_TYPE_LE_AUDIO) {
        bt_dmgr_report_id("[BT_DM][LINK_RECORD][I] Get le audio conn info fail 0x%x", 1, conn_info);
        ...
        bt_dmgr_report_id("[BT_DM][LINK_RECORD][W] Can't find bond info", 0);
