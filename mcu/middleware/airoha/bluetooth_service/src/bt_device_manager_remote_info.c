...
typedef struct {
    ...
    char remote_name[BT_GAP_MAX_DEVICE_NAME_LENGTH + 1];
...
static bt_status_t bt_device_manager_remote_info_update(bt_bd_addr_t addr, bt_device_manager_remote_info_mask_t type, void *data)
{
    ...
    if (0 == BT_DEVICE_MANAGER_MAX_PAIR_NUM || !memcmp(&temp_addr, (uint8_t *)addr, sizeof(bt_bd_addr_t))) {
...
static bt_status_t bt_device_manager_remote_non_flush_info_update(bt_bd_addr_t addr, bt_dm_remote_non_flush_info_mask_t type, void *data)
{
    ...
    if (NULL == data) {
        bt_dmgr_report_id("[BT_DM][REMOTE][E] Update non flush info buffer is null", 0);
        return BT_STATUS_FAIL;
    }
    bt_dmgr_report_id("[BT_DM][REMOTE][I] Non flush info update type 0x%x", 1, type);
    bt_device_manager_db_mutex_take();
    ...
    switch (type) {
        case BT_DM_REMOTE_NON_FLUSH_INFO_MASK_NAME:
            ...
            g_dm_remote_non_flush_list_cnt.item[find_index].info_valid_set |= BT_DM_REMOTE_NON_FLUSH_INFO_MASK_NAME;
            break;
        case BT_DM_REMOTE_NON_FLUSH_INFO_MASK_VERSION:
            ...
            g_dm_remote_non_flush_list_cnt.item[find_index].info_valid_set |= BT_DM_REMOTE_NON_FLUSH_INFO_MASK_VERSION;
            break;
        case BT_DM_REMOTE_NON_FLUSH_INFO_MASK_PNP:
            ...
            g_dm_remote_non_flush_list_cnt.item[find_index].info_valid_set |= BT_DM_REMOTE_NON_FLUSH_INFO_MASK_PNP;
            break;
        case BT_DM_REMOTE_NON_FLUSH_INFO_MASK_COD:
            g_dm_remote_non_flush_list_cnt.item[find_index].cod = (uint32_t)data;
            g_dm_remote_non_flush_list_cnt.item[find_index].info_valid_set |= BT_DM_REMOTE_NON_FLUSH_INFO_MASK_COD;
            break;
        default:
            bt_dmgr_report_id("[BT_DM][REMOTE][E] Update non flush info with invalid type:0x%x", 1, type);
            bt_device_manager_db_mutex_give();
            return BT_STATUS_FAIL;
    }
    if (BT_STATUS_SUCCESS == bt_device_manager_remote_info_find(addr, BT_DEVICE_MANAGER_REMOTE_INFO_MASK_PAIRED, NULL) && 
            (0x0F == (g_dm_remote_non_flush_list_cnt.item[find_index].info_valid_set & 0x0F) ||
             BT_DM_REMOTE_NON_FLUSH_INFO_MASK_PNP & (g_dm_remote_non_flush_list_cnt.item[find_index].info_valid_set))) {
...
static bt_status_t bt_device_manager_remote_non_flush_info_find(bt_bd_addr_t addr, bt_dm_remote_non_flush_info_mask_t type, void *data, uint16_t data_length)
{
    ...
    if (NULL == data || 0 == data_length) {
        bt_dmgr_report_id("[BT_DM][REMOTE][E] Find non flush info buffer is null", 0);
        return BT_STATUS_FAIL;
    }
    ...
    switch (type) {
        ...
        case BT_DM_REMOTE_NON_FLUSH_INFO_MASK_COD:
            ...
            *(uint32_t *)data = g_dm_remote_non_flush_list_cnt.item[index].cod;
            break;
        default:
            bt_dmgr_report_id("[BT_DM][REMOTE][E] Find non flush info with invalid type:0x%x", 1, type);
            return BT_STATUS_FAIL;
    }
    return BT_STATUS_SUCCESS;
}
...
{
    bt_device_manager_db_remote_paired_info_internal_t profile_info;
    if (BT_STATUS_SUCCESS != bt_device_manager_remote_info_find(addr, BT_DEVICE_MANAGER_REMOTE_INFO_MASK_PAIRED, &profile_info)) {
        return BT_STATUS_FAIL;
    }
