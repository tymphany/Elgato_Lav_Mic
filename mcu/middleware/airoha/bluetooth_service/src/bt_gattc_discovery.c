...
static bt_status_t bt_gattc_discovery_add_user_triggered(uint16_t conn_handle, bt_gattc_discovery_user_t user)
{
    LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_add_user_triggered, conn_handle: 0x%04x, user: %d", 2, conn_handle, user);
    ...
    if (user_add == NULL) {
        LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_add_user_triggered: failed malloc size: %d", 1, sizeof(user_triggered_t));
    ...
    LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_add_user_triggered, user_triggered_count: %d", 1, cntx->user_triggered_count);
    return BT_STATUS_SUCCESS;
}
...
static bt_gattc_discovery_user_t bt_gattc_discovery_get_user_triggered(uint16_t conn_handle)
{
    ...
    bt_gattc_discovery_context_t *cntx = bt_gattc_discovery_get_cntx_by_handle(conn_handle);
    LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_get_user_triggered, conn_handle: 0x%04x", 1, conn_handle);
...
static bt_status_t  bt_gattc_discovery_save_connection_info(void *buff)
{
    ...
    for (i = 0; i < BT_LE_CONNECTION_NUM; i++) {
        ...
        if (BT_HANDLE_INVALID == g_discovery_context[i].conn_handle) {
            LOG_MSGID_I(BT_GATTC, "first connect, connection handle=0x%04x", 1, conn_ind->connection_handle);
            ...
            LOG_MSGID_I(BT_GATTC, "reconnect, connection handle=0x%04x", 1, g_discovery_context[i].conn_handle);
            break;
        }
    }
...
static bt_status_t bt_gattc_discovery_delete_connection_info(void *buff)
{
    ...
    for (i = 0; i < BT_LE_CONNECTION_NUM ; i++) {
        if (disconn_ind->connection_handle == g_discovery_context[i].conn_handle) {
            LOG_MSGID_I(BT_GATTC, "disconnection context:%02x, connection_handle = %02x", 2, &g_discovery_context[i], g_discovery_context[i].conn_handle);
...
bt_gattc_discovery_context_t *bt_gattc_discovery_get_cntx_by_handle(uint16_t conn_handle)
{
    ...
    if (i == BT_LE_CONNECTION_NUM) {
        LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_get_cntx_by_handle, not connected!", 0);
    }
    return NULL;
}
...
static bt_gattc_discovery_buffer_size_t bt_gattc_discovery_get_buffer_size(bt_gattc_discovery_service_t *user_buffer)
{
    ...
    LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_get_buffer_size, size: %d, buffer: 0x%04x", 2, buffer_size.all_size, user_buffer);
    return buffer_size;
}
...
static void bt_gattc_discovery_copy_charc_buffer(bt_gattc_discovery_characteristic_t *dest, bt_gattc_discovery_characteristic_t *src, uint8_t charac_count)
{
    ...
    for (i = 0; i < charac_count; i++) {
        LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_copy_charc_buffer, chara_index: %d, descr_count_found:%d", 2, i, src[i].descr_count_found);
...
static void bt_gattc_discovery_copy_inc_serv_buffer(bt_gattc_discovery_included_service_t *dest, bt_gattc_discovery_included_service_t *src, uint8_t inc_serv_count)
{
    ...
    for (i = 0; i < inc_serv_count; i++) {
        LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_copy_inc_serv_buffer, inc_serv_index: %d, char_count_found:%d", 2, i, src[i].char_count_found);
...
static void bt_gattc_discovery_copy_discovery_buffer(bt_gattc_discovery_service_t *user_buffer, bt_gattc_discovery_service_t *discovery_buffer)
{
    ...
    LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_copy_discovery_buffer, chara_count_found: %d, inc_serv_count_found:%d", 2, chara_count_found, inc_serv_count_found);
...
static uint8_t bt_gattc_discovery_find_next_resigtered_serv_index_by_user(uint8_t serv_index_start, bt_gattc_discovery_user_t user)
{
    ...
    LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_find_next_resigtered_serv_index_by_user, start_serv_index:%d, user:%d", 2, serv_index_start,user);
...
static void bt_gattc_discovery_change_discovery_state(bt_gattc_discovery_context_t *cntx, bt_gattc_conn_discovery_state_t state)
{
    ...
    LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_change_discovery_state, conn_handle:0x%04x, state:%d", 2, cntx->conn_handle, state);
...
static bt_status_t bt_gattc_discovery_copy_service_uuid(ble_uuid_t *dest, ble_uuid_t *src)
{
    ...
    if (BLE_UUID_TYPE_16BIT == dest->type) {
        ...
        LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_copy_service_uuid, uuid16 = 0x%04x", 1, dest->uuid.uuid16);
    } else if (BLE_UUID_TYPE_32BIT == dest->type) {
        /* uuid is 32 bit. */
        dest->uuid.uuid32 = src->uuid.uuid32;
        LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_copy_service_uuid, uuid32 = 0x%04x", 1, dest->uuid.uuid32);
    } else if (BLE_UUID_TYPE_128BIT == dest->type) {
        /* uuid is 128 bit. */
        bt_utils_memcpy(dest->uuid.uuid, src->uuid.uuid, 16);
        for (int i = 15; i >= 0; i--) {
            LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_copy_service_uuid, uuid128[%d] = 0x%04x", 2, i, dest->uuid.uuid[i]);
        }
    } else {
        LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_copy_service_uuid error uuid! \r\n", 0);
        return BT_STATUS_FAIL;
    }
...
static void bt_gattc_discovery_parse_primary_service(bt_gattc_find_by_type_value_rsp_t *rsp, bt_gattc_discovery_context_t *cntx)
{
    ...
    for (uint8_t i = 0; i < num_of_data; i++) {
        LOG_MSGID_I(BT_GATTC, "serv_num: %d, starting_handle = 0x%04x, end_handle = 0x%04x \r\n", 3, i, *(uint16_t*)(attribute_data_list + i * 4), *(uint16_t*)(attribute_data_list + i * 4 + 2));
        LOG_MSGID_I(BT_GATTC, "multi instance service count: %d ", 1, multi_instance->service_count);
...
static void bt_gattc_discovery_parse_characteristic(bt_gattc_read_by_type_rsp_t *data, bt_gattc_discovery_context_t *cntx)
{
    ...
    if (characteristic_count < (*p_char_count_found + num_of_data)) {
        ...
        LOG_MSGID_I(BT_GATTC, "ble_gattc_parse_characteristic char_count not enough, cut char_num to: %d", 1, num_of_data);
    }
...
static void bt_gattc_discovery_descriptors_cnf(bt_status_t status, void *buff)
{
    ...
    switch (status) {
        case BT_STATUS_SUCCESS: {
            LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_descriptors_cnf: succsee:index = %d", 1, cntx->discovering_char_index);
        ...
        case BT_ATT_ERRCODE_CONTINUE: {
            LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_descriptors_cnf: continue:index = %d", 1, cntx->discovering_char_index);
            if (chara) {
                bt_gattc_discovery_parse_descriptor(p_event_data, chara);
            }
            break;
        ...
        case BT_ATT_ERRCODE_ATTRIBUTE_NOT_FOUND: {
            LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_descriptors_cnf: fail:index = %d", 1, cntx->discovering_char_index);
            cntx->discovering_char_index++;
            bt_gattc_discovery_all_descriptor(cntx, cntx->discovering_char_index);
            break;
        }
...
static void bt_gattc_discovery_get_device_addr(bt_bd_addr_t *addr, uint16_t conn_handle)
{
    ...
    if (conn_info->peer_addr.type == BT_ADDR_PUBLIC || conn_info->peer_addr.type == BT_ADDR_RANDOM) {
        bonded_info = bt_device_manager_le_get_bonding_info_by_addr(&conn_info->peer_addr.addr);
        if (bt_utils_memcmp(bonded_info->identity_addr.address.addr, null_address, sizeof(bt_bd_addr_t)) != 0){
            ...
            bt_utils_memcpy(addr, conn_info->peer_addr.addr, sizeof(bt_bd_addr_t));
        }
...
bt_gattc_discovery_status_t bt_gattc_discovery_start_internal(bt_gattc_discovery_context_t *cntx)
{
    ...
    if (cntx->discovery_buffer->service_uuid.uuid.uuid16 == 0x00) {
        bt_gattc_discovery_copy_service_uuid(&cntx->discovery_buffer->service_uuid, &serv_registered.service_uuid);
    }
    ...
    if (ret == BT_GATT_CACHE_STATUS_NOT_COMPLETE || ret == BT_GATT_CACHE_STATUS_SUCCESS) {
        ...
        timer_ret = bt_timer_ext_start(BT_GATTC_DISCOVERY_TIMER_ID, (uint32_t)cntx->conn_handle, 0, bt_gattc_discovery_timer_cb);
...
static bt_status_t bt_gattc_discovery_all_descriptor(bt_gattc_discovery_context_t *cntx, uint8_t index)
{
    ...
    if (index < char_num) {
        ...
        if (cntx->conn_discovery_state == BT_GATTC_CONN_DISCOVERY_STATE_DISCOVERY_INCLUDED_DESCR) {
            LOG_MSGID_I(BT_GATTC, "[Gattc]ble_gattc_find_all_descriptor_of_char done, discover included service idx:%d, completed!", 1, cntx->discovering_inc_serv_index);
            bt_gattc_discovery_included_service_completed(cntx);
        } else {
            LOG_MSGID_I(BT_GATTC, "[Gattc]ble_gattc_find_all_descriptor_of_char done, discover primary service idx:%d completed!", 1, cntx->discovering_serv_index);
            bt_gattc_discovery_change_discovery_state(cntx,  BT_GATTC_CONN_DISCOVERY_STATE_READY);
            bt_gattc_discovery_result_evt_trigger(cntx, true, 0);
            // bt_gattc_discovery_primary_service_completed(cntx);
        }
...
static void bt_gattc_discovery_included_service_completed(bt_gattc_discovery_context_t *cntx)
{
    ...
    if (cntx->discovering_inc_serv_index < cntx->discovery_buffer->included_srv_count_found) {
        LOG_MSGID_I(BT_GATTC, "[Gattc]bt_gattc_discovery_included_service_completed, start discover next included service idx:%d!", 1, cntx->discovering_inc_serv_index);
        bt_gattc_discovery_change_discovery_state(cntx,  BT_GATTC_CONN_DISCOVERY_STATE_DISCOVERY_INCLUDED_CHARC);
        bt_gattc_discover_characteristic(cntx);
    } else {
        LOG_MSGID_I(BT_GATTC, "[Gattc]bt_gattc_discovery_included_service_completed, all include services discover done, next dicover primary charac!", 0);
        bt_gattc_discovery_change_discovery_state(cntx,  BT_GATTC_CONN_DISCOVERY_STATE_DISCOVERY_PRIMERY_CHARC);
        bt_gattc_discover_characteristic(cntx);
    }
}
...
static void bt_gattc_discovery_primary_service_completed(bt_gattc_discovery_context_t *cntx)
{
    ...
    if (multi_instance->service_count != 0) {
        ...
        if (BT_STATUS_SUCCESS != bt_gattc_discovery_get_discovery_buffer(&cntx->discovery_buffer, serv_registered.service_info)) {
            LOG_MSGID_I(BT_GATTC, "[Gattc]bt_gattc_discovery_primary_service_completed, get buffer fail!", 0);
            return;
        }
        ...
        if (multi_instance->service_count == 0xff) { //use gatt caches
            ...
            if (ret == BT_GATT_CACHE_STATUS_NOT_COMPLETE || ret == BT_GATT_CACHE_STATUS_SUCCESS) {
                ...
                assert(0 && "[GATTC] get mutil service cache fail!, ");
            }
        }
    ...
    if (serv_index_next != 0xff) {
        ...
        if (bt_gattc_discovery_start_internal(cntx) != BT_STATUS_SUCCESS) {
            LOG_MSGID_E(BT_GATTC, "bt_gattc_discovery_primary_service_completed, assert!!", 0);
...
bt_gattc_discovery_status_t bt_gattc_discovery_register_service(bt_gattc_discovery_user_t user, bt_gattc_discovery_user_data_t *user_data)
{
    ...
    if (g_num_of_handlers_reg < BT_GATTC_DISCOVERY_MAX_USERS) {
        if (BT_STATUS_SUCCESS != bt_gattc_discovery_copy_service_uuid(&(g_registered_handlers[g_num_of_handlers_reg].service_uuid), &(user_data->uuid))){
            LOG_MSGID_I(BT_GATTC, "bt_gattc_discovery_registered_handler_set error uuid!", 0);
