...
uint32_t        bt_iot_device_white_list_get_iot_id(bt_iot_device_identify_information_t *identify_info)
{
    ...
    bt_dmgr_report_id("[BT_DM][IOT] bt_iot_device_white_list_get_iot_id, list_count:%d", 1, list_count);
    ...
    for (uint32_t index = 1; index < list_count; index++) {
        ...
        if (g_all_iot_case_list[index].cod != BT_IOT_DEVICE_INVALIDE_COD && g_all_iot_case_list[index].cod == identify_info->cod) {
            temp_matched += 1;
        }
    ...
    bt_dmgr_report_id("[BT_DM][IOT] bt_iot_device_white_list_get_iot_id, pnp_info.product_id:0x%x, pnp_info.vender_id:0x%x, pnp_info.version:0x%x, version_info.manufacturer_id:%d, version_info.version:%d, version_info.subversion:%d", 6,
                      identify_info->pnp_info.product_id, identify_info->pnp_info.vender_id, identify_info->pnp_info.version, identify_info->version_info.manufacturer_id, identify_info->version_info.version, identify_info->version_info.subversion);
    bt_dmgr_report_id("[BT_DM][IOT] bt_iot_device_white_list_get_iot_id, matched_item:%d, found_index:%d", 2, matched_item, found_index);
    return found_index;
}
...
{
    bt_dmgr_report_id("[BT_DM][IOT] Get iot list, addr:%x, ret_list:%x", 2, addr, ret_list);
    if (NULL == addr || NULL == ret_list) {
        return BT_STATUS_FAIL;
    }
    uint32_t iot_id = bt_device_manager_remote_find_iot_id((void *)addr);
    uint32_t list_count = sizeof(g_all_iot_case_list) / sizeof(bt_iot_device_all_iot_case_t);
    bt_dmgr_report_id("[BT_DM][IOT] Get iot list, index:%d, list_count:%d", 2, iot_id, list_count);
...
{
    bt_dmgr_report_id("[BT_DM][IOT] Get iot list, addr:%x", 1, addr);
    if (NULL == addr) {
        return NULL;
    }
    uint32_t iot_id = bt_device_manager_remote_find_iot_id((void *)addr);
    uint32_t list_count = sizeof(g_all_iot_case_list) / sizeof(bt_iot_device_all_iot_case_t);
    bt_dmgr_report_id("[BT_DM][IOT] Get iot list 2, index:%d, list_count:%d", 2, iot_id, list_count);
    if (iot_id >= list_count) {
        return NULL;
    }
