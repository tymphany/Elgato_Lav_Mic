        ...
        if ((handle != BT_HANDLE_INVALID) && (temp_cntx)) {
            ...
            if (rw == BT_GATTS_CALLBACK_WRITE) {
                ...
                if ((false == ble_air_has_real_connected()) || ((0 == *(uint16_t *)data) && (true == temp_cntx->is_real_connected))) {
                    ...
                    LOG_MSGID_I(AIR, "[AIR][LE] enable air cccd fail, is real connected = %d", 1, ble_air_has_real_connected());
                    return 0;
                }
 ...
 */
#if defined (AIR_BLE_ULTRA_LOW_LATENCY_ENABLE) || !defined (AIR_BLE_AUDIO_DONGLE_ENABLE)
...
#if (defined (AIR_BLE_AUDIO_DONGLE_ENABLE) || defined (AIR_BLE_ULTRA_LOW_LATENCY_ENABLE))
#ifdef AIR_BLE_ULTRA_LOW_LATENCY_ENABLE
    if ((conn_handle != BT_HANDLE_INVALID) && (buffer_t) && (buffer_t->is_real_connected) && (BT_ULL_ROLE_CLIENT == bt_ull_le_srv_get_role())) {
        ...
        BLEAIR_MUTEX_UNLOCK();
        return read_size;
    } else if ((conn_handle != BT_HANDLE_INVALID) && (buffer_t) && (BT_ULL_ROLE_SERVER == bt_ull_le_srv_get_role())) {
        if (buffer_t->receive_buffer_length > size) {
            read_size = size;
        } else {
            read_size = buffer_t->receive_buffer_length;
