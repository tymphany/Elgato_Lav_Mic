    ...
    if (info && info->length && (BT_AWS_MCE_ROLE_PARTNER == info->role)) {
        ...
        if (context_num > 0) {
            ...
            if (BT_STATUS_SUCCESS == gatt_over_bredr_air_find_free_conn_handle(&i)) {
                ...
                LOG_MSGID_I(GOBEAIR, "[GATT][BREDR][RHO] rho update new hci handle = %02x", 1, (uint16_t)info->profile_info->gatt_handle);
                ...
                if (g_gobe_air_cntx[i].is_real_connected) {
...
static uint8_t bt_gobe_service_desc_list[] = {
