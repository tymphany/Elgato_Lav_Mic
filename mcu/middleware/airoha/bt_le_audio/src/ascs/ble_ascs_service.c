...
typedef struct {
    ...
    uint16_t size;
    uint8_t *data;
} ble_ascs_ase_control_point_prepare_write_t;

ble_ascs_ase_control_point_prepare_write_t g_ascs_ase_control_point_prepare_write;

static void ble_ascs_ase_clear_prepare_write_data(void)
{
    g_ascs_ase_control_point_prepare_write.handle = BT_HANDLE_INVALID;
    g_ascs_ase_control_point_prepare_write.size = 0;

    if (NULL != g_ascs_ase_control_point_prepare_write.data) {
        le_audio_free(g_ascs_ase_control_point_prepare_write.data);
        g_ascs_ase_control_point_prepare_write.data = NULL;
    }
}

static uint32_t ble_ascs_ase_update_prepare_write_data(bt_handle_t handle, void *data, uint16_t size, uint16_t offset)
{
    uint8_t *temp;

    LE_AUDIO_LOG_I(BT_LE_AUDIO_ASCS_027, 5,
        handle, size, offset, g_ascs_ase_control_point_prepare_write.data, g_ascs_ase_control_point_prepare_write.size);

    if (0 == offset) {
        if (NULL != g_ascs_ase_control_point_prepare_write.data) {
            ble_ascs_ase_clear_prepare_write_data();
        }

        g_ascs_ase_control_point_prepare_write.handle = handle;
        g_ascs_ase_control_point_prepare_write.size = size;
        g_ascs_ase_control_point_prepare_write.data = le_audio_malloc(size);
        memcpy(g_ascs_ase_control_point_prepare_write.data, data, size);

    } else {
        if (NULL == g_ascs_ase_control_point_prepare_write.data ||
            offset != g_ascs_ase_control_point_prepare_write.size) {
            return 0;
        }

        if (NULL != (temp = le_audio_malloc(g_ascs_ase_control_point_prepare_write.size + size))) {
            memcpy(temp, g_ascs_ase_control_point_prepare_write.data, g_ascs_ase_control_point_prepare_write.size);
            memcpy(&temp[g_ascs_ase_control_point_prepare_write.size], data, size);
            g_ascs_ase_control_point_prepare_write.size = g_ascs_ase_control_point_prepare_write.size + size;
        } else {
            g_ascs_ase_control_point_prepare_write.size = 0;
        }

        le_audio_free(g_ascs_ase_control_point_prepare_write.data);
        g_ascs_ase_control_point_prepare_write.data = temp;
    }
...
static uint32_t ble_ascs_ase_control_point_value_callback(const uint8_t rw, uint16_t handle, void *data, uint16_t size, uint16_t offset)
{
    ...
    if (handle != BT_HANDLE_INVALID) {
        if (rw == BT_GATTS_CALLBACK_WRITE) {
            ...
            return ble_ascs_ase_update_prepare_write_data(handle, data, size, offset);

        } else if (rw == BT_GATTS_CALLBACK_EXECUTE_WRITE_WRITE) {
            if (NULL != g_ascs_ase_control_point_prepare_write.data) {
                ble_ascs_gatt_request_handler(BLE_ASCS_WRITE_ASE_CONTROL_POINT, handle, BLE_ASCS_CHARC_NORMAL,
                                                g_ascs_ase_control_point_prepare_write.data,
                                                g_ascs_ase_control_point_prepare_write.size, offset);
                ble_ascs_ase_clear_prepare_write_data();
        ...
        } else if (rw == BT_GATTS_CALLBACK_EXECUTE_WRITE_CANCEL) {
            ble_ascs_ase_clear_prepare_write_data();
            return BT_STATUS_SUCCESS;
        }
    }
...
ble_ascs_default_qos_parameters_t *ble_ascs_get_qos_preference(uint8_t target_latency, uint8_t frame_duration, uint16_t configed_octets_per_codec_frame)
{
    ...
    if (frame_duration == FRAME_DURATIONS_10_MS) {
        ...
        return (target_latency == TARGET_HIGHER_RELIABILITY ? (&g_default_qos_high_reliability_10ms[preference_idx]) : (&g_default_qos_low_latency_10ms[preference_idx]));
        ...
        return (target_latency == TARGET_HIGHER_RELIABILITY ? (&g_default_qos_high_reliability_7p5ms[preference_idx]) : (&g_default_qos_low_latency_7p5ms[preference_idx]));
    }
