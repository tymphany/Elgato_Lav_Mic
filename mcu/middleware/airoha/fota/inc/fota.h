 ...
 * File: fota.h
 *
 * Description: This file declares some FOTA SDK APIs.
 *
 ...
 *@addtogroup FOTA
 *@{
 * This section introduces the FOTA interface APIs including terms and acronyms, supported features, details on how to use this interface, FOTA function groups, enums, structures and functions.
 * This interface writes into a pre-defined register flag. The bootloader enters the update process based on the value written in the flag. The return error code indicates if the flag is written successfully or not.
 * After the update is complete, the UA automatically resets the register flag.
 * Note, it is recommended to call the reboot API after calling the FOTA interface, because fota_trigger_update() does not contain reboot functionality.
 * There is no dedicated FOTA read or write SDK API, it is suggested to use HAL flash SDK API to write data to specified address of flash.
...
/**
 * @brief
 * Get the upgrade result for the last FOTA.
 * @param[out] result The upgrade result, TRUE: succeed, FALSE: fail.
 */
void fota_get_upgrade_result(bool *result);


/**
 * @brief
 * Check if the upgrade flag is set or not.
 * @param[out] upgrade_flag_set TRUE: set, FALSE: not set.
 */
void fota_check_upgrade_flag(bool *upgrade_flag_set);


/**
 * @brief
 * Initialize the flash driver and configurate the flash for FOTA. It should be
 * called before any other FOTA API is called.
 */
...
/**
 * @brief
 * Check whether the NVDM incompatible flag is set.
 * @return TRUE, it is set; FALSE, it is not set.
 */
...
/**
 * @brief
 * Clear the NVDM incompatible flag.
 * @return FOTA_ERRCODE_SUCCESS, Success; Otherwise, Fail.
 */
...
/**
 * @brief
 * Get the bootloader FOTA update result information and clear it after that.
 * @param[out] pInfo the FOTA update result information.
...
/**
 * @brief
 * Check whether the FOTA is executed in the bootloader.
 * @return If FOTA is executed in the bootloader, true. Otherwise, false.
 */
