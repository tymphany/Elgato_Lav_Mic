 ...
 * File: fota.c
 *
 * Description: This file defines some FOTA SDK APIs.
 *
...
{
    return FOTA_ERRCODE_SUCCESS == fota_upgrade_flag_set() ? FOTA_TRIGGER_SUCCESS : FOTA_TRIGGER_FAIL;
}
...
void fota_get_upgrade_result(bool *result)
{
    ...
    if (result) {
        ...
        if (FOTA_ERRCODE_SUCCESS == ret && 0x0101 == state) {
            *result = TRUE;
            return;
        }
