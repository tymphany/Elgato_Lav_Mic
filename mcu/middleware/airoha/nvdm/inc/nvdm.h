 ...
 *           uint32_t size;
 *           uint8_t buffer[64] = STA_IPADDR_DEFAULT_VALUE;
 *           char group_name[64];
 ...
 *           }
 *
 *           status = nvdm_write_data_item("STA", "IpAddr", NVDM_DATA_ITEM_TYPE_STRING, buffer, sizeof(STA_IPADDR_DEFAULT_VALUE));
 *           if (status != NVDM_STATUS_OK) {
 *               //Error handler;
 *           }
 *
 ...
 *                   size = sizeof(buffer);
 *                   status = nvdm_read_data_item(group_name, data_item_name, buffer, &size);
 *                   if (status != NVDM_STATUS_OK) {
 *                       //Error handler;
 *                   }
 *                   //Handle the value;
 ...
 *    @endcode
 *
 * - NVDM support allows the user to manage data items (i.e. read, write, and delete) and query APIs.
 *   These operations are mutually exclusive. Therefore, if an operation is occurring, the next operation
 *   is prevented from starting until the first operation is complete.
 *
...
/** @brief This enum defines the action of NVDM reset API. */
typedef enum {
    NVDM_ACTION_KEEP = 0x0,              /**< Keeps the data items specified by the user. */
    NVDM_ACTION_DELETE = 0x1,            /**< Deletes the data items specified by the user. */
} nvdm_reset_action_t;


/** @brief This enum defines the garbage collection types supported in NVDM. */
typedef enum {
#ifdef SYSTEM_DAEMON_TASK_ENABLE
    NVDM_GC_IN_DAEMON = 0x0,             /**< Garbage collection in a dedicated daemon task. */
#endif
    NVDM_GC_IN_CURR = 0x1,               /**< Complete garbage collection in the context of the current task. */
} nvdm_gc_type_t;


/** @brief This enum defines return type of NVDM APIs. */
typedef enum {
    NVDM_STATUS_ITEM_MERGED_BY_BW = -7,  /**< The previous non-blocking write data is merged by a new blocking write.*/
    NVDM_STATUS_ITEM_MERGED_BY_NBW = -6, /**< The previous non-blocking write data is merged by a new non-blocking write. */
    ...
    NVDM_STATUS_INCORRECT_CHECKSUM = -2, /**< The NVDM found a checksum error when reading the data item or the buffer has changed when writing the data item. */
...
/** @brief This structure defines the spatial information of a single NVDM partition. */
typedef struct {
    uint32_t free;        /**< The number of bytes in the entire NVDM partition that have not been written. */
    uint32_t dirty;       /**< The number of bytes in the entire NVDM partition that have been written or marked as invalid. */
    uint32_t valid;       /**< The number of valid bytes that can be obtained by #nvdm_read_data_item in the entire NVDM partition. */
    uint32_t capacity;    /**< The actual number of bytes that can be stored in the entire NVDM partition. */
    uint32_t phy_size;    /**< The actual number of bytes occupied by the entire NVDM. */
} nvdm_space_info_t;


/** @brief This structure defines a single data item input into the NVDM reset API. */
typedef struct {
    const char *group_name;              /**< The group name of the data item, such as "Bluetooth". */
    const char *item_name;               /**< The name of the data item, such as "address" of the "Bluetooth". */
} nvdm_item_id_t;
...
/**
 * @brief     This function is used to write or update a data item in flash.
 *            If there are existing data items in flash, the NVDM invalidates old values and replaces them with new ones.
 ...
 *            User should check the return status to verify whether the data is successfully written to flash.
 ...
 *                #NVDM_STATUS_INCORRECT_CHECKSUM, if some data in the buffer has been modified while the data is being written. \n
 * @note      Call this API in tasks or function callbacks, but not in the interrupt handlers.
 *            NVDM will use the buffer passed in by the user for checksum calculation, so the caller must ensure that the data will not be modified during the writing process!
 *            When using this API, the user does not need to specify the NVDM partition where the data will be stored, the driver will use the existing data to determine which partition to use. The 0 partition is used by default.
 */
...
/**
 * @brief     This function is used to create, write or update a data item in flash.
 *            If there are existing data items in flash, the NVDM invalidates old values and replaces them with new ones.
 *            No other operation is performed while the data is being written to the flash.
 *            This function returns after the data is completely written to the flash.
 *            User should check the return status to verify whether the data is successfully written to flash.
 * @param[in] partition   indicates which partition the item needs to be stored in.
 * @param[in] type   is the data item type, which defines how the value is displayed.
 * @param[in] group_name   is the group name of the data item, such as "Bluetooth".
 * @param[in] data_item_name   is the name of the data item, such as "address" of the "Bluetooth".
 * @param[in] buffer   is a pointer to the data item's content.
 ...
 * @note      Call this API in tasks or function callbacks, but not in the interrupt handlers.
 *            NVDM will use the buffer passed in by the user for checksum calculation, so the caller must ensure that the data will not be modified during the writing process!
 *            When using this API, the user need to specify the NVDM partition where the data will be stored.
 */
...
/**
 * @brief     This function deletes the data item in the NVDM specified by the user.
 * @param[in] array   is the array pointer the user wants to operate.
 * @param[in] len   is the items count of the input parameter array.
 * @param[in] action   is the action the user wants to take on the data element.
 * @return
 *                #NVDM_STATUS_OK, if the operation completed successfully. \n
 *                #NVDM_STATUS_ERROR, if an unknown error occurred. \n
 *                #NVDM_STATUS_INVALID_PARAMETER, if the parameter is invalid. \n
 * @note      This function will block other NVDM APIs.
 *            It is generally used to restore the factory settings and is called before the system scheduler is started.
 */
nvdm_status_t nvdm_reset_items(const nvdm_item_id_t *array, uint32_t len, nvdm_reset_action_t action);


/**
 * @brief     Through this API, the user can obtain the current nvdm space information,
 *            and use the returned information to determine the recycling parameters passed to the #nvdm_trigger_garbage_collection.
 *            User should not call #nvdm_query_space_information() before #nvdm_init().
 * @param[in] partition   indicates which NVDM Partition information needs to be obtained.
 * @param[in, out] info   A pointer to a structure passed by the user that can store NVDM space information.
 *                        If num is not 1, this pointer may point to an array.
 * @return
 *                #NVDM_STATUS_OK, if the operation completed successfully. \n
 *                #NVDM_STATUS_INVALID_PARAMETER, if the input parameters do not meet expectations. \n
 *                #NVDM_STATUS_ERROR, if an unknown error occurred. \n
 */
...
/**
 * @brief     This function can be used to actively trigger garbage collection to minimize
 *            the side effects caused by passively triggering garbage collection when writing a nvdm item.
 *            User should not call #nvdm_trigger_garbage_collection() before #nvdm_init().
 * @param[in] partition   indicates the partition from which garbage will be collected.
 * @param[in] type   indicates the type of garbage collection supported by NVDM driver.
 * @param[in] gc_bytes   indicates the amount of data that the user wants to reclaim.
 *                       In particular, 0 means that all dirty data will be reclaimed.
 * @return
 *                #NVDM_STATUS_OK, if the operation completed successfully. \n
 *                #NVDM_STATUS_INVALID_PARAMETER, if the type is not expected or the driver cannot reclaim the number of bytes specified by the user. \n
 *                #NVDM_STATUS_ERROR, if an unknown error occurred. \n
 * @note      This function will block NVDM APIs related to writing data.
 *            It is generally called when the system load is not heavy.
 */
