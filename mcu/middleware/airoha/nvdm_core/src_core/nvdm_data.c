...
static int32_t search_data_item_by_name(uint32_t partition, const char *group_name, const char *data_item_name, uint32_t *hasename)
{
    ...
    for (i = 0; i < p_cfg->total_item_count; i++) {
        ...
        if (hash == p_info->hdrs[i].hash_name) {
            peb_read_data(partition, p_info->hdrs[i].pnum, p_info->hdrs[i].offset + DATA_ITEM_HEADER_SIZE, (uint8_t *)str, p_info->hdrs[i].group_name_size);
            if (strcmp(str, group_name)) {
                continue;
            }
            peb_read_data(partition, p_info->hdrs[i].pnum, p_info->hdrs[i].offset + DATA_ITEM_HEADER_SIZE + p_info->hdrs[i].group_name_size, (uint8_t *)str, p_info->hdrs[i].data_item_name_size);
                                  ...
                                  uint32_t *size)
{
    ...
    if (*size < p_info->hdrs[index].value_size) {
        *size = 0;
        nvdm_log_msgid_error(nvdm_119, 3, 'R',
                             p_info->hdrs[index].value_size, *size
                            );
        nvdm_port_protect_mutex_give();
        return NVDM_STATUS_INVALID_PARAMETER;
    }
    ...
    checksum1 = calculate_data_item_checksum(&p_info->hdrs[index],
                                             partition,
                                             p_info->hdrs[index].pnum,
                                             p_info->hdrs[index].offset + DATA_ITEM_HEADER_SIZE);
    offset = p_info->hdrs[index].offset + DATA_ITEM_HEADER_SIZE + p_info->hdrs[index].group_name_size + p_info->hdrs[index].data_item_name_size + p_info->hdrs[index].value_size;
    peb_read_data(partition, p_info->hdrs[index].pnum, offset, (uint8_t *)&checksum2, DATA_ITEM_CHECKSUM_SIZE);
    ...
    offset = p_info->hdrs[index].offset + DATA_ITEM_HEADER_SIZE + p_info->hdrs[index].group_name_size + p_info->hdrs[index].data_item_name_size;
    peb_read_data(partition, p_info->hdrs[index].pnum, offset, buffer, p_info->hdrs[index].value_size);
    buffer += p_info->hdrs[index].value_size;
    memset(buffer, 0, *size - p_info->hdrs[index].value_size);

    *size = p_info->hdrs[index].value_size;
    if (*size == 0) {
...
static int32_t cmp_item_data_in_buffer_and_flash(uint32_t partition, const uint8_t *buffer, const data_item_header_t *header)
{
                                    ...
                                    uint32_t size)
{
    ...
    if (index < 0) {
        ...
        append = 0;
        if (p_info->hdrs[index].value_size == size) {
             ...
             */
            nvdm_port_protect_mutex_take();
            cmp_res = cmp_item_data_in_buffer_and_flash(partition, buffer, &(p_info->hdrs[index]));
            if (cmp_res == 0) {
                nvdm_port_protect_mutex_give();
                nvdm_port_mutex_give();
                return NVDM_STATUS_OK;
    ...
    if (append) {
        ...
        added_size = (int32_t)size - (int32_t)p_info->hdrs[index].value_size;
    }
    ...
    p_old_item_header = &(p_info->hdrs[index]);
    if (append) {
...
static void data_item_delete(uint32_t partition, uint32_t index)
{
    ...
    size = DATA_ITEM_HEADER_SIZE +
           p_info->hdrs[index].group_name_size +
           p_info->hdrs[index].data_item_name_size +
           p_info->hdrs[index].value_size +
           DATA_ITEM_CHECKSUM_SIZE;
    peb_add_drity(partition, p_info->hdrs[index].pnum, size);
    /* free the data item header in memory */
    p_info->hdrs[index].value_size = 0;
...
nvdm_status_t nvdm_delete_group(const char *group_name)
{
    ...
    for (partition = 0; partition < g_ncb.partition_count; partition++) {
        ...
        for (index = 0; index < p_cfg->total_item_count; index++) {
            ...
            if (p_info->hdrs[index].value_size == 0) {
                continue;
            }
            ...
            peb_read_data(partition,
                          p_info->hdrs[index].pnum,
                          p_info->hdrs[index].offset + DATA_ITEM_HEADER_SIZE,
                          (uint8_t *)str, p_info->hdrs[index].group_name_size
                         );
        ...
        for (partition = 0; partition < g_ncb.partition_count; partition++) {
            ...
            for (i = 0; i < p_cfg->total_item_count; i++) {
                ...
                if (is_frist_group_name == true) {
                    ...
                    peb_read_data(partition,
                                  p_info->hdrs[i].pnum,
                                  p_info->hdrs[i].offset + DATA_ITEM_HEADER_SIZE,
                                  (uint8_t *)search_str,
                                  p_info->hdrs[i].group_name_size
                                 );
...
void data_item_scan(uint32_t partition, int32_t pnum)
{
    ...
    while (offset < scan_limit) {
        ...
        if (p_info->hdrs[curr_index].sequence_number == MAX_WRITE_SEQUENCE_NUMBER) {
            /* we find this frist time */
            memcpy(&p_info->hdrs[curr_index], &data_item_header, sizeof(data_item_header_t));
            p_info->curr_item_count++;
            if (p_info->curr_item_count > p_cfg->total_item_count) {
            ...
            if (p_info->hdrs[curr_index].sequence_number < data_item_header.sequence_number) {
                ...
                size = p_info->hdrs[curr_index].value_size +
                       p_info->hdrs[curr_index].group_name_size +
                       p_info->hdrs[curr_index].data_item_name_size +
        ...
        switch (data_item_header.status) {
            ...
            case DATA_ITEM_STATUS_VALID:
                ...
                if ((p_info->hdrs[curr_index].pnum != data_item_header.pnum) ||
                    ...
                    memcpy(&tmp_hdr, &p_info->hdrs[curr_index], DATA_ITEM_HEADER_SIZE);
                    ...
                    nvdm_port_protect_mutex_take();
                    memcpy(&p_info->hdrs[curr_index], &tmp_hdr, DATA_ITEM_HEADER_SIZE);
                    nvdm_port_protect_mutex_give();
                }
                break;
            default:
