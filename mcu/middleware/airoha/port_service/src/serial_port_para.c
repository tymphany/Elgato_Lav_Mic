...
#ifdef MTK_PORT_SERVICE_NVDM_ENABLE
#include "nvdm.h"
...
void serial_port_uart_reset_para(serial_port_setting_t *setting)
{
    ...
    for (device = SERIAL_PORT_DEV_UART_TYPE_BEGIN; device <= SERIAL_PORT_DEV_UART_TYPE_END; device++) {
        ...
        memset(port_setting_uart, 0, sizeof(serial_port_setting_uart_t));
        port_setting_uart->baudrate = UART_DEFAULT_BAUDRATE;
    }
}

extern const serial_port_operation_t g_serial_port_handlers[];
...
serial_port_status_t serial_port_config_dump_dev_number(uint32_t *user_count, serial_port_assign_t *port_assign)
{
    ...
    status = nvdm_read_data_item(NVDM_GROUP_PORT_SETTING, NVDM_PORT_SETTING_ITEM_PORT_ASSIGN, (uint8_t *)serial_port_assign, &setting_size);
...
serial_port_status_t serial_port_config_write_dev_number(const char *user_name, serial_port_dev_t device)
{
    ...
    if (status == NVDM_STATUS_ITEM_NOT_FOUND) {
        ...
        nvdm_write_data_item(NVDM_GROUP_PORT_SETTING, NVDM_PORT_SETTING_ITEM_PORT_ASSIGN, NVDM_DATA_ITEM_TYPE_RAW_DATA, (const uint8_t *)serial_port_assign, sizeof(serial_port_assign_t));
    } else if (status == NVDM_STATUS_OK) {
        found = false;
        setting_count = setting_size / sizeof(serial_port_assign_t);
        for (i = 0; i < setting_count; i++) {
        ...
        if (found == false) {
            ...
            nvdm_write_data_item(NVDM_GROUP_PORT_SETTING, NVDM_PORT_SETTING_ITEM_PORT_ASSIGN, NVDM_DATA_ITEM_TYPE_RAW_DATA, (const uint8_t *)serial_port_assign, setting_size);
        } else {
            serial_port_assign[i].device = device;
            nvdm_write_data_item(NVDM_GROUP_PORT_SETTING, NVDM_PORT_SETTING_ITEM_PORT_ASSIGN, NVDM_DATA_ITEM_TYPE_RAW_DATA, (const uint8_t *)serial_port_assign, setting_size);
        }
...
static void serial_port_config_check_default_setting(serial_port_setting_t *serial_port_setting)
{
    ...
    uint32_t setting_size, original_size;
    nvdm_status_t status;

    /* check whether we can add more user? */
    setting_size = sizeof(serial_port_setting_t);
    status = nvdm_query_data_item_length(NVDM_GROUP_PORT_SETTING, NVDM_PORT_SETTING_ITEM_PORT_CONFIG, &original_size);
    if (status == NVDM_STATUS_OK) {
        if (original_size != setting_size) {
            nvdm_delete_group(NVDM_GROUP_PORT_SETTING);
        }
    }

    status = nvdm_read_data_item(NVDM_GROUP_PORT_SETTING, NVDM_PORT_SETTING_ITEM_PORT_CONFIG, (uint8_t *)serial_port_setting, &setting_size);
    if (status == NVDM_STATUS_ITEM_NOT_FOUND) {
        ...
        setting_size = sizeof(serial_port_setting_t);
        nvdm_write_data_item(NVDM_GROUP_PORT_SETTING, NVDM_PORT_SETTING_ITEM_PORT_CONFIG, NVDM_DATA_ITEM_TYPE_RAW_DATA, (const uint8_t *)serial_port_setting, setting_size);
    } else if (status != NVDM_STATUS_OK) {
        SERIAL_PORT_ASSERT();
    }
...
serial_port_status_t serial_port_config_read_dev_setting(serial_port_dev_t device, serial_port_dev_setting_t *dev_setting)
{
    ...
    return SERIAL_PORT_STATUS_OK;
#else
    dev_setting->port_setting_uart.baudrate = UART_DEFAULT_BAUDRATE;
...
serial_port_status_t serial_port_config_write_dev_setting(serial_port_dev_t device, serial_port_dev_setting_t *dev_setting)
{
    ...
    nvdm_write_data_item(NVDM_GROUP_PORT_SETTING, NVDM_PORT_SETTING_ITEM_PORT_CONFIG, NVDM_DATA_ITEM_TYPE_RAW_DATA, (const uint8_t *)&serial_port_setting, sizeof(serial_port_setting_t));
