...
static uint8_t *g_serial_port_send_vfifo_buffer[HAL_UART_MAX];
static uint8_t *g_serial_port_receive_vfifo_buffer[HAL_UART_MAX];

static serial_port_register_callback_t g_serial_port_register_callback[HAL_UART_MAX] = {NULL};
static const serial_port_dev_t g_serial_port_uart_port[] = {
...
static void serial_port_uart_callback(hal_uart_callback_event_t event, void *user_data)
{
    ...
    if (g_serial_port_register_callback[uart_port] != NULL) {
        if (event == HAL_UART_EVENT_READY_TO_READ) {
            ...
            g_serial_port_register_callback[uart_port](uart_port, SERIAL_PORT_EVENT_WAKEUP_SLEEP, NULL);
        }
    }
}
...
serial_port_status_t serial_port_uart_init(serial_port_dev_t device, serial_port_open_para_t *para, void *priv_data)
{
    ...
    g_serial_port_send_vfifo_buffer[uart_port] = (uint8_t *)pvPortMallocNC(para->tx_buffer_size);
    if (g_serial_port_send_vfifo_buffer[uart_port] == NULL) {
        return SERIAL_PORT_STATUS_FAIL;
    }
    g_serial_port_receive_vfifo_buffer[uart_port] = (uint8_t *)pvPortMallocNC(para->rx_buffer_size);
    if (g_serial_port_receive_vfifo_buffer[uart_port] == NULL) {
        return SERIAL_PORT_STATUS_FAIL;
    }
#else
    g_serial_port_send_vfifo_buffer[uart_port] = (uint8_t *)pvPortMallocNC(SERIAL_PORT_SEND_BUFFER_SIZE);
    if (g_serial_port_send_vfifo_buffer[uart_port] == NULL) {
        return SERIAL_PORT_STATUS_FAIL;
    }
    g_serial_port_receive_vfifo_buffer[uart_port] = (uint8_t *)pvPortMallocNC(SERIAL_PORT_RECEIVE_BUFFER_SIZE);
    if (g_serial_port_receive_vfifo_buffer[uart_port] == NULL) {
        return SERIAL_PORT_STATUS_FAIL;
    }
#endif
...
#ifdef MTK_PORT_SERVICE_SLIM_ENABLE
    dma_config.receive_vfifo_buffer_size = (para->rx_buffer_size);
    dma_config.receive_vfifo_threshold_size = ((para->rx_buffer_size) * 2) / 3;
    dma_config.send_vfifo_buffer_size = (para->tx_buffer_size);
    dma_config.send_vfifo_threshold_size = (para->tx_buffer_size) / 5;
#else
    ...
    dma_config.send_vfifo_threshold_size    = (SERIAL_PORT_SEND_BUFFER_SIZE) / 5;//SERIAL_PORT_SEND_BUFFER_SIZE / 10;
#endif
    ...
    status = hal_uart_register_callback(uart_port, serial_port_uart_callback, (void *)&g_serial_port_uart_port[device]);
    if (status != HAL_UART_STATUS_OK) {
        return SERIAL_PORT_STATUS_FAIL;
    }
...
serial_port_status_t serial_port_uart_deinit(serial_port_dev_t device)
{
    ...
    vPortFreeNC(g_serial_port_send_vfifo_buffer[uart_port]);
    vPortFreeNC(g_serial_port_receive_vfifo_buffer[uart_port]);
