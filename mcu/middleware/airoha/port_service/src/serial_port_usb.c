...
serial_port_status_t serial_port_usb_init(serial_port_dev_t device, serial_port_open_para_t *para, void *priv_data)
{
    ...
    if (USB2UARTPort[usb_port].RingBuffers.rx_buffer == NULL) {
        USB2UARTPort[usb_port].RingBuffers.rx_buffer = (uint8_t *)USB_Get_Memory(length_rx);
    }
    ...
    if (USB2UARTPort[usb_port].RingBuffers.tx_buffer == NULL) {
        USB2UARTPort[usb_port].RingBuffers.tx_buffer = (uint8_t *)USB_Get_Memory(length_tx);
    }
    ...
    log_hal_msgid_info("reg. usb_callback port:%d\n", 1, usb_port);

    if (USB_Get_Device_State() != DEVSTATE_CONFIG) {
        log_hal_msgid_info("serial_port_usb open NOT ready\n", 0);
...
#ifdef MTK_USB2UART_DMA_THRU
            serial_port_write_data->ret_size = USB2UART_Thru_PutBytes(dev, serial_port_write_data->data, serial_port_write_data->size);
#else
...
serial_port_status_t serial_port_usb_deinit(serial_port_dev_t port)
{
    ...
    if (USB2UARTPort[usb_port].RingBuffers.rx_buffer != NULL) {
        USB_Free_Memory((void *)USB2UARTPort[usb_port].RingBuffers.rx_buffer);
        USB2UARTPort[usb_port].RingBuffers.rx_buffer = NULL;
    }


    if (USB2UARTPort[usb_port].RingBuffers.tx_buffer != NULL) {
        USB_Free_Memory((void *)USB2UARTPort[usb_port].RingBuffers.tx_buffer);
