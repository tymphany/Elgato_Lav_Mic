...
#ifdef MTK_GATT_OVER_BREDR_ENABLE
    uint32_t handle_goe = race_get_serial_port_handle(SERIAL_PORT_DEV_BT_GATT_OVER_BREDR);
    RACE_LOG_MSGID_I("notify_aws_state: handle_goe: %x,",1, handle_goe);
#endif
...
#if 0
        else if (handle_goe != 0 && handle_goe != 0xFF) {
            dev = SERIAL_PORT_DEV_BT_GATT_OVER_BREDR;
        } 
#endif
    else {
