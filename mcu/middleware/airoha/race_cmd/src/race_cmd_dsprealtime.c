...
uint32_t race_dsprt_peq_realtime_data(uint8_t phase_id, uint8_t setting_mode, uint32_t target_bt_clk, uint8_t *p_coef, uint32_t coef_size, am_feature_type_t audio_path_id)
{
    ...
    RACE_LOG_MSGID_I("race_dsprt_peq_realtime_data phase_id:%d,setting_mode:%d,target_bt_clk:%d\n", 3, phase_id,setting_mode,target_bt_clk);
    ...
    if (role == BT_AWS_MCE_ROLE_AGENT) {
        *setting_mode = PEQ_SYNC;
        ...
        if (ret == BT_STATUS_FAIL) {
            RACE_LOG_MSGID_W("get current bt clock FAIL\n", 0);
        }
        ret = bt_sink_srv_bt_clock_addition(target_bt_clk, NULL, PEQ_FW_LATENCY * 1000);
        if (ret == BT_STATUS_FAIL) {
            RACE_LOG_MSGID_W("get target bt clock FAIL with duration %d us\n", 1, PEQ_FW_LATENCY * 1000);
        }
...
void *RACE_DSPREALTIME_PEQ_HDR(ptr_race_pkt_t pCmdMsg, uint8_t channel_id)
{
    ...
    if((pCmd->peq_PhaseAndPath == PEQ_AEQ_CONTROL)||(pCmd->peq_PhaseAndPath == PEQ_AEQ_DETECT_CONTROL)){
        ...
        if (param != NULL) {
            param->type = pCmd->peq_PhaseAndPath;
            param->action = pCmd->Peq_param[0];
...
void *RACE_DSPREALTIME_AIRDUMP_HDR(ptr_race_pkt_t pCmdMsg, uint8_t channel_id)
{
    ...
    RACE_LOG_MSGID_I("[RACE_DSPREALTIME_AIRDUMP_HDR] num %d, handle=0x%08X", 2,num, gap_handle);
        ...
        if (pCmd) {
            if (pCmd->AirDumpEn == 1) {
                ...
                if (num > 0) {
                    ...
                    RACE_LOG_MSGID_I("[RACE_DSPREALTIME_AIRDUMP_HDR] Exit sniff mode ret 0x%x, %x", 2, status,BT_STATUS_SUCCESS);
                    status = bt_gap_write_link_policy(gap_handle, &setting);
                    RACE_LOG_MSGID_I("[RACE_DSPREALTIME_AIRDUMP_HDR] Disable sniff mode ret 0x%x", 1, status);
                }
                ...
                status = bt_gap_write_link_policy(gap_handle, &setting);
                RACE_LOG_MSGID_I("[RACE_DSPREALTIME_AIRDUMP_HDR] Enable sniff mode ret 0x%x", 1, status);
            }
        }
...
void *RACE_DSPREALTIME_SELF_FITTING_CONFIG_HDR(ptr_race_pkt_t pCmdMsg, uint8_t channel_id)
{
    ...
    RACE_LOG_MSGID_I("[SELF_FITTING] recv SELF_FITTING CONFIG RACE CMD, id=0x%x, type=%d, data_lenth=%d",
            3, race_id, pCmd->config_type, data_lenth);
    ...
    if (pCmdMsg->hdr.length >= 4) { //hdr.length = hdr.id(2bytes) + pAnc_cmd->status(1byte) + pAnc_cmd->anc_id(1byte) + pAnc_cmd->param(0/1/2/4 bytes)
        switch (pAnc_cmd->param.header.ancId) {
            ...
            case RACE_ANC_ENTER_MP_MODE: {
                ...
                /* Enter MP mode, Disable switch filter ramp. */
                hal_audio_dsp_controller_send_message(MSG_MCU2DSP_COMMON_ANC_SET_PARAM, (unsigned short)(0xF << 12), 1, false);
...
#endif
                /* Leave MP mode, Enable switch filter ramp. */
                hal_audio_dsp_controller_send_message(MSG_MCU2DSP_COMMON_ANC_SET_PARAM, (unsigned short)(0xF << 12), 2, false);
...
//                            }
        ...
        case ADAPTIVE_ANC_STREAM_MODE: {
            ...
            if (pEvt) {
                ...
                if (pCmd->enable) { //enable stream
                    RACE_LOG_MSGID_I("[RACE][Adapt_ANC] Enable Adaptive ANC Stream.", 0);
        ...
        case ADAPTIVE_ANC_STREAM_STATUS: {
            RACE_LOG_MSGID_I("[RACE][Adapt_ANC] Query Adaptive ANC Stream Status.", 0);
            bt_aws_mce_role_t role;
            //send 5B
            pEvt = RACE_ClaimPacket(RACE_TYPE_RESPONSE, pCmd->Hdr.id, sizeof(RACE_DSPREALTIME_ANC_ADAPTIVE_CHECK_EVT_STRU), channel_id);
            if (pEvt) {
                ...
                RACE_LOG_MSGID_I("[RACE][Adapt_ANC] Query Adaptive ANC Stream Status, %d", 1, adapt_anc_stream_status);
...
dsp_realtime_calibration_e race_dsprealtime_dchs_cosys_ctrl_cmd(ptr_race_pkt_t pRaceHeaderCmd, uint16_t length, uint8_t channel_id)
{
    ...
    if (result == DSP_REALTIME_STATUS_RELAY_COMMAND || result == DSP_REALTIME_STATUS_PROCESS_AND_RELAY_COMMAND) {
        ...
        dchs_relay_cmd_param.header.param_size =  sizeof(uint8_t)*4 + length;
...
{
    bt_aws_mce_role_t role = bt_connection_manager_device_local_info_get_aws_role();
    RACE_LOG_MSGID_I("[ADAPTIVE_EQ] adaptive_eq_control_response, [%02X] status=%d type=%d channel_id=%d",
                     4, role, status, type, channel_id);
    if (role != BT_AWS_MCE_ROLE_AGENT && role != BT_AWS_MCE_ROLE_NONE) {
        return;
    }
...
{
    bt_aws_mce_role_t role = bt_connection_manager_device_local_info_get_aws_role();
    RACE_LOG_MSGID_I("[ADAPTIVE_EQ] adaptive_eq_detect_status_response, [%02X] is_response=%d is_start=%d status=%d channel_id=%d",
                     5, role, is_response, is_start, status, channel_id);
    if (role != BT_AWS_MCE_ROLE_AGENT && role != BT_AWS_MCE_ROLE_NONE) {
        return;
    }
