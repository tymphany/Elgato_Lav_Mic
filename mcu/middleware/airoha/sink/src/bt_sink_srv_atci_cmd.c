...
void bt_sink_srv_aws_mce_test(uint8_t type)
{
    ...
    if (aws_run_count < repeat_times) {
        ...
        if (aws_packet == NULL) {
            bt_utils_assert(0 && "Error: No extra heap");
        }
        ...
        bt_sink_srv_report_id("[AWS_TEST] summary, ave: %d, max: %d, fail_num: %d", 3,
                              sum_rx_diff / repeat_times, max_rx_diff, fail_num);
...
static int16_t bt_sink_srv_cmd_entry(const char *string)
{
    ...
    if (0 == bt_sink_srv_memcmp(string, CMD_PARAM("ANS"))) { //ANSWER
        ...
        bt_sink_srv_report_id("[Sink][ATCI]get call state", 0);
...
uint32_t bt_sink_srv_atci_get_element_attributes(bt_sink_srv_avrcp_get_element_attributes_cnf_t *params, uint32_t *parsered_num)
{
    ...
    vPortFree(response);
#endif
    bt_sink_srv_report_id("get_element_attributes-parsered_length:%d", 1, parsered_length);
