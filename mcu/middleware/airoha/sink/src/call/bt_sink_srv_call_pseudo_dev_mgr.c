...
static void bt_sink_srv_call_psd_audio_resource_callback(audio_src_srv_resource_manager_handle_t *handle, audio_src_srv_resource_manager_event_t event);
#endif
...
#ifndef BT_SINK_DUAL_ANT_ENABLE
static bt_sink_srv_call_pseudo_dev_t *bt_source_srv_call_psd_find_device_by_handle( void *handle)
{
    uint32_t i = 0;
    for (i = 0; i < BT_SINK_SRV_CALL_PSD_NUM; i++) {
...
static void bt_sink_srv_call_psd_audio_resource_callback(audio_src_srv_resource_manager_handle_t *handle,
    ...
    if (dev->flag & BT_SINK_SRV_CALL_PSD_FLAG_ADD_WAIT_LIST) {
        ...
        if (BT_SINK_SRV_CALL_AUDIO_SCO  == dev->audio_type ||
            ...
            BT_SINK_SRV_CALL_AUDIO_NULL == dev->audio_type) {
                ...
                bt_sink_srv_report_id("[CALL][PSD][MGR]dual-chip role:slave ", 0);
    ...
    void *data)
{
    ...
    switch (dev->audio_src->substate) {
        case BT_SINK_SRV_CALL_PSD_SUB_STATE_MIC_RES_TAKING: {
            bt_sink_srv_call_psd_sub_state_mic_resource_taking(dev, event, data);
        }
        break;
