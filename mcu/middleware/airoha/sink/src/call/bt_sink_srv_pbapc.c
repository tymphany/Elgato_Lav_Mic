...
static void bt_sink_srv_pbapc_add_number(bt_bd_addr_t *address_p, bt_sink_srv_pbapc_record_t *record)
{
    ...
    bt_sink_srv_report_id("[Sink][PBAPC] record_p:0x%x", 1, record_p);
}
...
static bool bt_sink_srv_pbapc_search_number(bt_bd_addr_t *address, bt_sink_srv_pbapc_record_t *record)
{
    ...
    bt_sink_srv_report_id("[Sink][PBAPC] result:%d", 1, result);
    return result;
}
...
bt_status_t bt_sink_srv_pbapc_common_callback(bt_msg_type_t msg, bt_status_t status, void *buffer)
{
    ...
    if (msg >= BT_PBAPC_CONNECT_CNF && msg <= BT_PBAPC_DISCONNECT_IND) {
        ...
        bt_sink_srv_report_id("[Sink][PBAPC] SDK msg:0x%x", 1, msg);
    }
#else
    bt_sink_srv_report_id("[Sink][PBAPC] SDK msg:0x%x", 1, msg);
#endif /* __BT_SINK_SRV_DEBUG_INFO__ */
...
bt_status_t bt_sink_srv_pbapc_action_handler(bt_sink_srv_action_t action, void *parameters)
{

    bt_sink_srv_report_id("[Sink][PBAPC] bt_sink_srv_pbapc_action_handler, action:0x%x", 1, action);
    ...
    switch (action) {
        ...
        case BT_SINK_SRV_ACTION_PROFILE_DISCONNECT: {
            ...
            if (action_param->profile_connection_mask & BT_SINK_SRV_PROFILE_PBAPC) {
                ...
                if (NULL != bt_sink_srv_pbapc_p && bt_sink_srv_pbapc_p->pbapc_handle != 0) {
                    bt_sink_srv_report_id("[Sink][PBAPC] call pbapc profile disconnect", 0);
                                                   ...
                                                   bt_sink_srv_pbapc_get_name_callback_t callback)
{
    ...
    bt_sink_srv_report_id("[Sink][PBAPC] get name status:0x%x", 1, result);
    return result;
}
...
bt_status_t bt_sink_srv_pbapc_connect(bt_bd_addr_t *address)
{
    ...
    bt_sink_srv_report_id("[Sink][PBAPC] connect status:0x%x", 1, status);
    return status;
}
#else /**/
bt_status_t bt_sink_srv_pbapc_connect(bt_bd_addr_t *address)
