                ...
                if (mode <= CAP_AM_UNICAST_MODE_END) {
                    ...
                    if (sub_msg == AUD_CMD_COMPLETE) {
                        ...
                        if (mode <= CAP_AM_UNICAST_CALL_MODE_END) {
                            ...
                            bt_sink_srv_cap_am_take_give_mic_resource(mode, false);
#endif
#endif
                        }
...
static void bt_sink_srv_cap_am_play_callback(audio_src_srv_handle_t *handle)
{
    ...
    le_audio_log("[CAP][AM] play callback, mode:%d, state:%d, switch_suspending:%d, switch_suspending_mode:%d",
...
#ifdef BT_SINK_DUAL_ANT_ENABLE
#ifdef BT_SINK_DUAL_ANT_ROLE_SLAVE
    else if ((mode <= CAP_AM_UNICAST_CALL_MODE_END || ul_only) && g_cap_am_ctl[mode].sub_state != CAP_AM_SUB_STATE_TAKING_MIC_RES) {
        bt_sink_srv_cap_am_take_give_mic_resource(mode, true);
        return;
    }
#endif
#endif
...
static void bt_sink_srv_cap_am_stop_callback(audio_src_srv_handle_t *handle)
{
    ...
    le_audio_log("[CAP][AM] stop callback, mode:%d, state:%d, switch_suspending:%d, switch_suspending_mode:%d",
        4, mode, state, g_cap_am_switch_suspending, g_cap_am_switch_suspending_mode);
#ifdef AIR_BT_CODEC_BLE_ENABLED
...
static void bt_sink_srv_cap_am_reject_timer_callback(TimerHandle_t timer_id)
{
    ...
    le_audio_log("[CAP][AM] reject timer timeout, mode:%d, state:%d", 3, g_cap_am_reject_mode, state, config.reject_config.reject_operation);
...
void bt_sink_srv_cap_am_audio_start(bt_sink_srv_cap_am_mode mode)
{
    ...
    if(state == CAP_AM_STATE_IDLE || state == CAP_AM_STATE_SUSPENDING) {
        ...
        if (mode >= CAP_AM_UNICAST_MUSIC_MODE_START && mode <= CAP_AM_UNICAST_MUSIC_MODE_END) {
            ...
            if (bt_le_audio_sink_call_get_first_call_state(bt_sink_srv_cap_get_ble_link_by_streaming_mode(mode), 0xFF) != NULL) {
...
#ifdef BT_SINK_DUAL_ANT_ENABLE
bt_sink_srv_cap_am_mode bt_sink_srv_cap_am_is_taking_mic_resource(void)
{
    for (uint8_t i = 0; i < CAP_MAX_DEVICE_NUM; i++) {
        if (g_cap_am_ctl[i].sub_state == CAP_AM_SUB_STATE_TAKING_MIC_RES) {
            le_audio_log("[CAP][AM] is taking mic resource, mode:%d", 1, i);
            return i;
        }
    }
    return CAP_INVALID_UINT8;
...
void bt_sink_srv_cap_am_take_give_mic_resource(bt_sink_srv_cap_am_mode mode, bool take_mic)
{
    if (mode >= CAP_MAX_DEVICE_NUM) {
        return;
    }

    g_cap_am_ctl[mode].sub_state = (take_mic ? CAP_AM_SUB_STATE_TAKING_MIC_RES : CAP_INVALID_UINT8);

    bt_sink_srv_dual_ant_data_t notify;
    notify.type = BT_SINK_DUAL_ANT_TYPE_CALL;
    notify.call_info.esco_state = take_mic;
    bt_sink_srv_dual_ant_notify(false, &notify);
    le_audio_log("[CAP][AM] Take/Give mic resource:%d", 1, take_mic);
}

void bt_sink_srv_cap_am_take_mic_resource_response(void)
{
    bt_sink_srv_cap_am_mode mode = bt_sink_srv_cap_am_is_taking_mic_resource();
    if (mode >= CAP_MAX_DEVICE_NUM) {
        return;
    }
