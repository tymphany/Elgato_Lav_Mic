        ...
        bt_sink_srv_state_manager_device_t *device)
{
    bt_sink_srv_report_id(
            "[Sink][StaMgr][PseDev]play psedev, IN: 0x%x(0x%x)",
            2,
            device->dummy_device,
            device->dummy_device->state);
    ...
    bt_sink_srv_report_id(
            "[Sink][StaMgr][PseDev]play psedev, OUT: 0x%x(0x%x)",
            2,
            device->dummy_device,
            device->dummy_device->state);
}
...
{
    bt_sink_srv_report_id(
            "[Sink][StaMgr][PseDev]stop psedev, IN: 0x%x(0x%x)",
            2,
            device->dummy_device,
            device->dummy_device->state);
    ...
    bt_sink_srv_report_id(
            "[Sink][StaMgr][PseDev]stop psedev, OUT: 0x%x(0x%x)",
            2,
            device->dummy_device,
            device->dummy_device->state);
}
...
static void bt_sink_srv_state_manager_play_callback(audio_src_srv_handle_t *handle)
{
    ...
    bt_sink_srv_report_id("[Sink][StaMgr][PseDev]play callback, IN: 0x%x(0x%x)", 2, handle, handle->state);

    audio_src_srv_update_state(handle, AUDIO_SRC_SRV_EVT_PLAYING);
    bt_sink_srv_state_manager_run_next_state(context, device);

    bt_sink_srv_report_id("[Sink][StaMgr][PseDev]play callback, OUT: 0x%x(0x%x)", 2, handle, handle->state);
}
...
static void bt_sink_srv_state_manager_stop_callback(audio_src_srv_handle_t *handle)
{
    bt_sink_srv_state_manager_context_t *context = bt_sink_srv_state_manager_get_context();
    bt_sink_srv_state_manager_device_t *device = bt_sink_srv_state_manager_get_device_by_psedev(context, handle);
    bt_sink_srv_report_id("[Sink][StaMgr][PseDev]stop callback, IN: 0x%x(0x%x)", 2, handle, handle->state);

    audio_src_srv_update_state(handle, AUDIO_SRC_SRV_EVT_READY);
    bt_sink_srv_state_manager_run_next_state(context, device);

    bt_sink_srv_report_id("[Sink][StaMgr][PseDev]stop callback, OUT: 0x%x(0x%x)", 2, handle, handle->state);
}
        ...
        audio_src_srv_handle_t *interrupt)
{
    bt_sink_srv_state_manager_context_t *context = bt_sink_srv_state_manager_get_context();
    bt_sink_srv_state_manager_device_t *device = bt_sink_srv_state_manager_get_device_by_psedev(context, handle);
    bt_sink_srv_report_id("[Sink][StaMgr][PseDev]suspend callback, IN: 0x%x(0x%x)", 2, handle, handle->state);
    ...
    bt_sink_srv_report_id("[Sink][StaMgr][PseDev]suspend callback, OUT: 0x%x(0x%x)", 2, handle, handle->state);
}
...
static void bt_sink_srv_state_manager_reject_callback(audio_src_srv_handle_t *handle)
{
    bt_sink_srv_state_manager_context_t *context = bt_sink_srv_state_manager_get_context();
    bt_sink_srv_state_manager_device_t *device = bt_sink_srv_state_manager_get_device_by_psedev(context, handle);
    bt_sink_srv_report_id("[Sink][StaMgr][PseDev]reject callback, IN: 0x%x(0x%x)", 2, handle, handle->state);

    audio_src_srv_add_waiting_list(handle);
    bt_sink_srv_state_manager_set_next_state(context, device, BT_SINK_SRV_STATE_MANAGER_NEXT_STATE_INVALID);

    bt_sink_srv_report_id("[Sink][StaMgr][PseDev]reject callback, OUT: 0x%x(0x%x)", 2, handle, handle->state);
}
