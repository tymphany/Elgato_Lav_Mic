...
static void bt_sink_srv_state_manager_update_edr_call_devices(bt_sink_srv_state_manager_context_t *context);
static void bt_sink_srv_state_manager_update_edr_media_devices(bt_sink_srv_state_manager_context_t *context);
static void bt_sink_srv_state_manager_update_le_devices(bt_sink_srv_state_manager_context_t *context);
...
static void bt_sink_srv_state_manager_update_edr_call_devices(bt_sink_srv_state_manager_context_t *context)
{
    ...
    for (uint32_t i = 0; i < BT_SINK_SRV_STATE_MANAGER_MAX_DEVICE_NUM; i++) {
        ...
        bt_sink_srv_report_id(
                "[Sink][StaMgr]update EDR call devices, device: 0x%x(call_state: 0x%x, call_audio_state: 0x%x)",
                3,
                &context->devices[i],
        ...
        bool is_sync)
{
    ...
    bt_sink_srv_report_id("[Sink][StaMgr]notify state change, role: 0x%x link: 0x%x", 2, role, link);
#endif
    ...
    if (BT_AWS_MCE_ROLE_PARTNER == role && BT_AWS_MCE_SRV_LINK_NORMAL == link) {
        if (!is_sync && !BT_SINK_SRV_STATE_MANAGER_IS_MEDIA_STATE(state)) {
...
#if defined(MTK_AWS_MCE_ENABLE) && defined(AIR_MULTI_POINT_ENABLE)
    if (BT_AWS_MCE_ROLE_AGENT == role && BT_AWS_MCE_SRV_LINK_NORMAL == link && !BT_SINK_SRV_STATE_MANAGER_IS_MEDIA_STATE(state)) {
        ...
        bt_sink_srv_state_t state)
{
    ...
    if (BT_AWS_MCE_ROLE_AGENT != role || BT_AWS_MCE_SRV_LINK_NORMAL != link) {
        bt_sink_srv_report_id("[Sink][StaMgr]sync state, unexpected role 0x%x or link 0x%x", 2, role, link);
        return;
    }
        ...
        case BT_AWS_MCE_STATE_CHANGED_IND: {
            ...
            if (BT_AWS_MCE_ROLE_AGENT == bt_connection_manager_device_local_info_get_aws_role()) {
                if ((NULL != state_change) &&
                    ...
                    (!BT_SINK_SRV_STATE_MANAGER_IS_MEDIA_STATE(context->previous_state))) {
