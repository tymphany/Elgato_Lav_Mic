...
void smchg_nvkey_init(void)
{
    ...
    if (nvkey_status != NVKEY_STATUS_OK) {
        log_hal_msgid_error("smchg, 1wire, nvkey 0xF01A fail", 0);
        assert(0);
    }
    log_hal_msgid_info("smchg, 1wire, chg_type[%d], uart_sel[%d], com_br[%d], log_br[%d], out_case_timer[%d], com_timer[%d], race_timer[%d], vbus_thrd[%d], vbat_thrd[%d], 1wire_log[%d]", 10,
                       smchg_cfg.chg_type, smchg_cfg.uart_sel, smchg_1wire_baudrate_report(smchg_cfg.com_mode_baud_rate),
                       smchg_1wire_baudrate_report(smchg_cfg.log_mode_baud_rate), smchg_cfg.out_of_case_chk_timer, smchg_cfg.com_mode_chk_timer,
