...
{
    return (Timer_Handle_t)xTimerCreate((const char *) timer_name,
                                        (const TickType_t)(timer_ms * portTICK_PERIOD_MS),
                                        (const UBaseType_t)auto_reload,
                                        (void *const)pv_timer_id,
                                        (TimerCallbackFunction_t)hanlder);
}

int32_t ui_shell_al_timer_start(Timer_Handle_t timer_handler,
                                uint16_t  time_to_wait)
{
    return (int32_t)xTimerStart((TimerHandle_t)timer_handler, (const TickType_t)time_to_wait);
...
int32_t ui_shell_al_timer_stop(Timer_Handle_t  timer_handler,
                               uint16_t    time_to_wait)
{
    return (int32_t)xTimerStop((TimerHandle_t)timer_handler, (const TickType_t)time_to_wait);
...
{
    return (int32_t)xTimerDelete((TimerHandle_t)timer_handler, (const TickType_t)timer_to_wait);
}
