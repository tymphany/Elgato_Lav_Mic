    ...
    size_t data_len)
{
    ...
    switch (event_id) {
        case EVENT_ID_SHELL_SYSTEM_ON_CREATE:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity1_1 create", 0);
        ...
        case EVENT_ID_SHELL_SYSTEM_ON_DESTROY:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity1_1 destroy", 0);
            if (self->local_context) {
                ui_shell_al_free(self->local_context);
            }
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_RESUME:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity1_1 resume", 0);
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_PAUSE:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity1_1 pause", 0);
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_REFRESH:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity1_1 refresh", 0);
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_RESULT:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity1_1 result", 0);
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_REQUEST_ALLOWANCE: {
            uint32_t request_id = (uint32_t)extra_data;
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity1_1 request allowance : %d", 1, request_id);
    ...
    switch (key_num) {
        case APP1_CARE_KEY_ID:
            if (key_event == AIRO_KEY_DOUBLE_CLICK) { // key event
                char *result_data = ui_shell_al_malloc(sizeof("RESULT_A"));
                ui_shell_al_memcpy(result_data, "RESULT_A", sizeof("RESULT_A"));
                ret = true;
                if (extra_data && data_len) {
                    UI_SHELL_LOG_I("extra data = %s", extra_data);
                }
                if (!ui_shell_set_result(self, result_data, sizeof("RESULT_A"))) {
                ...
                UI_SHELL_LOG_MSGID_I("Go back to idle : %x", 1, (uint32_t)self);
                ui_shell_back_to_idle(self);
                if (extra_data && data_len) {
                    UI_SHELL_LOG_I("extra data = %s", extra_data);
                }
                ret = true;
        ...
        case TEST_ALLOW_KEY_ID:
            if (key_event == AIRO_KEY_SLONG && memcmp(extra_data, "1_1", sizeof("1_1")) == 0) { // key event
...
bool ui_shell_ut_activity1_1_proc(
    struct _ui_shell_activity *self,
