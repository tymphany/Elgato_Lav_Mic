...
#include "ui_shell_ut_activity2_2.h"
    ...
    size_t data_len)
{
    ...
    switch (event_id) {
        case EVENT_ID_SHELL_SYSTEM_ON_CREATE:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity2_1 create", 0);
            self->local_context = ui_shell_al_malloc(sizeof(local_context_type_t));
            ((local_context_type_t *)self->local_context)->refuse_destory = false;
            if (extra_data) {
                UI_SHELL_LOG_I("extra data for activity1 create : %s", extra_data);
            }
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_DESTROY:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity2_1 destroy", 0);
            if (self->local_context && !((local_context_type_t *)self->local_context)->refuse_destory) {
                ui_shell_al_free(self->local_context);
            } else {
                UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity2_1 refused destory", 0);
                ret = false;
            }
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_RESUME:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity2_1 resume", 0);
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_PAUSE:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity2_1 pause", 0);
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_REFRESH:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity2_1 refresh", 0);
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_RESULT:
            UI_SHELL_LOG_MSGID_I("ui_shell_ut_activity2_1 result", 0);
            if (extra_data) {
                UI_SHELL_LOG_I("extra data for activity2_1 result : %s", extra_data);
            }
            break;
        case EVENT_ID_SHELL_SYSTEM_ON_REQUEST_ALLOWANCE: {
            uint32_t request_id = (uint32_t)extra_data;
            UI_SHELL_LOG_MSGID_I("activity2_1 request allowance : %d", 1, request_id);
    ...
    switch (key_num) {
        case APP2_CARE_KEY_ID:
            if (key_event == AIRO_KEY_DOUBLE_CLICK) { // key event
                char *result_data = ui_shell_al_malloc(sizeof("RESULT_B"));
                ui_shell_al_memcpy(result_data, "RESULT_B", sizeof("RESULT_B"));
                ret = true;
                if (extra_data && data_len) {
                    UI_SHELL_LOG_I("extra data = %s", extra_data);
                }
                if (!ui_shell_set_result(self, result_data, sizeof("RESULT_B"))) {
                    UI_SHELL_LOG_MSGID_I("Set result fail, free result data", 0);
                    ui_shell_al_free(result_data);
                }
                ret = true;
            } else if (key_event == AIRO_KEY_TRIPLE_CLICK) {
                ...
                UI_SHELL_LOG_MSGID_I("start activity : %x", 1, (uint32_t)self);
                ui_shell_start_activity(self, ui_shell_ut_activity2_2_proc, ACTIVITY_PRIORITY_HIGHEST, NULL, 0);
                if (extra_data && data_len) {
                    UI_SHELL_LOG_I("extra data = %s", extra_data);
                }
                ret = true;
            } else if (key_event == AIRO_KEY_LONG_PRESS_2) {
                UI_SHELL_LOG_MSGID_I("record refulse destory", 0);
                ((local_context_type_t *)self->local_context)->refuse_destory =
                    !((local_context_type_t *)self->local_context)->refuse_destory;
                if (extra_data && data_len) {
                    UI_SHELL_LOG_I("extra data = %s", extra_data);
                }
        ...
        case TEST_ALLOW_KEY_ID:
            if (key_event == AIRO_KEY_SLONG && memcmp(extra_data, "2_1", sizeof("2_1")) == 0) { // key event
...
bool ui_shell_ut_activity2_1_proc(
    struct _ui_shell_activity *self,
