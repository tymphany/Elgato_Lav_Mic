...
*************************************************************/
bool g_usb_high_speed_enable = false;
...
void USB_Init(usb_dev_type_t type)
{
    ...
    LOG_MSGID_I(USB, "USB_Init, high speed mode enable [%x]", 1, g_usb_high_speed_enable);
    hal_usb_set_speed(g_usb_high_speed_enable);
...
static bool USB_Cmd_SetAddress(Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    LOG_MSGID_I(USB_DEBUG, "USB_Cmd_SetAddress", 0);
...
#ifdef AIR_USB_HID_ENABLE
extern usb_hid_dscr_hid_t hid_dscr;
extern uint8_t hid_report_dscr[];
extern uint16_t hid_report_dscr_length;
#endif
...
static bool USB_Cmd_GetDescriptor(Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    LOG_MSGID_I(USB_DEBUG, "USB_Cmd_GetDescriptor wValue:0x%X", 1, pcmd->wValue);
    ...
    if ((pcmd->bmRequestType != USB_CMD_STDDEVIN) && pcmd->bmRequestType != USB_CMD_STDIFIN) {
        ...
        switch (pcmd->wValue & USB_CMD_DESCMASK) {
            ...
            case USB_CMD_HID_DESC:
                USB_Generate_EP0Data(pep0state, pcmd, &hid_dscr, hid_dscr.bLength);
                break;
            case USB_CMD_HID_REPORT:
                USB_Generate_EP0Data(pep0state, pcmd, hid_report_dscr, hid_report_dscr_length);
                break;
...
static bool USB_Cmd_GetConfiguration(Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    LOG_MSGID_I(USB_DEBUG, "USB_Cmd_GetConfiguration", 0);
...
{
    LOG_MSGID_I(USB_DEBUG, "USB_Cmd_SetInterface", 0);
...
{
    LOG_MSGID_I(USB_DEBUG, "USB_Cmd_GetInterface", 0);
...
static bool USB_Cmd_SetFeature(Usb_Command *pcmd, bool bset)
{
    ...
    LOG_MSGID_I(USB_DEBUG, "USB_Cmd_SetFeature", 0);
...
static bool USB_Cmd_GetStatus(Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    LOG_MSGID_I(USB_DEBUG, "USB_Cmd_GetStatus", 0);
...
static void USB_Stdcmd(Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    /* Phone detection : default OFF */
    bool usb_host_detect_enable = false;
    if (usb_host_detect_enable) {
...
bool USB_Check_EP0_DataLen(uint32_t ep_num, uint32_t expected_byte)
{
    ...
    while (byte < expected_byte) {
        ...
        if (dma_timeout_duration > USB_GPT_TIMEOUT_1ms) {
            ...
            LOG_MSGID_I(USB_DEBUG, "USB_Check_EP0_DataLen wait %d us to get enough data", 1, dma_timeout_duration);
        }
    }
...
static void USB_Endpoint0_Idle(void)
{
    ...
    LOG_MSGID_I(USB_DEBUG, "USB_Endpoint0_Idle bmRequestType:0x%x, bRequest:0x%x, wValue:0x%x, wIndex:0x%x, wLength0x%x", 5,
        ...
        case USB_CMD_CLASSREQ:
            ...
            if ((gUsbDevice.cmd.bmRequestType == USB_CMD_CLASSIFIN) || (gUsbDevice.cmd.bmRequestType == USB_CMD_CLASSIFOUT)) {
                ...
                if (gUsbDevice.if_info[(gUsbDevice.cmd.wIndex & 0xff)].if_class_specific_hdlr != NULL) {
                    // LOG_MSGID_I(USB_DEBUG, "USB_Endpoint0_Idle if_class_specific_hdlr if_info:0x%X", 1, (gUsbDevice.cmd.wIndex & 0xff));
        ...
        case USB_CMD_VENDREQ:
#ifdef AIR_USB_XBOX_ENABLE
            LOG_MSGID_I(USB_DEBUG, "USB_Endpoint0_Idle device_type:%x", 1, gUsbDevice.device_type);
            ...
            if (gUsbDevice.cmd.bRequest == bMS_Vendor_Code) {
                ...
                if (gUsbDevice.cmd.wIndex == EXTENDED_COMPATIBLE_ID) {
                    LOG_MSGID_I(USB_DEBUG, "USB_Endpoint0_Idle EXTENDED_COMPATIBLE_ID", 0);
                    ...
                    LOG_MSGID_I(USB_DEBUG, "USB_Endpoint0_Idle EXTENDED_COMPATIBLE", 0);
...
void USB_Endpoint0_Tx(void)
{
    ...
    //LOG_MSGID_I(USB_DEBUG, "USB_Endpoint0_Tx", 0);
...
static void USB_Endpoint0_Hdlr(void)
{
    ...
    //LOG_MSGID_I(USB_DEBUG, "USB_Endpoint0_Hdlr\n", 0);
