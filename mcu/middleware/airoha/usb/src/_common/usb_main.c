            ...
            if (xQueueReceive(usb_queue_handle, &msgs, portMAX_DELAY)) {
                ...
                switch (msgs.msg_id) {
                    ...
                    case USB_AUDIO_SET_VOLUME: {
                            ...
                            LOG_MSGID_I(USB_MAIN, "usb_task_main speaker volume ep:0x%02X ch:%d vol:%d(raw:%04x) dB:%d", 5, ep_addr, ch_index+1, volume_0to100, volume_cur, volume_dB);
                            if( volumechange_cb){
                                volumechange_cb( ep_addr, ch_index+1, volume_0to100, volume_dB);
                             }
                        }
                        break;
#ifdef AIR_USB_AUDIO_1_MIC_ENABLE
                    case USB_MIC_SET_VOLUME: {
                            ...
                            LOG_MSGID_I(USB_MAIN, "usb_task_main mic volume ep:0x%02X ch:%d vol:%d(raw:%04x) dB:%d", 5, ep_addr, ch_index+1, volume_0to100, volume_cur, volume_dB);
                            if (USB_Audio[0].volumechange_cb_mic) {
                                USB_Audio[0].volumechange_cb_mic( ep_addr, ch_index+1, volume_0to100, volume_dB);
                            }
                        }
                        break;
#endif
...
{
#ifdef AIR_USB_HID_ENABLE
    USB_Register_CreateFunc("HID", USB_HID_Create, USB_HID_Reset,
                            USB_HID_Enable, (usb_speed_if_func_ptr)USB_HID_Speed_Reset, NULL);
...
void usb_dev_app_init_audio()
{
    ...
    for(uint8_t i=0;i<USB_AUDIO_MAX_CHANNEL_NUM;++i)
    {
...
#ifdef AIR_USB_HID_ENABLE
    USB_Register_CreateFunc("AUDIO HID", USB_HID_Create, USB_HID_Reset,
                            USB_HID_Enable, (usb_speed_if_func_ptr)USB_HID_Speed_Reset, NULL);
#endif
...
#ifdef AIR_NVDM_ENABLE
extern bool    g_usb_high_speed_enable;
...
static void usb_device_cfg_nvkey_read(void)
{
    ...
    if (nvkey_status == NVKEY_STATUS_OK) {
        g_usb_high_speed_enable = devcfg.hs_fs;
