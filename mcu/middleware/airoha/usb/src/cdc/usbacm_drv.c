...
static uint8_t *USB2Uart_shortpkg_addr[MAX_USB_PORT_NUM];
static bool USB2Uart_shortpkg_force[MAX_USB_PORT_NUM] = {false};
...
*************************************************************/
/*
    Determine transmit data If return value is large than 0,
    the caller should send the data in parameter.
...
static void USB2UART_Update_Transmit_Data(uint32_t port)
{
    ...
    /* Update buffer information */
    {
        ...
        if (!(USB2UARTPort[usb_port].Tx_Buffer.Read <= USB2UARTPort[usb_port].Tx_Buffer.Length)) {
            LOG_MSGID_E(common, "USB2UART_Update_Transmit_Data, ASSERT\n", 0);
        }
...
void USB2UART_Thru_DMATransmit(uint32_t port, uint8_t *buffaddr, uint16_t length)
{
    uint32_t usb_port = USB_PORT[port];
    uint8_t ep_num = 0;
    uint32_t savedMask;
    ...
    if (length != 0) {
        ...
        if (length % HAL_USB_MAX_PACKET_SIZE_ENDPOINT_BULK_FULL_SPEED == 0) {
            USB2Uart_shortpkg_force[usb_port] = true;
            USB2Uart_shortpkg_addr[usb_port] = buffaddr + length - 4;
            length -= 4;
        }

        hal_usb_start_dma_channel(ep_num, HAL_USB_EP_DIRECTION_TX, HAL_USB_EP_TRANSFER_BULK, (void *)buffaddr, length, dma_callback, false, HAL_USB_DMA1_TYPE);
...
void USB2UART_DMATransmit(uint32_t port)
{
    ...
    if (length != 0) {
        ...
        if (usb_dma_setup == true) {
            ...
            hal_usb_start_dma_channel(ep_num, HAL_USB_EP_DIRECTION_TX, HAL_USB_EP_TRANSFER_BULK, (void *)(hal_memview_mcu_to_infrasys(addr)), length, dma_callback, false, HAL_USB_DMA1_TYPE);
        }
        /* else part must because of cable out, so we do not need to care */
    } else {
        g_UsbACM[usb_port].setup_dma = false;
...
void USB2UART_Tx_DMA_Callback(uint32_t port)
{
    ...
    // LOG_I(common, "USB2UART_Tx_DMA_Callback, usb port = %d", usb_port);
    ...
    */
    ...
    // Callback after USB2UART_Update_Transmit_Data
...
void USB2UART_Thru_Tx_DMA_Callback(uint32_t port)
{
    uint32_t usb_port = USB_PORT[port];

    // LOG_I(common, "USB2UART_Tx_DMA_Callback, usb port = %d", usb_port);
    if (USB2Uart_shortpkg_force[usb_port] == true) {
#if 0
        hal_usb_write_endpoint_fifo(g_UsbACM[usb_port].txpipe->byEP, 4, (void *)USB2Uart_shortpkg_addr[usb_port]);
...
#else
        USB2UART_Thru_DMATransmit(port, (void *)USB2Uart_shortpkg_addr[usb_port], 4);
#endif
        USB2Uart_shortpkg_force[usb_port] = false;
        return;
    }
...
void USB_Acm_CommIf_Create(uint8_t port, void *ifname)
{
    ...
    g_UsbACM[port].comm_ep_intr_info = USB_Get_Intr_Tx_Ep(&ep_id);
    ...
    memcpy(&(g_UsbACM[port].comm_if_info->ifdscr), cdcacm_com_if_dscr, USB_CDC_IF_LENGTH);
    ...
    ((Usb_Cdc_If_Dscr *)g_UsbACM[port].comm_if_info->ifdscr.classif)->iInterface = USB_String_Get_Id_By_Usage(USB_STRING_USAGE_ACM_COMM);
...
void USB_Acm_DataIf_Create(uint8_t port, void *ifname)
{
    ...
    g_UsbACM[port].data_if_info->ifdscr.stdif.iInterface = USB_String_Get_Id_By_Usage(USB_STRING_USAGE_ACM_DATA);
    if (port == 0) {
...
void USB_Acm_Default_Setting(uint8_t port)
{
    ...
    g_UsbACM[port].line_coding.bDataBits = len_8 ;
    g_UsbACM[port].line_coding.dwDTERate = UART_BAUD_115200 ;
}
...
void USB_Acm_Ep0_Command(uint8_t port, Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    switch (pcmd->bRequest) {
        ...
        case USB_ACM_GET_LINE_CODING:
            ...
            //LOG_I(common, "USB_ACM_GET_LINE_CODING\n");
...
void USB_Acm_BulkOut_Hdr(uint8_t port)
{
    ...
    if (nCount != 0) {
        ...
        if ((nCount > max_bulk_pkt_size) || (RoomLeft < nCount)) {
            ...
            LOG_MSGID_E(common, "ASSERT, USB_Acm_BulkOut_Hdr\n", 0);
        }
