...
/* Syslog create module for usbms_adap.c */
#include "syslog.h"
log_create_module_variant(USB_MS_ADAP, DEBUG_LOG_ON, PRINT_LEVEL_INFO);
...
static void USB_Ms_Check_CURRENT_DRV(uint8_t LUN)
{
    if (USBMS_DISKDRV_API[CURRENT_DRV][LUN] == NULL) {
        LOG_MSGID_E(USB_MS_ADAP, "ASSERT\n", 0);
    }
}
...
uint8_t USB_Ms_Get_Max_LUN(void)
{
    ...
    if (USBMS_MAX_DISK[CURRENT_DRV] == 0) {
        LOG_MSGID_E(USB_MS_ADAP, "ASSERT, (uint32_t)USBMS_MAX_DISK[CURRENT_DRV] \n", 0);
    }
...
uint8_t USB_Ms_Register_DiskDriver(USB_DiskDriver_STRUCT *disk_api)
{
    ...
    LOG_MSGID_I(USB_MS_ADAP, "USB_Ms_Register_DiskDriver, USBMS_MAX_DISK[CURRENT_DRV]-1 = %d\n", 1, USBMS_MAX_DISK[CURRENT_DRV] - 1);
    return (USBMS_MAX_DISK[CURRENT_DRV] - 1);
}
void USB_Ms_DeRegister_DiskDriver(uint8_t disk_index)
{
    ...
    if ((disk_index > USBMS_MAX_DISK[CURRENT_DRV]) /*|| (USBMS_MAX_DISK[CURRENT_DRV] == 1)*/) {
        LOG_MSGID_E(USB_MS_ADAP, "ASSERT, disk_index:%d\n", 1, disk_index);
    }
