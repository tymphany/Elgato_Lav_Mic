...
void USB_Ms_If_Create(void *ifname)
{
    ...
    g_UsbMS.ms_if_info->ifdscr.stdif.iInterface = 1;    //griffy
...
{
    uint8_t index;
    LOG_MSGID_I(hal, "USB_Init_Ms_Status\n", 0);
    ...
    if (g_UsbMS.disk_buffer == NULL) {
        LOG_MSGID_I(hal, "USB_Init - Malloc disk_buffer\n", 0);
        g_UsbMS.disk_buffer = (void *)USB_Get_Memory(USBMS_MAX_BUFFERSIZE);
    }
...
void USB_Release_Ms_Status(void)
{
    ...
    if (g_UsbMS.disk_buffer != NULL) {
        USB_Free_Memory((void *)g_UsbMS.disk_buffer);
    ...
    switch (pcmd->bRequest) {
        ...
        case USB_MS_GETMAXLUN:
            if ((pcmd->bmRequestType != USB_CMD_CLASSIFIN) || (pcmd->wValue != 0)) {
                ...
                LOG_MSGID_I(hal, "USB_Ms_Get_Max_LUN, lun=%d\n", 1, g_UsbMS.max_LUN);
...
static void USB_Ms_BulkOut_Hdlr(void)
{
    ...
    if (nCount == 31) {
        ...
        /* Error handling */
        LOG_MSGID_E(hal, "USBMS_RX_WRONG_CBW_LENGTH=%d\n", 1, nCount);
