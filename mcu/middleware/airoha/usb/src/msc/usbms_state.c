...
void USB_Ms_State_Main(USB_Ms_State event, usb_ms_rec_done_conf_struct *local_para_ptr)
{
    ...
    if ((event != MSG_ID_USB_MSDRV_RESET_IND) && ((g_UsbMS.usb_reset == true) || (g_UsbMS.ms_is_reset == true))) { //receive reset
        LOG_MSGID_E(hal, "USB_Ms_State_Main, do nothing return\n", 0);
        return;
    }
    ...
    switch (event) {
        ...
        default:
            //ASSERT(0);
            LOG_MSGID_E(hal, "ASSERT\n", 0);
            break;
    }

}

#define WORKAROUND_BTA_37334
#ifdef WORKAROUND_BTA_37334
#include "FreeRTOS.h"
#include "task.h"
#endif
    ...
    switch (g_UsbMS.nState) {
        case USBMS_IDLE:
            if (ptr->nBytesRecv != 31) {
                LOG_MSGID_E(hal, "ASSERT, nBytesRecv = %d\n", 1, ptr->nBytesRecv);
            }
        ...
        case USBMS_RX:
            ...
            if ((CBWCB[0] == USBMS_WRITE10) || (CBWCB[0] == USBMS_WRITE12)) {
...
#ifdef WORKAROUND_BTA_37334
                static uint32_t s_USBMS_RX_run_cnt;
                ++s_USBMS_RX_run_cnt;
                if( s_USBMS_RX_run_cnt > 500 ) {
                    s_USBMS_RX_run_cnt = 0;
                    vTaskDelay(5);
                }
#endif
                ...
                LOG_MSGID_E(hal, "ASSERT, CBW = %d\n", 1, (uint32_t)CBW->CBWCB[0]);
            }
            break;
...
//  case USBMS_GETNEXTCMD:
        ...
        case USBMS_TX:
        default:
            LOG_MSGID_E(hal, "ASSERT, g_UsbMS.nState = %d\n", 1, g_UsbMS.nState);
            break;
    }
}
    ...
    if (g_UsbMS.power_off_state[LUN] == false) {
        switch (msdc_status) {
            ...
            case USB_STORAGE_DEV_STATUS_NOMEDIA:
                ...
                break;
            default:
                LOG_MSGID_E(hal, "Checkmedia_Exist:ASSERT\n", 0);
                break;
        }
    } else {
        g_UsbMS.sensePtr = (uint8_t *)senseNoMedia;
...
static void USB_Ms_Cmd_Inquiry(uint8_t LUN, UsbMs_CBW *CBW)
{
    LOG_MSGID_I(hal, "USB_Ms_Cmd_Inquiry\n", 0);
    if (CBW->dCBWDataTransferLength == 0) {
        USB_Ms_State_Checkmedia_Exist(LUN);
...
static void USB_Ms_Cmd_Read_Capacity(uint8_t LUN, UsbMs_CBW *CBW)
{
    ...
    USB_Ms_State_Checkmedia_Exist(LUN);
    LOG_MSGID_I(hal, "USB_Ms_Cmd_Read_Capacity\n", 0);
    if (g_UsbMS.CSWStatusError == 0x00) {
...
static void USB_Ms_Cmd_Read_FormatCapacity(uint8_t LUN, UsbMs_CBW *CBW)
{
    ...
    LOG_MSGID_I(hal, "USB_Ms_Cmd_Read_FormatCapacity\n", 0);
    len = (uint8_t)CBWCB[7];
    len <<= 8;
    len |= (uint8_t)CBWCB[8];
...
static void USB_Ms_Read_Fail_Handler(uint8_t LUN, UsbMs_CBW *CBW)
{
    ...
    If this happens, airoha device will send CSW of 13 bytes out to PC,
