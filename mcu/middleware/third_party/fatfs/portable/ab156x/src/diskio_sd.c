...
static BYTE rcvr_spi(void)
{
    ...
    /* Now send_length == 0 but send_data is Not NULL,
     * called hal_spi_master_send_and_receive_polling will
     * return INVALID_PARAMETER, so Fix it
     */
...
)
{
    ...
    if ((Drive0_Stat & STA_NOINIT) == 0) {
        return Drive0_Stat;    /* SD card has been initialized */
    }
    ...
    if (ty) {			 /* Initialization succeded */
        ...
        /*the max freqency is HAL_SPI_MASTER_CLOCK_MAX_FREQUENCY which defined at hal_platform.h */
        set_frequency(HAL_SPI_MASTER_CLOCK_MAX_FREQUENCY / 2);
...
)
{
    ...
    while (retry < 3) {
        // printf("[R] retry=%d,sector=%d,count=%d\r\n",retry,sector,count);
       // status = hal_sd_read_blocks(fatfs_sd_port, (uint32_t *)buff, sector, count);
        status = hal_sd_read_blocks_dma_blocking(fatfs_sd_port, (uint32_t *)buff, sector, count);
...
)
{
    ...
    while (retry < 3) {
        // printf("[W] retry=%d,sector=%d,count=%d\r\n",retry,sector,count);
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &time[0]);
        //status = hal_sd_write_blocks(fatfs_sd_port, (uint32_t *)buff, sector, count);
        status = hal_sd_write_blocks_dma_blocking(fatfs_sd_port, (uint32_t *)buff, sector, count);
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &time[1]);
        // printf("mkfs time=%d\r\n",time[1] -time[0]);
        if (HAL_SD_STATUS_OK == status) {
            // printf("done!\r\n");
            return RES_OK;
