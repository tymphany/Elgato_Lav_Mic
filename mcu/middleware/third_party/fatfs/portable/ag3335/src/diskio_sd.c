...
DSTATUS sd_disk_state = STA_NOINIT;	/* Physical drive status */
...
void power_off(void)
{
    sd_disk_state |= STA_NOINIT;        /* Set STA_NOINIT */
}
...
)
{
    ...
    if (sd_disk_state & STA_NODISK) {
        return sd_disk_state;    /* No card in the socket */
    }
    ...
    if (ty) {			 /* Initialization succeded */
        sd_disk_state &= ~STA_NOINIT;		/* Clear STA_NOINIT */
        set_frequency(20000000); /*the max freqency is 60MHz*/
    } else {			/* Initialization failed */
        power_off();
    }

    return sd_disk_state;
}
...
)
{
    ...
    while (retry < 3) {
        ...
        if (HAL_SD_STATUS_OK == result) {
            sd_disk_state &= ~STA_NOINIT;	/* Clear STA_NOINIT flag */
            return sd_disk_state;
        } else {
            retry++;
        }
