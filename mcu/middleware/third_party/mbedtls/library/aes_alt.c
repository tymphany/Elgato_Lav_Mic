                          ...
                          unsigned char output[16])
{
    ...
    unsigned char *output_temp= (unsigned char *)pvPortMallocNC(48);
    unsigned char *input_temp= (unsigned char *)pvPortMallocNC(16);
    if (output_temp == NULL || input_temp == NULL) {
        mbedtls_printf("mbedtls_aes_crypt_ecb, malloc fail");
        return -1;
    }
    memset(output_temp, 0, 48);
    memset(input_temp, 0, 16);
    memcpy(input_temp, input, 16);

    output_buf.buffer = (uint8_t*)output_temp;
    output_buf.length = 48;
    input_buf.buffer = (uint8_t*)input_temp;
    input_buf.length = 16;
    ...
    if ( mode == 0 ) /* decrypt */
    {
        do {
            ...
            //printf("mbedtls hal_aes_ecb_decrypt, ret=%d\r\n", ret);
            if (ret == -100) {
                hal_gpt_delay_ms(1);
            }
        }while(ret == -100);
    ...
    else
    {
        do {
            ...
            //printf("mbedtls hal_aes_ecb_encrypt, ret=%d\r\n", ret);
            if (ret == -100) {
                hal_gpt_delay_ms(1);
            }
        }while(ret == -100);
