...
typedef enum {
    ...
    AUDIO_ANC_CONTROL_EVENT_BYPASS_PATH_REQUEST   = 1 << 10, /**< The event related to anc bypass path control.   */
...
typedef enum {
    ...
    AUDIO_ANC_CONTROL_EXTEND_RAMP_TYPE_SIDETONE_FF,                  /**< */
...
typedef enum {
    AUDIO_ANC_FF_LEFT_CH_MIC = 0,       /**< L channel of feedforward anc. */
    AUDIO_ANC_FB_LEFT_CH_MIC,           /**< L channel of feedback anc. */
    AUDIO_ANC_FF_RIGHT_CH_MIC,          /**< R channel of feedforward anc. */
    AUDIO_ANC_FB_RIGHT_CH_MIC,          /**< R channel of feedback anc. */
...
#ifdef AIR_ANC_V3
/** @brief The support bypass path ANC function. */
typedef struct {
    uint8_t     sub_event;            /**< Bypass path request event.   */
} audio_anc_control_bypass_path_req_param_t;
#endif
...
#ifdef AIR_ANC_V3
        audio_anc_control_bypass_path_req_param_t     bypass_param;   /**< Bypass path request parameter.*/
#endif
...
typedef struct audio_anc_control_misc_s {
    ...
    uint8_t  extend_sidetone_control;                  /**< Extend use.   */
    int32_t  sidetone_scenario_gain;                   /**< Extend use.   */
    union {
...
/**
 * @brief     This function is used to set ANC volume for sidetone usage.
 * @param[in] side_tone_gain is the gain of ANC path which would implement as sidetone.
 * @param[in] *control_misc is the other parameter which user wants to be set.
 * @return    Return AUDIO_ANC_CONTROL_EXECUTION_SUCCESS if the operation is successful. Otherwise, error occurs.
...
/**
 * @brief     This function is used to seamless switch filter for specific anc channle.
 * @param[in] filter_num is which specific anc channel for seamless switch filter.
 * @param[in] *coef is for input coefficient address.
 * @param[in] *control_misc is the other parameter which user wants to be set.
 * @return    Return AUDIO_ANC_CONTROL_EXECUTION_SUCCESS if the operation is successful. Otherwise, an error occurs.
 */
 audio_anc_control_result_t audio_anc_control_switch_specific_filter(audio_anc_control_filter_id_t filter_num, void *coef, audio_anc_control_misc_t *control_misc);
