...
typedef struct {
    ...
    bt_clock_t *pta;
} avm_direct_optimize_pta_param_t;
...
void avm_direct_cal_pta(uint32_t asi_dur, bt_clock_t *pta, bt_clock_t *clk_base, bt_role_t role);

bt_clock_t *avm_direct_get_sink_latency_by_tick();

int16_t avm_direct_cal_drift(int32_t n, const uint32_t *x, const int32_t *y);

void avm_direct_clear_drift_parameters();
...
void avm_direct_optimize_base_pta(avm_direct_optimize_pta_param_t *optimize_para);

bool avm_direct_calculate_pta_according_base(uint32_t frequence, uint32_t asi_duration, bt_clock_t *clock_base, bt_clock_t *clock_target);
