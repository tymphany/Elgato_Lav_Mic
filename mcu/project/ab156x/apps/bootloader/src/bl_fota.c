...
#include "bl_fota.h"
#include "hal_flash.h"
#include "hal_flash_disk.h" /* get_block_size_from_address */
...
#else
ATTR_ZIDATA_IN_TCM static uint32_t page_buffer[BL_FOTA_PAGE_SIZE];
#endif
...
bl_fota_status_t bl_fota_do_update()
{
    ...
    the_block_size = 0x1000;

    if (flash_block_size_enum == HAL_FLASH_BLOCK_32K) {
        the_block_size = 0x8000;
    } else if (flash_block_size_enum == HAL_FLASH_BLOCK_64K) {
        the_block_size = 0x10000;
    }
    BL_PRINT(LOG_DEBUG, "block size =%x (%d) \n\r", the_block_size, flash_block_size_enum);
