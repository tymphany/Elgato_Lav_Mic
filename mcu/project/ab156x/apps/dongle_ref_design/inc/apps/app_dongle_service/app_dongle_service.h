...
typedef enum {
    ...
    APP_DONGLE_SERVICE_HEADSET_EVENT_HADNSHAKE_DONE            = 0x02,
...
} app_dongle_service_headset_event_t;
...
#define APP_DONGLE_SERVICE_MODE_SWITCH_EVENT        0x01
...
bool app_dongle_service_is_headset_connected(void);
uint32_t app_dongle_service_get_headset_battery_level(void);
bool app_dongle_service_is_headset_charging(void);
uint32_t app_dongle_service_get_headset_fw_version(void);
...
} __attribute__((packed)) app_dongle_service_volume_change_t;

void app_dongle_service_update_volume_status(uint32_t volume_status, uint8_t which);

typedef struct {
    bool (*headset_connected)();
    bool (*headset_disconnected)();
    void (*headset_headset_power_off)();
    void (*headset_battery_level_changed)(uint32_t new_level);
    void (*headset_battery_charging_changed)(bool charging);
...
uint8_t app_dongle_service_get_headset_type(void);
