 ...
 * @{
 * @addtogroup apps_dongle_ref_design dongle_ref_design
 * @{

@par Overview
  - Application description
    - This application demonstrates dongle features based on the chip for the production of dongle.
  - Application features
    - 1. User Interface, such as LEDs for indication, keys for function control.
    - 2. Bluetooth audio. Streaming audio to an audio device. The streaming audio data is from USB.
    - 3. In USB module, support HID report to execute FW update and synchronize volume control with host.
  For more details on AWS, please contact Airoha customer support.
...
@par Run the application
  - How to build the dongle_ref_design application
  - GCC version
    - make command "./build.sh <board> dongle_ref_design -f=feature_<board>.mk"
      under the SDK/mcu root folder.
    - make command "./build.sh <board> dongle_ref_design -fm=feature_<board>.mk -fd0=feature_<board>.mk" 
      under the SDK root folder.
  - How to download the dongle_ref_design application
    - Install and use Flash Tool ("sdk_root/tools/PC_tool_Win.zip") to download the
      application binary dongle_ref_design.bin.
  - How to run each feature
    - Power on the board and use the smartphone to search for and connect to the BT device.
      Then use BT device as a dongle.
