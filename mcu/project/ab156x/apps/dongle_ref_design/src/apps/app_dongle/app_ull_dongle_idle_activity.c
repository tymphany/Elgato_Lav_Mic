...
#define APP_WIRELESS_MIC_MODE_MERGE         (1)
#define APP_WIRELESS_MIC_MODE_SPLIT_LEFT    (6)
#define APP_WIRELESS_MIC_MODE_SPLIT_RIGHT   (7)
            ...
            if (NVKEY_STATUS_OK == nvkey_ret) {
                ...
                for (i = 0; i < tx_mic_amount; i++) {
                    ...
                    if (connected_record_id[i] == 0xFF) {
                        ...
                        nvkey_record[new_record_count].mode_id = nvkey_record_count > 0 ? nvkey_record[0].mode_id : APP_WIRELESS_MIC_MODE_MERGE;
        ...
        case KEY_DONGLE_SWITCH_SPLIT_MERGE_MODE: {
            ...
            if (NULL != nvkey_record) {
                if (nvkey_record_count > 0) {
                    uint8_t i;
                    if (nvkey_record[0].mode_id == 1) {
                        for (i = 0; i < nvkey_record_count; i++) {
                            if (i % 2 == 0) {
                                nvkey_record[i].mode_id = APP_WIRELESS_MIC_MODE_SPLIT_LEFT;
                            } else {
                                nvkey_record[i].mode_id = APP_WIRELESS_MIC_MODE_SPLIT_RIGHT;
                            }
                        }
                    } else {
                        for (i = 0; i < nvkey_record_count; i++) {
                            nvkey_record[i].mode_id = APP_WIRELESS_MIC_MODE_MERGE;
                        }
                    }
