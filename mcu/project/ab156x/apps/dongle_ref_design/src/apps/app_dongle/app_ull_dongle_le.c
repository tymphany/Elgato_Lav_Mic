...
#define APP_ULL_DONGLE_LE_STATE_ENABLED       1
#define APP_ULL_DONGLE_LE_STATE_DISABLING     2
...
static bt_status_t app_ull_dongle_le_add_black_list(bt_addr_t *addr)
{
    uint8_t i;
...
static void app_ull_dongle_le_update_bonded_info(void)
{
...
#ifdef APP_ULL_DONGLE_LE_DEBUG   /* just For test */
    {
        ...
        while (i > 0) {
            i--;
            tempaddr = g_app_ull_le_bonded_info.addr[i*BT_BD_ADDR_LEN];
...
static bool app_ull_dongle_le_is_ull_adv(bt_gap_le_ext_advertising_report_ind_t *adv_report)
{
    ...
    if (NULL != (uuid = bt_ull_le_srv_get_uuid())) {
...
bt_status_t app_ull_dongle_le_scan_device(app_ull_dongle_le_scan_t scan_type)
{
    ...
    switch (scan_type) {
        ...
        case APP_ULL_DONGLE_LE_SCAN_BONDED_DEVICE: {
            ...
            g_app_ull_le_ctrl.curr_conn = APP_ULL_DONGLE_LE_CONN_BONDED_DEVICE;
            if (BT_STATUS_SUCCESS != (status = app_ull_dongle_le_start_scan_device(APP_ULL_DONGLE_LE_SCAN_BONDED_DEVICE, true))) {
                g_app_ull_le_ctrl.curr_conn = APP_ULL_DONGLE_LE_CONN_NONE;
            }
...
static void app_ull_dongle_le_handle_scan_cnf(bt_status_t status)
{
    ...
    switch (g_app_ull_le_ctrl.curr_scan) {
        ...
        case APP_ULL_DONGLE_LE_SCAN_CS_DISABLING: {
            ...
            if (APP_ULL_DONGLE_LE_SCAN_ENABLING == g_app_ull_le_ctrl.next_scan) {
                bool use_white_list = (APP_ULL_DONGLE_LE_CONN_BONDED_DEVICE == g_app_ull_le_ctrl.curr_conn) ? true : false;
...
static void app_ull_dongle_le_handle_adv_report_ind(bt_status_t status, bt_gap_le_ext_advertising_report_ind_t *ind)
{
    ...
    if (APP_ULL_DONGLE_LE_CONN_COORDINATED_SET_BY_SIRK < g_app_ull_le_ctrl.curr_conn) {
...
static void app_ull_dongle_le_handle_connect_ind(bt_status_t status, bt_gap_le_connection_ind_t *ind)
{
    ...
    APPS_LOG_MSGID_I(APP_ULL_LOG_TAG"LE_CONNECT_IND, handle:0x%x status:0x%x", 2, ind->connection_handle, status);
...
static void app_ull_dongle_le_handle_bonding_complete_ind(bt_status_t status, bt_gap_le_bonding_complete_ind_t *ind)
{
    ...
    if ((!ind) || (NULL == (p_info = app_ull_dongle_le_get_link_info(ind->handle))) || (BT_STATUS_SUCCESS != status)) {
        return;
    }
    app_ull_dongle_le_update_bonded_info();
...
static void app_ull_dongle_le_handle_ull_connect_ind(bt_ull_le_connected_info_t *ind)
{
    ...
    uint8_t link_num;
    APPS_LOG_MSGID_I(APP_ULL_LOG_TAG"Group device addr:type: %d, addr: %x-%x-%x-%x-%x-%x", 7, ind->group_device_addr[0].type,
    ...
    if ((!ind) || (BT_HANDLE_INVALID == ind->conn_handle)) {
    ...
    if (BT_STATUS_SUCCESS != ind->status) {
        ...
        /* add to white list */
        if (app_ull_dongle_le_is_bonded_device(&(app_ull_dongle_le_get_link_info(ind->conn_handle)->addr))) {
            bt_status_t ret = bt_gap_le_set_white_list(BT_GAP_LE_ADD_TO_WHITE_LIST, &(app_ull_dongle_le_get_link_info(ind->conn_handle)->addr));
            APPS_LOG_MSGID_I(APP_ULL_LOG_TAG"[APP][U] add_white_list, ret:%x", 1, ret);
        }
#if defined (MTK_RACE_CMD_ENABLE) && defined (AIR_WIRELESS_MIC_ENABLE)
        ...
        if (1 == link_num) {/**the  firsrt connection always trigger by app dongle cm module, so notify it the result */
...
static void app_ull_dongle_le_handle_disconnect_ind(bt_status_t status, bt_gap_le_disconnect_ind_t *ind)
{
    ...
    if (APP_ULL_DONGLE_LE_INVALID_LINK_IDX == (link_idx = app_ull_dongle_le_get_link_idx(ind->connection_handle))) {
        APPS_LOG_MSGID_E(APP_ULL_LOG_TAG"[APP][U] LE_DISCONNECT_IND, link not exist (hdl:%x)", 1, ind->connection_handle);
        return;
    }
    dis_ind.ret = status;
    ...
    if ((BT_STATUS_SUCCESS != status) || (!ind)) {
        APPS_LOG_MSGID_E(APP_ULL_LOG_TAG"[APP][U] LE_DISCONNECT_IND Fail, handle:%x status:%x", 2, ind->connection_handle, status);
        return;
    }
...
bt_status_t app_ull_dongle_le_disconnect_all_device(void)
{
    ...
    while (0 != i) {
        i--;
        if (BT_HANDLE_INVALID != g_app_ull_le_link_info[i].handle) {
            if (BT_STATUS_SUCCESS != app_ull_dongle_le_disconnect(g_app_ull_le_link_info[i].handle)) {
