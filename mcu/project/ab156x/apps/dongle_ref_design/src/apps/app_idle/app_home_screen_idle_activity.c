...
static bool homescreen_app_usb_audio_event_proc(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    ...
    switch (event_id) {
        ...
        case APPS_EVENTS_USB_CONFIG_CHECKER: {
            ...
            APPS_LOG_MSGID_I(UI_SHELL_IDLE_BT_CONN_ACTIVITY", USB configure state : %d", 1, state);
        ...
        case APPS_EVENTS_USB_AUDIO_RESET: {
            //APPS_LOG_MSGID_I(UI_SHELL_IDLE_BT_CONN_ACTIVITY", USB reset, Turn BT on", 0);
            //app_bt_state_service_set_bt_on_off(true, false, true, false);
...
bool bt_cm_check_connect_request(bt_bd_addr_ptr_t address, uint32_t cod)
{
    ...
    if (0 != memcmp(s_app_homescreen_context.ull_peer_addr, address, sizeof(bt_bd_addr_t))) {
        allow_connect = false;
