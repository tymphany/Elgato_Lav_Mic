                                            ...
                                            apps_config_led_manager_aws_sync_priority_t priority)
{
    ...
    bool ret;                   /* The return value of calling the LED module APIs. */
...
static bool _process_foreground_led_pattern(uint8_t index, uint16_t timeout, bool need_delay, uint32_t gpt_time)
{
    ...
    xTimerChangePeriod(s_timer_handle, timeout * LED_FG_TIMEOUT_UNIT_MS / portTICK_PERIOD_MS, 0);
...
void app_config_led_sync(void *param)
{
    if (param) {
        ...
        if (LED_PATTERN_FG == p_pattern_info->pattern_type) {
            ...
            bt_sink_srv_convert_bt_clock_2_gpt_count(&p_pattern_info->bt_clock, &target_gpt_time);
            APPS_LOG_MSGID_I(LOG_TAG"app_config_led_sync, remain sync gpt time:%d", 1, target_gpt_time);
            _process_foreground_led_pattern(p_pattern_info->pattern_index, p_pattern_info->timeout, true, target_gpt_time);
            ...
            if (LED_PATTERN_INVALID_INDEX == p_pattern_info->pattern_index) {
                ...
                if (p_pattern_info->sync_priority >= s_background_self_priority) {
                    ...
                    uint32_t target_gpt_time;
                    bt_sink_srv_convert_bt_clock_2_gpt_count(&p_pattern_info->bt_clock, &target_gpt_time);
                    APPS_LOG_MSGID_I(LOG_TAG"app_config_led_sync, target gpt_time:%d", 1, target_gpt_time);
                    _process_background_led_pattern(p_pattern_info->pattern_index, true, true, target_gpt_time);
                } else {
