...
#if defined(AIR_USB_AUDIO_2_SPK_ENABLE) && defined(AIR_USB_AUDIO_ENABLE) && !defined(APPS_USB_AUDIO_SUPPORT)
...
uint8_t apps_event_usb_get_speaker_enable_info(uint8_t interface_id, uint8_t *sample_size, uint8_t *channel)
{
    ...
    *channel = s_speaker_enable_control[interface_id].channel;
    ...
    USB_EVENT_LOG_I("get_speaker_enable_info[%d], enable: %d, sample_size: %d, channel: %d, acount: %d", 5, interface_id, enabled, *sample_size, *channel, count);
    return enabled;
}
