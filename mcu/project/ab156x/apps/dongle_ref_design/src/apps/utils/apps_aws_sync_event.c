                                           ...
                                           uint32_t event_id, const void *extra_data, uint32_t extra_data_len)
{
    ...
    if (!data_array) {
        ...
        APPS_LOG_MSGID_I(LOG_TAG"apps_aws_sync_event_send_extra group-event: 0x%x-0x%x, len: %d", 3, event_group, event_id, extra_data_len);
...
bt_aws_mce_report_info_t *s_last_aws_report_info = NULL;
