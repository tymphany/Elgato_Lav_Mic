...
#if (defined(AIR_LE_AUDIO_ENABLE) && defined(AIR_LE_AUDIO_UNICAST_ENABLE)) || defined (AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
static bt_gap_le_smp_pairing_config_t pairing_config_le_audio = {//MITM, Bond, OOB
    ...
    .oob_data_flag = BT_GAP_LE_SMP_OOB_DATA_NOT_PRESENTED,
    .initiator_key_distribution = BT_GAP_LE_SMP_KEY_DISTRIBUTE_ENCKEY | BT_GAP_LE_SMP_KEY_DISTRIBUTE_SIGN,
    .responder_key_distribution = BT_GAP_LE_SMP_KEY_DISTRIBUTE_ENCKEY | BT_GAP_LE_SMP_KEY_DISTRIBUTE_IDKEY | BT_GAP_LE_SMP_KEY_DISTRIBUTE_SIGN,
};
#endif
...
extern void app_le_audio_ucst_handle_connect_ind(bt_status_t ret, bt_gap_le_connection_ind_t *ind);
#endif
...
void bt_app_common_adv_ind_cb(bt_msg_type_t msg, bt_status_t status, void *buff)
{
    ...
    if ((report->address.type & 0x0D) == BT_ADDR_PUBLIC) {
        ...
        snprintf((char *)addr_buff, 50, "Public, %02x:%02x:%02x:%02x:%02x:%02x", report->address.addr[5],
                 report->address.addr[4],
                 report->address.addr[3],
                 report->address.addr[2],
                 report->address.addr[1],
        ...
        snprintf((char *)addr_buff, 50, "Random, %02x:%02x:%02x:%02x:%02x:%02x", report->address.addr[5],
                 report->address.addr[4],
                 report->address.addr[3],
                 report->address.addr[2],
                 report->address.addr[1],
    ...
    while (count < report->data_length) {
        ...
        if (ad_data_type == BT_GAP_LE_AD_TYPE_NAME_COMPLETE) {
            ...
            snprintf((char *)report_string, 90, "ADV report %s Complete Name: %s\r\n", addr_buff, local_buff);
...
extern bt_status_t bt_gap_le_srv_bond(bt_handle_t connection_handle, bt_gap_le_smp_pairing_config_t *pairing_config);
static bt_status_t bt_app_common_event_callback(bt_msg_type_t msg, bt_status_t status, void *buff)
{
    ...
    switch (msg) {
...
#ifdef AIR_LE_AUDIO_BA_ENABLE
            app_le_audio_ba_handle_connect_ind(status, connection_ind);
#endif
#endif
            bt_gap_le_srv_bond(connection_ind->connection_handle, &pairing_config_le_audio);
#else
        ...
        case BT_GAP_LE_BONDING_COMPLETE_IND: {
            ...
            LOG_MSGID_I(BT_APP, "Bond complete, conn_handle(0x%04x).\r\n", 1, ind->handle);
            sta = bt_gattc_discovery_start(BT_GATTC_DISCOVERY_USER_LE_AUDIO, ind->handle, false);
            LOG_MSGID_I(BT_APP, "start discovery, status is %d\r\n", 1, sta);
#endif
        }
        break;
...
{
    LOG_MSGID_I(BT_APP, "[APP][COMMON] connection handle = %02x, is_previously_bonded(%d)", 2, ind->handle, ind->is_previously_bonded);
...
#if (defined(AIR_LE_AUDIO_ENABLE) && defined(AIR_LE_AUDIO_UNICAST_ENABLE)) || defined (AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
    LOG_MSGID_I(BT_APP,"[APP][COMMON] connection handle is le audio link", 0);
...
static bt_status_t bt_app_common_security_param_init(void)
{
    ...
    /*Generate LTK. */
    bt_app_common_generate_random_key((uint8_t *)&local_key.encryption_info.ltk, size);
    ...
    /*Generate Rand. */
    bt_app_common_generate_random_key((uint8_t *)&local_key.master_id.rand, 8);
    key_ptr = (uint8_t *)&local_key.master_id.rand;
    LOG_MSGID_I(BT_APP, "[BT]local random:%x%x%x%x %x%x%x%x", 8,
                key_ptr[0], key_ptr[1], key_ptr[2], key_ptr[3],
                key_ptr[4], key_ptr[5], key_ptr[6], key_ptr[7]);
    /*Generate CSRK. */
    bt_app_common_generate_random_key((uint8_t *)&local_key.signing_info.csrk, size);
...
#endif
#if defined (AIR_BT_ULTRA_LOW_LATENCY_ENABLE) || defined (AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
    bt_ull_init(BT_ULL_ROLE_SERVER, bt_ulla_callback);
#endif
