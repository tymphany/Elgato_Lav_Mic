...
extern bt_status_t ble_vcp_read_volume_state(bt_handle_t handle);
extern ble_tmap_role_t ble_tmas_get_role(void);
extern bt_status_t ble_vcp_read_volume_flags(bt_handle_t handle);
extern bt_status_t ble_vcp_volume_down(bt_handle_t handle);
extern bt_status_t ble_vcp_volume_up(bt_handle_t handle);
extern bt_status_t ble_vcp_unmute_volume_down(bt_handle_t handle);
extern bt_status_t ble_vcp_unmute_volume_up(bt_handle_t handle);
...
extern bt_status_t ble_micp_read_mute_req(bt_handle_t handle);
...
static uint32_t bt_app_comm_at_cmd_le_audio_conn_addr_hdl(char *pChar)
{
    ...
    ret = app_le_audio_ucst_connect_device(&addr);
...
static uint32_t bt_app_comm_at_cmd_le_audio_mic_hdl(char *pChar, atci_response_t *response)
{
    ...
    if (0 == memcmp(pChar, "MUTE", 4)) {
        /* AT+LEAUDIO=MIC,MUTE */
        result = bt_le_audio_source_send_action(BT_LE_AUDIO_SOURCE_ACTION_MIC_MUTE, NULL);

    } else if (0 == memcmp(pChar, "UNMUTE", 6)) {
        /* AT+LEAUDIO=MIC,UNMUTE */
        result = bt_le_audio_source_send_action(BT_LE_AUDIO_SOURCE_ACTION_MIC_UNMUTE, NULL);
        ...
        for (i = 0; i < APP_LE_AUDIO_UCST_LINK_MAX_NUM; i++) {
            if (BT_HANDLE_INVALID != (handle = app_le_audio_ucst_get_handle(i))) {
                result = ble_micp_read_mute_req(handle);
                break;
            }
        }
    }
...
static uint32_t bt_app_comm_at_cmd_le_audio_volume_hdl(char *pChar)
{
    ...
    if (0 == memcmp(pChar, "MUTE", 4)) {
        ...
        for (i = 0; i < APP_LE_AUDIO_UCST_LINK_MAX_NUM; i++) {
            if (BT_HANDLE_INVALID != (handle = app_le_audio_ucst_get_handle(i))) {
                ble_vcp_read_volume_state(handle);
            }
        }
        ...
        for (i = 0; i < APP_LE_AUDIO_UCST_LINK_MAX_NUM; i++) {
            if (BT_HANDLE_INVALID != (handle = app_le_audio_ucst_get_handle(i))) {
                ble_vcp_read_volume_flags(handle);
            }
        }
        ...
        for (i = 0; i < APP_LE_AUDIO_UCST_LINK_MAX_NUM; i++) {
            if (BT_HANDLE_INVALID != (handle = app_le_audio_ucst_get_handle(i))) {
                ble_vcp_unmute_volume_down(handle);
            }
        }
        ...
        for (i = 0; i < APP_LE_AUDIO_UCST_LINK_MAX_NUM; i++) {
            if (BT_HANDLE_INVALID != (handle = app_le_audio_ucst_get_handle(i))) {
                ble_vcp_unmute_volume_up(handle);
            }
        }
    }
#endif
...
static uint32_t bt_app_comm_at_cmd_le_audio_unicast_hdl(char *pChar)
{
    ...
    if (0 == memcmp(pChar, "START", 5)) {
        ...
        for (i = 0; i < APP_LE_AUDIO_UCST_LINK_MAX_NUM; i++) {
            if (BT_HANDLE_INVALID != (handle = app_le_audio_ucst_get_handle(i))) {
                app_le_audio_ucst_config_codec(handle);
            }
        }
...
static uint32_t bt_app_comm_at_cmd_le_audio_set_codec_hdl(char *pChar)
{
    ...
    config->param[0].codec_specific_configuration_length = 16;
