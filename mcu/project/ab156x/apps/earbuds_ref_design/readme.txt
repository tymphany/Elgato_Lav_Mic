 ...
 * @{
 * @addtogroup apps_earbuds_ref_design earbuds_ref_design
 * @{
...
@par Overview
  ...
  - Application features
    ...
    - 3. Bluetooth music. Streaming music from an audio device. Streaming controls include:
         playing or pausing the music; moving to the previous or next track; Advanced Wireless
         Stereo with Multi-Cast Extension (AWS_MCE) feature for high quality streaming between
         two earbuds. For more details on AWS_MCE, please contact Airoha customer support.
...
@par Run the application
  - How to build the earbuds_ref_design application
  - GCC version
    - make command "./build.sh <board> earbuds_ref_design -f=feature_<board>.mk"
      under the SDK/mcu root folder.
    - make command "./build.sh <board> earbuds_ref_design -fm=feature_<board>.mk -fd0=feature_<board>.mk" 
      under the SDK root folder.
  - How to download the earbuds_ref_design application
    - Install and use Flash Tool ("sdk_root/tools/PC_tool_Win.zip") to download the
      application binary earbuds_ref_design.bin.
  - How to run each feature
    - Power on the board and use the smartphone to search for and connect to the BT device.
      Then use BT device as a earbuds.
