...
{
    uint8_t i = 0;
    bt_cm_link_info_t *link_info = NULL;
    for (i = 0; i < APP_CONN_MAX_DEVICE_NUM; i++) {
        if (local_ctx->conn_device[i].conn_service != BT_CM_PROFILE_SERVICE_MASK_NONE
            && 0 != memcmp(local_ctx->conn_device[i].addr, zero_bt_addr, sizeof(bt_bd_addr_t))) {
            link_info = bt_cm_get_link_information(local_ctx->conn_device[i].addr);
            if (link_info == NULL || BT_CM_ACL_LINK_CONNECTED > link_info->link_state) {
                APPS_LOG_MSGID_W(UI_SHELL_IDLE_BT_CONN_ACTIVITY", remove the disconnected device %d", 1, i);
                memcpy(&(local_ctx->conn_device[i].addr), &zero_bt_addr, sizeof(bt_bd_addr_t));
                local_ctx->conn_device[i].conn_service = BT_CM_PROFILE_SERVICE_MASK_NONE;
                local_ctx->conn_device_num --;
            }
        }
    }
                ...
                if ((remote_update->connected_service & ~remote_update->pre_connected_service)
                    ...
                    if (local_ctx->connection_state && bt_conn_component_add_device(false, local_ctx, remote_update)) {
                        ...
                        if (local_ctx->conn_device_num == 0) {
                            ...
                            bt_conn_component_update_mmi();
#ifdef MTK_AWS_MCE_ENABLE
                            APPS_LOG_MSGID_I(UI_SHELL_IDLE_BT_CONN_ACTIVITY", aws acl false.", 0);
