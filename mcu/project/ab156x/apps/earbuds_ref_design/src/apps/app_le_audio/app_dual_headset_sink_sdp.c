...
static bool app_lea_dhss_is_device_peer = FALSE;
...
bool app_le_audio_dhss_read_peer_le_addr(void)
{
    ...
    app_lea_dhss_is_device_peer = TRUE;
...
bool app_le_audio_dhss_get_le_addr_list(uint8_t *buf)
{
    ...
    if (!app_lea_dhss_is_device_peer) {
...
void app_le_audio_dhss_set_peer_le_addr(bt_addr_type_t type, uint8_t *addr)
{
    ...
    APPS_LOG_MSGID_I(LOG_TAG" set_peer_le_addr, is_paired=%d", 1, app_lea_dhss_is_device_peer);
...
bool app_le_audio_dhss_is_data_ready(void)
{
    ...
    if (!app_lea_dhss_is_device_peer || !app_lea_dhss_read_local_le_addr) {
        APPS_LOG_MSGID_E(LOG_TAG" is_data_ready, peer=%d local=%d",
                         2, app_lea_dhss_is_device_peer, app_lea_dhss_read_local_le_addr);
    }
    return (app_lea_dhss_is_device_peer && app_lea_dhss_read_local_le_addr);
}
