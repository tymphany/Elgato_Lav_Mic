...
#ifdef MTK_AWS_MCE_ENABLE
    //After One earbud scan the source adddress, then the other earbud add the address to white list and scan
    if (BT_STATUS_SUCCESS == ble_bap_set_white_list_ex(BT_GAP_LE_ADD_TO_WHITE_LIST, app_bis_ctx.peer_src_addr_type, app_bis_ctx.peer_src_addr)) {
    ...
    if (bt_sink_srv_cap_stream_is_scanning_broadcast_source()) {
        ...
        if (bt_status == BT_STATUS_SUCCESS) {
            if (app_bis_ctx.state != APP_LEA_BIS_STATE_SCANNING) {
                app_bis_ctx.state = APP_LEA_BIS_STATE_SCANNING;
            }
...
static bool app_le_audio_bis_start(bool start)
{
    ...
    APPS_LOG_MSGID_I(LOG_TAG" bis_start, is_bis_streaming=%d start=%d success=%d",
                     3, is_bis_streaming, start, success);
    return success;
}
...
{
    APPS_LOG_MSGID_I(LOG_TAG" bis_action_by_peer, sync_feature=%d", 1, app_bis_sync_feature);
    ...
    if (app_bis_sync_feature) {
        ...
        if (app_bis_ctx.peer_state == APP_LEA_BIS_STATE_STREAMING) {
            if (app_bis_ctx.state != APP_LEA_BIS_STATE_STREAMING
                ...
                // After One earbud's BIS was established, then the other earbud start to sync BIS state
                // After Power on, earbuds will sync state to each other
                if (app_bis_ctx.state != APP_LEA_BIS_STATE_SCANNING) {
                    ...
                    APPS_LOG_MSGID_I(LOG_TAG" AWS Data Event, already scanning", 0);
                }
            } else {
                APPS_LOG_MSGID_I(LOG_TAG" AWS Data Event, not sync and start", 0);
            }
        }
        // Scan action by peer source address and scanning state
        if (app_bis_ctx.peer_state == APP_LEA_BIS_STATE_SCANNING) {
            if (app_bis_ctx.state != APP_LEA_BIS_STATE_SCANNING
                && memcmp(peer_addr, empty_addr, BT_BD_ADDR_LEN) != 0) {
                memcpy(app_bis_ctx.src_addr, peer_addr, BT_BD_ADDR_LEN);
                // After One earbud scan the source adddress, then the other earbud add the address to white list and scan
                APPS_LOG_MSGID_I(LOG_TAG" AWS Data Event, scan white list %02X:%02X:%02X:%02X:%02X:%02X",
                                 6, peer_addr[5], peer_addr[4], peer_addr[3],
                                 peer_addr[2], peer_addr[1], peer_addr[0]);
                app_le_audio_bis_start(TRUE);
            } else {
                                                 ...
                                                 void *extra_data, size_t data_len)
{
    switch (event_id) {
        ...
        case EVENT_ID_LE_AUDIO_BIS_STOP_STREAMING: {
            ...
            ble_bap_set_white_list_ex(BT_GAP_LE_REMOVE_FROM_WHITE_LIST, app_bis_ctx.peer_src_addr_type, app_bis_ctx.peer_src_addr);
#endif
            app_le_audio_bis_set_state(APP_LEA_BIS_STATE_IDLE);
            break;
        }
        ...
        case EVENT_ID_LE_AUDIO_BIS_SCAN_WHITE_LIST: {
            ...
            APPS_LOG_MSGID_I(LOG_TAG" SCAN_SRC_ADDR event, src_addr_type=%02X, src_addr=%02X:%02X:%02X:%02X:%02X:%02X",
