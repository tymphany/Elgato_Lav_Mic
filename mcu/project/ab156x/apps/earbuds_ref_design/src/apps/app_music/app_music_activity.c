    ...
    switch (key_action) {
        ...
        case KEY_VOICE_DN: {
            ...
            if (app_music_get_ull_is_streaming()) {
                ...
                app_music_set_ull_volume(volume_up, rotary_data);
            } else
#endif
            {
...
static bool app_music_check_and_end_music(struct _ui_shell_activity *self, void *extra_data)
{
    ...
    APPS_LOG_MSGID_I(APP_MUSIC_ACTI" app_music_check_and_end_music, cur=%d, pre=%d, isAutoPaused=%d, music_playing=%d, in_ear_config=%d",
                               ...
                               size_t data_len)
{
    ...
    switch (event_id) {
        case BT_ULL_EVENT_LE_STREAMING_STOP_IND: {
            ...
            if (false == app_music_ull2_downlink_enable)
            {
                app_music_finish_music_activity(self,local_ctx);
                APPS_LOG_MSGID_I(APP_MUSIC_ACTI" app_music_proc_ull_events-STOP_IND end music", 0);
            }
            break;
        }
