...
static bt_status_t _role_handover_update_data_callback(bt_role_handover_update_info_t *info)
{
...
#endif
    {
        if (info->role == BT_AWS_MCE_ROLE_PARTNER) {
            ...
            APPS_LOG_MSGID_I(APP_MUSIC_TAG" [RHO] update_data_callback, recv role=0x%02X,old[isAutoPaused=%d:isPlaying=%d], new[isAutoPaused=%d:isPlaying=%d]",
                             5, info->role, s_app_music_context.isAutoPaused, s_app_music_context.music_playing,
                             isAutoPaused, music_playing);
    ...
    switch (event_id) {
        case BT_ULL_EVENT_LE_STREAMING_START_IND: {
            ...
            if (!local_ctx->music_playing
                && (false == app_music_ull2_uplink_enable && true == app_music_ull2_downlink_enable) ) {
