...
#if 0//def AIR_BT_AUDIO_SYNC_ENABLE
...
static void app_call_rx_eq_type_load(void)
{
    ...
    if (status == NVKEY_STATUS_ITEM_NOT_FOUND) {
        ...
        nvkey_write_data(NVID_APP_CALL_RX_EQ_TYPE, (const uint8_t *)&(s_app_rx_eq_context.call_rx_eq_type), size);
    }
#endif
}
...
static bool app_call_rx_eq_type_local_set(app_call_rx_eq_type_t type, uint32_t target_gpt)
{
    ...
    /*Todo: set rx eq type. */
#if 0
    cm4_dsp_audio_sync_scenario_type_t scenario = (s_app_rx_eq_context.call_type == BT_SINK_SRV_DEVICE_EDR) ? MCU2DSP_SYNC_REQUEST_HFP : MCU2DSP_SYNC_REQUEST_BLE;
    bool set_eq_ret = voice_set_RX_EQ_mode(scenario, type);
    if (!set_eq_ret) {
        return false;
    }
#endif
    ...
    if (target_gpt != 0) {
        ...
        bt_sink_srv_am_id_t aud_id = (s_app_rx_eq_context.call_type == BT_SINK_SRV_DEVICE_EDR) ? AM_HFP : AM_BLE;
...
app_call_rx_eq_set_status_t app_call_rx_eq_type_set(app_call_rx_eq_type_t type)
{
    ...
    if (s_app_rx_eq_context.curr_sink_state <= BT_SINK_SRV_STATE_STREAMING) {
        set_status = APP_CALL_RX_EQ_SET_NO_CALL_STATE;
        goto exit;
    }
    ...
    if (BT_AWS_MCE_SRV_LINK_NONE != bt_aws_mce_srv_get_link_type()) {
        ...
        bool sync_ret = app_call_rx_eq_type_sync(s_app_rx_eq_context.call_rx_eq_type, bt_clk);
        if (sync_ret == true) {
...
exit:
    APPS_LOG_MSGID_I(LOG_TAG" set_rx_eq: type=%d, status=%d", 2, type, set_status);
        ...
        if (event_group == EVENT_GROUP_UI_SHELL_APP_INTERACTION
         && action == APPS_EVENTS_INTERACTION_SYNC_CALL_RX_EQ_STATUS) {
            if (BT_AWS_MCE_ROLE_PARTNER == bt_connection_manager_device_local_info_get_aws_role()) {
                ...
                APPS_LOG_MSGID_I(LOG_TAG" [AWS_DATA] bt_clk 0x%08x 0x%08x", 2, rx_eq_sync_data->bt_clk.nclk, rx_eq_sync_data->bt_clk.nclk_intra);
                ...
                case BT_SINK_SRV_EVENT_STATE_CHANGE: {
                    ...
                    if (param->current > BT_SINK_SRV_STATE_STREAMING) {
                        ...
                        if (get_ret == BT_STATUS_SUCCESS ) {
                            rx_eq_context->call_type = curr_device_state.type;
                            APPS_LOG_MSGID_I(LOG_TAG" sink state change: call_type=%d.", 1, rx_eq_context->call_type);
                        }
                    }
                    break;
                }
