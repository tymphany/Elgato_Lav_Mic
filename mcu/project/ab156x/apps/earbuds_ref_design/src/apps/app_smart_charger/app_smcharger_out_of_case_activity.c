...
#define LOG_TAG "[SMCharger][OUT_OF_CASE]"
...
static bool smcharger_out_of_case_proc_ui_shell_group(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    ...
    switch (event_id) {
        case EVENT_ID_SHELL_SYSTEM_ON_CREATE: {
            ...
            app_smcharger_battery_state_t battery_state = app_smcharger_get_battery_state();
            if (battery_state == SMCHARGER_BATTERY_STATE_SHUTDOWN) {
                APPS_LOG_MSGID_I(LOG_TAG" ShutDown in create", 0);
                need_power_off = true;
                app_smcharger_power_off(TRUE);
            } else if (battery_state == SMCHARGER_BATTERY_STATE_LOW_CAP) {
                APPS_LOG_MSGID_I(LOG_TAG" LowCap in create", 0);
...
bool app_smcharger_out_of_case_activity_proc(struct _ui_shell_activity *self, uint32_t event_group, uint32_t event_id,
                                             void *extra_data, size_t data_len)
{
    ...
    switch (event_group) {
        ...
        case EVENT_GROUP_UI_SHELL_CHARGER_CASE: {
            ...
            ret = false;
            if (event_id == SMCHARGER_EVENT_CHARGER_IN) {
