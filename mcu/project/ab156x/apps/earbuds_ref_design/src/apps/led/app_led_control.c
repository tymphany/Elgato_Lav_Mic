...
enum {
    BT_SYNC_TYPE_START = 1,
    BT_SYNC_TYPE_INFO  = 2,
    BT_SYNC_TYPE_STOP  = 3
};

enum {
    BT_SYNC_FLAG_PHONE = 0,
    BT_SYNC_FLAG_AWS   = 1
};
...
void    app_led_bt_sync_callback(uint8_t type, BT_TIME_STRU syncTime, uint32_t gpt_count, uint8_t aws_flag)
{
    uint32_t bt_tm_ms = 0;
    ...
    if ((type == BT_SYNC_TYPE_START || type == BT_SYNC_TYPE_INFO) && led_control_is_twinkle_mode(p_style) && (temp != 0)) {
...
#define     BT_TICK_PER_SECOND          1000000
    hal_gpt_status_t status;
    bt_pka_sync_register_callback(0, app_led_bt_sync_callback, 13 * BT_TICK_PER_SECOND);
