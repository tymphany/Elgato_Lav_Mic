...
#if ((defined(AIR_SWIFT_PAIR_ENABLE) && (defined(AIR_CUST_PAIR_ENABLE) || defined(APP_SWIFT_PAIR_LE_EDR_SECURE_MODE))) || (defined(AIR_LE_AUDIO_ENABLE) && defined(AIR_LE_AUDIO_DUALMODE_ENABLE)))
...
#if defined(AIR_BT_FAST_PAIR_SASS_ENABLE) && defined(AIR_BT_SINK_SRV_STATE_MANAGER_ENABLE)
    if ((NULL != current_device && BT_SINK_SRV_DEVICE_EDR == current_device->type)
        && (NULL != coming_device && BT_SINK_SRV_DEVICE_EDR == coming_device->type)) {
