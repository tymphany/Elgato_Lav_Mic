...
BT_ALIGNMENT4(
    static char bt_avrcp_ex_link_cb_buf[BT_AVRCP_LINK_EX_BUF_SIZE]
);
#else
BT_ALIGNMENT4(
...
BT_ALIGNMENT4(
    static char bt_hid_link_cb_buf[BT_HID_LINK_BUF_SIZE]
);
#endif
...
BT_ALIGNMENT4(
    static char le_otp_cb_buf[BT_LE_OTP_BUF_SIZE]
);
#endif
...
static void bt_demo_init_local_address(void)
{
    ...
    bt_bd_addr_t *aws_addr = bt_device_manager_aws_local_info_get_fixed_address();
    if (!memcmp(local_addr, temp_addr, sizeof(bt_bd_addr_t)) && !memcmp(aws_addr, temp_addr, sizeof(bt_bd_addr_t))) {
        bt_demo_generate_local_address(&temp_addr);
        bt_device_manager_store_local_address(&temp_addr);
    } else if (!memcmp(local_addr, temp_addr, sizeof(bt_bd_addr_t))) {
        bt_device_manager_store_local_address(aws_addr);
    } else if (!memcmp(aws_addr, temp_addr, sizeof(bt_bd_addr_t))) {
        bt_device_manager_store_local_address(local_addr);
    } else {
...
#endif
#ifdef AIR_BT_HID_ENABLE
    bt_memory_init_control_block(BT_MEMORY_CONTROL_BLOCK_HID, bt_hid_link_cb_buf, BT_HID_LINK_BUF_SIZE);
#endif
#ifdef AIR_BLE_ULTRA_LOW_LATENCY_ENABLE
...
#endif
#ifdef AIR_LE_OTP_ENABLE
    bt_memory_init_control_block(BT_MEMORY_CONTROL_BLOCK_LE_OTP, le_otp_cb_buf, BT_LE_OTP_BUF_SIZE);
#endif
#ifdef BT_EATT_ENABLE
...
//    uint32_t EDR_table_size = sizeof(tx_pwr_BTD_EDR_table);
    ...
    bt_firmware_type_set(BT_FIRMWARE_TYPE_EARBUDS);
