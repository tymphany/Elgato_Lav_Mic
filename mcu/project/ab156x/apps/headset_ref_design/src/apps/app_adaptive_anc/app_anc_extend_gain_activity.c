...
static bool app_anc_extend_gain_control_wn(bt_aws_mce_role_t role, uint8_t action)
{
    ...
    APPS_LOG_MSGID_I(LOG_TAG" gain_control_wn, [%02X] action=%d, uu=%d ret=%d",
                     3, role, action, s_app_anc_gain_control.uu_enable, ret);
                                          ...
                                          audio_anc_control_gain_t gain_2, uint32_t target_gpt)
{
    ...
    if ((type == AUDIO_ANC_CONTROL_EXTEND_RAMP_TYPE_USER_UNAWARE && s_app_anc_gain_control.uu_suspend)
        || (type == AUDIO_ANC_CONTROL_EXTEND_RAMP_TYPE_WIND_NOISE && s_app_anc_gain_control.wn_suspend)
        || (type == AUDIO_ANC_CONTROL_EXTEND_RAMP_TYPE_ENVIRONMENT_DETECTION && s_app_anc_gain_control.ed_suspend)) {
        return success;
    }
...
static bool app_anc_extend_gain_proc_anc_extend_gain_race_group(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    ...
    app_anc_service_state_t anc_state = app_anc_server_get_state();
