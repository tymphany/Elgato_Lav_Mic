...
#ifdef AIR_BTA_IC_PREMIUM_G3
    model_count += 2; // add en-US and hi-IN into the supported list.
#endif /* AIR_BTA_IC_PREMIUM_G3 */
...
#ifdef AIR_BTA_IC_PREMIUM_G3
    for (model_index = 0; model_index < model_count - 2; model_index ++) {
#else
...
#ifdef AIR_BTA_IC_PREMIUM_G3
    // Append the en-US and hi-IN to the supported list.
    memcpy((void *)(supported_locale_list[model_count - 2].name), "en-US", strlen((char *)("en-US")));
    memcpy((void *)(supported_locale_list[model_count - 1].name), "hi-IN", strlen((char *)("hi-IN")));
#endif /* AIR_BTA_IC_PREMIUM_G3 */
...
#ifdef MTK_ANC_ENABLE
            state.valueType = AMA_VALUE_TYPE_BOOLEAN;
            if (anc_enabled && (anc_type < AUDIO_ANC_CONTROL_PASS_THRU_TYPE_DEFAULT)) {
                state.value.boolean = true;
            } else {
                state.value.boolean = false;
            }
