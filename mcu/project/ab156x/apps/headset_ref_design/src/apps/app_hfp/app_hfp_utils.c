...
bool app_hfp_set_auto_accept_incoming_call(uint8_t isAutoaccept)
{
    ...
    APPS_LOG_MSGID_I(APP_HFP_UTILS" [AUTO ACCEPT] set in ear control value: status=%d, isEnable=%d.", 2, status, isAutoaccept);
    return ret;
}
...
bool app_hfp_mute_mic(bool mute)
{
    ...
    APPS_LOG_MSGID_I(APP_HFP_UTILS" [MUTE] mute_mic=%d, bt_status=0x%08X",
                     2, mute, bt_status);
    return ret;
}
