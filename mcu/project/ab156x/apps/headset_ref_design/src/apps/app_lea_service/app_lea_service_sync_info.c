...
//    app_lea_sync_info_print(peer_info);
    ...
    if (role == BT_AWS_MCE_ROLE_PARTNER
        ...
        app_lea_sync_start_peer_adv(peer_info);
            ...
            && peer_info->adv_timeout == APP_LE_AUDIO_SWITCH_LEA_ADV_TIME
            && local_info.adv_mode != APP_LEA_ADV_MODE_GENERAL) {
            APPS_LOG_MSGID_W(LOG_TAG" sync_info_handle, [%02X] Switch_LEA General ADV by peer",
                             1, role);
            app_lea_service_start_advertising(APP_LEA_ADV_MODE_GENERAL, FALSE, APP_LE_AUDIO_SWITCH_LEA_ADV_TIME);
     } else if (peer_info->adv_mode == APP_LEA_ADV_MODE_TARGET_ALL
             && local_info.adv_mode == APP_LEA_ADV_MODE_TARGET_ALL
             && peer_info->bond_num > local_info.bond_num) {
         APPS_LOG_MSGID_W(LOG_TAG" sync_info_handle, [%02X] Switch_LEA General ADV by peer more bond_info",
                          1, role);
         app_lea_service_start_advertising(APP_LEA_ADV_MODE_GENERAL, FALSE, APP_LE_AUDIO_SWITCH_LEA_ADV_TIME);
     }
