...
bt_addr_t    app_lea_adv_target_addr[APP_LEA_MAX_TARGET_NUM];
                                             ...
                                             const uint8_t *addr)
{
    ...
    bt_addr_t bt_addr = {0};
    if (addr != NULL) {
        bt_addr.type = addr_type;
        memcpy(bt_addr.addr, addr, BT_BD_ADDR_LEN);
        bt_addr_param = &bt_addr;
    }
    bt_status_t bt_status = bt_gap_le_set_white_list(op, bt_addr_param);
        ...
        case APP_LEA_TARGET_ADD_ADDR: {
            for (int i = 0; i < APP_LEA_MAX_TARGET_NUM; i++) {
                ...
                if (APP_LEA_IS_EMPTY_ADDR(target_addr, empty_addr)) {
                    ...
                    app_lea_target_update_white_list(BT_GAP_LE_ADD_TO_WHITE_LIST, addr_type, addr);
                    ret = TRUE;
                    break;
        ...
        case APP_LEA_TARGET_REMOVE_ADDR: {
            for (int i = 0; i < APP_LEA_MAX_TARGET_NUM; i++) {
                ...
                if (memcmp(addr, target_addr, BT_BD_ADDR_LEN) == 0) {
                    ...
                    memset(target_addr, 0, BT_BD_ADDR_LEN);
                    app_lea_adv_target_addr_num -= 1;
                    app_lea_target_update_white_list(BT_GAP_LE_REMOVE_FROM_WHITE_LIST, addr_type, addr);
                    ret = TRUE;
                    break;
                }
            }
...
/**                                     Public API                                 */
/**================================================================================*/
void app_lea_clear_target_addr(void)
{
    bt_aws_mce_role_t role = bt_connection_manager_device_local_info_get_aws_role();
