...
bool app_music_remove_avrcp_status(bt_sink_srv_event_param_t *sink_event)
{
    bool ret = false;
    if (sink_event == NULL || s_app_music_avrcp_status.avrcp_num == 0) {
        return ret;
    }
    ...
    for (i=0; i<APP_MUISC_AVRCP_RECORD_MAX_NUM; i++) {
        if (s_app_music_avrcp_status.avrcp_device[i].is_playing
            && (0 == memcmp(s_app_music_avrcp_status.avrcp_device[i].remote_addr, sink_event->avrcp_status_change.address, sizeof(bt_bd_addr_t)))) {
    ...
    if (s_app_music_avrcp_status.avrcp_num == 1) {
...
#if defined(MTK_AWS_MCE_ENABLE) && !defined (AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
...
#ifdef APP_MUSIC_ADJUST_VOLUME_VIA_ADDRESS_ENABLE
                if (sink_action == BT_SINK_SRV_ACTION_VOLUME_UP || sink_action == BT_SINK_SRV_ACTION_VOLUME_DOWN) {
#if defined(MTK_AWS_MCE_ENABLE)
                                        ...
                                        void *extra_data, size_t data_len)
{
    ...
    if (event_id == BT_SINK_SRV_EVENT_STATE_CHANGE) {
        ...
        if (BT_AVRCP_STATUS_PLAY_PLAYING == avrcp_status) {
            ...
            app_music_remove_avrcp_status(event);
...
static bool app_music_idle_check_and_start_music(struct _ui_shell_activity *self, void *extra_data)
{
    ...
    APPS_LOG_MSGID_I(APP_MUSIC_UTILS" check_and_start_music, cur=%d, pre=%d, isAutoPaused=%d, music_playing=%d, in_ear_config=%d",
                     5, sta_info->current, sta_info->previous, ctx->isAutoPaused, ctx->music_playing, temp_music_in_ear_config);
...
bool app_music_set_ull_volume(bool volume_up, uint32_t volume)
{
    ...
    bt_status = bt_ull_action(BT_ULL_ACTION_SET_STREAMING_VOLUME, &volume_param, sizeof(bt_ull_volume_t));
    if (bt_status == BT_STATUS_SUCCESS) {
        ret = true;
    }
#endif
