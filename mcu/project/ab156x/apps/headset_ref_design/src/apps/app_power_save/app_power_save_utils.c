...
static bool app_power_saving_is_ull_steamming = false;
...
void app_power_update_ull2_streamming_state(uint32_t event_id)
{
#if defined(AIR_BT_ULTRA_LOW_LATENCY_ENABLE) || defined(AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
    if (BT_ULL_EVENT_LE_STREAMING_START_IND == event_id) {
       app_power_saving_is_ull_steamming = true;
    } else if (BT_ULL_EVENT_LE_STREAMING_STOP_IND == event_id) {
       app_power_saving_is_ull_steamming = false;
    }
    APPS_LOG_MSGID_I(LOG_TAG" app_power_saving_is_ull_steamming = %d",1, app_power_saving_is_ull_steamming);
    ...
    *type = APP_POWER_SAVING_TYPE_NO_CONNECTION;
    ...
    if (self && self->local_context) {
        ...
        if (target_mode == APP_POWER_SAVING_TARGET_MODE_NORMAL
            ...
            && app_power_saving_get_cfg()->silence_detect_enable
#if defined(AIR_BT_ULTRA_LOW_LATENCY_ENABLE) || defined(AIR_BLE_ULTRA_LOW_LATENCY_ENABLE)
            && ( (false == app_power_saving_is_ull_steamming) && BT_SINK_SRV_STATE_STREAMING > bt_sink_srv_get_state() )
#else
            && BT_SINK_SRV_STATE_STREAMING > bt_sink_srv_get_state()
#endif
) {
            APPS_LOG_MSGID_I(LOG_TAG" Need power off by silence %d", 1,app_power_saving_is_ull_steamming);
            *type = APP_POWER_SAVING_TYPE_AUDIO_SILENCE;
            target_mode = APP_POWER_SAVING_TARGET_MODE_SYSTEM_OFF;
        }
#endif
