...
#if defined(AIR_BLE_ULTRA_LOW_LATENCY_ENABLE) && defined (MTK_AWS_MCE_ENABLE)
...
#endif
void app_le_ull_set_advertising_enable(bool enable);
static atci_status_t app_le_ull_atci_hdl(atci_parse_cmd_param_t *parse_cmd)
{
    char *pChar = NULL;
...
static uint32_t app_le_ull_init()
{
    ...
    bt_ull_le_srv_init(BT_ULL_ROLE_CLIENT, app_le_ull_callback);
    bt_ull_atci_init();
    bt_ull_le_device_info_t *dev_info = &g_ull_dev_info;
...
#ifdef AIR_BLE_ULL_TAKEOVER_ENABLE
#if !defined(AIR_DCHS_MODE_ENABLE) && !defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_MASTER_ENABLE)
    ...
    switch (action) {
        ...
        case KEY_MUTE_MIC: {
            if (!app_ull_is_le_ull_connected()) {
                /* Can not unmute during the disconnected states*/
                break;
            }
