...
static void app_usb_audio_cfg_callback(app_audio_trans_mgr_usr_type_t type, app_audio_trans_mgr_usr_cfg_t* cfg)
{
...
#else
        cfg->trans_cfg.scenario_sub_id = AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT_IEM;
    ...
    codec_param->format = s_sample_sizes[param_idx] == 3 ? AFE_PCM_FORMAT_S24_LE : AFE_PCM_FORMAT_S16_LE;
...
static void app_usb_audio_init()
{
...
#endif
    usb_audio_cfg.rconfig_type = WIRED_AUDIO_CONFIG_OP_VOL_LEVEL_MUSIC_DUL;
    ...
    usb_audio_cfg.rconfig_type = WIRED_AUDIO_CONFIG_OP_VOL_LEVEL_VOICE_DUL;
    usb_audio_cfg.ctrl_type = APP_AUDIO_TRANS_MGR_CTRL_TYPE_RESOURCE_CTRL;
...
/* workaround start */
        ...
        case APPS_EVENTS_INTERACTION_USB_IN_RESUME_AFTER_VP: {
            if (voice_prompt_get_current_index() == VOICE_PROMPT_VP_INDEX_INVALID) {
                if (s_started[APP_USB_AUDIO_IDX_AUDIO_IN1]) {
                    app_audio_trans_mgr_start_audio(s_usb_in1_handle);
                }
                if (s_started[APP_USB_AUDIO_IDX_AUDIO_IN2]) {
    ...
    size_t data_len)
{
    ...
    switch (action) {
        ...
        case KEY_AVRCP_PAUSE: {
            apps_config_state_t sta = apps_config_key_get_mmi_state();
            USB_AUDIO_LOG_I("idle activity play pause, mmi_sta = %d", 1, sta);
    ...
    size_t data_len)
{
    ...
    switch (event_id) {
        ...
        case APPS_EVENTS_USB_AUDIO_VOLUME: {
            ...
            uint8_t vol_l = (uint8_t)((vol->left_volume * USB_AUDIO_MAX_LEVEL) / USB_AUDIO_MAX_VOLUME + 0.5f);
            uint8_t vol_r = (uint8_t)((vol->right_volume * USB_AUDIO_MAX_LEVEL) / USB_AUDIO_MAX_VOLUME + 0.5f);
            app_audio_trans_mgr_set_volume(get_handle_by_port(vol->port_type, vol->port_num), vol_l, vol_r);
            #ifdef AIR_AUDIO_DETACHABLE_MIC_ENABLE
        ...
        case APPS_EVENTS_USB_AUDIO_SET_CHANNELS: {
            ...
            if (channel_info->channel_nums > 0) {
                ...
                if (!s_started[idx]) {
                    ...
                    /* workaround: do not restart usb out if it was suspended. */
                    #ifdef AIR_USB_AUDIO_OUT_ENABLE
                    if (idx == APP_USB_AUDIO_IDX_AUDIO_OUT && usb_out_suspend) {
                        app_audio_trans_mgr_deinit_audio(handle);
                        app_audio_trans_mgr_init_audio(handle);
