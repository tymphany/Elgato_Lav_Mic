...
#ifdef AIR_USB_ENABLE
    usb_dev_type_t  usb_device_type = s_app_wireless_mic_context.usb_type;
#endif
    if (AUD_ID_INVALID == s_app_wireless_mic_context.transmit_id
#ifdef AIR_USB_ENABLE
            || usb_device_type == USB_MASS_STORAGE
#endif
            || app_fota_get_ota_ongoing()
        ) {
#ifdef AIR_USB_ENABLE
        APPS_LOG_MSGID_W(LOG_TAG" start_audio_transmitter fail: id=%d, usb_type=%d.",
                2, s_app_wireless_mic_context.transmit_id, usb_device_type);
#endif
        goto start_exit;
    }
...
//            if (s_app_wireless_mic_context.is_write_done) {
//                ui_shell_send_event(false, EVENT_PRIORITY_HIGHEST, EVENT_GROUP_UI_SHELL_WIRELESS_MIC, APPS_EVENTS_INTERACTION_WIRELESS_MIC_AUDIO_DATA,
//                                    NULL, 0, NULL, 0);
//                s_app_wireless_mic_context.is_write_done = false;
//            }
...
{
    return (s_app_wireless_mic_context.usb_type == USB_MASS_STORAGE);
}


static void app_wireless_mic_usb_plug_in_cb(usb_evt_t event, void* usb_data, void* user_data) {
    APPS_LOG_MSGID_I(LOG_TAG" app_wireless_mic_usb_plug_in_cb: type[%d]", 1, s_app_wireless_mic_context.usb_type);
    usb_set_device_type(s_app_wireless_mic_context.usb_type);
    s_app_wireless_mic_context.usb_plug_in = true;
    ui_shell_send_event(false, EVENT_PRIORITY_HIGHEST, EVENT_GROUP_UI_SHELL_APP_INTERACTION,
                APPS_EVENTS_INTERACTION_USB_PLUG_STATE,
                (void *)s_app_wireless_mic_context.usb_plug_in, 0, NULL, 0);

}

static void app_wireless_mic_usb_plug_out_cb(usb_evt_t event, void* usb_data, void* user_data) {
    bool is_real_plug = is_vusb_ready();
    APPS_LOG_MSGID_I(LOG_TAG" app_wireless_mic_usb_plug_out_cb: type[%d], is_real_plug=%d", 2, s_app_wireless_mic_context.usb_type, is_real_plug);
    if (is_real_plug) {
        return;
    }
    s_app_wireless_mic_context.usb_plug_in = false;
    ui_shell_send_event(false, EVENT_PRIORITY_HIGHEST, EVENT_GROUP_UI_SHELL_APP_INTERACTION,
                        APPS_EVENTS_INTERACTION_USB_PLUG_STATE,
                        (void *)s_app_wireless_mic_context.usb_plug_in, 0, NULL, 0);
}
...
#endif
#ifdef AIR_USB_ENABLE
            s_app_wireless_mic_context.usb_type = USB_AUDIO;
            usb_evt_register_cb(USB_USER_APP, USB_EVT_PLUG_IN, app_wireless_mic_usb_plug_in_cb, NULL);
            usb_evt_register_cb(USB_USER_APP, USB_EVT_PLUG_OUT, app_wireless_mic_usb_plug_out_cb, NULL);
#endif
            break;
        }
...
#ifdef AIR_USB_ENABLE
            if (USB_MASS_STORAGE == s_app_wireless_mic_context.usb_type) {
                s_app_wireless_mic_context.usb_type = USB_AUDIO;
            } else if (USB_AUDIO == s_app_wireless_mic_context.usb_type) {
                if (s_app_wireless_mic_context.is_transmitter_start) {
                    app_wireless_mic_stop_audio_transmitter();
                }
                s_app_wireless_mic_context.usb_type = USB_MASS_STORAGE;

            }
            usb_drv_disable();
            usb_drv_enable();
            APPS_LOG_MSGID_I(LOG_TAG" switch usb type: type=%d",
                             1, s_app_wireless_mic_context.usb_type);
#endif
            break;
        }
        default:
...
bool app_wireless_mic_idle_proc_audio_data()
{
    ...
    do {
        ...
        if (status == AUDIO_TRANSMITTER_STATUS_SUCCESS) {
#ifdef APP_WIRELESS_MIC_DEBUG
            // APPS_LOG_MSGID_I(LOG_TAG" audio_transmitter_callback: get data addr=0x%x, len=%d, data_count=%d",
            //                  3, data1, len, s_app_wireless_mic_context.data_count);
#endif
            if (len > 0) {
