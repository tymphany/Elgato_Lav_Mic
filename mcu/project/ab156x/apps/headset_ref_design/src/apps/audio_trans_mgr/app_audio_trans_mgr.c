...
#if defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_MASTER_ENABLE) || defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_SLAVE_ENABLE)
    if (u_ctx->usr_info.ctrl_type == APP_AUDIO_TRANS_MGR_CTRL_TYPE_AUD_SRC_CTRL && u_ctx->state == APP_AUDIO_TRANS_MGR_TRANS_STA_STOPING) {
...
static void __app_audio_trans_mgr_set_transmitter_volume(app_audio_trans_mgr_usr_ctx_t *u_ctx)
{
    ...
    //config.wired_audio_runtime_config.vol_level.vol_level_l = u_ctx->usr_info.mute > 0 ? 0 : u_ctx->usr_info.volume_l;
    //config.wired_audio_runtime_config.vol_level.vol_level_r = u_ctx->usr_info.mute > 0 ? 0 : u_ctx->usr_info.volume_r;
