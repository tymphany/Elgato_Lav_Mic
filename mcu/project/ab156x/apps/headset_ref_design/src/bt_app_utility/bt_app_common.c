    ...
    switch (msg) {
        ...
        case BT_GAP_LE_SET_ADVERTISING_CNF: {
            ...
            if (state == BT_ROLE_HANDOVER_STATE_ONGOING) {
                ...
                } else if (bt_app_adv_restart_flag && bt_app_common_is_connection_info_list_empty()) {
                    bt_role_handover_reply_prepare_request(BT_ROLE_HANDOVER_MODULE_BLE_APP);
                }
            }
#endif
    ...
    switch (event) {
        ...
        case BT_ROLE_HANDOVER_PREPARE_REQ_IND: {
            if (role == BT_AWS_MCE_ROLE_AGENT) {
                ...
                if (conn_handle && multi_va_manager_get_current_va_type() != MULTI_VA_TYPE_GSOUND) {
                    bt_app_common_le_disconnect(conn_handle);
                }
#endif
            }
...
static bt_status_t bt_app_common_get_pairing_config(bt_gap_le_bonding_start_ind_t *ind)
{
    ...
    if (conn_info != NULL) {
        LOG_MSGID_I(BT_APP, "[APP][COMMON] get connection link type = %02x by handle = %02x", 2, conn_info->link_type, ind->handle);
        if (conn_info->link_type == BT_GAP_LE_SRV_LINK_TYPE_LE_AUDIO) {
...
#elif defined(APP_SWIFT_PAIR_LE_EDR_SECURE_MODE)
    uint16_t swift_conn_handle = app_swift_pair_get_conn_handle();
    if (ind->handle == swift_conn_handle) {
