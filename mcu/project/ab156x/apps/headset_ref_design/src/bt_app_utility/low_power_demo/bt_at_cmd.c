...
static uint8_t g_local_mp3_enable = false;
#endif
...
#if 0
        if (!g_local_mp3_enable && t->pvTaskCode == audio_player_task_main) {
            LOG_MSGID_I(common, "skip audio_task", 0);
            continue;
        }
#endif
...
//extern void bt_sink_srv_switch_aac_codec(bool enable);
