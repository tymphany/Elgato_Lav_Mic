...
void system_init(void)
{
...
#ifdef AIR_BTA_IC_PREMIUM_G3
    /* Enable Sysram */
    *(volatile uint32_t *)0x422E0E00 |= (1 << 8);
    ...
    *(volatile uint32_t *)0x42040B20 = 0xFFFFFFFF;
...
#ifdef AIR_BTA_IC_PREMIUM_G3
    /* sys init done log */
    LOG_I(common, "[CM33]system initialize done[%s]", build_date_time_str);
    ...
    LOG_MSGID_I(common, "DUAL_CHIP_MIXING_MODE_ROLE [MASTER]", 0);
#elif defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_SLAVE_ENABLE)
    LOG_MSGID_I(common, "DUAL_CHIP_MIXING_MODE_ROLE [SLAVE]", 0);
#else
    LOG_MSGID_W(common, "DUAL_CHIP_MIXING_MODE_ROLE [NONE]", 0);
#endif
