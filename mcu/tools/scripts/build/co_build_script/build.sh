...
check_variable_value () {
   ...
   #echo ${dsp_feature}
    ...
    if [ ! -f ${mcu_feaure_file} ] || [ -z ${mcu_feaure_file} ] ; then
        echo Feature file ${mcu_feaure_file} not exist/defined!
        echo "Feature option compare status : FAIL"
        echo "./build.sh $fake_board $project" >> .variable_check_fail_cases
        exit 1
    fi

    if [ ! -f ${dsp_feaure_file} ] || [ -z ${dsp_feaure_file} ] ; then
        echo Feature file ${dsp_feaure_file} not exist/defined!
        echo "Feature option compare status : FAIL"
        echo "./build.sh $fake_board $project" >> .variable_check_fail_cases
        exit 1
    fi
